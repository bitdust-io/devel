...
def send(customer_idurl, packet_id, format_type, key_id, remote_idurl, query_items=[]):
    ...
    if not my_keys.is_key_registered(key_id):
        ...
        return p2p_service.SendFailNoRequest(
            customer_idurl, packet_id, response="key not registered"
        )
    if _Debug:
        lg.out(
            _DebugLevel,
    ...
    if os.path.isdir(ownerdir):
        try:
            ...
            lg.exc()
            return p2p_service.SendFailNoRequest(
                customer_idurl, packet_id, response="list files query processing error"
            )
    else:
...
def TreeSummary(ownerdir, key_alias=None):
    ...
    bpio.traverse_dir_recursive(
        lambda realpath, subpath, name: cb(out, realpath, subpath, name), ownerdir
    )
    src = out.getvalue()
    out.close()
    return src
