...
from lib import packetid, serialization, strng
from logs import lg
...
from supplier import list_files, local_tester
...
def verify_ownership(newpacket, raise_exception=False):
    ...
    packet_key_id = my_keys.latest_key_id(
        my_keys.make_key_id(
            packet_key_alias, creator_idurl, creator_glob_id=packet_owner_id
        )
    )
    ...
    if _Debug:
        lg.args(
            ...
            customer_id=packet_owner_id,
            key_registered=packet_key_id_registered,
        )
    if newpacket.Command == commands.Data():
        if owner_idurl == creator_idurl:
            ...
            if packet_key_id_registered:
                if _Debug:
                    lg.dbg(
                        _DebugLevel,
                        "OK, scenario 10: data sender is not my customer but packet key is registered",
                    )
                return customer_idurl, creator_idurl
            lg.err(
                "FAIL, scenario 6: data sender is not my customer and also packet key is not registered"
            )
            if raise_exception:
        ...
        if _Debug:
            lg.dbg(
                _DebugLevel, "non-authorized user is trying to store data on the supplier"
            )
        return None, None
    ...
    if driver.is_enabled("service_proxy_server"):
        if _Debug:
            lg.dbg(
                _DebugLevel,
                "IGNORE, scenario 9: received Data() was not authorized, but proxy router service is enabled",
            )
        return None, None
    # TODO:
...
def do_notify_supplier_file_modified(
    key_alias, remote_path, action, customer_idurl, authorized_idurl
):
...
def on_data(newpacket):
    ...
    if id_url.is_not_in(
        customer_idurl, known_customers_quoats, as_field=False, as_bin=True
    ):
        lg.err(
            "customer space is broken, no info about donated space can be found for %s"
            % newpacket
        )
        p2p_service.SendFail(
            newpacket,
            "customer space is broken, no info found about donated space",
    ...
    p2p_service.SendAck(
        newpacket, response=strng.to_text(sz), remote_idurl=authorized_idurl
    )
    reactor.callLater(0, local_tester.TestSpaceTime)  # @UndefinedVariable
    if key_alias != "master" and data_changed:
        if remote_path == settings.BackupIndexFileName():
            do_notify_supplier_file_modified(
                key_alias, remote_path, "write", customer_idurl, authorized_idurl
            )
        else:
...
def on_retrieve(newpacket):
    ...
    if not glob_path["path"]:
        lg.err("got incorrect PacketID")
        p2p_service.SendFail(
            newpacket, "incorrect path", remote_idurl=newpacket.CreatorID
        )
        return False
    if not glob_path["idurl"]:
        lg.warn("no customer global id found in PacketID: %s" % newpacket.PacketID)
        p2p_service.SendFail(
            newpacket, "incorrect retrieve request", remote_idurl=newpacket.CreatorID
        )
        return False
    key_id = glob_path["key_id"]
    recipient_idurl = newpacket.OwnerID
    if (
        newpacket.CreatorID != glob_path["idurl"]
        ...
        if not my_keys.is_key_registered(key_id):
            ...
            p2p_service.SendFail(
                newpacket, "key not registered", remote_idurl=newpacket.CreatorID
            )
            return False
        verified = False
        if _Debug:
        ...
        if not verified:
            lg.warn(
                "request is not authorized, test sample signature verification failed"
            )
            return False
    ...
    if not os.path.exists(filename):
        ...
        p2p_service.SendFail(
            newpacket,
            "did not found requested file locally",
            remote_idurl=recipient_idurl,
        )
        return False
    if not os.access(filename, os.R_OK):
        lg.warn("no read access to requested packet %s" % filename)
        p2p_service.SendFail(
            newpacket, "failed reading requested file", remote_idurl=recipient_idurl
        )
        return False
    ...
    if not data:
        ...
        p2p_service.SendFail(
            newpacket, "empty data on disk", remote_idurl=recipient_idurl
        )
        return False
    ...
    if stored_packet is None:
        ...
        p2p_service.SendFail(
            newpacket, "unserialize failed", remote_idurl=recipient_idurl
        )
        return False
    if not stored_packet.Valid():
        lg.warn("Stored packet is not Valid %s" % filename)
        p2p_service.SendFail(
            newpacket, "stored packet is not valid", remote_idurl=recipient_idurl
        )
        return False
    ...
    if _Debug:
        lg.args(
            ...
            fn=filename,
            recipient=recipient_idurl,
        )
    if recipient_idurl == stored_packet.OwnerID:
        if _Debug:
            lg.dbg(
                _DebugLevel,
                "from request %r : sending %r back to owner: %s"
    ...
    if _Debug:
        lg.dbg(
            _DebugLevel,
            "from request %r : returning data owned by %s to %s"
...
def on_delete_backup(newpacket):
    ...
    if _Debug:
        lg.dbg(
            _DebugLevel,
            "from [%s] with %d IDs, %d were removed"
        ...
        if id_url.is_cached(old_idurl) and id_url.is_cached(customer_idurl_bin):
            if old_idurl == id_url.field(customer_idurl_bin):
                ...
                if latest_customer_idurl_bin != customer_idurl_bin:
                    all_meta_info[latest_customer_idurl_bin] = all_meta_info.pop(
                        customer_idurl_bin
                    )
