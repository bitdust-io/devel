...
from lib import nameurl, serialization, strng
from logs import lg
...
class FamilyMember(automat.Automat):
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---DISCONNECTED---
            ...
            if event == "shutdown":
                ...
                event == "family-refresh"
                or event == "family-join"
                or event == "family-leave"
            ):
        ...
        # ---DHT_READ---
            ...
            if (
                ...
                and not self.isLeaving(*args, **kwargs)
            ):
                self.state = "CONNECTED"
                self.Attempts = 0
                self.doNotifyConnected(*args, **kwargs)
            elif event == "dht-value-not-exist" or (
                event == "dht-value-exist" and not self.isMyPositionOK(*args, **kwargs)
            ):
                ...
                event == "dht-value-exist"
                and self.isMyPositionOK(*args, **kwargs)
                and self.isLeaving(*args, **kwargs)
            ):
        ...
        # ---SUPPLIERS---
            ...
            if event == "shutdown":
                ...
                event == "all-suppliers-agree" or event == "timer-10sec"
            ) and not self.isFamilyModified(*args, **kwargs):
                self.state = "CONNECTED"
                self.Attempts = 0
                self.doNotifyConnected(*args, **kwargs)
            elif (
                event == "timer-10sec" or event == "all-suppliers-agree"
            ) and self.isFamilyModified(*args, **kwargs):
                self.state = "DHT_WRITE"
                self.doDHTWrite(*args, **kwargs)
        ...
        # ---DHT_WRITE---
            ...
            if (
                ...
                self.state = "DHT_READ"
                self.doDHTRead(*args, **kwargs)
            elif event == "shutdown" or (
                (event == "dht-write-fail" or event == "dht-write-ok")
                and self.isLeaving(*args, **kwargs)
            ):
                self.state = "CLOSED"
                self.doDestroyMe(*args, **kwargs)
    ...
    def doInit(self, *args, **kwargs):
        ...
        self.refresh_period = (
            DHT_RECORD_REFRESH_INTERVAL * settings.DefaultDesiredSuppliers()
        )
    ...
    def doRebuildFamily(self, *args, **kwargs):
        ...
        merged_info = None
        if latest_revision > 0:
            merged_info = self._do_merge_revisions(
                dht_info_valid, my_info_valid, latest_revision
            )
        if not merged_info:
        ...
        if self.transaction:
            ...
            if known_ecc_map:
                ...
                self.refresh_period = (
                    DHT_RECORD_REFRESH_INTERVAL * expected_suppliers_count
                )
    ...
    def doDHTRead(self, *args, **kwargs):
        ...
        d = dht_relations.read_customer_suppliers(
            self.customer_idurl, as_fields=False, use_cache=False
        )
    ...
    def doNotifyConnected(self, *args, **kwargs):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "family_member.doNotifyConnected\n            my_info=%r\n            dht_info=%r\n            requests=%r"
                % (
                    self.my_info,
                    self.dht_info,
                    self.requests,
    ...
    def _do_create_possible_revision(self, latest_revision):
        ...
        try:
            ...
            possible_position = -1
        possible_suppliers = id_url.to_bin_list(
            local_customer_meta_info.get("family_snapshot") or []
        )
    ...
    def _do_create_revision_from_another_supplier(
        self, another_revision, another_suppliers, another_ecc_map
    ):
        local_customer_meta_info = contactsdb.get_customer_meta_info(self.customer_idurl)
        try:
            possible_position = int(local_customer_meta_info.get("position", -1))
    ...
    def _do_merge_revisions(self, dht_info, my_info, latest_revision):
        ...
        if not merged_info["ecc_map"]:
            known_ecc_map = contactsdb.get_customer_meta_info(self.customer_idurl).get(
                "ecc_map", None
            )
        ...
        if merged_info["ecc_map"]:
            ...
            if len(merged_info["suppliers"]) < expected_suppliers_count:
                ...
                merged_info["suppliers"] = merged_info["suppliers"][
                    :expected_suppliers_count
                ]
        if merged_info["revision"] != latest_revision:
            lg.info(
                "will switch known revision %d to the latest: %d"
                % (
    ...
    def _do_increment_revision(self, possible_transaction):
        if (
            self.customer_idurl.to_original()
            != possible_transaction["customer_idurl"].to_original()
        ):
            ...
            lg.info(
                "incremented family revision after customer %r identity rotated: %r"
                % (
                    self.customer_idurl,
            ...
            if self.dht_info["suppliers"] == possible_transaction["suppliers"]:
                if self.dht_info["ecc_map"] == possible_transaction["ecc_map"]:
                    if (
                        ...
                        and self.current_request["command"] == "family-leave"
                    ):
                        if _Debug:
                            lg.out(
                                _DebugLevel,
                                'family_member._do_increment_revision will re-publish latest DHT info because processing "family-leave" request',
                            )
                    else:
                        if _Debug:
                            lg.out(
    ...
    def _do_process_family_join_request(self, merged_info, current_request):
        ...
        if (
            ...
            and current_request.get("position")
            and current_request["position"] >= current_request_expected_suppliers_count
        ):
            lg.warn(
        ...
        if (
            merged_info["ecc_map"]
            and current_request["ecc_map"]
            and current_request["ecc_map"] != merged_info["ecc_map"]
        ):
            lg.info(
        ...
        if not merged_info["ecc_map"]:
            ...
            if known_ecc_map:
                lg.warn(
                    "unknown ecc_map, will populate known value from customer meta info: %s"
                    % known_ecc_map
                )
                merged_info["ecc_map"] = known_ecc_map
        if not merged_info["ecc_map"]:
        ...
        if current_request["position"] is not None and current_request["position"] >= 0:
            ...
            if (
                existing_position >= 0
                and existing_position != current_request["position"]
            ):
                merged_info["suppliers"][existing_position] = b""
                merged_info["suppliers"][current_request["position"]] = current_request[
                    "supplier_idurl"
                ]
                if _Debug:
                    lg.out(
        ...
        if current_request["supplier_idurl"] not in merged_info["suppliers"]:
            if b"" in merged_info["suppliers"]:
                ...
                merged_info["suppliers"][first_empty_position] = current_request[
                    "supplier_idurl"
                ]
                if _Debug:
                    lg.out(
        ...
        if current_request.get("family_snapshot"):
            for supplier_position in range(len(merged_info["suppliers"])):
                if supplier_position < len(current_request["family_snapshot"]):
                    if (
                        not merged_info["suppliers"][supplier_position]
                        and current_request["family_snapshot"][supplier_position]
                    ):
                        merged_info["suppliers"][supplier_position] = current_request[
                            "family_snapshot"
                        ][supplier_position]
                        if _Debug:
                            lg.out(
                                _DebugLevel,
                                "    found empty supplier at position %d and populated from current request: %s"
                                % (
                                    supplier_position,
                                    merged_info["suppliers"][supplier_position],
                                ),
                            )
    ...
    def _do_process_family_leave_request(self, merged_info, current_request):
        ...
        if current_request.get("ecc_map"):
            if (
                merged_info["ecc_map"]
                and current_request.get("ecc_map")
                and current_request.get("ecc_map") != merged_info["ecc_map"]
            ):
                lg.info(
                    'from "family-leave" request, detected ecc_map change %s -> %s for customer %s'
                    % (
                        merged_info["ecc_map"],
                        current_request["ecc_map"],
                        self.customer_idurl,
            ...
            if not merged_info["ecc_map"] and current_request["ecc_map"]:
                lg.info(
                    'from "family-leave" request, detected ecc_map was set to %s for the first time for customer %s'
                    % (current_request["ecc_map"], self.customer_idurl)
                )
                merged_info["ecc_map"] = current_request["ecc_map"]
        ...
        if existing_position < 0:
            if _Debug:
                lg.dbg(
                    _DebugLevel,
                    "supplier %r not found in customer family %r, probably already left"
                    % (
                        current_request["supplier_idurl"],
                        self.customer_idurl,
                    ),
            ...
            if existing_position < expected_suppliers_count:
                ...
                if _Debug:
                    lg.info(
                        "erasing supplier %r from customer family %r"
                        % (
                            current_request["supplier_idurl"],
                            self.customer_idurl,
                        )
    ...
    def _do_process_family_refresh_request(self, merged_info):
        ...
        if (
            ...
            and len(merged_info["suppliers"]) != my_expected_suppliers_count
        ):
            lg.warn(
    ...
    def _do_write_transaction(self, retries):
        if _Debug:
            lg.out(
                _DebugLevel,
                "family_member._do_write_transaction  suppliers=%d  retries=%d"
                % (
    ...
    def _on_dht_read_success(self, dht_result):
        if (
            dht_result
            and isinstance(dht_result, dict)
            and len(dht_result.get("suppliers", [])) > 0
        ):
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "family_member._on_dht_read_success  result with %d suppliers"
                    % len(dht_result.get("suppliers", [])),
                )
            ...
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "family_member._on_dht_read_success  result with %s"
                    % type(dht_result),
                )
            self.dht_info = None
            self.dht_value_exists = False
    ...
    def _on_dht_write_success(self, dht_result):
        if _Debug:
            lg.out(
                _DebugLevel,
                "family_member._on_dht_write_success  result: %r" % type(dht_result),
            )
    ...
    def _on_dht_write_failed(self, err, retries):
        ...
        if err_msg.count("current revision is") and retries < 3:
            try:
                current_revision = re.search("current revision is (\d+)", err_msg).group(
                    1
                )
            ...
            if _Debug:
                lg.warn(
                    'recognized "DHT write operation failed" because of late revision, increase revision to %d and retry'
                    % current_revision
                )
    ...
    def _on_incoming_suppliers_list(self, inp):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "family_member._on_incoming_suppliers_list with %s" % incoming_packet,
            )
        if not self.my_info:
            if _Debug:
        ...
        try:
            ...
            return p2p_service.SendFail(
                incoming_packet, response=serialization.DictToBytes(self.my_info)
            )
        if _Debug:
            lg.args(
                _DebugLevel,
                another_revision=another_revision,
                another_ecc_map=another_ecc_map,
                another_suppliers_list=another_suppliers_list,
            )
        ...
        if my_known_position < 0:
            if _Debug:
                lg.args(
                    _DebugLevel,
                    my_known_position=my_known_position,
                    another_suppliers_list=another_suppliers_list,
                    customer_idurl=self.customer_idurl,
                )
            lg.warn(
                "another supplier is trying to remove me from the family of customer %s"
                % self.customer_idurl
            )
            return p2p_service.SendFail(
                incoming_packet, response=serialization.DictToBytes(self.my_info)
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "    all correct, refresh customer family for %r" % self.customer_idurl,
            )
        self.automat("family-refresh")
        return p2p_service.SendAck(incoming_packet)
    ...
    def _on_supplier_fail(self, response, info):
        ...
        if _Debug:
            lg.args(
                ...
                supplier_idurl=response.OwnerID.to_bin(),
            )
        self.automat(
            "one-supplier-not-agree",
            ecc_map=ecc_map,
            suppliers_list=suppliers_list,
            supplier_idurl=response.OwnerID.to_bin(),
        )
