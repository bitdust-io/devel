...
from transport import callback, packet_in, packet_out
...
def verify():
    ...
    def _on_verified_one(t_result, proto):
        ...
        if _Debug:
            lg.out(
                _DebugLevel - 2,
                "        verified %s transport, result=%r" % (proto, t_result),
            )
...
def inbox(info):
    ...
    if len(data) == 0:
        lg.err(
            "gateway.inbox ERROR zero byte file from %s://%s" % (info.proto, info.host)
        )
        return None
...
def send_file(remote_idurl, proto, host, filename, description="", pkt_out=None):
    ...
    if not is_installed(proto):
        ...
        return False
    filtered = callback.run_file_sending_filter_callbacks(
        remote_idurl, proto, host, filename, description, pkt_out
    )
    if filtered is not None:
        return filtered
    result_defer = transport(proto).call(
        "send_file", remote_idurl, filename, host, description
    )
    callback.run_begin_file_sending_callbacks(
        result_defer, remote_idurl, proto, host, filename, description, pkt_out
    )
    return True
...
def send_file_single(remote_idurl, proto, host, filename, description="", pkt_out=None):
    ...
    if not is_installed(proto):
        ...
        return False
    result_defer = transport(proto).call(
        "send_file_single", remote_idurl, filename, host, description
    )
    callback.run_begin_file_sending_callbacks(
        result_defer, remote_idurl, proto, host, filename, description, pkt_out
    )
...
def find_active_stream(proto, stream_id=None, transfer_id=None):
    ...
    return transport(proto).call(
        "find_stream", stream_id=stream_id, transfer_id=transfer_id
    )
...
def cancel_outbox_file(proto, host, filename, why=None):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "gateway.cancel_outbox_file : %s:%s %s, why: %s"
...
def shutdown_all_inbox_packets():
    if _Debug:
        lg.out(
            _DebugLevel,
            "gateway.shutdown_all_inbox_packets, %d live objects at the moment"
...
def on_identity_received(newpacket, send_ack=True):
    ...
    if not identitycache.HasKey(idurl):
        lg.info(
            "received new identity %s rev %r" % (idurl, newidentity.getRevisionValue())
        )
    if my_id.isLocalIdentityReady():
        if newidentity.getPublicKey() == my_id.getLocalIdentity().getPublicKey():
            if (
                newidentity.getRevisionValue()
                > my_id.getLocalIdentity().getRevisionValue()
            ):
                lg.warn(
                    "received my own identity from another user, but with higher revision number"
                )
                reactor.callLater(
                    0,
                    my_id.rebuildLocalIdentity,
                    new_revision=newidentity.getRevisionValue() + 1,
                )  # @UndefinedVariable
                return False
    ...
    if latest_identity:
        if latest_identity.getRevisionValue() > newidentity.getRevisionValue():
            ...
            lg.warn(
                "received new identity with out-dated revision number %d from %r, known revision is %d"
                % (
                    newidentity.getRevisionValue(),
                    idurl,
    ...
    if not send_ack:
        if _Debug:
            lg.dbg(
                _DebugLevel,
                "%s  idurl=%s  remoteID=%r  skip sending Ack()"
    ...
    if newpacket.OwnerID == idurl:
        if _Debug:
            lg.dbg(
                _DebugLevel,
                "%s  idurl=%s  remoteID=%r  sending wide Ack()"
                % (newpacket.PacketID, idurl, newpacket.RemoteID),
            )
    else:
        if _Debug:
            lg.dbg(
                _DebugLevel,
                "%s  idurl=%s  remoteID=%r  but packet ownerID=%s   sending wide Ack()"
                % (
                    newpacket.PacketID,
                    idurl,
                    newpacket.RemoteID,
...
def on_outbox_packet(
    ...
    if started_packets:
        for active_packet, _ in started_packets:
            ...
            lg.warn(
                "skip creating new outbox packet because found similar pending packet: %r"
                % active_packet
            )
            return active_packet
    pkt_out = packet_out.create(
        outpacket, wide, callbacks, target, route, response_timeout, keep_alive
    )
...
def on_receiving_started(proto, host, options_modified=None):
    if _Debug:
        lg.out(
            _DebugLevel - 2,
            "gateway.on_receiving_started %r host=%r" % (proto.upper(), host),
        )
    transport(proto).automat("receiving-started", (proto, host, options_modified))
    events.send(
        "gateway-receiving-started",
        data=dict(
            proto=proto,
            host=host,
...
def on_receiving_failed(proto, error_code=None):
    if _Debug:
        lg.out(
            _DebugLevel - 2,
            "gateway.on_receiving_failed %s    error=[%s]"
            % (proto.upper(), str(error_code)),
        )
    transport(proto).automat("failed")
    events.send(
        "gateway-receiving-failed",
        data=dict(
            proto=proto,
            error=error_code,
        ),
...
def on_disconnected(proto, result=None):
    if _Debug:
        lg.out(
            _DebugLevel - 2,
            "gateway.on_disconnected %s    result=%s" % (proto.upper(), str(result)),
        )
...
def on_register_file_sending(
    proto, host, receiver_idurl, filename, size=0, description=""
):
    """
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "gateway.on_register_file_sending %s %s to %r"
...
def on_cancelled_file_sending(
    proto, host, filename, size, description="", error_message=None
):
    pkt_out, work_item = packet_out.search(proto, host, filename)
    if pkt_out is None:
        if _Debug:
    ...
    def lookupProcedure(self, procedurePath):
        try:
            ...
            raise xmlrpc.NoSuchFunction(
                self.NOT_FOUND, "procedure %s not found: %s" % (procedurePath, e)
            )
