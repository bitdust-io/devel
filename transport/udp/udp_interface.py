...
class GateInterface:
    ...
    def send_file_single(self, remote_idurl, filename, host, description=""):
        ...
        return self.send_file(
            self, remote_idurl, filename, host, description, keep_alive=False
        )
            ...
            while i < len(sess.file_queue.outboxQueue):
                ...
                if fn == filename:
                    if _Debug:
                        lg.out(
                            14,
                            "udp_interface.cancel_outbox_file removed %s in %s"
...
def interface_transport_initialized():
    if proxy():
        return (
            proxy().callRemote("transport_initialized", "udp").addErrback(proxy_errback)
        )
...
def interface_receiving_started(host, new_options={}):
    if proxy():
        return (
            proxy()
            .callRemote("receiving_started", "udp", host, new_options)
...
def interface_receiving_failed(error_code=None):
    if proxy():
        return (
            proxy()
            .callRemote("receiving_failed", "udp", error_code)
            .addErrback(proxy_errback)
        )
    lg.warn("transport_udp is not ready")
    return fail(Exception("transport_udp is not ready")).addErrback(proxy_errback)
...
def interface_register_file_receiving(host, sender_idurl, filename, size):
    if proxy():
        return (
            proxy()
            .callRemote(
                "register_file_receiving", "udp", host, sender_idurl, filename, size
            )
            .addErrback(proxy_errback)
        )
    lg.warn("transport_udp is not ready")
...
def interface_unregister_file_sending(
    transfer_id, status, bytes_sent, error_message=None
):
    if proxy():
        return (
            proxy()
...
def interface_unregister_file_receiving(
    transfer_id, status, bytes_received, error_message=None
):
    if proxy():
        return (
            proxy()
...
def interface_cancelled_file_sending(
    host, filename, size, description=None, error_message=None
):
    if proxy():
        return (
            proxy()
...
def interface_cancelled_file_receiving(host, filename, size, error_message=None):
    if proxy():
        return (
            proxy()
            .callRemote(
                "cancelled_file_receiving", "udp", host, filename, size, error_message
            )
            .addErrback(proxy_errback)
        )
    lg.warn("transport_udp is not ready")
