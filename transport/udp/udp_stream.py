        ...
        if self.limit_send_bytes_per_sec > 0:
            ...
            if relative_time > 0.0:
                ...
                if current_rate > self.limit_send_bytes_per_sec:
                    if _Debug:
                        lg.out(
                            self.debug_level, 'SKIP BLOCK LIMIT SENDING %d : %r>%r' %
...
#            return
        ...
        #--- decide to send the block now
            if time_sent == - \
                    1 or timed_out or (acks_missed > 2 and ((acks_missed % 3) == 0)):
    ...
    def doSendingLoop(self, arg):
        ...
        if self.loop is None:
            next_iteration = min(
                MAX_BLOCKS_INTERVAL,
                self._rtt_current() *
                self.resend_inactivity_counter)
            self.loop = reactor.callLater(
                next_iteration, self.automat, 'iterate')
            return
        if self.loop.called:
            next_iteration = min(
                MAX_BLOCKS_INTERVAL,
        ...
        if lg.is_debug(self.debug_level):
            ...
            if relative_time - self.last_progress_report > 1.0:
                ...
                if _Debug:
                    lg.out(
                        self.debug_level,
                        'udp_stream[%d] | %r%% received | %d/%d/%d | %dbps | %r sec dt | from %s' %
                        (self.stream_id,
                         round(
                             100.0 *
                             (
                            ...
                            self.consumer.size,
                            int(
                             self.current_receive_bytes_per_sec),
                            round(
                             relative_time -
                             self.last_received_block_time,
                             4),
                            self.producer.session.peer_id))
        if self.loop is None:
            next_iteration = min(
                MAX_ACKS_INTERVAL, max(
                    RTT_MIN_LIMIT / 2.0, self._block_period_avarage() * self.resend_inactivity_counter))
            self.loop = reactor.callLater(
                next_iteration, self.automat, 'iterate')
            return
        if self.loop.called:
...
#            reactor.callLater(0, self.automat, 'iterate')
    ...
    def on_block_received(self, inpt):
        if self.consumer and getattr(
            self.consumer,
            'on_received_raw_data',
                None):
            ...
            if block_id != -1:
                ...
                if _Debug:
                    lg.out(
                        self.debug_level + 6, 'in-> BLOCK %d %r EMPTY %d %d' %
            ...
            try:
                ...
                if len(acks) > 0:
                    ...
                    if pause_time == 0.0 and eof_flag is not None and eof_flag:
                        ...
                        if _Debug:
                            lg.out(
                                self.debug_level, '    ZERO FINISH %d eof:%r acked:%d tail:%d' %
                ...
                for block_id in acks:
                    try:
                        ...
                        if _Debug:
                            lg.out(
                                self.debug_level + 6, '    ack received but block %d not found, stream_id=%d' %
    ...
    def _send_blocks(self, blocks_to_send):
        ...
        if new_blocks_counter == 0:
            if relative_time - self.last_block_sent_time > MAX_BLOCKS_INTERVAL:
                ...
                self.producer.do_send_data(
                    self.stream_id, self.consumer, output)
                self.output_blocks_counter += 1
                if _Debug:
                    lg.out(
