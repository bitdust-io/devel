...
def balance_streams_limits():
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "udp_stream.balance_streams_limits in:%r out:%r avarage:%r streams:%d"
            % (
...
class UDPStream(automat.Automat):
    ...
    def __init__(self, stream_id, consumer, producer):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "udp_stream.__init__ %d peer_id:%s session:%s"
    ...
    def A(self, event, *args, **kwargs):
        ...
        if self.state == "SENDING":
            if event == "iterate":
                ...
                event == "ack-received"
                and not self.isEOF(*args, **kwargs)
                and not self.isPaused(*args, **kwargs)
            ):
                self.doResendBlocks(*args, **kwargs)
                ...
                event == "ack-received" and self.isPaused(*args, **kwargs)
            ):
    ...
    def doInit(self, *args, **kwargs):
        ...
        self.output_limit_bytes_per_sec = get_global_output_limit_bytes_per_sec() / len(
            streams()
        )
        self.input_limit_bytes_per_sec = get_global_input_limit_bytes_per_sec() / len(
            streams()
        )
    ...
    def doReportSendDone(self, *args, **kwargs):
        ...
        if _Debug:
            lg.out(
                self.debug_level,
                "udp_stream.doReportSendDone %r %r"
    ...
    def doCloseStream(self, *args, **kwargs):
        ...
        if _Debug:
            ...
            lg.out(
                self.debug_level,
                "udp_stream.doCloseStream %d %s" % (self.stream_id, pir_id),
            )
            lg.out(
                self.debug_level,
                "    in:%d|%d|%r acks:%d|%d dups:%d|%d out:%d|%d|%d|%d rate:%r|%r"
                % (
    ...
    def doUpdateLimits(self, *args, **kwargs):
        ...
        if _Debug:
            lg.out(
                self.debug_level + 6,
                "udp_stream[%d].doUpdateLimits in=%r out=%r (remote=%r)"
                % (
                    self.stream_id,
                    self.input_limit_bytes_per_sec,
                    self.output_limit_bytes_per_sec,
        ...
        if block_id != -1:
            ...
            if eof and not self.eof:
                ...
                if _Debug:
                    lg.out(
                        self.debug_level,
                        "    EOF flag set !!!!!!!! : %d" % self.stream_id,
                    )
            if _Debug:
                lg.out(
                    self.debug_level + 6,
                    "in-> BLOCK %d %r %d-%d %d %d %d"
                    % (
                        self.stream_id,
                        self.eof,
                        block_id,
    ...
    def on_ack_received(self, inpt):
        ...
        if pause_time == 0.0 and eof_flag:
            ...
            if _Debug:
                sum_not_acked_blocks = sum(
                    [len(block[0]) for block in list(self.output_blocks.values())]
                )
                try:
                ...
                lg.out(
                    self.debug_level,
                    "    EOF state found in ACK %d acked:%d not acked:%d total:%d"
        ...
        for block_id in acks:
            ...
            if (
                block_id not in self.output_blocks_ids
                or block_id not in self.output_blocks
            ):
                ...
                if _Debug:
                    lg.out(
                        self.debug_level + 6,
                        "    GARBAGE ACK, block %d not found, stream_id=%d"
        ...
        if not self.eof and eof:
            ...
            if _Debug:
                lg.out(
                    self.debug_level,
                    "    in-> ACK %d : EOF RICHED !!!!!!!!" % self.stream_id,
                )
        if eof_flag is None and len(acks) == 0 and pause_time == 0.0:
            ...
            if _Debug:
                lg.out(
                    self.debug_level,
                    "    in-> ACK %d : REMOTE SIDE CLOSED STREAM !!!!!!!!"
                    % self.stream_id,
                )
        if _Debug:
            try:
    ...
    def on_consume(self, data):
        if self.consumer:
            ...
            if self.output_quality_counter > BLOCKS_PER_ACK * WINDOW_SIZE:
                error_rate = float(self.output_blocks_errors_counter) / (
                    self.output_quality_counter
                )
    ...
    def on_close(self):
        if _Debug:
            lg.out(
                self.debug_level,
                'udp_stream.UDPStream[%d].on_close, send "close" event to the stream, state=%s'
    ...
    def _sending_loop(self):
        ...
        if lg.is_debug(self.debug_level):
            if (
                self.output_quality_counter
                and relative_time - self.last_progress_report > POOLING_INTERVAL * 50.0
            ):
                if _Debug:
                    lg.out(
                        self.debug_level,
                        "udp_stream[%d]|%d/%r%%|garb.:%d/%d|err.:%r%%/%r%%|%rbps|pkt:%d/%d|RTT:%r|lag:%d|last:%r/%r|buf:%d|N:%d"
                        % (
                            self.stream_id,
                            ...
                            round(
                                100.0
                                * (float(self.output_bytes_acked) / self.consumer.size),
                                2,
                            ),
                            ...
                            round(
                                100.0
                                * (
                                    self.output_blocks_success_counter
                                    / self.output_quality_counter
                                ),
                                2,
                            ),
        ...
        if lg.is_debug(self.debug_level):
            ...
            if relative_time - self.last_progress_report > POOLING_INTERVAL * 50.0:
                if _Debug:
                    lg.out(
                        self.debug_level,
                        "udp_stream[%d] | %d/%r%% | garb.:%d/%d/%r%% | %d bps | b.:%d/%d | pkt.:%d/%d | last: %r | buf: %d | N:%d"
                        % (
                            self.stream_id,
                            # --- percent received
                            self.input_block_id_current,
                            round(
                                100.0
                                * (
                                    float(self.consumer.bytes_received)
                                    / self.consumer.size
                                ),
                                2,
                            ),
                            ...
                            round(
                                ...
                                * (
                                    float(
                                        self.input_duplicated_blocks
                                        + self.input_old_blocks
                                    )
                                    / float(self.input_block_id_current + 1)
                                ),
                                2,
                            ),
                            # --- receiving speed
    ...
    def _resend_blocks(self):
        ...
        if current_limit > 0 and relative_time > 0:
            ...
            if (
                current_rate > current_limit
                and last_block_sent_delta < RTT_MAX_LIMIT / 2.0
            ):
        ...
        if self.state == "SENDING" or self.state == "PAUSE":
            sending_was_limited = (
                relative_time - self.output_limit_iteration_last_time < SENDING_TIMEOUT
            )
            input_ack_timed_out = (
                relative_time - self.input_ack_last_time > SENDING_TIMEOUT
            )
        ...
        if self.input_acks_counter > 0:
            if last_block_sent_delta < RTT_MAX_LIMIT:
                current_out_in_rate = self.output_blocks_counter / float(
                    self.input_acks_counter
                )
                if current_out_in_rate > float(BLOCKS_PER_ACK) * 1.5:
                    ...
                    if _Debug:
                        lg.out(
                            self.debug_level + 6,
                            "SKIP SENDING %d, too few acks:%d blocks:%d"
                            % (
                                self.stream_id,
                                self.input_acks_counter,
                                self.output_blocks_counter,
        ...
        if last_block_sent_delta < RTT_MAX_LIMIT:
            ...
            possible_out_in_ratio = float(possible_blocks_out) / float(
                self.input_acks_counter
            )
            if possible_out_in_ratio > float(BLOCKS_PER_ACK) * (
                1.0 + ACCEPTABLE_ERRORS_RATE
            ):
        ...
        for block_id in blocks_not_acked:
            ...
            if relative_time - time_sent < block_position * rtt_current:
                continue
            error_rate = float(self.output_blocks_errors_counter + 1) / (
                self.output_quality_counter + 1.0
            )
            if error_rate > ACCEPTABLE_ERRORS_RATE:
        ...
        for block_id in blocks_not_acked:
            ...
            if missed_acks < int(WINDOW_SIZE):
                continue
            error_rate = float(self.output_blocks_errors_counter + 1) / float(
                self.output_quality_counter + 1.0
            )
            if error_rate > ACCEPTABLE_ERRORS_RATE:
                too_much_errors = True
        ...
        for block_id in blocks_not_acked:
            ...
            if self.output_error_last_time < RTT_MAX_LIMIT:
                if self.output_quality_counter > BLOCKS_PER_ACK:
                    error_rate = float(self.output_blocks_errors_counter) / float(
                        self.output_quality_counter
                    )
                    if error_rate > ACCEPTABLE_ERRORS_RATE:
                        too_much_errors = True
        ...
        if len(blocks_to_send_now) > 0:
            ...
            return
        if (
            last_block_sent_delta > RECEIVING_TIMEOUT / 3.0
            and len(self.output_blocks_ids) > 0
        ):
    ...
    def _resend_ack(self):
        ...
        if self.input_limit_bytes_per_sec > 0 and relative_time > 0:
            ...
            if self.input_bytes_received > max_receive_available:
                ...
                pause_time = (
                    self.input_bytes_received - max_receive_available
                ) / self.input_limit_bytes_per_sec
                if pause_time < 0:
        ...
        receiving_was_limited = (
            relative_time - self.input_limit_iteration_last_time < RECEIVING_TIMEOUT
        )
        input_block_timed_out = (
            relative_time - self.input_block_last_time > RECEIVING_TIMEOUT
        )
        if not receiving_was_limited and input_block_timed_out:
            ...
            if _Debug:
                lg.out(
                    self.debug_level,
                    "TIMEOUT RECEIVING %d rtt=%r, last block:%r, last limit:%r reltime:%r, eof:%r, blocks to ack:%d"
                    % (
                        self.stream_id,
                        self._rtt_current(),
                        round(relative_time - self.input_block_last_time, 4),
