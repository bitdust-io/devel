...
class FileQueue:
    ...
    def report_failed_outbox_queue(self, error_message):
        for filename, description, result_defer, keep_alive in self.outboxQueue:
            self.on_failed_outbox_queue_item(
                filename, description, error_message, result_defer, keep_alive
            )
    ...
    def do_send_data(self, stream_id, outfile, output):
        ...
        #                 return True
        newoutput = "".join(
            (struct.pack("i", stream_id), struct.pack("i", outfile.size), output)
        )
    ...
    def append_outbox_file(
        self, filename, description="", result_defer=None, keep_alive=True
    ):
    ...
    def process_outbox_queue(self):
        ...
        while len(self.outboxQueue) > 0:
            ...
            if not os.path.isfile(filename):
                self.on_failed_outbox_queue_item(
                    filename, description, "file not exist", result_defer, keep_alive
                )
                continue
            try:
            ...
            self.start_outbox_file(
                filename, filesize, description, result_defer, keep_alive
            )
        return has_reads
    ...
    def start_outbox_file(
        self, filename, filesize, description, result_defer, keep_alive
    ):
        from transport.udp import udp_interface, udp_stream
    ...
    def start_inbox_file(self, stream_id, data_size):
        ...
        if _Debug:
            lg.out(
                12,
                "udp_file_queue.start_inbox_file %d %d %s"
        ...
        d = udp_interface.interface_register_file_receiving(
            self.session.peer_id,
            self.session.peer_idurl,
            self.inboxFiles[stream_id].filename,
            self.inboxFiles[stream_id].size,
        )
    ...
    def report_inbox_file(self, infile):
        ...
        if _Debug:
            lg.out(
                ...
                % (
                    os.path.basename(infile.filename),
                    infile.transfer_id,
                    infile.status,
                    infile.bytes_received,
    ...
    def on_received_data_packet(self, payload):
        ...
        if stream_id not in list(self.streams.keys()):
            ...
            if len(self.streams) >= 2 * MAX_SIMULTANEOUS_STREAMS_PER_SESSION:
                ...
                if _Debug:
                    lg.warn(
                        "SEND ZERO ACK, too many active streams: %d  skipped: %s %s"
    ...
    def on_inbox_file_registered(self, response, stream_id):
        ...
        if _Debug:
            lg.out(
                18,
                "udp_file_queue.on_inbox_file_registered %d transfer_id=%r status=%s"
    ...
    def on_inbox_file_register_failed(self, err, stream_id):
        lg.out(
            2,
            "udp_file_queue.on_inbox_file_register_failed ERROR failed to register, stream_id=%s, err: %s"
            % (str(stream_id), err),
        )
        self.session.automat("shutdown")
    ...
    def on_outbox_file_registered(self, response, stream_id):
        ...
        if _Debug:
            lg.out(
                18,
                "udp_file_queue.on_outbox_file_registered %d transfer_id=%r status=%s"
    ...
    def on_outbox_file_register_failed(self, err, stream_id):
        lg.out(
            2,
            "udp_file_queue.on_outbox_file_register_failed ERROR failed to register, stream_id=%s :\n%s"
    ...
    def on_failed_outbox_queue_item(
        ...
        if _Debug:
            lg.out(
                18,
                "udp_file_queue.failed_outbox_queue_item %s because %s"
    ...
    def on_timeout_receiving(self, stream_id):
        ...
        if _Debug:
            lg.out(
                18,
                "udp_file_queue.on_timeout_receiving stream_id=%s %d : %s"
...
class InboxFile:
        ...
        if _Debug:
            lg.out(
                ...
                % (
                    ...
                    self.stream_id,
                    str(self.queue.session.peer_address),
                    self.size,
                ),
            )
    ...
    def close(self):
        if _Debug:
            lg.out(
                18,
                "udp_file_queue.InboxFile.close %d : %d received"
...
class OutboxFile:
    ...
    def __del__(self):
        ...
        if _Debug:
            lg.out(
                18,
                "udp_file_queue.OutboxFile.__del__ {%s} [%d] file:%r"
    ...
    def close(self):
        if _Debug:
            lg.out(
                18,
                "udp_file_queue.OutboxFile.close %s %d/%d"
    ...
    def cancel(self):
        if _Debug:
            lg.out(
                18,
                "udp_file_queue.OutboxFile.cancel %s %d/%d"
                % (self.stream_id, self.bytes_sent, self.bytes_delivered),
            )
        self.cancelled = True
        self.status = "failed"
            ...
            if not self.buffer:
                ...
                if not data:
                    if _Debug:
                        lg.out(
                            18,
                            "udp_file_queue.OutboxFile.process reach EOF state %d"
                            % self.stream_id,
                        )
                    self.eof = True
                    break
