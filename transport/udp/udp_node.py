...
from transport.udp import udp_connector, udp_session
...
class UDPNode(automat.Automat):
    ...
    def A(self, event, *args, **kwargs):
        ...
        if self.state == "LISTEN":
            if (
                ...
                event == "connect"
                and self.isKnowMyAddress(*args, **kwargs)
                and not self.isKnownUser(*args, **kwargs)
            ):
    ...
    def isKnownUser(self, *args, **kwargs):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "udp_node.isKnownUser %s not found in %s"
    ...
    def doStartStunClient(self, *args, **kwargs):
        ...
        if driver.is_on("service_my_ip_port"):
            ...
            self.automat(
                "stun-success",
                ("unknown", misc.readExternalIP() or "127.0.0.1", settings.getUDPPort()),
            )
    ...
    def doDisconnect(self, *args, **kwargs):
        ...
        lg.out(
            ...
            % (
                len(list(udp_session.sessions().values())),
                len(list(udp_connector.connectors().values())),
            ),
        )
    ...
    def _got_my_address(self, value, key):
        ...
        try:
            ...
            if _Debug:
                lg.out(
                    4,
                    "udp_node._got_my_address ERROR   wrong key in response: %r" % value,
                )
                lg.exc()
            self.automat("dht-write-failed")
            return
    ...
    def _got_my_incoming(self, value, key, position):
        if not isinstance(value, dict):
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "udp_node._got_my_incoming no incoming at position: %d" % position,
                )
    ...
    def _failed_my_incoming(self, err, key, position):
        if _Debug:
            lg.out(
                _DebugLevel,
                "udp_node._got_my_incoming incoming empty: %s" % str(position),
            )
        self.automat("dht-read-result", None)
