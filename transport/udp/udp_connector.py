...
class DHTUDPConnector(automat.Automat):
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---DHT_LOOP---
            ...
            if event == "dht-read-failed":
                ...
                event == "dht-read-success"
                and self.KeyPosition < 10
                and not self.isMyIncoming(*args, **kwargs)
            ):
                self.KeyPosition += 1
                self.doDHTReadIncoming(*args, **kwargs)
    ...
    def _got_peer_incoming(self, value, key, position):
        if _Debug:
            lg.out(
                _DebugLevel,
                "udp_connector._got_peer_incoming at position %d: %d"
        ...
        try:
            ...
            incoming_user_address = (
                incoming_user_address[0],
                int(incoming_user_address[1]),
            )
        except:
            lg.out(2, "%r" % incoming)
            lg.exc()
    ...
    def _got_peer_address(self, value, key):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "udp_connector._got_peer_address %s:%d ~ %s"
