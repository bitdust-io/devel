...
def run_inbox_callbacks(newpacket, info, status, error_message):
    ...
    if _Debug:
        lg.out(
            _DebugLevel, "callback.run_inbox_callbacks for %s from %s" % (newpacket, info)
        )
    handled = False
...
def run_file_sending_filter_callbacks(
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "callback.run_file_sending_filter_callbacks %s to %s:%s at %s"
...
def run_begin_file_sending_callbacks(
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "callback.run_begin_file_sending_callbacks %s to %s:%s at %s"
            % (description, proto, host, remote_idurl),
        )
    handled = False
