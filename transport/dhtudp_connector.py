...
"""
.. module:: dhtudp_connector
.. role:: red
BitPie.NET dhtudp_connector() Automat
...
def create(node, peer_id):
    """
    """
    dhnio.Dprint(12, 'dhtudp_connector.create peer_id=%s' % peer_id)
...
class DHTUDPConnector(automat.Automat):
    """
    This class implements all the functionality of the ``dhtudp_connector()`` state machine.
    """
    ...
    def __init__(self, node, peer_id):
        ...
        name = 'dhtudp_connector[%s]' % self.peer_id
    ...
    def doStartNewSession(self, arg):
        ...
        Action method.
        """
        # dhnio.Dprint(10, 'dhtudp_connector.doStartNewSession wants to start a new session')
        peer_address = arg
        s = dhtudp_session.get(peer_address)
        if s:
            return
        s = dhtudp_session.create(self.node, peer_address, self.peer_id)
    ...
    def _got_peer_address(self, value):
        # dhnio.Dprint(18, 'dhtudp_connector._got_peer_address  %r' % value)
        if type(value) != dict:
