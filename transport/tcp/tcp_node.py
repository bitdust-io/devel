...
def receive(options):
    ...
    try:
        _Listener = reactor.listenTCP(
            _InternalPort, TCPFactory(None, keep_alive=True)
        )  # @UndefinedVariable
...
def connect_to(host, keep_alive=True):
    ...
    for peeraddr, connections in opened_connections().items():
        if peeraddr == host:
            lg.warn(
                'already connected to "%s" with %d connections' % (host, len(connections))
            )
            return True
    ...
    if _Debug:
        lg.out(
            _DebugLevel, 'tcp_node.connect_to "%s", keep_alive=%s' % (host, keep_alive)
        )
    ...
    connection.connector = reactor.connectTCP(
        host[0], host[1], connection, timeout=_ConnectionTimeout
    )  # @UndefinedVariable
    return False
...
def send(filename, remoteaddress, description=None, keep_alive=True):
    ...
    connection.connector = reactor.connectTCP(
        remoteaddress[0], remoteaddress[1], connection, timeout=_ConnectionTimeout
    )  # @UndefinedVariable
    if not keep_alive:
        if _Debug:
            lg.out(
    ...
    for connections in opened_connections().values():
        for connection in connections:
            if (
                ...
                and connection.peer_address == host
                or connection.peer_external_address
                and connection.peer_external_address == host
            ):
                i = 0
