...
def make_stream_id():
    ...
    _StreamCounter += 1
    return (
        random.randint(10, 99) * 1000000
        + random.randint(10, 99) * 10000
        + _StreamCounter % 10000
    )
...
class TCPFileStream:
    ...
    def abort_files(self, reason="connection closed"):
        ...
        if _Debug:
            lg.args(
                _DebugLevel,
                reason=reason,
                inbox_file_ids_to_remove=inbox_file_ids_to_remove,
                outbox_file_ids_to_remove=outbox_file_ids_to_remove,
            )
        ...
        for file_id in outbox_file_ids_to_remove:
            if self.sender.is_sending(file_id):
                ...
                if reason == "disconnecting":
                    self.send_data(
                        tcp_connection.CMD_ABORT,
                        struct.pack("i", file_id) + b" " + strng.to_bin(reason),
                    )
    ...
    def data_received(self, payload):
        ...
        if file_id not in self.inboxFiles:
            if len(self.inboxFiles) >= 2 * MAX_SIMULTANEOUS_OUTGOING_FILES:
                ...
                # that session!
                lg.warn(
                    "too many incoming files, close connection %s" % str(self.connection)
                )
    ...
    def ok_received(self, payload):
        ...
        if file_id not in self.outboxFiles:
            lg.warn(
                "did not found %r in outboxFiles: %r" % (file_id, self.outboxFiles.keys())
            )
            return
    ...
    def on_inbox_file_register_failed(self, err, file_id):
        lg.warn(
            "failed to register file_id=%r session=%r err: %s"
            % (file_id, self.session, str(err))
        )
        self.connection.automat("disconnect")
    ...
    def create_outbox_file(
        ...
        outfile = OutboxFile(
            self, filename, file_id, filesize, description, result_defer, keep_alive
        )
        if keep_alive:
            d = tcp_interface.interface_register_file_sending(
                self.connection.getAddress(),
                self.connection.peer_idurl,
                filename,
                description,
            )
    ...
    def on_outbox_file_register_failed(self, err, file_id):
        lg.warn(
            "failed to register file_id=%r connection=%r err: %s"
            % (str(file_id), self.connection, str(err))
        )
        self.connection.automat("disconnect")
    ...
    def outbox_file_done(self, file_id, status, error_message=None):
        ...
        if outfile.transfer_id:
            self.report_outbox_file(
                outfile.transfer_id, status, outfile.get_bytes_sent(), error_message
            )
...
class InboxFile:
    def __init__(self, stream, file_id, file_size):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "<<<TCP-IN %s with %d bytes write to %s"
    ...
    def close(self):
        if _Debug:
            lg.out(
                _DebugLevel,
                "<<<TCP-IN %s CLOSED with %s | %s"
                % (
                    self.file_id,
                    self.stream.connection.peer_address,
                    self.stream.connection.peer_external_address,
                ),
            )
        if self.fin:
            os.close(self.fin)
...
class OutboxFile:
    def __init__(
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                ">>>TCP-OUT %s with %d bytes reading from %s"
                % (self.file_id, self.size, self.filename),
            )
    ...
    def start(self):
        d = self.stream.sender.startFileTransfer(
            self.file_id, self.fout, self.send_chunk, self.transform_data
        )
...
class MultipleFilesSender:
    ...
    def startFileTransfer(self, file_id, file_object, writer, transform):
        ...
        if _Debug:
            lg.args(
                ...
                file_id,
                file_object,
                [fid for fid in self.active_files.keys()],
            )
    ...
    def stopFileTransfer(self, file_id, reason="cancelled"):
        ...
        deferred, _, _, _ = self.active_files.pop(
            file_id,
            (
                None,
                None,
    ...
    def resumeProducing(self):
        ...
        for file_id in self.active_files.keys():
            deferred, file_object, writer, transform = self.active_files.get(
                file_id,
                (
                    None,
                    None,
