...
def proxy(instance=None):
    ...
    if instance is False:
        if _Debug:
            lg.out(
                4,
                "tcp_interface.proxy killing existing gate instance: %d" % id(_GateProxy),
            )
        _GateProxy = None
        return None
    if instance is not None:
        ...
        if _Debug:
            lg.out(
                4, "tcp_interface.proxy created new gate instance: %d" % id(_GateProxy)
            )
    return _GateProxy
...
class GateInterface:
    ...
    def send_file(self, remote_idurl, filename, host, description=""):
        """ """
        return tcp_node.send(
            filename, net_misc.normalize_address(host), description, keep_alive=True
        )

    def send_file_single(self, remote_idurl, filename, host, description=""):
        """ """
        return tcp_node.send(
            filename, net_misc.normalize_address(host), description, keep_alive=False
        )
    ...
    def find_session(self, host=None, idurl=None):
        ...
        if _Debug:
            lg.dbg(
                _DebugLevel,
                "looking for %r, known connections: %r"
                % (
                    search_for_host,
...
def interface_register_file_sending(
    host, receiver_idurl, filename, size=0, description=""
):
    if proxy():
        return (
            proxy()
...
def interface_cancelled_file_sending(
    host, filename, size=0, description=None, error_message=None
):
    if proxy():
        return (
            proxy()
...
def main():
    ...
    if len(sys.argv) >= 5 and sys.argv[1] == "send":
        tcp_node.send(
            sys.argv[2], net_misc.normalize_address((sys.argv[3], int(sys.argv[4])))
        )
        reactor.run()  # @UndefinedVariable
        return
