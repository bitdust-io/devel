...
class FileStream:
    ...
    def check_sliding_window(self):
        ...
        for block_info, tm in self.sliding_window.items():
            ...
            if not self.outboxFiles.has_key(file_id):
                # dhnio.Dprint(2, 'dhtudp_stream.checkWindow WARNING unknown file_id [%d] from %s' % (file_id, nameurl.GetName(self.remote_idurl)))
                continue
            if not self.outboxFiles[file_id].blocks.has_key(block_id):
                dhnio.Dprint(2, 'dhtudp_stream.checkWindow WARNING unknown block_id: %d' % block_id)
                continue
        ...
        for block_info in to_remove:
            ...
            # dhnio.Dprint(8, 'dhtudp_stream.doCheckWindow (%d,%d) removed, window=%d' % (file_id, block_id, len(self.sliding_window)))
        for block_info, tm in to_update.items():
            self.sliding_window[block_info] = tm
            # dhnio.Dprint(8, 'dhtudp_stream.doCheckWindow (%s) resending to %s, window=%d' % (block_info, self.remote_address, len(self.sliding_window)))
        if len(to_remove) > 0:
            self.sliding_window_size = int( self.sliding_window_size / 2.0 )
            if self.sliding_window_size < MIN_WINDOW_SIZE:
                self.sliding_window_size = MIN_WINDOW_SIZE
            # dhnio.Dprint(8, 'dhtudp_stream.doCheckWindow decreased, window=%d' % self.sliding_window_size)
        del to_remove
        del to_update
        return 
    ...
    def adjust_ack_timeout(self, timeouts_ratio):
        if timeouts_ratio >= MAX_TIMEOUTS_RATIO:
            ...
            if self.ack_timeout > MAX_ACK_TIME_OUT:
                ...
                dhnio.Dprint(6, 'dhtudp_stream.data_received NETERROR Ack timeout, close session %s' % str(self.session))
    ...
    def timeout_incoming_files(self):
        for file_id in self.inboxFiles.keys():
            if self.inboxFiles[file_id].is_timed_out():
                dhnio.Dprint(6, 'dhtudp_stream.data_received WARNING inbox file is timed out, close session %s' % str(self.session))
                self.session.automat('shutdown')
                return True
        return False
        ...
        if not self.inboxFiles.has_key(file_id):
            if self.receivedFiles.has_key(file_id):
                if time.time() - self.receivedFiles[file_id] > 30:
                    ...
                    dhnio.Dprint(6, 'dhtudp_stream.data_received NETERROR too old Ack packets, drop session %s' % str(self.session)) 
                    self.session.automat('shutdown')
                    return
            ...
            if len(self.inboxFiles) >= 2 * MAX_SIMULTANEOUS_OUTGOING_FILES:
                ...
                dhnio.Dprint(6, 'dhtudp_stream.data_received WARNING too many incoming files, close session %s' % str(self.session))
                self.session.automat('shutdown') 
                return
            self.create_inbox_file(file_id, num_blocks)
        if len(inp_data) != data_size:
            dhnio.Dprint(6, 'dhtudp_stream.data_received WARNING incorrect datagram received from %s, not full data' % str(self.session.peer_address))
            dhnio.Dprint(6, 'dhtudp_stream.data_received close session %s' % str(self.session))
            self.session.automat('shutdown') 
            return
    ...
    def ack_received(self, payload):
        ...
        if not self.outboxFiles.has_key(file_id):
            ...
            #     dhnio.Dprint(8, 'dhtudp_stream.ack_received WARNING unknown file_id in REPORT packet received from %s: [%d]' % (self.remote_address, file_id))
            return
        try:
            block_id = struct.unpack('i', inp.read(4))[0]
        except:
        ...
        # dhnio.Dprint(8, 'dhtudp_stream.doReceiveData REPORT on [%d,%d] received, blocks=%d, window=%d' % (file_id, block_id, len(self.outboxFiles[file_id]), self.sliding_window_size))
...
#                    infile.transfer_id, 'finished', infile.get_bytes_received())
    ...
    def on_inbox_file_register_failed(self, err, file_id):
        dhnio.Dprint(2, 'dhtudp_stream.on_inbox_file_register_failed ERROR failed to register, file_id=%s' % (str(file_id)))
        dhnio.Dprint(6, 'dhtudp_stream.on_inbox_file_register_failed close session %s' % self.session)
        self.session.automat('shutdown')
    ...
    def on_outbox_file_register_failed(self, err, file_id):
        dhnio.Dprint(2, 'dhtudp_stream.on_outbox_file_register_failed ERROR failed to register, file_id=%s :\n%s' % (str(file_id), str(err)))
        dhnio.Dprint(6, 'dhtudp_stream.on_outbox_file_register_failed close session %s' % self.session)
        self.session.automat('shutdown')
...
#                infile.transfer_id, status, infile.get_bytes_received(), error_message)
            ...
            dhnio.Dprint(6, 'dhtudp_stream.file_received WARNING transfer_id is None, file_id=%s' % (str(file_id)))
        self.close_inbox_file(file_id)
        self.receivedFiles[file_id] = time.time()
        self.erase_old_file_ids()

    def outbox_file_done(self, file_id, status, error_message=None):
        dhnio.Dprint(18, 'dhtudp_stream.outbox_file_done %s %s because %s' % (file_id, status, error_message))
        try:
            outfile = self.outboxFiles[file_id]
        except:
            dhnio.DprintException()
    ...
    def failed_outbox_queue_item(self, filename, description='', error_message='', result_defer=None, single=False):
        dhnio.Dprint(18, 'dhtudp_stream.failed_outbox_queue_item %s because %s' % (filename, error_message))
        if not single:
    ...
    def report_outbox_file(self, transfer_id, status, bytes_sent, error_message=None):    
        dhnio.Dprint(18, 'dhtudp_stream.report_outbox_file %s %s %d' % (transfer_id, status, bytes_sent))
    ...
    def report_inbox_file(self, transfer_id, status, bytes_received, error_message=None):
        dhnio.Dprint(18, 'dhtudp_stream.report_inbox_file %s %s %d' % (transfer_id, status, bytes_received))
        dhtudp_interface.interface_unregister_file_receiving(
...
class InboxFile():
    def __init__(self, stream, file_id, num_blocks):
        ...
        # dhnio.Dprint(6, 'dhtudp_stream.InboxFile {%s} [%d] from %s' % (self.transfer_id, self.file_id, str(self.stream.remote_address)))

    def close(self):
        try:
            if self.bytes_received > 0 and self.bytes_extra > self.bytes_received * 0.1:
                dhnio.Dprint(10, 'dhtudp_stream.InboxFile.close WARNING %s%% garbage traffic from %s' % (
...
class OutboxFile():
    ...
    def cancel(self):
        dhnio.Dprint(6, 'dhtudp_stream.OutboxFile.cancel timeout=%d' % self.timeout)
        self.timeout = 0
    ...
    def report_block(self, block_id):
        if not self.blocks.has_key(block_id):
            dhnio.Dprint(10, 'dhtudp_stream.report_block WARNING unknown block_id from %s: [%d]' % (str(self.stream.session.peer_address), block_id))
            return
    ...
    def send_block(self, block_id):
        ...
        if not self.blocks.has_key(block_id):
            dhnio.Dprint(8, 'dhtudp_stream.send_block WARNING block_id=%d not found, file_id=%s, transfer_id=%s, blocks: %d' % (
