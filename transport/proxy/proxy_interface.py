...
class GateInterface:
    def init(self, xml_rpc_url_or_object):
        ...
        from transport.proxy import proxy_receiver, proxy_sender
    ...
    def build_contacts(self, id_obj):
        ...
        if _Debug:
            lg.out(
                4,
                "proxy_interface.build_contacts %s : %r"
    ...
    def verify_contacts(self, id_obj):
        ...
        if (
            not proxy_receiver.A()
            or not proxy_receiver.GetRouterIDURL()
...
def interface_transport_initialized(xmlrpcurl):
    if proxy():
        return (
            proxy()
            .callRemote("transport_initialized", "proxy", xmlrpcurl)
...
def interface_receiving_started(host, new_options={}):
    if proxy():
        return (
            proxy()
            .callRemote("receiving_started", "proxy", host, new_options)
            .addErrback(proxy_errback)
        )
    lg.warn("transport_proxy is not ready")
    return fail(Exception("transport_proxy is not ready")).addErrback(proxy_errback)
...
def interface_receiving_failed(error_code=None):
    if proxy():
        return (
            proxy()
            .callRemote("receiving_failed", "proxy", error_code)
            .addErrback(proxy_errback)
        )
    lg.warn("transport_proxy is not ready")
    return fail(Exception("transport_proxy is not ready")).addErrback(proxy_errback)
...
def interface_disconnected(result=None):
    if proxy():
        return (
            proxy().callRemote("disconnected", "proxy", result).addErrback(proxy_errback)
        )
    lg.warn("transport_proxy is not ready")
    # return fail(Exception('transport_proxy is not ready')).addErrback(proxy_errback)
