...
from lib import net_misc, packetid, serialization, strng
from logs import lg
from main import config
from p2p import commands, lookup, online_status
...
def WriteMyOriginalIdentitySource(new_identity_xml_src):
    return config.conf().setData(
        "services/proxy-transport/my-original-identity", new_identity_xml_src
    )
...
def VerifyExistingRouter():
    ...
    if not ReadCurrentRouter() and ReadMyOriginalIdentitySource():
        lg.err(
            "current router is not set, but some wrong data found as original identity"
        )
        return False
    return True
...
class ProxyReceiver(automat.Automat):
    ...
    def __repr__(self):
        return "%s_%s_%s(%s)" % (
            self.id,
            self.router_id,
            (self.router_connection_info or {}).get("repr", "?"),
            self.state,
        )
        ...
        j.update(
            {
                ...
                "host": net_misc.pack_address(self.router_proto_host[1])
                if self.router_proto_host
                else "",
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---ACK?---
            ...
            if event == "ack-received":
                ...
                (event == "sending-failed" and self.Retries > 3)
                or event == "ack-timeout"
                or event == "fail-received"
                or event == "timer-15sec"
            ):
    ...
    def doLoadRouterInfo(self, *args, **kwargs):
        ...
        if _Debug:
            lg.out(
                _DebugLevel, "proxy_receiver.doLoadRouterInfo : %s" % self.router_idurl
            )
    ...
    def doSendMyIdentity(self, *args, **kwargs):
        ...
        reactor.callLater(
            0,
            self._do_send_identity_to_router,
            my_id.getLocalIdentity().serialize(),
            failed_event="fail-received",
        )  # @UndefinedVariable
        identity_source = (
            config.conf().getData("services/proxy-transport/my-original-identity").strip()
        )
        if identity_source:
            if _Debug:
                lg.out(
                    _DebugLevel,
                    '    also sending identity loaded from "my-original-identity" config',
                )
            reactor.callLater(
                0,
                self._do_send_identity_to_router,
    ...
    def doStartListening(self, *args, **kwargs):
        ...
        try:
            ...
            try:
                ...
                s = (
                    config.conf()
                    .getString("services/proxy-transport/current-router")
                    .strip()
                )
        ...
        if previous_identity:
            ...
            if _Debug:
                lg.out(
                    _DebugLevel, "PREVIOUS ORIGINAL IDENTITY:\n%s\n" % previous_identity
                )
        ...
        lg.info(
            "router %s is connected at %s://%s"
            % (
                self.router_idurl,
                self.router_proto_host[0],
    ...
    def doStopListening(self, *args, **kwargs):
        ...
        if self.router_connection_info:
            ...
            if active_router_session_machine_index is not None:
                active_router_session_machine = automat.by_index(
            ...
            if not active_router_session_machine:
                ...
                if active_router_sessions:
                    active_router_session_machine = automat.by_index(
                        active_router_sessions[0].index
                    )
            if active_router_session_machine is not None:
                ...
                lg.info(
                    "removed callback from router active session: %r"
    ...
    def doUpdateRouterID(self, *args, **kwargs):
        ...
        if self.router_idurl != newidentity.getIDURL():
            lg.warn(
                "router idurl is unrecognized from response %r != %r"
                % (
                    self.router_idurl,
                    newidentity.getIDURL(),
                )
            )
            return
    ...
    def doCheckPingRouter(self, *args, **kwargs):
        ...
        """
        if self.router_connection_info:
            gateway.send_keep_alive(
                self.router_connection_info["proto"], self.router_connection_info["host"]
            )
        ...
        if live_time < 60.0:
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "proxy_receiver.doCheckPingRouter OK, latest packet received %f sec ago"
                    % live_time,
                )
            return
        if _Debug:
            lg.out(
                _DebugLevel, "proxy_receiver.doCheckPingRouter to %s" % self.router_idurl
            )
        identity_source = (
            config.conf().getData("services/proxy-transport/my-original-identity").strip()
        )
        ...
        self._do_send_identity_to_router(
            identity_source, failed_event="router-disconnected"
        )
    ...
    def _do_process_inbox_packet(self, *args, **kwargs):
        ...
        if newpacket.Command == commands.RelayAck():
            try:
                ack_info = serialization.BytesToDict(
                    data, keys_to_text=True, values_to_text=True
                )
            except:
                lg.exc()
                return
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "<<<Relay-ACK %s:%s from %s://%s with %d bytes %s"
                    % (
                        ack_info["command"],
                        ack_info["packet_id"],
                        info.proto,
            ...
            if _PacketLogFileEnabled:
                lg.out(
                    ...
                    % (
                        ...
                        ack_info["packet_id"],
                        info.bytes_received,
                        global_id.UrlToGlobalID(ack_info["from"]),
                        global_id.UrlToGlobalID(ack_info["to"]),
                        info.transfer_id,
                    ),
                    log_name="packet",
                    showtime=True,
        ...
        if newpacket.Command == commands.RelayFail():
            try:
                fail_info = serialization.BytesToDict(
                    data, keys_to_text=True, values_to_text=True
                )
            except:
                lg.exc()
            ...
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "<<<Relay-FAIL %s:%s from %s://%s with %d bytes %s"
                    % (
                        fail_info["command"],
                        fail_info["packet_id"],
                        info.proto,
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "<<<Relay-IN %s from %s://%s with %d bytes"
        ...
        if routed_packet.Command in [
            ...
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "    found routed identity in relay packet %s" % routed_packet,
                )
            try:
        ...
        if (
            newpacket.Command == commands.RelayIn()
            and routed_packet.Command == commands.Fail()
        ):
            if (
                routed_packet.Payload == b"route not exist"
                or routed_packet.Payload == b"route already closed"
            ):
                for pout in packet_out.search_by_packet_id(routed_packet.PacketID):
                    lg.warn(
                        "received %r from %r, outgoing packet is failed: %r"
                        % (
    ...
    def _do_send_identity_to_router(self, identity_source, failed_event):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "proxy_receiver._do_send_identity_to_router to %s" % self.router_idurl,
            )
            lg.out(
                _DebugLevel,
                "        contacts=%r, sources=%r"
        ...
        packet_out.create(
            ...
            wide=True,
            callbacks={
                commands.Ack(): lambda response, info: self.automat(
                    "ack-received", (response, info)
                ),
                commands.Fail(): lambda x: self.automat(failed_event),
                None: lambda pkt_out: self.automat("ack-timeout", pkt_out),
                "failed": lambda pkt_out, error_message: self.automat(
                    "sending-failed", (pkt_out, error_message)
                ),
            },
            keep_alive=True,
            response_timeout=30,
        ...
        for idurl in idurls:
            ...
            if len(myprotos.intersection(remoteprotos)) > 0:
                ...
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "proxy_receiver._on_nodes_lookup_finished found : %r"
                        % self.possible_router_idurl,
                    )
    ...
    def _find_random_node(self, attempts):
        ...
        preferred_routers_raw = (
            config.conf().getData("services/proxy-transport/preferred-routers").strip()
        )
        ...
        if preferred_routers:
            ...
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "proxy_receiver._find_random_node selected random item from preferred_routers: %r"
                    % self.possible_router_idurl,
                )
            idcache_defer = identitycache.immediatelyCaching(self.possible_router_idurl)
            idcache_defer.addCallback(
                lambda *args: self.automat("found-one-node", self.possible_router_idurl)
            )
        ...
        if tsk:
            tsk.result_defer.addCallback(
                self._on_nodes_lookup_finished, attempts=attempts
            )
    ...
    def _on_request_service_ack(self, response, info):
        ...
        if response.PacketID not in self.request_service_packet_id:
            lg.warn(
                "wrong PacketID in response: %s, but outgoing was : %s"
        ...
        if response.PacketID in self.request_service_packet_id:
            ...
            lg.warn(
                "%s was not found in pending requests: %s"
                % (response.PacketID, self.request_service_packet_id)
            )
        if _Debug:
            lg.out(
                _DebugLevel,
                "proxy_receiver._on_request_service_ack : %s" % str(response.Payload),
            )
        if self.router_idurl != response.CreatorID:
            lg.err(
                "received unexpected response from another node: %r ~ %r"
                % (
                    self.router_idurl,
                    response.CreatorID,
                )
        ...
        if not active_router_sessions:
            active_router_sessions = gateway.find_active_session(
                info.proto, idurl=id_url.to_bin(response.CreatorID)
            )
        if not active_router_sessions:
            lg.err(
            ...
            if _Debug:
                lg.args(
                    ...
                    router_idurl=self.router_idurl,
                    ack_packet=info,
                    active_sessions=gateway.list_active_sessions(info.proto),
                )
            self.automat("service-refused", (response, info))
            return
        ...
        if active_router_session_machine is None:
            lg.err(
                "did not found proxy router session state machine instance: %s"
                % self.router_connection_info
            )
            self.router_connection_info = None
            if _Debug:
            ...
            return
        lg.info(
            "found active session for proxy router: %s" % active_router_session_machine
        )
    ...
    def _on_inbox_packet_received(self, newpacket, info, status, error_message):
        if (
            newpacket.Command == commands.Identity()
            and newpacket.CreatorID == self.router_idurl
            and newpacket.RemoteID == my_id.getIDURL()
        ):
    ...
    def _on_router_contact_status_connected(
        self, oldstate, newstate, event_string, *args, **kwargs
    ):
        lg.info(
            'router %r contact status online: %s->%s after "%s"'
            % (
                self.router_idurl,
                oldstate,
                newstate,
    ...
    def _on_router_contact_status_offline(
        self, oldstate, newstate, event_string, *args, **kwargs
    ):
        lg.warn(
            'router %r contact status offline: %s->%s after "%s"'
            % (
                self.router_idurl,
                oldstate,
                newstate,
    ...
    def _on_router_session_disconnected(
        self, oldstate, newstate, event_string, *args, **kwargs
    ):
        lg.err(
            "router session disconnected: %s->%s because of %r"
            % (oldstate, newstate, event_string)
        )
        self.automat("router-disconnected")
    ...
    def _on_queue_item_status_changed(self, pkt_out, status, error=""):
        ...
        if pkt_out.remote_idurl != proxy_receiver.GetRouterIDURL():
            return False
        lg.err(
            "connection failed with current proxy router, must reconnect to another router: %r %r %r"
            % (
                pkt_out,
                status,
                error,
