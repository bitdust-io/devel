...
from contacts import identitycache, identitydb
from lib import nameurl, net_misc, serialization, strng
from logs import lg
from main import config, events
from p2p import commands, network_connector, p2p_service
from services import driver
from transport import callback, gateway, packet_in, packet_out
from userid import global_id, id_url, identity, my_id
...
class ProxyRouter(automat.Automat):
    ...
    def state_changed(self, oldstate, newstate, event, *args, **kwargs):
        ...
        if oldstate != "TRANSPORTS?" and newstate == "TRANSPORTS?":
            if network_connector.A().state == "CONNECTED":
                reactor.callLater(
                    0, self.automat, "network-connected"
                )  # @UndefinedVariable
            elif network_connector.A().state == "DISCONNECTED":
                reactor.callLater(
                    0, self.automat, "network-disconnected"
                )  # @UndefinedVariable
    ...
    def doSaveRouteProtoHost(self, *args, **kwargs):
        ...
        if idurl not in self.routes:
            ...
            if new_address not in self.routes[idurl]["address"]:
                ...
                lg.info(
                    "added new active address %r for %s, currently %d active addresses"
                    % (
                        new_address,
                        nameurl.GetName(idurl),
                        len(self.routes[idurl]["address"]),
                    )
                )
    ...
    def doSendFail(self, *args, **kwargs):
        ...
        if self.closed_routes.get(receiver_idurl.original(), 0) or self.closed_routes.get(
            receiver_idurl.to_bin(), 0
        ):
    ...
    def _get_session_proto_host(self, sender_idurl, info=None):
        ...
        if (info is not None and not connection_info) or not connection_info.get("index"):
            ...
            if info:
                active_user_sessions = gateway.find_active_session(
                    info.proto, idurl=sender_idurl.original()
                )
                if not active_user_sessions:
                    active_user_sessions = gateway.find_active_session(
                        info.proto, idurl=sender_idurl.to_bin()
                    )
            if not active_user_sessions:
                lg.warn(
                    "route with %s found but no active sessions found : %r"
                    % (
                        sender_idurl,
                        info,
                    )
        ...
        if not hosts:
            lg.warn(
                "found active user session but host is empty in %r, will try to use recorded info"
                % active_user_session_machine
            )
            hosts = route_info["address"]
        if len(hosts) == 0:
            lg.warn(
                "route with %s do not have actual info about the host, will use identity contacts instead"
                % sender_idurl
            )
        ...
        if len(hosts) > 1:
            lg.warn(
                "found more then one channel with %s : %r"
                % (
                    sender_idurl,
                    hosts,
                )
            )
        receiver_proto, receiver_host = strng.to_bin(hosts[0][0]), strng.to_bin(
            hosts[0][1]
        )
        if _Debug:
            lg.args(
                ...
                host=receiver_host,
                user_session=active_user_session_machine,
            )
        ...
        if request.Command == commands.RequestService():
            if len(self.routes) >= _MaxRoutesNumber:
                ...
                if user_idurl.original() != cached_ident.getIDURL().original():
                    lg.warn(
                        "incoming identity is not belong to request packet creator: %r != %r"
                        % (user_idurl.original(), cached_ident.getIDURL().original())
                    )
                    return
                identitycache.UpdateAfterChecking(
                    cached_ident.getIDURL().original(), idsrc
                )
                if user_idurl.original() not in list(
                    self.routes.keys()
                ) and user_idurl.to_bin() not in list(self.routes.keys()):
                    oldnew = "NEW"
                else:
                    oldnew = "OLD"
                ...
                if not active_user_sessions:
                    active_user_sessions = gateway.find_active_session(
                        info.proto, idurl=user_idurl.original()
                    )
                if not active_user_sessions:
                    active_user_sessions = gateway.find_active_session(
                        info.proto, idurl=user_idurl.to_bin()
                    )
                if active_user_sessions:
                    ...
                    active_user_session_machine = automat.by_index(
                    ...
                    if active_user_session_machine:
                        ...
                        active_user_session_machine.addStateChangedCallback(
                            cb=lambda oldstate, newstate, event_string, *args, **kwargs: self._on_user_session_disconnected(
                                user_idurl.original(),
                                oldstate,
                                newstate,
                                event_string,
                        ...
                        lg.info(
                            "connected %s routed user %r and set active session: %r"
                            % (oldnew.upper(), user_idurl, active_user_session_machine)
                        )
                    else:
                        lg.err(
                            "not found session state machine by index %s"
                            % user_connection_info["index"]
                        )
                else:
                    if _Debug:
                        lg.dbg(
                            _DebugLevel,
                            "active connection with user %s at %s:%s not yet exist"
                            % (
                                user_idurl.original(),
                                info.proto,
                                info.host,
                ...
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "proxy_server.doProcessRequest !!!!!!! ACCEPTED %s ROUTE for %r  contacts=%s"
                        % (
                            oldnew.upper(),
                            user_idurl,
                            self.routes.get(user_idurl.original(), {}).get("contacts"),
                        ),
                    )
        # --- commands.CancelService()
        elif request.Command == commands.CancelService():
            if user_idurl.original() in list(
                self.routes.keys()
            ) or user_idurl.to_bin() in list(self.routes.keys()):
                # cancel existing route
                active_user_session_machine_index = (
                    self.routes.get(user_idurl.original(), {})
                    .get("connection_info", {})
                    .get("index", None)
                )
                if active_user_session_machine_index is None:
                    active_user_session_machine_index = (
                        self.routes.get(user_idurl.to_bin(), {})
                        .get("connection_info", {})
                        .get("index", None)
                    )
                if active_user_session_machine_index is not None:
                    ...
                    if active_user_session_machine is not None:
                        active_user_session_machine.removeStateChangedCallback(
                            callback_id="proxy_router"
                        )
                ...
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "proxy_server.doProcessRequest !!!!!!! CANCELLED ROUTE for %r"
                        % user_idurl.original(),
                    )
            else:
                p2p_service.SendAck(request, "rejected", wide=True)
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "proxy_server.doProcessRequest CancelService rejected : %r is not found in routes"
                        % user_idurl.original(),
                    )
    ...
    def _do_forward_inbox_packet(self, *args, **kwargs):
        ...
        if _Debug:
            lg.args(
                _DebugLevel, newpacket=newpacket, info=info, receiver_idurl=receiver_idurl
            )
        if not route_info:
            lg.warn(
        ...
        if not connection_info or not connection_info.get("index"):
            active_user_sessions = gateway.find_active_session(
                info.proto, idurl=receiver_idurl.original()
            )
            if not active_user_sessions:
                active_user_sessions = gateway.find_active_session(
                    info.proto, idurl=receiver_idurl.to_bin()
                )
            if not active_user_sessions:
                lg.warn(
                    'route with %s found but no active sessions found with %s://%s, fire "routed-session-disconnected" event'
                    % (
                        receiver_idurl,
                        info.proto,
                        info.host,
            ...
            if active_user_session_machine:
                if receiver_idurl.original() in self.routes:
                    self.routes[receiver_idurl.original()][
                        "connection_info"
                    ] = user_connection_info
                    lg.info(
                        "found and remember active connection info: %r"
                        % user_connection_info
                    )
                if receiver_idurl.to_bin() in self.routes:
                    self.routes[receiver_idurl.to_bin()][
                        "connection_info"
                    ] = user_connection_info
                    lg.info(
                        "found and remember active connection info (for latest IDURL): %r"
                        % user_connection_info
                    )
        if not active_user_session_machine:
            if connection_info.get("index"):
        ...
        if not active_user_session_machine:
            lg.warn(
                'route with %s found but no active user session, fire "routed-session-disconnected" event'
                % receiver_idurl
            )
            self.automat("routed-session-disconnected", receiver_idurl)
            return
        if not active_user_session_machine.is_connected():
            lg.warn(
                'route with %s found but session is not connected, fire "routed-session-disconnected" event'
                % receiver_idurl
            )
            self.automat("routed-session-disconnected", receiver_idurl)
            return
        ...
        if not hosts:
            lg.warn(
                "found active user session but host is empty in %r, try use recorded info"
                % active_user_session_machine
            )
            hosts = route_info["address"]
        if len(hosts) == 0:
        ...
        if len(hosts) > 1:
            lg.warn(
                "found more then one channel with receiver %s : %r"
                % (
                    receiver_idurl,
                    hosts,
                )
        ...
        raw_data, pout = self._do_send_relay_packet(
            ...
            failed_callback=lambda pkt_out, msg: self._on_routed_in_packet_failed(
                pkt_out, msg, newpacket, info, receiver_idurl
            ),
        )
        if _Debug:
            lg.out(
    ...
    def _do_set_contacts_override(self, *args, **kwargs):
        if _Debug:
            lg.out(
                _DebugLevel,
                "proxy_router.doSetContactsOverride identity for %s" % args[0].CreatorID,
            )
        ...
        try:
            current_contacts = identity.identity(
                xmlsrc=current_overridden_identity
            ).getContacts()
        except:
            current_contacts = []
        ...
        if _Debug:
            lg.out(
                _DebugLevel, "    current overridden contacts is : %s" % current_contacts
            )
            lg.out(
                _DebugLevel,
                "    new override contacts will be : %s" % new_ident.getContacts(),
            )
    ...
    def _do_forward_outbox_packet(self, outpacket_info_tuple):
        ...
        try:
            ...
            padded_data = key.DecryptWithSessionKey(
                session_key, block.EncryptedData, session_key_type=block.SessionKeyType
            )
        ...
        del block
        if (
            identitycache.HasKey(sender_idurl)
            and identitycache.HasKey(receiver_idurl)
            and not is_retry
        ):
            return self._do_verify_routed_data(
                newpacket,
                info,
                sender_idurl,
                receiver_idurl,
        ...
        lg.warn(
            "will send routed data after caching, is_retry=%s sender_idurl=%r receiver_idurl=%r"
            % (
                is_retry,
                sender_idurl,
                receiver_idurl,
        ...
        if _Debug:
            d.addErrback(
                ...
                debug=_Debug,
                debug_level=_DebugLevel,
                method="_do_forward_outbox_packet",
            )
        d.addErrback(
            lambda err: self._do_verify_routed_data(
                newpacket,
                info,
                None,
                None,
    ...
    def _do_check_cached_idurl(
        ...
        if _Debug:
            lg.args(
                _DebugLevel,
                sender_id_rev=sender_id_rev,
                receiver_id_rev=receiver_id_rev,
                is_retry=is_retry,
        ...
        if sender_idurl in self.routes:
            ...
            if (
                sender_ident
                and sender_id_rev is not None
                and sender_id_rev != sender_ident.getRevisionValue()
            ):
                route_changed = True
                self.routes[sender_idurl][
                    "identity_rev"
                ] = sender_ident.getRevisionValue()
        ...
        if receiver_idurl in self.routes:
            ...
            if (
                receiver_ident
                and receiver_id_rev is not None
                and receiver_id_rev != receiver_ident.getRevisionValue()
            ):
                route_changed = True
                self.routes[receiver_idurl][
                    "identity_rev"
                ] = receiver_ident.getRevisionValue()
                self.closed_routes.pop(receiver_idurl, None)
        if some_failed:
            self._do_verify_routed_data(
                newpacket,
                info,
                None,
                None,
    ...
    def _do_verify_routed_data(
        ...
        if sender_idurl is None or receiver_idurl is None:
            lg.warn(
                "failed sending %r, sender or receiver IDURL was not cached" % newpacket
            )
            self._do_send_fail_packet(
                newpacket,
                info,
                wide,
                response_timeout,
                ...
                newpacket.CreatorID,
                receiver_idurl,
                "sender or receiver IDURL was not found",
            )
            return
        ...
        if not is_signature_valid:
            lg.err(
                "new packet from %s is NOT VALID:\n\n%r\n\n\n%r\n"
        ...
        if receiver_idurl.to_bin() == my_id.getIDURL().to_bin():
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "        proxy_router() passing by INCOMING packet %r from %s to me"
        ...
        if False:
            ...
            route_closed_time = max(
                self.closed_routes.get(receiver_idurl.original(), 0),
                self.closed_routes.get(receiver_idurl.to_bin(), 0),
            )
            if _Debug:
                lg.args(
                    _DebugLevel, route_closed_time=route_closed_time, time=time.time()
                )
            if time.time() - route_closed_time < 900:
                # route was closed but just recently - can not send outgoing data
                lg.err(
                    "can not send routed data, route with %s already closed"
                    % (receiver_idurl)
                )
                self._do_send_fail_packet(
                    routed_packet,
        ...
        if (
            receiver_idurl.original() in routes_keys
            or receiver_idurl.to_bin() in routes_keys
        ):
            # if both node A and node B are behind my router node I need to send routed packet directly to B
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "        proxy_router() ROUTED (same router) packet %s from %s to %s"
                    % (routed_packet, sender_idurl, receiver_idurl),
                )
            self.event(
                "routed-inbox-packet-received", (receiver_idurl, routed_packet, info)
            )
            return
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                ">>>Route-OUT %d bytes from %s at %s://%s :"
                % (
                    len(routed_data),
            ...
            lg.out(
                _DebugLevel,
                "    routed to %s : %s" % (nameurl.GetName(receiver_idurl), pout),
            )
    ...
    def _do_send_fail_packet(
        ...
        if not publickey:
            lg.err(
                "%r : but can not send RelayFail(), identity %r is not cached"
                % (
                    error,
                    newpacket.CreatorID,
                )
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "<<<Route-FAIL %s from %s:%s   sent to %s://%s with %d bytes in %s"
                % (
                    str(newpacket),
                    strng.to_text(info.proto),
                    strng.to_text(info.host),
    ...
    def _do_register_route(self, idurl, ident_obj):
        ...
        oldnew = ""
        if idurl.original() not in list(
            self.routes.keys()
        ) and idurl.to_bin() not in list(self.routes.keys()):
            # accept new route
            oldnew = "NEW"
        ...
        if _Debug:
            lg.args(
                _DebugLevel, idurl=idurl, oldnew=oldnew, rev=ident_obj.getRevisionValue()
            )
        ...
        self.routes[idurl.original()]["contacts"] = ident_obj.getContactsAsTuples(
            as_text=True
        )
    ...
    def _do_unregister_route(self, idurl):
        ...
        active_user_session_machine_index = (
            (self.routes.get(idurl.original()) or {}).get("connection_info") or {}
        ).get("index", None)
        if active_user_session_machine_index is None:
            active_user_session_machine_index = (
                (self.routes.get(idurl.to_bin()) or {}).get("connection_info") or {}
            ).get("index", None)
        if active_user_session_machine_index is not None:
            active_user_session_machine = automat.by_index(
                active_user_session_machine_index
            ...
            if active_user_session_machine is not None:
                ...
                lg.info(
                    'removed "proxy_router" callback from active user session %r'
                    % active_user_session_machine
                )
    ...
    def _on_routed_in_packet_failed(self, pkt_out, msg, newpacket, info, receiver_idurl):
        lg.err(
            "routed packet transfer failed: %r %r %r %r %r"
            % (pkt_out, msg, newpacket, info, receiver_idurl)
        )
        p2p_service.SendFail(
            newpacket, "routed packet transfer failed", remote_idurl=newpacket.CreatorID
        )

    def _on_routed_out_packet_sent(
        self,
        pkt_out,
        msg,
        newpacket,
        ...
        if not publickey:
            lg.err(
                "routed packet sent but can not send RelayAck(), identity %r is not cached"
                % newpacket.CreatorID
            )
            return
        receiver_proto, receiver_host = self._get_session_proto_host(sender_idurl, info)
    ...
    def _on_routed_out_packet_failed(
        self,
        pkt_out,
        msg,
        newpacket,
        ...
        if not publickey:
            lg.err(
                "routed packet delivery failed but can not send RelayFail(), identity %r is not cached"
                % newpacket.CreatorID
            )
            return
        receiver_proto, receiver_host = self._get_session_proto_host(sender_idurl, info)
    ...
    def _on_first_inbox_packet_received(self, newpacket, info, status, error_message):
        if _Debug:
            lg.out(
                _DebugLevel,
                "proxy_router._on_first_inbox_packet_received %s from %s://%s"
                % (
                    newpacket,
                    info.proto,
                    info.host,
            ...
            lg.out(
                ...
                "    creator=%s owner=%s"
                % (
                    newpacket.CreatorID.original(),
                    newpacket.OwnerID.original(),
                ),
            )
            lg.out(
                _DebugLevel,
                "    sender=%s remote_id=%s"
                % (
                    info.sender_idurl,
            ...
            for k, v in self.routes.items():
                lg.out(
                    _DebugLevel * 2,
                    "        route with %r :  address=%s  contacts=%s"
                    % (
                        k,
                        v.get("address"),
                        v.get("contacts"),
        ...
        if newpacket.RemoteID == my_id.getIDURL():
            ...
            if newpacket.Command in [
                ...
                if (
                    newpacket.CreatorID.original() in list(self.routes.keys())
                    or newpacket.CreatorID.to_bin() in list(self.routes.keys())
                    or newpacket.OwnerID.original() in list(self.routes.keys())
                    or newpacket.OwnerID.to_bin() in list(self.routes.keys())
                ):
            ...
            # and this is not a Relay packet, Identity
                ...
                if newpacket.CreatorID.original() in list(
                    self.routes.keys()
                ) or newpacket.CreatorID.to_bin() in list(self.routes.keys()):
                    ...
                    if _Debug:
                        lg.out(
                            _DebugLevel, '        sending "known-identity-received" event'
                        )
                ...
                if _Debug:
                    lg.out(
                        _DebugLevel, '        sending "unknown-identity-received" event'
                    )
        ...
        receiver_idurl = None
        known_remote_id = newpacket.RemoteID.original() in list(
            self.routes.keys()
        ) or newpacket.RemoteID.to_bin() in list(self.routes.keys())
        known_creator_id = newpacket.CreatorID.original() in list(
            self.routes.keys()
        ) or newpacket.CreatorID.to_bin() in list(self.routes.keys())
        known_owner_id = newpacket.OwnerID.original() in list(
            self.routes.keys()
        ) or newpacket.OwnerID.to_bin() in list(self.routes.keys())
        if known_remote_id:
    ...
    def _on_first_outbox_packet_direct(
        self,
        outpacket,
        wide,
        callbacks,
        ...
        route = {
            "packet": outpacket,
            "remoteid": receiver_idurl,
            "description": "direct_%s[%s]_%s"
            % (outpacket.Command, outpacket.PacketID, nameurl.GetName(receiver_idurl)),
            "proto": receiver_proto,
            "host": receiver_host,
        }
    ...
    def _on_file_sending_filter(
        self, remote_idurl, proto, host, filename, description, pkt_out
    ):
        ...
        if not receiver_proto or not receiver_host:
            ...
            lg.warn(
                "did not found the real host for outgoing %r addressed to my own host"
                % pkt_out
            )
            return False
        if _Debug:
            lg.dbg(
                _DebugLevel,
                "switched %s://%s to %s://%s for outgoing %r"
                % (proto, host, receiver_proto, receiver_host, pkt_out),
            )
        if _PacketLogFileEnabled:
            lg.out(
                0,
        ...
        result_defer = gateway.transport(receiver_proto).call(
            "send_file", remote_idurl, filename, receiver_host, description
        )
        callback.run_begin_file_sending_callbacks(
            result_defer,
            remote_idurl,
            receiver_proto,
            receiver_host,
    ...
    def _on_network_connector_state_changed(
        self, oldstate, newstate, event, *args, **kwargs
    ):
    ...
    def _on_finish_file_sending(self, pkt_out, item, status, size, error_message):
        ...
        if Command != commands.Ack():
            return False
        if RemoteID.original() not in list(
            self.routes.keys()
        ) and RemoteID.to_bin() not in list(self.routes.keys()):
            return False
        found = False
        to_remove = []
        for ack_packet_id, ack_remote_idurl in self.acks.items():
            if PacketID.lower() == ack_packet_id.lower() and RemoteID == ack_remote_idurl:
                if _Debug:
                    lg.dbg(
                        _DebugLevel,
                        "found outgoing Ack() packet %r to %r"
                        % (ack_packet_id, ack_remote_idurl),
                    )
                to_remove.append(ack_packet_id)
                # TODO: clean up self.acks for un-acked requests
                self.automat(
                    "request-route-ack-sent",
                    (RemoteID, pkt_out, item, status, size, error_message),
                )
                found = True
    ...
    def _on_user_session_disconnected(
        self, user_id, oldstate, newstate, event_string, *args, **kwargs
    ):
    ...
    def _on_identity_url_changed(self, evt):
        ...
        if old in self.routes and new not in self.routes:
            ...
            lg.info(
                "replaced route for user after identity rotate detected : %r -> %r"
                % (old, new)
            )
    ...
    def _load_routes(self):
        ...
        try:
            dct = serialization.BytesToDict(
                strng.to_bin(src), keys_to_text=True, values_to_text=True
            )
        except:
            dct = {}
    ...
    def _write_route(self, user_idurl):
        ...
        newsrc = strng.to_text(
            serialization.DictToBytes(dct, keys_to_text=True, values_to_text=True)
        )
    ...
    def _remove_route(self, user_idurl):
        ...
        if user_idurl_txt in dct:
            ...
            if _Debug:
                lg.out(
                    _DebugLevel, "proxy_router._remove_route %d bytes wrote" % len(newsrc)
                )
