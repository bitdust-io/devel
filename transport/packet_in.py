...
def process(newpacket, info):
    ...
    if not id_url.is_cached(newpacket.RemoteID):
        ...
        d.addErrback(
            lambda err: lg.err(
                "incoming remote ID is unknown, failed caching remote %s identity: %s"
                % (newpacket.RemoteID, str(err))
            )
            and None
        )
        return d
    ...
    if not identitycache.HasKey(newpacket.CreatorID):
        ...
        d.addErrback(
            lambda err: lg.err(
                "failed caching remote %s identity: %s" % (newpacket.CreatorID, str(err))
            )
            and None
        )
        return d
...
def handle(newpacket, info):
    ...
    if not is_signature_valid:
        ...
        lg.warn(
            "signature is not valid for %r from %r|%r to %r"
    ...
    try:
        handled = (
            callback.run_inbox_callbacks(newpacket, info, info.status, info.error_message)
            or handled
        )
    except:
        lg.exc()
    if not handled and newpacket.Command not in [
        ...
        lg.warn(
            "incoming %s from [%s://%s] was NOT HANDLED"
            % (newpacket, info.proto, info.host)
        )
        if _PacketLogFileEnabled:
            lg.out(
...
class PacketIn(automat.Automat):
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---RECEIVING---
            ...
            elif event == "unregister-item" and not self.isTransferFinished(
                *args, **kwargs
            ):
                self.state = "FAILED"
                ...
                and self.isTransferFinished(*args, **kwargs)
                and not self.isRemoteIdentityCached(*args, **kwargs)
            ):
                self.state = "CACHING"
                ...
                event == "unregister-item"
                and self.isTransferFinished(*args, **kwargs)
                and self.isRemoteIdentityCached(*args, **kwargs)
            ):
                self.state = "INBOX?"
    ...
    def doInit(self, *args, **kwargs):
        ...
        reactor.callLater(
            0, callback.run_begin_file_receiving_callbacks, self
        )  # @UndefinedVariable
    ...
    def doEraseInputFile(self, *args, **kwargs):
        ...
        """
        reactor.callLater(
            1, tmpfile.throw_out, self.filename, "received"
        )  # @UndefinedVariable
    ...
    def doReportReceived(self, *args, **kwargs):
        ...
        """
        newpacket = args[0]
        p2p_stats.count_inbox(
            self.sender_idurl, self.proto, self.status, self.bytes_received
        )
    ...
    def doReportFailed(self, *args, **kwargs):
        ...
        if _PacketLogFileEnabled:
            lg.out(
                0,
                '                \033[0;49;31mIN FAILED with status "%s" from %s://%s TID:%s\033[0m'
    ...
    def doReportCacheFailed(self, *args, **kwargs):
        ...
        if _PacketLogFileEnabled:
            lg.out(
                0,
                '                \033[0;49;31mIN CACHE FAILED with "%s" for %s TID:%s\033[0m'
