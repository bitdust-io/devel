...
def send(filename, remoteaddress, description=None, single=False):
    ...
    # dhnio.Dprint(18, 'tcp_node.send     %s opened, %d started' % (
        ...
        for connection in connections:
            for in_file in connection.inboxFiles.vaues():
                if in_file.transfer_id and in_file.transfer_id == transferID:
                    ...
                    return True
    dhnio.Dprint(8, 'tcp_node.cancel_file_receiving WARNING %r not found' % transferID)
    return False
        ...
        for connection in connections:
            for out_file in connection.stream.outboxFiles.values():
                if out_file.transfer_id and out_file.transfer_id == transferID:
                    out_file.cancel()
                    return True
    dhnio.Dprint(8, 'tcp_node.cancel_file_sending WARNING %r not found' % transferID)
    return False
...
class TCPFactory(protocol.ClientFactory):
    ...
    def clientConnectionFailed(self, connector, reason):
        ...
        self.pendingoutboxfiles = []
        dhnio.Dprint(18, 'tcp_node.clientConnectionFailed from %s  :   %s closed, %d more started' % (
