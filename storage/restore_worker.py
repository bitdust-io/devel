...
class RestoreWorker(automat.Automat):
    ...
    def __init__(
        self, BackupID, OutputFile, KeyID=None, ecc_map=None, debug_level=_DebugLevel, log_events=False, log_transitions=_Debug, publish_events=False, **kwargs
    ):
        """
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---REQUESTED---
            ...
            if event == 'data-receiving-started':
                ...
                self.doRequestPackets(*args, **kwargs)
                self.Attempts += 1
            elif (event == 'abort' or (event == 'request-failed' and (not self.isStillCorrectable(*args, **kwargs) or self.Attempts >= 3))) or (
                (event == 'instant' or event == 'request-finished') and not self.isBlockReceiving(*args, **kwargs) and not self.isBlockFixable(*args, **kwargs)
            ):
                self.state = 'FAILED'
        ...
        # ---RECEIVING---
            ...
            if event == 'data-receiving-stopped' and self.isStillCorrectable(*args, **kwargs):
                ...
                self.doSavePacket(*args, **kwargs)
            elif (event == 'abort' or ((event == 'request-failed' or event == 'data-receiving-stopped') and not self.isStillCorrectable(*args, **kwargs))) or (
                (event == 'instant' or event == 'request-finished') and not self.isBlockReceiving(*args, **kwargs) and not self.isBlockFixable(*args, **kwargs)
            ):
                self.state = 'FAILED'
                self.doDeleteAllRequests(*args, **kwargs)
