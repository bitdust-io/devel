...
from p2p import online_status, propagate
from raid import eccmap, raid_worker
from services import driver
from stream import data_receiver, io_throttle
from system import bpio, tmpfile
...
class RestoreWorker(automat.Automat):
    ...
    def state_changed(self, oldstate, newstate, event, *args, **kwargs):
        ...
        """
        if event != "instant":
            if newstate in ["REQUESTED", "RECEIVING",] and oldstate not in [
                "REQUESTED",
                "RECEIVING",
            ]:
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---REQUESTED---
            ...
            if event == "data-receiving-started":
                ...
                and self.isStillCorrectable(*args, **kwargs)
                and self.Attempts < 3
            ):
                self.doScanExistingPackets(*args, **kwargs)
                self.doRequestPackets(*args, **kwargs)
                ...
                or (
                    event == "request-failed"
                    and (
                        not self.isStillCorrectable(*args, **kwargs) or self.Attempts >= 3
                    )
                )
            ) or (
                (event == "instant" or event == "request-finished")
                and not self.isBlockReceiving(*args, **kwargs)
                and not self.isBlockFixable(*args, **kwargs)
            ):
                self.state = "FAILED"
                ...
                event == "instant" or event == "request-finished"
            ) and self.isBlockFixable(*args, **kwargs):
                self.state = "RAID"
                self.doReadRaid(*args, **kwargs)
        # ---RECEIVING---
        elif self.state == "RECEIVING":
            if event == "data-receiving-stopped" and self.isStillCorrectable(
                *args, **kwargs
            ):
                self.state = "REQUESTED"
                self.doScanExistingPackets(*args, **kwargs)
                ...
                event == "abort"
                or (
                    (event == "request-failed" or event == "data-receiving-stopped")
                    and not self.isStillCorrectable(*args, **kwargs)
                )
            ) or (
                (event == "instant" or event == "request-finished")
                and not self.isBlockReceiving(*args, **kwargs)
    ...
    def isStillCorrectable(self, *args, **kwargs):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "restore_worker.isStillCorrectable max_errors=%d, fails=%d"
                % (
                    self.max_errors,
    ...
    def isBlockFixable(self, *args, **kwargs):
        ...
        if _Debug:
            lg.args(
                ...
                result=result,
                OnHandData=self.OnHandData,
                OnHandParity=self.OnHandParity,
            )
        return result
    ...
    def doInit(self, *args, **kwargs):
        ...
        self.known_suppliers = [
            _f for _f in contactsdb.suppliers(customer_idurl=self.customer_idurl) if _f
        ]
        if not self.EccMap:
        ...
        if not self.EccMap:
            ...
            if known_eccmap_dict:
                ...
                if known_ecc_map:
                    ...
                    lg.info(
                        "ECC map %r recognized from active share %r"
                        % (
                            self.EccMap,
                            active_share,
                        )
    ...
    def doStartNewBlock(self, *args, **kwargs):
        ...
        if _Debug:
            lg.out(
                _DebugLevel, "restore_worker.doStartNewBlock " + str(self.block_number)
            )
        self.OnHandData = [
            False,
    ...
    def doScanExistingPackets(self, *args, **kwargs):
        ...
        for SupplierNumber in range(self.EccMap.datasegments):
            PacketID = packetid.MakePacketID(
                self.backup_id, self.block_number, SupplierNumber, "Data"
            )
            customerID, remotePath = packetid.SplitPacketID(PacketID)
            self.OnHandData[SupplierNumber] = bool(
                os.path.exists(
                    os.path.join(settings.getLocalBackupsDir(), customerID, remotePath)
                )
            )
        for SupplierNumber in range(self.EccMap.paritysegments):
            PacketID = packetid.MakePacketID(
                self.backup_id, self.block_number, SupplierNumber, "Parity"
            )
            customerID, remotePath = packetid.SplitPacketID(PacketID)
            self.OnHandParity[SupplierNumber] = bool(
                os.path.exists(
                    os.path.join(settings.getLocalBackupsDir(), customerID, remotePath)
                )
            )
    ...
    def doRestoreBlock(self, *args, **kwargs):
        ...
        try:
            ...
            blockdata = blockbits[
                splitindex + 1 : splitindex + 1 + datalength
            ]  # remove padding from raidmake/ECC
            newblock = encrypted.Unserialize(
                blockdata, decrypt_key=self.key_id
            )  # convert to object
        except:
            if _Debug:
        ...
        if not newblock:
            lg.warn(
                "block read/unserialize failed from %d bytes of data" % len(blockbits)
            )
            self.automat("block-failed")
            return
        self.automat(
    ...
    def doSavePacket(self, *args, **kwargs):
        ...
        filename = os.path.join(
            settings.getLocalBackupsDir(), customer_id, glob_path["path"]
        )
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "restore_worker.doSavePacket %s saved to %s" % (packetID, filename),
            )
    ...
    def doReadRaid(self, *args, **kwargs):
        ...
        _, outfilename = tmpfile.make(
            "restore",
            extension=".raid",
            prefix=self.backup_id.replace(":", "_").replace("@", "_").replace("/", "_")
            + "_"
        ...
        inputpath = os.path.join(
            settings.getLocalBackupsDir(), self.customer_id, self.path_id
        )
        task_params = (
            outfilename,
            self.EccMap.name,
    ...
    def doRemoveTempFile(self, *args, **kwargs):
        ...
        if settings.getBackupsKeepLocalCopies():
            return
        from storage import backup_matrix, backup_rebuilder
        ...
        for supplierNum in range(
            ...
            for dataORparity in [
                ...
                packetID = packetid.MakePacketID(
                    self.backup_id, self.block_number, supplierNum, dataORparity
                )
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "restore_worker.doRemoveTempFile %d files were removed" % count,
            )
    ...
    def _do_check_run_requests(self):
        if _Debug:
            lg.out(
                _DebugLevel,
                "restore_worker._do_check_run_requests for %s at block %d"
                % (
                    self.backup_id,
                    self.block_number,
                ),
        ...
        packetsToRequest = []
        for SupplierNumber in range(self.EccMap.datasegments):
            request_packet_id = packetid.MakePacketID(
                self.backup_id, self.block_number, SupplierNumber, "Data"
            )
            if self.OnHandData[SupplierNumber]:
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "        SKIP, OnHandData is True for supplier %d"
                        % SupplierNumber,
                    )
            ...
            if request_packet_id in self.block_requests:
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "        SKIP, request for packet %r already sent to IO queue for supplier %d"
                        % (
                            request_packet_id,
                            SupplierNumber,
                        ),
        ...
        for SupplierNumber in range(self.EccMap.paritysegments):
            ...
            if self.OnHandParity[SupplierNumber]:
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "        SKIP, OnHandParity is True for supplier %d"
                        % SupplierNumber,
                    )
                if request_packet_id not in self.block_requests:
                    self.block_requests[request_packet_id] = True
        ...
        for SupplierID, packetID in packetsToRequest:
            if io_throttle.HasPacketInRequestQueue(SupplierID, packetID):
                ...
                lg.warn(
                    "packet already in IO queue for supplier %s : %s"
                    % (
                        SupplierID,
                        packetID,
                    )
            ...
            if _Debug:
                lg.dbg(
                    _DebugLevel,
                    "sent request %r to %r, other requests: %r"
        ...
        if requests_made:
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "        requested %d packets for block %d"
                    % (
                        requests_made,
                        self.block_number,
                    ),
        ...
        if pending_count > 0:
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "        nothing for request, currently %d pending packets for block %d"
                    % (
                        pending_count,
                        self.block_number,
                    ),
        ...
        if failed_count > self.max_errors:
            lg.err(
                "all requests finished and %d packets failed, not possible to read data for block %d"
                % (
                    failed_count,
                    self.block_number,
                )
            ...
            reactor.callLater(
                0, self.automat, "request-failed", None
            )  # @UndefinedVariable
            return
        if _Debug:
            lg.out(
                _DebugLevel,
                "        all requests finished for block %d : %r"
                % (
                    self.block_number,
    ...
    def _on_block_restored(self, restored_blocks, filename):
        if _Debug:
            lg.out(
                _DebugLevel,
                "restore_worker._on_block_restored at %s with result: %s"
                % (filename, restored_blocks),
            )
        ...
        if packet_id not in self.block_requests:
            ...
            for req_packet_id in self.block_requests:
                ...
                if resp["version"] == req["version"] and resp["path"] == req["path"]:
                    if (
                        resp["key_alias"] == req["key_alias"]
                        and resp["user"] == req["user"]
                    ):
    ...
    def _on_data_receiver_state_changed(
