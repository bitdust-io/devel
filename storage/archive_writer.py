...
from lib import misc, packetid
from logs import lg
from main import settings
from p2p import commands, p2p_service
from raid import eccmap
from storage import backup, backup_fs, backup_tar
...
class ArchiveWriter(automat.Automat):
    ...
    def doCheckFinished(self, *args, **kwargs):
        ...
        for block_num in self.packets_out.keys():
            ...
            if (
                block_packets_failed > self.correctable_errors * 2
            ):  # because each packet also have Parity()
                lg.err(
    ...
    def _do_start_archive_backup(self):
        archive_id, local_path = self.local_data_callback(
            self.queue_id, self.archive_info
        )
        ...
        backupPipe = backup_tar.backuptarfile_thread(
            local_path, arcname=arcname, compress=compress_mode
        )
    ...
    def _do_send_packets(self, backup_id, block_num):
        ...
        archive_snapshot_dir = os.path.join(
            settings.getLocalBackupsDir(), customer_id, path_id, version_name
        )
        if _Debug:
            lg.args(
                _DebugLevel,
        ...
        for supplier_num in range(len(self.suppliers_list)):
            ...
            for dataORparity in (
                ...
                packet_id = packetid.MakePacketID(
                    backup_id, block_num, supplier_num, dataORparity
                )
                ...
                p2p_service.SendData(
                    ...
                    packetID=packet_id,
                    callbacks={
                        commands.Ack(): lambda newpacket, _: self.automat(
                            "ack", newpacket=newpacket
                        ),
                        commands.Fail(): lambda newpacket, _: self.automat(
                            "fail", newpacket=newpacket
                        ),
                    },
                )
