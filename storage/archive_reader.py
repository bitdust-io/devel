...
from lib import packetid, serialization
from logs import lg
from p2p import commands, p2p_service
from raid import eccmap
from storage import backup_fs, backup_matrix, backup_tar, restore_worker
from system import local_fs, tmpfile
...
class ArchiveReader(automat.Automat):
    ...
    def doInit(self, *args, **kwargs):
        ...
        self.group_key_id = my_keys.make_key_id(
            alias=self.queue_alias, creator_glob_id=self.queue_owner_id
        )
        self.suppliers_list = []
    ...
    def doRequestTheirListFiles(self, *args, **kwargs):
        ...
        Action method.
        """
        groups.create_archive_folder(
            self.group_key_id, force_path_id=self.archive_folder_path
        )
    ...
    def doExtractArchive(self, *args, **kwargs):
        ...
        Action method.
        """
        self._do_extract_archive(
            backup_id=kwargs["backup_id"],
            tarfilename=kwargs["tarfilename"],
            backup_index=kwargs["backup_index"],
        )
    ...
    def _do_request_list_files(self, suppliers_list):
        ...
        for supplier_pos, supplier_idurl in enumerate(suppliers_list):
            ...
            outpacket = p2p_service.SendListFiles(
                ...
                timeout=30,
                callbacks={
                    commands.Fail(): lambda resp, info: self._on_list_files_failed(
                        supplier_pos
                    ),
        ...
        self.request_list_files_timer = reactor.callLater(
            30, self._on_request_list_files_timeout
        )  # @UndefinedVariable

    def _do_select_archive_snapshots(self):
        iterID_and_path = backup_fs.WalkByID(
            self.archive_folder_path,
            iterID=backup_fs.fsID(self.queue_owner_idurl, self.queue_alias),
        )
        if iterID_and_path is None:
            lg.err(
                "did not found archive folder in the catalog: %r"
                % self.archive_folder_path
            )
        ...
        if not snapshots_list:
            lg.err(
                "no available snapshots found in archive list: %r"
        ...
        for backup_id in snapshots_list:
            ...
            try:
                ...
                continue
            if (
                self.start_sequence_id is not None
                and self.start_sequence_id > snapshot_sequence_id
            ):
                continue
            if (
                self.end_sequence_id is not None
                and self.end_sequence_id < snapshot_sequence_id
            ):
                continue
    ...
    def _do_restore_next_backup(self, backup_index):
        ...
        rw.MyDeferred.addCallback(
            self._on_restore_done, backup_id, outfd, outfilename, backup_index
        )
        if _Debug:
        ...
        rw.MyDeferred.addErrback(
            self._on_restore_failed, backup_id, outfd, outfilename, backup_index
        )
        rw.automat("init")
    ...
    def _do_extract_archive(self, backup_id, tarfilename, backup_index):
        ...
        d.addCallback(
            self._on_extract_done, backup_id, tarfilename, snapshot_dir, backup_index
        )
    ...
    def _on_extract_failed(self, err, backupID, source_filename, output_location):
        lg.err(
            "archive %r extract failed from %r to %r with: %r"
    ...
    def _on_extract_done(
        self, retcode, backupID, source_filename, output_location, backup_index
    ):
