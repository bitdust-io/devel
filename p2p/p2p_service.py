...
def inbox(newpacket, info, status, error_message):
    ...
    if not newpacket.Valid():              
        io.log(1, 'p2p_service.inbox ERROR new packet is not Valid')
        return False
  
    if newpacket.CreatorID != misc.getLocalID() and newpacket.RemoteID != misc.getLocalID():
        io.log(1, "p2p_service.inbox  ERROR packet is NOT for us")
        io.log(1, "p2p_service.inbox  getLocalID=" + misc.getLocalID() )
        io.log(1, "p2p_service.inbox  CreatorID=" + newpacket.CreatorID )
        io.log(1, "p2p_service.inbox  RemoteID=" + newpacket.RemoteID )
        io.log(1, "p2p_service.inbox  PacketID=" + newpacket.PacketID )
        return False
    ...
    if not commandhandled:
        io.log(6, "p2p_service.inbox WARNING [%s] from %s|%s (%s://%s) NOT handled" % (
...
def outbox(outpacket):
    io.log(6, "p2p_service.outbox [%s] to %s" % (outpacket.Command, nameurl.GetName(outpacket.RemoteID)))
    return True
...
def makeFilename(customerID, packetID):
    ...
    if not packetid.Valid(packetID): # SECURITY
        if packetID not in [    settings.BackupInfoFileName(), 
            ...
            # io.log(1, "p2p_service.makeFilename ERROR failed packetID format: " + packetID )
            return ''
    if not contacts.IsCustomer(customerID):  # SECURITY
        io.log(4, "p2p_service.makeFilename WARNING %s not a customer: %s" % (customerID, str(contacts.getCustomerNames())))
        return ''
...
def SendAck(packettoack, response=''):
    result = packet.Signed(commands.Ack(), misc.getLocalID(), misc.getLocalID(), 
                                 packettoack.PacketID, response, packettoack.OwnerID)
    io.log(8, "p2p_service.SendAck %s to %s" % (result.PacketID, result.RemoteID))
    gate.outbox(result)
    return result
    

def Ack(newpacket):
    io.log(8, "p2p_service.Ack %s from [%s] : %s" % (newpacket.PacketID, newpacket.CreatorID, newpacket.Payload))
    # for p in packet_out.search_by_packet_id(newpacket.CreatorID, newpacket.PacketID):
    #     io.log(8, '        found matched outbox packet : %r' % p)
    #     p.automat('ack', newpacket)
     
    
def SendFail(request, response=''):
    result = packet.Signed(commands.Fail(), misc.getLocalID(), misc.getLocalID(), 
                                 request.PacketID, response, request.OwnerID) # request.CreatorID)
    io.log(8, "transport_control.SendFail %s to %s" % (result.PacketID, result.RemoteID))
    gate.outbox(result)
    return result
    

def Fail(newpacket):
    io.log(8, "p2p_service.Fail from [%s]: %s" % (newpacket.CreatorID, newpacket.Payload))
...
def Identity(newpacket):
    ...
    #     io.log(1,"p2p_service.Identity ERROR has non-Valid identity")
    #     return
    ...
    if not newpacket.Valid():
        ...
        io.log(6, "p2p_service.Identity WARNING not Valid packet from %s" % idurl)
        return

    if newpacket.OwnerID == idurl:
        SendAck(newpacket)
        io.log(8, "p2p_service.Identity from [%s], sent Ack" % nameurl.GetName(idurl))
    else:
        io.log(8, "p2p_service.Identity from [%s]" % nameurl.GetName(idurl))
...
def RequestIdentity(request):
    ...
    """
    io.log(6, "p2p_service.RequestIdentity starting")
    MyID = misc.getLocalID()
    RemoteID = request.OwnerID
    PacketID = request.PacketID
    identitystr = misc.getLocalIdentity().serialize()
    io.log(8, "p2p_service.RequestIdentity returning ")
    result = packet.Signed(commands.Identity(), MyID, MyID, PacketID, identitystr, RemoteID)
    gate.outbox(result, False)
       
def SendIdentity(remote_idurl, wide=False):
    """
    """
    io.log(8, "p2p_service.SendIdentity to %s" % nameurl.GetName(remote_idurl))
    result = packet.Signed(commands.Identity(), misc.getLocalID(), 
...
def RequestService(request):
    io.log(8, "p2p_service.RequestService")
    if request.Payload.startswith('storage'):
        ...
        if free_mb <= mb_for_customer:
            io.log(8, "    DENY - not enough space")
        ...
        io._write_dict(settings.CustomersSpaceFile(), space_dict)
...
def SendRequestService(remote_idurl, service_info, response_callback=None):
    io.log(8, "p2p_service.SendRequestService to %s [%s]" % (nameurl.GetName(remote_idurl), service_info))
    result = packet.Signed(commands.RequestService(), misc.getLocalID(), misc.getLocalID(), 
...
def CancelService(request):
    io.log(8, "p2p_service.CancelService")
    if request.Payload.startswith('storage'):
        if not contacts.IsCustomer(request.OwnerID):
            io.log(6, "p2p_service.CancelService WARNING got packet from %s, but he is not a customer" % request.OwnerID)
            return SendFail(request, 'not a customer')
        mb_donated = diskspace.GetMegaBytesFromString(settings.getCentralMegabytesDonated())
        space_dict = io._read_dict(settings.CustomersSpaceFile(), {'free': str(mb_donated)})
        if request.OwnerID not in space_dict.keys():
            io.log(6, "p2p_service.CancelService WARNING got packet from %s, but not found him in space dictionary" % request.OwnerID)
            return SendFail(request, 'not a customer')
        ...
        reactor.callLater(0, local_tester.TestUpdateCustomers)
        return SendAck(request, 'accepted')
    io.log(6, "p2p_service.CancelService WARNING got wrong payload in %s" % request)
    return SendFail(request, 'wrong service request')

def SendCancelService(remote_idurl, service_info, response_callback=None):
    io.log(8, "p2p_service.SendCancelService [%s]" % service_info)
    result = packet.Signed(commands.CancelService(), misc.getLocalID(), misc.getLocalID(), 
                                 packetid.UniqueID(), service_info, remote_idurl)
    gate.outbox(result, callbacks={
        commands.Ack(): response_callback,
        commands.Fail(): response_callback})
...
def ListFiles(request):
    ...
    io.log(8, "p2p_service.ListFiles from [%s], format is %s" % (nameurl.GetName(request.OwnerID), Payload))
    ...
    if not os.path.isdir(ownerdir):
        io.log(8, "p2p_service.ListFiles did not find customer dir " + ownerdir)
        src = PackListFiles('', Payload)
        result = packet.Signed(commands.Files(), MyID, MyID, PacketID, src, RemoteID)
        gate.outbox(result)
        return result
...
def Files(packet):
    ...
    """
    io.log(8, "p2p_service.Files from [%s]" % nameurl.GetName(packet.OwnerID))
...
def Data(request):
    ...
    if request.OwnerID == misc.getLocalID():
        io.log(8, "p2p_service.Data %r for us from %s" % (
...
#            return
    ...
    if not contacts.IsCustomer(request.OwnerID):  # SECURITY
        ...
        io.log(6, "p2p_service.Data WARNING %s not a customer, packetID=%s" % (request.OwnerID, request.PacketID))
    ...
    if filename == "":
        io.log(6,"p2p_service.Data WARNING got empty filename, bad customer or wrong packetID? ")
    ...
    if not os.path.exists(dirname):
        try:
            ...
            io.log(2, "p2p_service.Data ERROR can not create sub dir " + dirname)
    ...
    space_dict = io._read_dict(settings.CustomersSpaceFile(), {'free': str(mb_donated)})
    if request.OwnerID not in space_dict.keys():
        io.log(6, "p2p_service.Data WARNING no info about donated space for %s" % request.OwnerID)
        SendFail(request, 'no info about donated space')
        return
    used_space_dict = io._read_dict(settings.CustomersUsedSpaceFile(), {})
    if request.OwnerID in used_space_dict.keys():
        try:
            ...
            if bytes_donated_to_customer - bytes_used_by_customer < len(data):
                io.log(6, "p2p_service.Data WARNING no free space for %s" % request.OwnerID)
                SendFail(request, 'no free space')
                return
        except:
            io.exception()
    if not io.WriteFile(filename, data):
        io.log(2, "p2p_service.Data ERROR can not write to " + str(filename))
        SendFail(request, 'write error')
        return
    SendAck(request, str(len(request.Payload)))
    reactor.callLater(0, local_tester.TestSpaceTime)
    del data
    io.log(8, "p2p_service.Data saved from [%s/%s], packetID is %s" % (
...
def Retrieve(request):
    ...
    """
    if not contacts.IsCustomer(request.OwnerID):
        io.log(4, "p2p_service.Retrieve WARNING had unknown customer " + request.OwnerID)
        SendFail(request, 'not a customer')
        return
    filename = makeFilename(request.OwnerID, request.PacketID)
    if filename == '':
        io.log(4, "p2p_service.Retrieve WARNING had empty filename")
        SendFail(request, 'empty filename')
        return
    if not os.path.exists(filename):
        io.log(4, "p2p_service.Retrieve WARNING did not find requested packet " + filename)
        SendFail(request, 'did not find requested packet')
        return
    if not os.access(filename, os.R_OK):
        io.log(4, "p2p_service.Retrieve WARNING no read access to requested packet " + filename)
        SendFail(request, 'no read access to requested packet')
        return
    data = io.ReadBinaryFile(filename)
    if not data:
        io.log(4, "p2p_service.Retrieve WARNING empty data on disk " + filename)
        SendFail(request, 'empty data on disk')
        return
    packet = packet.Unserialize(data)
    del data 
    if packet is None:
        io.log(4, "p2p_service.Retrieve WARNING Unserialize fails, not Valid packet " + filename)
        SendFail(request, 'unserialize fails')
        return
    if not packet.Valid():
        io.log(4, "p2p_service.Retrieve WARNING unserialized packet is not Valid " + filename)
        SendFail(request, 'unserialized packet is not Valid')
        return
    io.log(8, "p2p_service.Retrieve sending %r back to %s" % (packet, nameurl.GetName(packet.CreatorID)))
    gate.outbox(packet)
...
def DeleteFile(request):
    ...
    for pathID in ids:
        ...
        if filename == "":
            ...
            if not os.path.exists(filename):
                io.log(1, "p2p_service.DeleteFile WARNING had unknown customer: %s or pathID is not correct or not exist: %s" % (nameurl.GetName(request.OwnerID), pathID))
                return
        if os.path.isfile(filename):
            ...
            try:
                io._dir_remove(filename)
                dirscount += 1
            except:
                io.exception()
        else:
            io.log(1, "p2p_service.DeleteFile WARNING path not found %s" % filename)
    io.log(8, "p2p_service.DeleteFile from [%s] with %d IDs, %d files and %d folders were removed" % (
        nameurl.GetName(request.OwnerID), len(ids), filescount, dirscount))
    SendAck(request)
    

def SendDeleteFile(SupplierID, pathID):
    io.log(8, "p2p_service.SendDeleteFile SupplierID=%s PathID=%s " % (SupplierID, pathID))
    MyID = misc.getLocalID()
    PacketID = pathID
    RemoteID = SupplierID
    result = packet.Signed(commands.DeleteFile(),  MyID, MyID, PacketID, "", RemoteID)
    gate.outbox(result)
    return result
    
    
def SendDeleteListPaths(SupplierID, ListPathIDs):
    io.log(8, "p2p_service.SendDeleteListPaths SupplierID=%s PathIDs number: %d" % (SupplierID, len(ListPathIDs)))
    MyID = misc.getLocalID()
    PacketID = packetid.UniqueID()
    RemoteID = SupplierID
    Payload = '\n'.join(ListPathIDs)
    result = packet.Signed(commands.DeleteFile(),  MyID, MyID, PacketID, Payload, RemoteID)
    gate.outbox(result)
    return result
...
def DeleteBackup(request):
    ...
    for backupID in ids:
        ...
        if filename == "":
            ...
            if not os.path.exists(filename):
                io.log(1, "p2p_service.DeleteBackup WARNING had unknown customer " + request.OwnerID + " or backupID " + backupID)
                return
        if os.path.isdir(filename):
            ...
            io.log(1, "p2p_service.DeleteBackup WARNING path not found %s" % filename)
    SendAck(request)
    io.log(8, "p2p_service.DeleteBackup from [%s] with %d IDs, %d were removed" % (nameurl.GetName(request.OwnerID), len(ids), count))


def SendDeleteBackup(SupplierID, BackupID):
    io.log(8, "p2p_service.SendDeleteBackup SupplierID=%s  BackupID=%s " % (SupplierID, BackupID))
    MyID = misc.getLocalID()
    PacketID = BackupID
    RemoteID = SupplierID
    result = packet.Signed(commands.DeleteBackup(),  MyID, MyID, PacketID, "", RemoteID)
    gate.outbox(result)
    return result

def SendDeleteListBackups(SupplierID, ListBackupIDs):
    io.log(8, "p2p_service.SendDeleteListBackups SupplierID=%s BackupIDs number: %d" % (SupplierID, len(ListBackupIDs)))
    MyID = misc.getLocalID()
    PacketID = packetid.UniqueID()
    RemoteID = SupplierID
    Payload = '\n'.join(ListBackupIDs)
    result = packet.Signed(commands.DeleteBackup(),  MyID, MyID, PacketID, Payload, RemoteID)
    gate.outbox(result)
    return result
...
def Correspondent(request):
    io.log(8, "p2p_service.Correspondent")
    MyID = misc.getLocalID()
    RemoteID = request.OwnerID
    PacketID = request.PacketID
...
def ListCustomerFiles(customer_idurl):
    ...
    def cb(realpath, subpath, name):
        if os.path.isdir(realpath):
            ...
            result.write('F%s\n' % subpath)
        return True
    io.traverse_dir_recursive(cb, customer_dir)
    src = result.getvalue()
    result.close()
    return src
...
def RequestListFiles(supplierNumORidurl):
    ...
    if not RemoteID:
        io.log(4, "p2p_service.RequestListFiles WARNING RemoteID is empty supplierNumORidurl=%s" % str(supplierNumORidurl))
        return None
    io.log(8, "p2p_service.RequestListFiles [%s]" % nameurl.GetName(RemoteID))
    MyID = misc.getLocalID()
    PacketID = packetid.UniqueID()
    Payload = settings.ListFilesFormat()
    result = packet.Signed(commands.ListFiles(), MyID, MyID, PacketID, Payload, RemoteID)
    gate.outbox(result)
    return result
...
def TreeSummary(ownerdir):
    ...
    def cb(result, realpath, subpath, name):
        ...
        del parityMissing
        return False
    io.traverse_dir_recursive(lambda realpath, subpath, name: cb(out, realpath, subpath, name), ownerdir)
    src = out.getvalue()
    out.close()
    return src
...
def RequestDeleteBackup(BackupID):
    ...
    """
    io.log(8, "p2p_service.RequestDeleteBackup with BackupID=" + str(BackupID))
...
def RequestDeleteListBackups(backupIDs):
    io.log(8, "p2p_service.RequestDeleteListBackups wish to delete %d backups" % len(backupIDs))
    for supplier in contacts.getSupplierIDs():
        if not supplier:
            continue
        found = False
...
def RequestDeleteListPaths(pathIDs):
    io.log(8, "p2p_service.RequestDeleteListPaths wish to delete %d paths" % len(pathIDs))
    for supplier in contacts.getSupplierIDs():
        if not supplier:
            continue
        found = False
...
def CheckWholeBackup(BackupID):
    io.log(8, "p2p_service.CheckWholeBackup with BackupID=" + BackupID)
