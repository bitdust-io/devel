...
def do_service_test(service_name, result_defer, wait_timeout):
    ...
    if _Debug:
        lg.args(
            _DebugLevel, service=service_name, state=svc_state, wait_timeout=wait_timeout
        )
    if svc_state == "STARTING":
        reactor.callLater(
            0.1, do_service_test, service_name, result_defer, wait_timeout
        )  # @UndefinedVariable
        return None
    if svc_state != "ON":
    ...
    if service_name == "service_network":
        reactor.callLater(
            0, do_service_test, "service_gateway", result_defer, wait_timeout
        )  # @UndefinedVariable
    elif service_name == "service_gateway":
        reactor.callLater(
            0, do_service_test, "service_p2p_hookups", result_defer, wait_timeout
        )  # @UndefinedVariable
        ...
        reactor.callLater(
            0, do_service_proxy_transport_test, result_defer
        )  # @UndefinedVariable
    else:
...
def do_service_restart(service_name, result_defer, wait_timeout):
    ...
    d.addErrback(
        lambda err: result_defer.callback(
            dict(
                error=err,
...
def on_service_proxy_transport_check_healthy(healthy, wait_timeout):
    ...
    d.addErrback(
        ...
        method="network_service.on_service_proxy_transport_check_healthy",
        ignore=True,
    )
            ...
            if p2p_connector_machine and p2p_connector_machine.state == "CONNECTED":
                ...
                if proxy_receiver_lookup:
                    ...
                    if (
                        ...
                        and proxy_receiver_machine.state == "LISTEN"
                    ):
                    ...
                    d.addCallback(
                        on_service_proxy_transport_check_healthy,
