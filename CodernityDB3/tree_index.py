...
from .index import DocIdNotFound, ElemNotFound, Index, IndexException
...
class IU_TreeBasedIndex(Index):
    ...
    def _count_props(self):
        ...
        self.leaf_heading_format = self.elements_counter_format + self.pointer_format * 2
    ...
    def _read_single_node_key(self, node_start, key_index):
        self.buckets.seek(self._calculate_key_position(node_start, key_index, b"n"))
    ...
    def _read_single_leaf_record(self, leaf_start, key_index):
        r = self._calculate_key_position(leaf_start, key_index, b"l")
    ...
    def _calculate_key_position(self, start, key_index, flag):
        ...
        if flag == b"l":
            return int(
                start + self.leaf_heading_size + key_index * self.single_leaf_record_size
            )
        elif flag == b"n":
        ...
        while True:
            if curr_key_index < nr_of_elements:
                ...
                if key != curr_key:
                    ...
                    raise DocIdNotFound
                elif doc_id == curr_doc_id and (
                    curr_status != b"d" and curr_status != "d"
                ):
    ...
    def _update_element(self, leaf_start, key_index, new_data):
        self.buckets.seek(
            self._calculate_key_position(leaf_start, key_index, b"l") + self.key_size
        )
        # Fix types
        ...
        self.buckets.write(struct.pack(b"<" + fixed_meta_format, *new_data))

    #        self._read_single_leaf_record.delete(leaf_start_position, key_index)

    def _delete_element(self, leaf_start, key_index):
        self.buckets.seek(
            self._calculate_key_position(leaf_start, key_index, b"l")
    ...
    def _choose_next_candidate_index_in_leaf(
        ...
        if buffer_start > candidate_start:
            ...
            move_buffer = None
        return (
            self._calculate_key_position(leaf_start, (imin + imax) // 2, b"l"),
            (imin + imax) // 2,
            move_buffer,
        )
    ...
    def _choose_next_candidate_index_in_node(
        ...
        if buffer_start > candidate_start:
            ...
            move_buffer = None
        return (
            self._calculate_key_position(node_start, (imin + imax) // 2, b"n"),
            (imin + imax) // 2,
            move_buffer,
        )
    ...
    def _find_key_in_leaf_using_binary_search(
        ...
        while imax != imin and imax > imin:
            ...
            if key < curr_key:
                if move_buffer == MOVE_BUFFER_PREV:
                    buffer_start, buffer_end = self._prev_buffer(buffer_start, buffer_end)
                else:
                ...
                if move_buffer == MOVE_BUFFER_NEXT:
                    buffer_start, buffer_end = self._next_buffer(buffer_start, buffer_end)
                else:
                    move_buffer is None
                imin = candidate_index + 1
                (
        ...
        if (
            doc_id
            and doc_id == curr_doc_id
            and (curr_status == b"o" or curr_status == "o")
        ):
            return (
                leaf_start,
                chosen_key_position,
            ...
            if (
                ...
                matching_record_index = self._leaf_linear_key_search(
                    key,
                    self._calculate_key_position(leaf_start, imin, b"l"),
                    imin,
                    chosen_key_position,
                )
            else:
        ...
        if curr_status == b"d" or curr_status == "d":
            ...
            if chosen_key_position < nr_of_elements - 1:
                ...
                if curr_start == b"d" or curr_start == "d":
                    ...
                    new_record_position = chosen_key_position + 1
                    nr_of_records_to_rewrite = nr_of_elements - chosen_key_position - 1
                    full_leaf = nr_of_elements == self.node_capacity
                    on_deleted = False
            else:
                new_record_position = chosen_key_position + 1
    ...
    def _find_key_in_node_using_binary_search(
        ...
        if (
            ...
            matching_record_index = self._node_linear_key_search(
                key,
                self._calculate_key_position(node_start, imin, b"n"),
                imin,
                chosen_key_position,
            )
        else:
    ...
    def _update_leaf_ready_data(
        ...
        start_position = self._calculate_key_position(leaf_start, start_index, b"l")
    ...
    def _update_leaf(
        ...
        if nr_of_records_to_rewrite == 0:  # just write at set position
            self.buckets.seek(
                self._calculate_key_position(leaf_start, new_record_position, b"l")
            )
            self.buckets.write(
                struct.pack(
                    b"<" + fixed_leaf,
            ...
            start = self._calculate_key_position(leaf_start, new_record_position, b"l")
    ...
    def _update_leaf_size_and_pointers(self, leaf_start, new_size, new_prev, new_next):
        ...
        self.buckets.write(
            struct.pack(
                b"<"
                + str(self.elements_counter_format + 2 * self.pointer_format).encode(),
                new_size,
                new_prev,
                new_next,
            )
    ...
    def _update_leaf_prev_pointer(self, leaf_start, pointer):
        self.buckets.seek(leaf_start + self.elements_counter_size)
        self.buckets.write(struct.pack(b"<" + str(self.pointer_format).encode(), pointer))
    ...
    def _update_size(self, start, new_size):
        self.buckets.seek(start)
        self.buckets.write(
            struct.pack(b"<" + str(self.elements_counter_format).encode(), new_size)
        )
    ...
    def _create_new_root_from_leaf(
        ...
        blanks = (
            (self.node_capacity - new_leaf_size) * self.single_leaf_record_size * b"\x00"
        )
        ...
        left_leaf_data += (
            (self.node_capacity - old_leaf_size) * self.single_leaf_record_size * b"\x00"
        )
            ...
            if nr_of_records_to_rewrite > half_size:  # insert key into first half of leaf
                self.buckets.seek(
                    self._calculate_key_position(
                        leaf_start, self.node_capacity - nr_of_records_to_rewrite, b"l"
                    )
                )
                ...
                self.buckets.seek(
                    self._calculate_key_position(leaf_start, old_leaf_size, b"l")
                )
                data = self.buckets.read(
    ...
    def _prepare_new_root_data(
        self, root_key, left_pointer, right_pointer, children_flag=b"n"
    ):
        new_root = struct.pack(
            b"<"
            + str(self.node_heading_format + self.single_node_record_format).encode(),
            1,
            children_flag,
            left_pointer,
            root_key,
    ...
    def _split_node(
        ...
        if create_new_root:
            ...
            if (
                ...
                # reading second half of node
                self.buckets.seek(
                    self._calculate_key_position(node_start, old_node_size, b"n")
                    + self.pointer_size
                )
                # read all keys with key>new_key
                data = self.buckets.read(
                ...
                self.buckets.seek(
                    self._calculate_key_position(
                        node_start, self.node_capacity - nr_of_keys_to_rewrite, b"n"
                    )
                    + self.pointer_size
                )
                # read all keys with key>new_key
    ...
    def insert_first_record_into_leaf(self, leaf_start, key, doc_id, start, size, status):
        ...
        self.buckets.seek(leaf_start)
        self.buckets.write(
            struct.pack(b"<" + str(self.elements_counter_format).encode(), 1)
        )
        self.buckets.seek(leaf_start + self.leaf_heading_size)
        self.buckets.write(
            struct.pack(
                b"<" + self.single_leaf_record_format.encode(),
                key,
                doc_id,
                start,
                size,
    ...
    def _update_node(self, new_key_position, nr_of_keys_to_rewrite, new_key, new_pointer):
        if nr_of_keys_to_rewrite == 0:
            ...
            self.buckets.write(
                struct.pack(
                    b"<" + str(self.key_format + self.pointer_format).encode(),
                    new_key,
                    new_pointer,
                )
            )
    ...
    def _insert_new_key_into_node(
        ...
        parent_prev_pointer = self._read_single_node_key(node_start, parent_key_index)[0]
        if (
            ...
            new_key_position = self.pointer_size + self._calculate_key_position(
                node_start, parent_key_index, b"n"
            )
            ...
            new_key_position = self.pointer_size + self._calculate_key_position(
                node_start, parent_key_index + 1, b"n"
            )
