...
class Initializer(automat.Automat):
    ...
    def A(self, event, *args, **kwargs):
        from main import installer, shutdowner
        ...
        # ---LOCAL---
            ...
            if (
                event == "init-local-done"
                and not self.isInstalled(*args, **kwargs)
                and self.isGUIPossible(*args, **kwargs)
            ):
                self.state = "INSTALL"
                installer.A("init")
                shutdowner.A("ready")
        ...
        # ---INSTALL---
            ...
            if not self.flagCmdLine and (
                event == "installer.state" and args[0] == "DONE"
            ):
                self.state = "STOPPING"
    ...
    def doInitInterfaces(self, *args, **kwargs):
        ...
        if settings.enableAPIAuthSecret():
            ...
            if not current_secret:
                ...
                lg.info(
                    "generated new API auth secret text and stored in %r"
                    % settings.APISecretFile()
                )
    ...
    def _check_install(self):
        ...
        try:
            ...
            if _Debug:
                lg.out(
                    _DebugLevel, "initializer._check_install fail loading private key "
                )
            return False
        try:
            ...
            if _Debug:
                lg.out(
                    _DebugLevel, "initializer._check_install fail init local identity "
                )
            return False
    ...
    def _init_local(self):
        ...
        from lib import misc, net_misc
        from p2p import commands
        from raid import eccmap
        from system import run_upnpc, tmpfile
        ...
        if _Debug:
            lg.dbg(
                _DebugLevel,
                "all local modules are initialized, ready to start the engine",
            )
    ...
    def _on_software_code_updated(self, evt):
        if _Debug:
            lg.out(
                _DebugLevel,
                'initializer._on_software_code_updated will RESTART BitDust now! "source-code-fetched" event received',
            )
        if False:
