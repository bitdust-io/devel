...
StarterFilename = os.path.basename(sys.argv[0].lower()) #'bitstarter.exe'
...
StarterFilename = 'bitstarter.exe'

LogFilePath = sharedPath('bitstarter.log', 'logs')
...
def main():
    ...
    if len(find_process_win32(search_list)) > 0:
        logwrite('another bitstarter.exe found working')
        return 0
    ...
    if new_version == cur_version and cur_version != '':
        ...
        # so they needed only during first start of the bitstarter.exe
