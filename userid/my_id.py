...
from userid import id_url, identity
...
def loadLocalIdentity():
    ...
    if not os.path.isfile(key_filename):
        ...
        if not os.path.isfile(keyfilenamelocation):
            lg.warn(
                "not possible to load local identity because master key file does not exist at specified location"
            )
            return False
        key_filename = bpio.ReadTextFile(keyfilenamelocation).strip()
        if not os.path.isfile(key_filename):
            lg.warn(
                "not possible to load local identity because master key file does not exist at default location"
            )
            return False
    if os.path.exists(lid_filename):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "my_id.loadLocalIdentity %d bytes read from %s"
                % (len(xmlid), lid_filename),
            )
    if not xmlid:
        if _Debug:
            lg.out(
                _DebugLevel,
                "my_id.loadLocalIdentity SKIPPED, local identity in %s is EMPTY !!!"
                % lid_filename,
            )
        return False
    ...
    if not lid.Valid():
        if _Debug:
            lg.out(
                _DebugLevel, "my_id.loadLocalIdentity ERROR loaded identity is not Valid"
            )
        return False
...
def saveLocalIdentity():
    ...
    setTransportOrder(getOrderFromContacts(_LocalIdentity))
    events.send(
        "local-identity-written",
        data=dict(idurl=_LocalIdentity.getIDURL(), filename=filename),
    )
    if _Debug:
        lg.out(
            _DebugLevel,
...
def forgetLocalIdentity():
    ...
    if not isLocalIdentityReady():
        if _Debug:
            lg.out(
                _DebugLevel, "my_id.forgetLocalIdentity ERROR local identity not exist!"
            )
        return False
    if _Debug:
...
def eraseLocalIdentity(do_backup=True):
    if do_backup:
        if os.path.isfile(settings.LocalIdentityFilename()):
            current_identity_xmlsrc = local_fs.ReadBinaryFile(
                settings.LocalIdentityFilename()
            )
            if current_identity_xmlsrc:
                fd, fname = tempfile.mkstemp(
                    prefix="localidentity_", dir=settings.MetaDataDir()
                )
                os.write(fd, current_identity_xmlsrc)
                os.close(fd)
                lg.info(
                    "created backup copy of my local identity in the file : %r" % fname
                )
    filename = bpio.portablePath(settings.LocalIdentityFilename())
    if not os.path.exists(filename):
        if _Debug:
            lg.out(
                _DebugLevel, "my_id.eraseLocalIdentity SKIP file %s not exist" % filename
            )
        return True
    if not os.path.isfile(filename):
        if _Debug:
            lg.out(
                _DebugLevel,
                "my_id.eraseLocalIdentity ERROR path %s is not a file" % filename,
            )
        return False
    try:
        os.remove(filename)
...
def buildDefaultIdentity(name="", ip="", idurls=[], revision=0):
    ...
    if len(new_contacts) == 0:
        ...
        if settings.enableUDP() and settings.enableUDPreceiving():
            ...
            new_contacts["udp"] = (
                b"udp://" + strng.to_bin(name.lower()) + b"@" + strng.to_bin(servername)
            )
    ...
    if changed:
        if save_identity:
            ...
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "    LOCAL IDENTITY CORRECT: %r" % getLocalIdentity().isCorrect(),
                )
                lg.out(
                    _DebugLevel,
                    "    LOCAL IDENTITY VALID: %r" % getLocalIdentity().Valid(),
                )
