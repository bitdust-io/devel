...
class IdRestorer(automat.Automat):
    ...
    def doVerifyAndRestore(self, *args, **kwargs):
        ...
        try:
            ...
            reactor.callLater(
                0.1, self.automat, "restore-failed", ("private key is not valid", "red")
            )  # @UndefinedVariable
            return
    ...
    def _on_my_dht_relations_discovered(self, dht_result):
        ...
        settings.config.conf().setInt(
            "services/customer/suppliers-number", dht_desired_suppliers_number
        )
        contactsdb.set_suppliers(dht_suppliers)
        contactsdb.save_suppliers()
        lg.info(
            "found and restored list of %d suppliers from DHT"
