...
def init():
    ...
    for one_user_dir in os.listdir(_IdentityHistoryDir):
        ...
        for one_ident_file in one_user_identity_files:
            one_ident_path = os.path.join(
                one_user_dir_path, strng.to_text(one_ident_file)
            )
            try:
                ...
                if not known_id_obj.isCorrect():
                    raise Exception(
                        "identity history in %r is broken, identity is not correct: %r"
                        % (one_user_dir, one_ident_path)
                    )
                if not known_id_obj.Valid():
                    raise Exception(
                        "identity history in %r is broken, identity is not valid: %r"
                        % (one_user_dir, one_ident_path)
                    )
            ...
            for known_idurl in reversed(known_sources):
                ...
                if one_pub_key not in _MergedIDURLs:
                    ...
                    if _Debug:
                        lg.out(
                            _DebugLevel,
                            "    new Public Key added: %s..." % one_pub_key[-10:],
                        )
                if one_revision in _MergedIDURLs[one_pub_key]:
                    if _MergedIDURLs[one_pub_key][one_revision] != known_idurl:
                        if _MergedIDURLs[one_pub_key][one_revision] not in known_sources:
                            lg.warn(
                                "rewriting existing identity revision %d : %r -> %r"
                                % (
                                    one_revision,
                                    _MergedIDURLs[one_pub_key][one_revision],
                                    known_idurl,
                                )
                            )
                    ...
                    if _Debug:
                        lg.out(
                            _DebugLevel,
                            "        revision %d merged with other %d known items"
...
def identity_cached(new_id_obj):
    ...
    if pub_key not in _KnownUsers:
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "id_url.identity_cached wrote first item for user %r in identity history: %r"
        ...
        if len(user_identity_files) == 0:
            lg.warn(
                "identity history for user %r is broken, public key is known, but no identity files found"
                % user_name
            )
        ...
        for id_file in user_identity_files:
            ...
            if not one_id_obj.isCorrect():
                lg.warn(
                    "identity history for user %r is broken, identity in the file %r is not correct"
                    % (user_name, identity_file_path)
                )
                for_cleanup.append(identity_file_path)
                continue
            if not one_id_obj.Valid():
                lg.warn(
                    "identity history for user %r is broken, identity in the file %r is not valid"
                    % (user_name, identity_file_path)
                )
                for_cleanup.append(identity_file_path)
                continue
            ...
            if latest_pub_key != one_id_obj.getPublicKey():
                lg.err(
                    "identity history for user %r is broken, public key not matching in the file %r"
                    % (user_name, identity_file_path)
                )
                for_cleanup.append(identity_file_path)
                continue
        ...
        if xmlsrc:
            ...
            if latest_id_obj.getPublicKey() != new_id_obj.getPublicKey():
                raise Exception(
                    "identity history for user %r is broken, public key not matching"
                    % user_name
                )
            if latest_id_obj.getIDName() != new_id_obj.getIDName():
                lg.warn(
                    "found another user name in identity history for user %r : %r"
                    % (user_name, latest_id_obj.getIDName())
                )
            if new_id_obj.getRevisionValue() in known_revisions:
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "id_url.identity_cached revision %d already known for user %r"
                        % (new_id_obj.getRevisionValue(), user_name),
                    )
            else:
                ...
                if latest_sources == new_sources:
                    local_fs.WriteBinaryFile(
                        latest_identity_file_path, new_id_obj.serialize()
                    )
                    if _Debug:
                        lg.out(
                            _DebugLevel,
                    ...
                    next_identity_file_path = os.path.join(
                        user_path, strng.to_text(next_identity_file)
                    )
                    local_fs.WriteBinaryFile(
                        next_identity_file_path, new_id_obj.serialize()
                    )
    ...
    for new_idurl in reversed(new_sources):
        if new_idurl not in _KnownIDURLs:
            ...
            if _Debug:
                lg.out(
                    _DebugLevel, "id_url.identity_cached new IDURL added: %r" % new_idurl
                )
        else:
        ...
        if pub_key not in _MergedIDURLs:
            ...
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "id_url.identity_cached new Public Key added: %s..." % pub_key[-10:],
                )
        ...
        if new_revision in _MergedIDURLs[pub_key]:
            if _MergedIDURLs[pub_key][new_revision] != new_idurl:
                if nameurl.GetName(
                    _MergedIDURLs[pub_key][new_revision]
                ) == nameurl.GetName(new_idurl):
    ...
    if _Debug:
        lg.args(
            _DebugLevel,
            is_identity_rotated=is_identity_rotated,
            latest_id_obj=bool(latest_id_obj),
        )
    if is_identity_rotated and latest_id_obj is not None:
        latest_revision = latest_id_obj.getRevisionValue()
        if _Debug:
            lg.args(
                _DebugLevel, new_revision=new_revision, latest_revision=latest_revision
            )
        if new_revision > latest_revision:
            lg.info(
                "found rotated identity after caching %r -> %r"
            ...
            if latest_id_obj.getIDURL(as_original=True) != new_id_obj.getIDURL(
                as_original=True
            ):
                events.send(
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "id_url.identity_cached revision %d for %r"
                % (new_revision, new_sources[0]),
            )
...
def field(idurl):
    ...
    if idurl not in _KnownIDURLs:
        if _Debug:
            lg.out(
                _DebugLevel,
                "id_url.field   will try to find %r in local identity cache" % idurl,
            )
...
def is_not_in(idurl, iterable_object, as_field=True, as_bin=False):
    ...
    """
    return not is_in(
        idurl=idurl, iterable_object=iterable_object, as_field=as_field, as_bin=as_bin
    )
...
def is_some_empty(iterable_object):
    ...
    """
    return is_in(
        ID_URL_FIELD(b""), iterable_object=iterable_object, as_field=True, as_bin=False
    )
...
def get_latest_ident(pub_key):
    ...
    if len(user_identity_files) == 0:
        lg.warn(
            "identity history is broken, public key is known, but no identity files found"
        )
    latest_revision = -1
    latest_ident = None
    known_revisions = set()
    ...
    for id_file in user_identity_files:
        ...
        if not one_id_obj.isCorrect():
            lg.warn(
                "identity history is broken, identity in the file %r is not correct"
                % identity_file_path
            )
            for_cleanup.append(identity_file_path)
            continue
        if not one_id_obj.Valid():
            lg.warn(
                "identity history is broken, identity in the file %r is not valid"
                % identity_file_path
            )
            for_cleanup.append(identity_file_path)
            continue
        if pub_key != one_id_obj.getPublicKey():
            lg.err(
                "identity history is broken, public key not matching in the file %r"
                % identity_file_path
            )
            for_cleanup.append(identity_file_path)
            continue
...
class ID_URL_FIELD(object):
    def __init__(self, idurl):
        ...
        if _Debug:
            lg.out(
                _DebugLevel * 2,
                "NEW ID_URL_FIELD(%r) with id=%r latest=%r"
                % (self.current, id(self), self.latest),
            )
    ...
    def __del__(self):
        try:
            if _Debug:
                lg.out(
                    _DebugLevel * 2,
                    "DELETED ID_URL_FIELD(%r) with id=%r latest=%r"
                    % (self.current, id(self), self.latest),
                )
        except:
            lg.exc()
    ...
    def __eq__(self, idurl):
        ...
        if not isinstance(idurl, ID_URL_FIELD):
            ...
            exc = TypeError(
                "tried to compare ID_URL_FIELD(%r) with %r of type %r"
                % (self.latest_as_string, idurl, type(idurl))
            )
            lg.exc(
                msg="called from %s.%s()" % (caller_modul, caller_method), exc_value=exc
            )
            raise exc
    ...
    def refresh(self, replace_original=True):
        ...
        if _Debug:
            lg.args(
                _DebugLevel,
                latest=self.latest_as_string,
                current=self.current_as_string,
                refreshed=True,
            )
        return True
