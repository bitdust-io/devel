...
import os
import re
from crypt import my_keys, signed
...
from p2p import commands, lookup, p2p_service, p2p_service_seeker, propagate
...
def register_group_member(A):
    ...
    if _Debug:
        lg.args(
            _DebugLevel,
            group_creator_idurl=A.group_creator_idurl,
            group_key_id=A.group_key_id,
        )
...
def unregister_group_member(A):
    ...
    if id_url.is_not_in(A.group_creator_idurl, _ActiveGroupMembersByIDURL):
        ...
        if A in _ActiveGroupMembersByIDURL[A.group_creator_idurl]:
            ...
            lg.warn(
                "group_member() instance not found for customer %r"
                % A.group_creator_idurl
            )
...
def start_group_members():
    def _start():
        ...
        for group_key_id, group_info in groups.active_groups().items():
            ...
            if not my_keys.is_key_registered(group_key_id):
                lg.err(
                    "can not start GroupMember because key %r is not registered"
                    % group_key_id
                )
                continue
    ...
    if _Debug:
        lg.args(
            _DebugLevel,
            active_groups=len(groups.active_groups()),
            idurls_to_be_updated=len(idurls_to_be_updated),
        )
...
class GroupMember(automat.Automat):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.member_sender_id = global_id.MakeGlobalID(
            idurl=self.member_idurl, key_alias=self.group_queue_alias
        )
    ...
    def __repr__(self):
        connected_brokers_short = [
            "+" if (self.connected_brokers or {}).get(p) else " "
            for p in range(groups.REQUIRED_BROKERS_COUNT)
        ]
        ...
        j.update(
            {
                ...
                "connected_brokers": {
                    p: id_url.idurl_to_id(b) for p, b in self.connected_brokers.items()
                },
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---DISCONNECTED---
            ...
            if event == "leave" or event == "shutdown":
                ...
                or (
                    event == "instant"
                    and self.isActive(*args, **kwargs)
                    and self.isDeadBroker(*args, **kwargs)
                )
            ):
                self.state = "DHT_READ?"
        ...
        # ---DHT_READ?---
            ...
            if event == "dht-read-failed":
                ...
                event == "brokers-changed"
                or event == "brokers-read"
                or event == "brokers-found"
                or event == "brokers-not-found"
            ):
                self.state = "BROKERS?"
        ...
        # ---BROKERS?---
            ...
            if event == "shutdown" or event == "leave":
                ...
                event == "top-broker-ping-failed"
                or event == "brokers-ping-failed"
                or event == "broker-lookup-failed"
                or event == "broker-connect-failed"
            ):
                self.state = "DISCONNECTED"
        ...
        # ---QUEUE?---
            ...
            if event == "queue-read-failed":
                ...
                event == "reconnect"
                or event == "brokers-changed"
                or event == "push-message-failed"
                or event == "replace-active-broker"
            ):
                self.state = "DHT_READ?"
                self.SyncedUp = False
                self.doMarkDeadBroker(event, *args, **kwargs)
    ...
    def doDHTReadBrokers(self, event, *args, **kwargs):
        ...
        if _Debug:
            result.addErrback(
                ...
                debug=_Debug,
                debug_level=_DebugLevel,
                method="group_member.doDHTReadBrokers",
            )
    ...
    def doConnectSingleBroker(self, event, *args, **kwargs):
        ...
        if event == "brokers-changed":
            target_broker = self._do_detect_target_broker(
                available_brokers=kwargs["connected_brokers"]
            )
        elif event == "brokers-read":
            target_broker = self._do_detect_target_broker(
                available_brokers=kwargs["known_brokers"]
            )
        elif event == "brokers-mismatch":
            groups.clear_brokers(self.group_creator_id)
            if "dht_brokers" in kwargs:
                target_broker = self._do_detect_target_broker(
                    available_brokers=kwargs["dht_brokers"]
                )
            elif "cooperated_brokers" in kwargs:
                target_broker = self._do_detect_target_broker(
                    available_brokers=kwargs["cooperated_brokers"]
                )
            else:
                target_broker = self._do_detect_target_broker()
        else:
        ...
        self._do_connect_target_broker(
            target_broker["broker_pos"], target_broker["broker_idurl"]
        )
    ...
    def doPingBrokers(self, *args, **kwargs):
        ...
        idurl_list = [
            cooperated_brokers.get(i) for i in range(groups.REQUIRED_BROKERS_COUNT)
        ]
    ...
    def doReadArchive(self, *args, **kwargs):
        ...
        result_defer.addErrback(
            self._on_read_archive_failed, received_messages, latest_known_sequence_id
        )
        ...
        if event == "leave":
            ...
            if kwargs.get("erase_key", False):
                if my_keys.is_key_registered(self.group_key_id):
                    ...
                    lg.warn(
                        "key %r not registered, can not be erased" % self.group_key_id
                    )
        else:
            groups.save_group_info(self.group_key_id)
    ...
    def _do_detect_target_broker(self, available_brokers=None, dht_brokers=None):
        if _Debug:
            lg.args(
                _DebugLevel,
                dead=self.dead_broker_id,
                available=len(available_brokers or []),
                dht=len(dht_brokers or []),
            )
    ...
    def _do_request_service_one_broker(self, broker_pos, broker_idurl):
        ...
        result = p2p_service_seeker.connect_known_node(
            remote_idurl=broker_idurl,
            service_name="service_message_broker",
            service_params=lambda idurl: self._do_prepare_service_request_params(
                idurl, broker_pos
            ),
            request_service_timeout=config.conf().getInt(
                "services/private-groups/broker-connect-timeout", 120
            ),
            attempts=1,
        )
    ...
    def _do_lookup_one_broker(self, broker_pos, skip_brokers):
        ...
        for known_broker_id in groups.known_brokers(self.group_creator_id):
            if known_broker_id:
                exclude_brokers.add(
                    id_url.to_bin(global_id.glob2idurl(known_broker_id, as_field=False))
                )
        ...
        if self.dead_broker_id:
            exclude_brokers.add(
                id_url.to_bin(global_id.glob2idurl(self.dead_broker_id, as_field=False))
            )
        preferred_brokers = []
        preferred_brokers_raw = (
            config.conf().getData("services/private-groups/preferred-brokers").strip()
        )
        ...
        if _Debug:
            lg.args(
                ...
                preferred_brokers=preferred_brokers,
            )
        if preferred_brokers:
    ...
    def _do_read_queue_messages(self, json_messages):
        ...
        for json_message in json_messages:
            try:
                msg_type = json_message.get("msg_type", "") or json_message.get(
                    "type", ""
                )  # TODO: need to cleanup that later
            ...
            (
                incoming_group_alias,
                incoming_group_creator_id,
                incoming_broker_id,
            ) = global_id.SplitGlobalQueueID(incoming_queue_id)
            incoming_group_creator_id = global_id.glob2idurl(
                incoming_group_creator_id
            ).to_id()
            incoming_group_key_id = global_id.MakeGlobalKeyID(
                incoming_group_alias, incoming_group_creator_id
            )
            ...
            for one_message in list_messages:
                if one_message["sequence_id"] > latest_known_sequence_id:
                    lg.warn(
                        "invalid item sequence_id %d   vs.  last_sequence_id %d known"
                        % (one_message["sequence_id"], latest_known_sequence_id)
                    )
                    continue
                group_message_object = message.GroupMessage.deserialize(
                    one_message["payload"]
                )
                if group_message_object is None:
                    lg.err(
                        "GroupMessage deserialize failed, can not extract message from payload of %d bytes"
                        % len(one_message["payload"])
                    )
                    continue
                try:
        ...
        for packet_id, owner_idurl in packets_to_ack.items():
            ...
            _, received_broker_id = global_id.SplitGlobalQueueID(
                received_queue_id, split_queue_alias=False
            )
        ...
        if (
            received_group_messages
            and len(found_broker_ids) > 0
            and self.active_broker_id not in found_broker_ids
        ):
            to_be_reconnected = True
            lg.warn(
        ...
        if to_be_reconnected:
            ...
            cooperated_brokers = {
                int(k): id_url.field(v) for k, v in cooperated_brokers.items()
            }
            if _Debug:
        ...
        if not received_group_messages:
            if self.group_key_id not in found_group_ids:
                if _Debug:
                    lg.dbg(
                        _DebugLevel,
                        "no messages for %r found in the incoming stream"
            ...
            if _Debug:
                lg.dbg(
                    _DebugLevel,
                    "no new messages, queue in sync, latest_known_sequence_id=%d"
                    % latest_known_sequence_id,
                )
            self.automat("queue-in-sync")
            if to_be_reconnected:
                if _Debug:
                    lg.dbg(_DebugLevel, "going to reconnect %r" % self)
                reactor.callLater(
                    0.01, self.automat, "reconnect", cooperated_brokers=cooperated_brokers
                )  # @UndefinedVariable
            return True
        received_group_messages.sort(key=lambda m: m["sequence_id"])
        ret = self._do_process_group_messages(
            received_group_messages, latest_known_sequence_id
        )
        if to_be_reconnected:
            if _Debug:
                lg.dbg(_DebugLevel, "going to reconnect %r" % self)
    ...
    def _do_process_group_messages(
        self, received_group_messages, latest_known_sequence_id
    ):
        if _Debug:
            lg.args(
                _DebugLevel,
        ...
        if _Debug:
            lg.dbg(
                _DebugLevel,
                "processed all messages, queue in sync, last_sequence_id=%d"
                % self.last_sequence_id,
            )
        self.automat("queue-in-sync")
        return True
    ...
    def _do_send_message_to_broker(
        self, json_payload=None, outgoing_counter=None, packet_id=None
    ):
        global CRITICAL_PUSH_MESSAGE_FAILS
        if CRITICAL_PUSH_MESSAGE_FAILS is None:
            CRITICAL_PUSH_MESSAGE_FAILS = int(
                os.environ.get("BITDUST_CRITICAL_PUSH_MESSAGE_FAILS", 2)
            )
        ...
        if outgoing_counter is None:
            ...
            if self.outgoing_messages[outgoing_counter]["last_attempt"] is not None:
                if utime.get_sec1970() - self.outgoing_messages[outgoing_counter][
                    "last_attempt"
                ] < config.conf().getInt("services/private-groups/message-ack-timeout"):
                    lg.warn(
                        "pending message %d already made attempt to send recently"
                        % outgoing_counter
                    )
                    return
        ...
        if _Debug:
            d.addErrback(
                ...
                debug=_Debug,
                debug_level=_DebugLevel,
                method="group_member._do_send_message_to_broker",
            )
    ...
    def _do_prepare_service_request_params(
        self, possible_broker_idurl, desired_broker_position=-1, action="queue-connect"
    ):
        if _Debug:
            lg.args(
                _DebugLevel,
    ...
    def _do_remember_brokers(self, *args, **kwargs):
        ...
        for position in range(groups.REQUIRED_BROKERS_COUNT):
            ...
            if current_connected_brokers.get(position) != self.connected_brokers.get(
                position
            ):
    ...
    def _on_broker_connected(self, response_info, broker_pos, *args, **kwargs):
        ...
        try:
            ...
            archive_folder_path = strng.to_text(
                ...
                if a and a[0]:
                    ...
                    if resp_payload.startswith("identity:"):
                        ...
                        if new_ident.isCorrect() and new_ident.Valid():
                            if identitycache.UpdateAfterChecking(
                                new_ident.getIDURL(), xml_src
                            ):
                                reactor.callLater(
                                    0.2, self.automat, "brokers-mismatch"
                                )  # @UndefinedVariable
                                return
                    if resp_payload.startswith("mismatch:"):
                        mismatch_info = jsn.loads(resp_payload[9:])
                        if "dht_brokers" in mismatch_info:
                            mismatch_info["dht_brokers"] = {
                                int(k): id_url.field(v)
                                for k, v in mismatch_info["dht_brokers"].items()
                            }
                        if "cooperated_brokers" in mismatch_info:
                            mismatch_info["cooperated_brokers"] = {
                                int(k): id_url.field(v)
                                for k, v in mismatch_info["cooperated_brokers"].items()
                            }
            except:
                lg.exc()
    ...
    def _on_message_to_broker_sent(self, response_packet, outgoing_counter, packet_id):
        ...
        if outgoing_counter not in self.outgoing_messages:
            raise Exception(
                "outgoing message with counter %d not found" % outgoing_counter
            )
        if response_packet and response_packet.Command == commands.Ack():
            ...
            return
        self._do_send_message_to_broker(
            json_payload=None, outgoing_counter=outgoing_counter, packet_id=None
        )
    ...
    def _on_read_archive_success(self, archive_messages, received_messages):
        if _Debug:
            lg.args(
                _DebugLevel,
                archive_messages=len(archive_messages),
    ...
    def _on_group_brokers_updated(self, evt):
        ...
        if _Debug:
            lg.args(
                ...
                changed=changed,
                this=repr(self),
                connected_brokers=d["connected_brokers"],
            )
        if changed:
            reactor.callLater(
                0.01,
