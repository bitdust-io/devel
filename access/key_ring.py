...
from crypt import encrypted, key, my_keys
...
from storage import backup_control, backup_matrix
...
def _do_request_service_keys_registry(
    key_id, idurl, include_private, include_signature, timeout, result
):
    p2p_service.SendRequestService(
        ...
        callbacks={
            ...
            commands.Fail(): lambda response, info: result.errback(
...
def _on_service_keys_registry_response(
    ...
    if not strng.to_text(response.Payload).startswith("accepted"):
        if not result.called:
            result.errback(
                Exception('request for "service_keys_registry" refused by remote node')
            )
        return None
    d = transfer_key(
        key_id,
...
def transfer_key(
    ...
    if include_signature and not my_keys.verify_key_info_signature(key_json):
        ...
        result.errback(
            Exception(
                'signature verification failed after making key info: "%s"' % key_id
            )
        )
        return result
    if _Debug:
    ...
    p2p_service.SendKey(
        ...
        packet_id=key_id,
        callbacks={
            commands.Ack(): lambda response, info: _on_transfer_key_response(
                response, info, key_id, result
            ),
            commands.Fail(): lambda response, info: _on_transfer_key_response(
                response, info, key_id, result
            ),
...
def share_key(
    key_id, trusted_idurl, include_private=False, include_signature=False, timeout=15
):
    """
...
def _on_audit_public_key_response(
    response, info, key_id, untrusted_idurl, test_sample, result
):
    try:
...
def audit_public_key(key_id, untrusted_idurl, timeout=10):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "key_ring.audit_public_key   testing %s from %s" % (key_id, untrusted_idurl),
        )
    key_id = my_keys.latest_key_id(key_id)
    result = Deferred()
    ...
    p2p_service.SendAuditKey(
        ...
        timeout=timeout,
        callbacks={
            commands.Ack(): lambda response, info: _on_audit_public_key_response(
                response, info, key_id, untrusted_idurl, public_test_sample, result
            ),
...
def _on_audit_private_key_response(
    response, info, key_id, untrusted_idurl, test_sample, result
):
    try:
        response_sample = base64.b64decode(response.Payload)
...
def audit_private_key(key_id, untrusted_idurl, timeout=10):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "key_ring.audit_private_key   testing %s from %s" % (key_id, untrusted_idurl),
        )
    key_id = my_keys.latest_key_id(key_id)
    result = Deferred()
    recipient_id_obj = identitycache.FromCache(untrusted_idurl)
    ...
    p2p_service.SendAuditKey(
        ...
        timeout=timeout,
        callbacks={
            commands.Ack(): lambda response, info: _on_audit_private_key_response(
                response, info, key_id, untrusted_idurl, private_test_sample, result
            ),
            commands.Fail(): lambda response, info: result.errback(Exception(response)),
            None: lambda pkt_out: result.errback(Exception("timeout")),  # timeout
        },
...
def on_key_received(newpacket, info, status, error_message):
    ...
    try:
        key_data = block.Data()
        key_json = serialization.BytesToDict(
            key_data, keys_to_text=True, values_to_text=True
        )
        ...
        if key_object.isSigned():
            if not my_keys.verify_key_info_signature(key_json):
                raise Exception(
                    "received key signature verification failed: %r" % key_json
                )
        ...
        if key_object.isPublic():
            ...
            if my_keys.is_key_registered(key_id):
                ...
                if my_keys.get_public_key_raw(key_id) != key_object.toPublicString():
                    ...
                    if not my_keys.register_key(key_id, key_object, label=key_label):
                        ...
                        lg.info(
                            "replaced existing key %s, is_public=%s"
            ...
            if not my_keys.register_key(key_id, key_object, label=key_label):
                ...
                lg.info(
                    "added new key %s, is_public=%s" % (key_id, key_object.isPublic())
                )
            p2p_service.SendAck(newpacket)
            if _Debug:
                lg.info(
        ...
        if my_keys.is_key_registered(key_id):
            ...
            if my_keys.is_key_private(key_id):
                ...
                if my_keys.get_private_key_raw(key_id) != key_object.toPrivateString():
                    ...
                    raise Exception(
                        "private key already registered and it is not matching with received copy"
                    )
                # this is the same private key
                p2p_service.SendAck(newpacket)
                lg.warn(
                    "received again an exact copy of already existing private key: %s, skip"
                    % key_id
                )
                return True
...
def on_audit_key_received(newpacket, info, status, error_message):
    ...
    if block is None:
        lg.out(
            2,
            "key_ring.on_audit_key_received ERROR reading data from %s"
            % newpacket.RemoteID,
        )
        return False
    try:
        ...
        json_payload = serialization.BytesToDict(
            raw_payload, keys_to_text=True, values_to_text=True
        )
    ...
    if public_sample:
        ...
        if _Debug:
            lg.info(
                "remote user %s requested audit of public key %s"
    ...
    if private_sample:
        ...
        if _Debug:
            lg.info(
                "remote user %s requested audit of private key %s"
                % (newpacket.OwnerID, key_id)
            )
        return True
...
def do_backup_key(key_id, keys_folder=None):
    ...
    if res["status"] == "OK" and res["result"] and res["result"].get("exist"):
        ...
        if global_key_path_id and backup_control.IsPathInProcess(global_key_path_id):
            lg.warn(
                "skip, another backup for key already started: %s" % global_key_path_id
            )
            ...
            if backup_id_list:
                ...
                if backup_job:
                    ...
                    backup_job.resultDefer.addCallback(
                        ...
                        if resp == "done"
                        else backup_result.errback(
                            Exception(
                                'failed to upload key "%s", task was not started: %r'
                                % (global_key_path, resp)
                            )
                        )
                    )
                    if _Debug:
                        backup_job.resultDefer.addErrback(
                            lg.errback,
                            debug=_Debug,
                            debug_level=_DebugLevel,
                            method="key_ring.do_backup_key",
                        )
                    ...
                    if _Debug:
                        lg.args(
                            _DebugLevel,
                            backup_id=backup_id,
                            global_key_path_id=global_key_path_id,
                        )
                ...
                lg.warn(
                    "did not found running backup id for path: %r" % global_key_path_id
                )
    else:
        res = api.file_create(global_key_path)
        if res["status"] != "OK":
            lg.err(
                'failed to create path "%s" in the catalog: %r' % (global_key_path, res)
            )
            return fail(
                Exception(
                    'failed to create path "%s" in the catalog: %r'
                    % (global_key_path, res)
                )
            )
    ...
    def _job_done(result):
        ...
        if result == "done":
            ...
            backup_result.errback(
                Exception('failed to upload key "%s", backup is %r' % (key_id, result))
            )
        return None
    ...
    def _task_started(resp):
        ...
        if not backupObj:
            backup_result.errback(
                Exception(
                    'failed to upload key "%s", task %r failed to start'
                    % (global_key_path, resp["version"])
                )
            )
            return None
...
def do_restore_key(key_id, is_private, keys_folder=None, wait_result=False):
    ...
    def _on_result(res):
        if not isinstance(res, dict):
            ...
            if wait_result:
                result.errback(
                    Exception('failed to download key "%s": %s' % (key_id, res))
                )
            return None
        if res["status"] != "OK":
            ...
            if wait_result:
                result.errback(
                    Exception('failed to download key "%s": %r' % (key_id, res))
                )
            return None
        if not my_keys.load_key(key_id, keys_folder):
            lg.err(
                'failed to read key "%s" from local folder "%s"' % (key_id, keys_folder)
            )
            if wait_result:
                result.errback(
                    Exception(
                        'failed to read key "%s" from local folder "%s"'
                        % (key_id, keys_folder)
                    )
                )
            return None
        if _Debug:
            lg.out(
                _DebugLevel,
                "key_ring.do_restore_key._on_result key_id=%s  is_private=%r : %r"
                % (key_id, is_private, res),
            )
        if wait_result:
            result.callback(res)
        return None
...
def do_delete_key(key_id, is_private):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "key_ring.do_delete_key key_id=%s  is_private=%r : %r"
            % (key_id, is_private, res),
        )
    return True
...
def on_files_received(newpacket, info):
    ...
    if list_files_global_id["key_alias"] == "master":
        if _Debug:
            lg.dbg(
                _DebugLevel,
                "ignore %s packet which seems to came from my own supplier" % newpacket,
            )
    ...
    if remote_files_changed:
        lg.info(
            "received updated list of files from external supplier %s for customer %s"
            % (external_supplier_idurl, trusted_customer_idurl)
        )
    return True
