...
class GroupAccessDonor(automat.Automat):
    ...
    def __init__(self, debug_level=0, log_events=False, log_transitions=False, publish_events=False, **kwargs):
        ...
        """
        super(GroupAccessDonor, self).__init__(name='group_access_donor', state='AT_STARTUP', debug_level=debug_level or _DebugLevel, log_events=log_events or _Debug, log_transitions=log_transitions or _Debug, publish_events=publish_events, **kwargs)
    ...
    def doReportDone(self, *args, **kwargs):
        ...
        Action method.
        """
        lg.info('share group key [%s] with %r finished with success' % (
            self.group_key_id,
            self.remote_idurl,
        ))
        events.send(
            'group-key-shared', data=dict(
    ...
    def doReportFailed(self, event, *args, **kwargs):
        ...
        Action method.
        """
        lg.warn('share group key [%s] with %s failed: %s' % (
            self.group_key_id,
            self.remote_idurl,
            args,
        ))
        ...
        events.send(
            'group-key-share-failed', data=dict(
                global_id=global_id.UrlToGlobalID(self.remote_idurl),
                remote_idurl=self.remote_idurl,
                group_key_id=self.group_key_id,
                reason=reason,
    ...
    def _on_user_priv_key_shared(self, response):
        lg.info('private group key %s was sent to %s' % (
            self.group_key_id,
            self.remote_idurl,
        ))
