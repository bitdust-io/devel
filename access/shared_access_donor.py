...
class SharedAccessDonor(automat.Automat):
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---PUB_KEY---
            ...
            if event == "ack":
                ...
                event == "timer-15sec" and not self.isSomeSuppliersAcked(*args, **kwargs)
            ):
                self.state = "CLOSED"
                self.doReportFailed(event, *args, **kwargs)
                self.doDestroyMe(*args, **kwargs)
            elif event == "all-suppliers-acked" or (
                event == "timer-2sec" and self.isSomeSuppliersAcked(*args, **kwargs)
            ):
                self.state = "PRIV_KEY"
    ...
    def doAuditUserMasterKey(self, *args, **kwargs):
        ...
        Action method.
        """
        master_key_id = my_keys.make_key_id(
            alias="master", creator_idurl=self.remote_idurl
        )
        d = key_ring.audit_private_key(master_key_id, self.remote_idurl)
        d.addCallback(
            lambda audit_result: (
                self.automat("audit-ok")
                if audit_result
                else self.automat(
                    "fail", Exception("remote user master key audit process failed")
                ),
            )
        )
        if _Debug:
            d.addErrback(
                ...
                method="shared_access_donor.doAuditUserMasterKey",
            )
    ...
    def doSendPrivKeyToUser(self, *args, **kwargs):
        ...
        """
        d = key_ring.share_key(
            self.key_id, self.remote_idurl, include_private=True, include_signature=True
        )
    ...
    def doSendMyListFiles(self, *args, **kwargs):
        ...
        json_list_files = backup_fs.Serialize(
            ...
            if strng.to_text(info.key_id) == strng.to_text(self.key_id)
            else False,
        )
        raw_list_files = serialization.DictToBytes(
            json_list_files, keys_to_text=True, values_to_text=True, encoding="utf-8"
        )
        if _Debug:
            lg.out(
                _DebugLevel,
                "shared_access_donor.doSendMyListFiles prepared list of files for %s :\n%s"
        ...
        p2p_service.SendFiles(
            ...
            packet_id=packet_id,
            callbacks={
                commands.Ack(): lambda response, _: self.automat(
                    "list-files-ok", response
                ),
                commands.Fail(): lambda response, _: self.automat(
                    "fail", Exception(str(response))
                ),
    ...
    def _on_supplier_pub_key_shared(self, response, supplier_idurl):
        ...
        self.automat("ack", response)
        lg.warn(
            "suppliers_acks=%d, suppliers_responses=%d"
            % (self.suppliers_acks, len(self.suppliers_responses))
        )
        return None
