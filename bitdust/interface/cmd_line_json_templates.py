...
TPL_DEVICES_LIST = tpl_4_items.format(tpl_status, tpl_execution, ls('{name} {url} active:{active}, {size} bits, routed:{meta.routed}, auth:{meta.auth_token}\n', or_text='there are currently no devices configured'), tpl_errors)
...
TPL_KEY_CREATE = tpl_5_items.format(tpl_status, tpl_execution, tpl_message, ls('\n\n[{key_id}]\ntype:{type} size:{size} fingerprint:{fingerprint}\n{public}\n{private}\n'), tpl_errors)

#------------------------------------------------------------------------------

TPL_BACKUPS_LIST = tpl_4_items.format(tpl_status, tpl_execution, ls('{global_id} {type} {size} {path} {.section versions}{.repeated section @}[{backup_id}: {size}]{.end}{.end} {.section key_id}<{@}>{.end}\n'), tpl_errors)
...
TPL_RESTORES_RUNNING_LIST = tpl_5_items.format(tpl_status, tpl_execution, ls('{backup_id}, currently {bytes_processed} bytes processed\n'), tpl_message, tpl_errors)
...
TPL_SUPPLIERS = tpl_4_items.format(tpl_status, tpl_execution, ls('{position}: {idurl}, since {connected}, keeps {.section fragments}{files}{.end} files, {contact_state}\n'), tpl_errors)
