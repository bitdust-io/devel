...
from p2p import online_status, p2p_connector, p2p_service, propagate
...
def synchronize_files(customer_idurl=None):
    ...
    def _on_list_files_orator_state_changed(
...
class ListFilesOrator(automat.Automat):
    ...
    def state_changed(self, oldstate, newstate, event, *args, **kwargs):
        if (
            self.target_customer_idurl is None
            or self.target_customer_idurl == my_id.getIDURL()
        ):
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---REMOTE_FILES---
            ...
            if event == "supplier-connected":
                ...
                event == "timer-20sec"
                and not self.isEnoughListFilesReceived(*args, **kwargs)
                and not self.isSomeConnecting(*args, **kwargs)
            ):
                self.state = "NO_FILES"
            elif (
                event == "timer-2sec" and self.isEnoughListFilesReceived(*args, **kwargs)
            ) or (
    ...
    def isAllListFilesReceived(self, *args, **kwargs):
        if _Debug:
            lg.out(
                _DebugLevel,
                "list_files_orator.isAllListFilesReceived need %d more"
                % len(self.requested_lf_packet_ids),
            )
    ...
    def isEnoughListFilesReceived(self, *args, **kwargs):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "list_files_orator.isEnoughListFilesReceived %d list files received"
    ...
    def doRequestFilesAllSuppliers(self, *args, **kwargs):
        ...
        if self.ping_required:
            ...
            propagate.ping_suppliers(customer_idurl=self.target_customer_idurl).addBoth(
                self._do_request
            )
        else:
            self._do_request()
    ...
    def doRequestFilesOneSupplier(self, *args, **kwargs):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "list_files_orator.doRequestFilesOneSupplier from %s" % supplier_idurl,
            )
    ...
    def _do_request(self, x=None):
        ...
        try:
            self.critical_suppliers_number = eccmap.GetCorrectableErrors(
                len(known_suppliers)
            )
        except:
            lg.warn(
                "number of known suppliers for customer %r is not standard"
                % self.target_customer_idurl
            )
    ...
    def _on_supplier_connected(self, evt):
        if id_url.field(evt.data["customer_idurl"]) == self.target_customer_idurl:
            if _Debug:
                lg.dbg(
                    _DebugLevel,
                    "for customer %r single supplier %r was connected"
