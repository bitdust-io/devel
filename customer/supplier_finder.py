...
from logs import lg
from p2p import lookup, online_status
...
class SupplierFinder(automat.Automat):
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---SERVICE?---
            ...
            if event == "supplier-connected":
                ...
                event == "timer-30sec" or event == "supplier-not-connected"
            ):
                self.state = "FAILED"
                self.doDestroyMe(*args, **kwargs)
                self.doReportFailed(*args, **kwargs)
    ...
    def doSupplierConnect(self, *args, **kwargs):
        ...
        Action method.
        """
        from customer import fire_hire, supplier_connector
        ...
        if position is None or position == -1:
            ...
            for i in range(len(current_suppliers)):
                ...
                if not supplier_idurl:
                    ...
                    break
                if id_url.is_in(
                    supplier_idurl, fire_hire.A().dismiss_list, as_field=False
                ):
                    position = i
                    break
    ...
    def doPopCandidate(self, *args, **kwargs):
        ...
        for idurl in _SuppliersToHire:
            if id_url.is_not_in(idurl, contactsdb.suppliers(), as_field=False):
                ...
                break
        lg.info(
            'populate supplier %r from "hire" list, %d more in the list'
    ...
    def _nodes_lookup_finished(self, idurls):
        ...
        for idurl in idurls:
            ...
            if not len(myprotos.intersection(remoteprotos)):
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "    skip %r because no matching protocols exists" % idurl,
                    )
                continue
            found_idurl = idurl
            break
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "    selected %r and will request supplier service" % found_idurl,
            )
