    ...
    if not skip_initialize:
        ...
        if os.environ.get("_DEBUG", "0") == "0":
            run_ssh_command_and_wait(
                node,
                "find /app/bitdust -type f -name '*.py' -exec sed -i -e 's/_Debug = True/_Debug = False/g' {} +",
            )
    bitdust_daemon = run_ssh_command_and_wait(
        node,
        "BITDUST_CRITICAL_PUSH_MESSAGE_FAILS=1 BITDUST_LOG_USE_COLORS=1 COVERAGE_PROCESS_START=/app/bitdust/.coverage_config bitdust daemon",
    )
    if verbose:
        dbg("\n" + bitdust_daemon[0].strip())
    assert bitdust_daemon[0].strip().startswith(
        "main BitDust process already started"
    ) or bitdust_daemon[0].strip().startswith(
...
def get_client_certificate(node):
    ...
    while True:
        ...
        apiclientcert = run_ssh_command_and_wait(
            node, "cat /root/.bitdust/metadata/apiclientcert"
        )[0]
    ...
    while True:
        ...
        apiclientcertkey = run_ssh_command_and_wait(
            node, "cat /root/.bitdust/metadata/apiclientcertkey"
        )[0]
    ...
    while True:
        ...
        apiservercert = run_ssh_command_and_wait(
            node, "cat /root/.bitdust/metadata/apiservercert"
        )[0]
...
async def get_client_certificate_async(node, loop):
    ...
    while True:
        ...
        apiclientcert = (
            await run_ssh_command_and_wait_async(
                node, "cat /root/.bitdust/metadata/apiclientcert", loop
            )
        )[0]
        if not apiclientcert:
    ...
    while True:
        ...
        apiclientcertkey = (
            await run_ssh_command_and_wait_async(
                node, "cat /root/.bitdust/metadata/apiclientcertkey", loop
            )
        )[0]
        if not apiclientcertkey:
            await asyncio.sleep(0.5)
    ...
    while True:
        ...
        apiservercert = (
            await run_ssh_command_and_wait_async(
                node, "cat /root/.bitdust/metadata/apiservercert", loop
            )
        )[0]
        if not apiservercert:
            await asyncio.sleep(0.5)
...
async def health_check_async(node, event_loop, verbose=False):
    async with aiohttp.ClientSession(
        loop=event_loop, connector=ssl_connection(node)
    ) as client:
        count = 0
        while True:
            if verbose:
            ...
            try:
                response = await client.get(
                    tunnel_url(node, "process/health/v1", verbose=verbose)
                )
...
def create_identity(node, identity_name):
    ...
    while True:
        ...
        if not response.status_code == 200 or (
            ...
            and response.json()["errors"][0] == "network connection error"
        ):
            count += 1
            continue
    ...
    async with aiohttp.ClientSession(
        ...
        for i in range(60):
            response_identity = await client.post(
                tunnel_url(node, "identity/create/v1", verbose=verbose),
...
def connect_network(node, delay=5, verbose=False):
    ...
    response = request_get(
        node, f"network/connected/v1?wait_timeout={delay}", verbose=verbose
    )
    ...
    while True:
        if count > 60:
            assert (
                False
            ), f"node {node} failed to connect to the network after many retries"
        response = request_get(
            node, f"network/connected/v1?wait_timeout={delay}", verbose=verbose
        )
        if response.json()["status"] == "OK":
...
async def connect_network_async(
    node, loop, attempts=30, delay=5, timeout=20, verbose=False
):
...
async def service_started_async(
    node, service_name, loop, expected_state="ON", attempts=60, delay=3, verbose=False
):
    async with aiohttp.ClientSession(loop=loop, connector=ssl_connection(node)) as client:
        current_state = None
        count = 0
        while current_state is None or current_state != expected_state:
            response = await client.get(
                tunnel_url(node, f"service/info/{service_name}/v1", verbose=verbose)
            )
            response_json = await response.json()
            ...
            if verbose:
                dbg(
                    f"\nservice/info/{service_name}/v1 [{node}] : %s"
                    % pprint.pformat(response_json)
                )
...
async def packet_list_async(
    node, loop, wait_all_finish=True, attempts=60, delay=3, verbose=False
):
    async with aiohttp.ClientSession(loop=loop, connector=ssl_connection(node)) as client:
        for i in range(attempts):
            response = await client.get(
                tunnel_url(node, "packet/list/v1", verbose=verbose), timeout=20
            )
            response_json = await response.json()
            if verbose:
                dbg(
...
def stop_daemon(node, skip_checks=False, verbose=False):
    ...
    if not skip_checks:
        ...
        assert (
            (
                resp.startswith("BitDust child processes found")
                and resp.endswith("BitDust stopped")
            )
...
async def stop_daemon_async(node, loop, skip_checks=False, verbose=False):
    ...
    bitdust_stop = await run_ssh_command_and_wait_async(
        node, "bitdust stop", loop, verbose=verbose
    )
    resp = bitdust_stop[0].strip()
    if skip_checks:
        if verbose:
...
async def start_one_proxy_server_async(proxy_server, loop):
    await start_proxy_server_async(
        ...
        health_check_interval_seconds=proxy_server.get(
            "health_check_interval_seconds", None
        ),
...
def report_one_node(node):
    main_log = run_ssh_command_and_wait(
        node, "cat /root/.bitdust/logs/stdout.log", verbose=False
    )[0].strip()
    ...
    api_log = run_ssh_command_and_wait(
        node, "cat /root/.bitdust/logs/api.log", verbose=False
    )[0].strip()
    num_apis = api_log.count(" *** ")
    packet_log = run_ssh_command_and_wait(
        node, "cat /root/.bitdust/logs/packet.log", verbose=False
    )[0].strip()
    ...
    event_log = run_ssh_command_and_wait(
        node, "cat /root/.bitdust/logs/event.log", verbose=False
    )[0].strip()
...
async def report_one_node_async(node, event_loop):
    main_log = await run_ssh_command_and_wait_async(
        node, "cat /root/.bitdust/logs/stdout.log", event_loop
    )
...
def print_exceptions_one_node(node):
    ...
    exceptions_out = run_ssh_command_and_wait(
        node, "cat /root/.bitdust/logs/exception_*.log 2>/dev/null", verbose=False
    )[0].strip()
    if exceptions_out:
        print(f"\n[{node}]:\n\n{exceptions_out}\n\n")
    # else:
...
async def print_exceptions_one_node_async(node, event_loop):
    ...
    exceptions_out = await run_ssh_command_and_wait_async(
        node,
        "cat /root/.bitdust/logs/exception_*.log 2>/dev/null",
        event_loop,
        verbose=False,
    )
...
def print_stdout_one_node(node):
    std_out = run_ssh_command_and_wait(node, "cat /root/.bitdust/logs/stdout.log")[
        0
    ].strip()
    if std_out:
        print(f"\n[{node}]:\n\n{std_out}\n\n")
...
async def clean_one_customer_async(node, event_loop, verbose=False):
    await run_ssh_command_and_wait_async(
        node, "rm -rf /%s/*" % node, event_loop, verbose=verbose
    )
