...
def test_alpha():
    ...
    t = threading.Timer(1.0, kw.message_send_v1, ['customer-1', 'master$customer-2@id-b_8084', random_message, ])
    t.start()
    kw.message_receive_v1('customer-2', expected_data=random_message, timeout=16, polling_timeout=15)
    ...
    kw.config_set_v1('customer-3', 'services/employer/candidates', 'http://id-a:8084/broker-1.xml,http://id-b:8084/broker-2.xml,http://id-a:8084/broker-3.xml,http://id-b:8084/broker-4.xml')
    ...
    assert new_proxy_sources != old_proxy_sources
    assert new_proxy_global_id != old_proxy_global_id
    assert new_proxy_idurl != old_proxy_idurl
    ...
    assert new_supplier_sources != old_supplier_sources
    assert new_supplier_global_id != old_supplier_global_id
    assert new_supplier_idurl != old_supplier_idurl
    ...
    assert new_broker_sources != old_broker_sources
    assert new_broker_global_id != old_broker_global_id
    assert new_broker_idurl != old_broker_idurl
    ...
    run_ssh_command_and_wait('customer-rotated', 'rm -rfv %s' % customer_rotated_download_filepath)[0].strip()
    ...
    old_folder_first_supplier = run_ssh_command_and_wait(first_supplier, f'ls -la ~/.bitdust/customers/{old_customer_global_id}/master/')[0].strip()
    new_folder_first_supplier = run_ssh_command_and_wait(first_supplier, f'ls -la ~/.bitdust/customers/{new_customer_global_id}/master/')[0].strip()
    assert old_folder_first_supplier == ''
    assert new_folder_first_supplier != ''
    old_folder_second_supplier = run_ssh_command_and_wait(second_supplier, f'ls -la ~/.bitdust/customers/{old_customer_global_id}/master/')[0].strip()
    new_folder_second_supplier = run_ssh_command_and_wait(second_supplier, f'ls -la ~/.bitdust/customers/{new_customer_global_id}/master/')[0].strip()
    ...
    run_ssh_command_and_wait('customer-3', 'rm -rfv %s' % customer_3_download_filepath)[0].strip()
    ...
    t = threading.Timer(1.0, kw.message_send_v1, ['customer-2', 'master$%s' % new_customer_global_id, random_message, 15, ])
    t.start()
    ...
    run_ssh_command_and_wait('customer-2', f'mkdir /customer_2/cat_mine/')
    run_ssh_command_and_wait('customer-2', f'mkdir /customer_2/cat_shared/')
    ...
    customer_2_cat_own = run_ssh_command_and_wait('customer-2', f'cat /customer_2/cat_mine/cat.txt')[0].strip()
    customer_2_cat_shared = run_ssh_command_and_wait('customer-2', f'cat /customer_2/cat_shared/cat.txt')[0].strip()
    assert customer_2_cat_own != customer_2_cat_shared
    assert len(customer_2_cat_own) == 1000
    assert len(customer_2_cat_shared) == 2000
    ...
    # customer_1_supplier_ids_before = list([x['global_id'] for x in customer_1_supplier_idurls_before])
