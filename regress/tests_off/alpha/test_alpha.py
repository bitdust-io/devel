...
import pytest
from testsupport import (  # @UnresolvedImport
    request_get,
    request_post,
    request_put,
...
def test_alpha():
    ...
    (
        old_proxy_rotated_info,
        old_customer_rotated_info,
        old_rotated_supplier_info,
        old_broker_rotated_info,
        old_customer_rotated_keys,
        new_proxy_rotated_info,
        new_customer_rotated_info,
        new_rotated_supplier_info,
        new_broker_rotated_info,
    ) = scenario9()
    ...
    # --- SCENARIO 12 end: customer-4 group chat with customer-2 but broker IDURL rotated
    scenario12_end(old_customer_4_info)

    # --- SCENARIO 11 end: customer-2 talk to customer-rotated
    scenario11_end(
        old_customer_rotated_info, new_customer_rotated_info, old_customer_2_info
    )
...
def prepare():
    ...
    kw.wait_suppliers_connected(
        CUSTOMERS_IDS, expected_min_suppliers=2, expected_max_suppliers=2
    )
...
def scenario1():
    ...
    response = request_get(
        "customer-1", f"user/search/user_name_not_exist/v1", timeout=30
    )
    assert response.json()["status"] == "OK", response.json()
...
def scenario3():
    ...
    t.start()
    kw.message_receive_v1(
        "customer-2", expected_data=random_message_1, timeout=16, polling_timeout=15
    )
...
def scenario4():
    ...
    (
        customer_1_local_filepath_cat,
        customer_1_remote_path_cat,
        customer_1_download_filepath_cat,
    ) = kw.verify_file_create_upload_start(
        node="customer-1",
    ...
    (
        customer_2_local_filepath_cat,
        customer_2_remote_path_cat,
        customer_2_download_filepath_cat,
    ) = kw.verify_file_create_upload_start(
        node="customer-2",
    ...
    run_ssh_command_and_wait(
        "customer-2", f"mkdir /customer_2/cat_mine/", verbose=ssh_cmd_verbose
    )
    run_ssh_command_and_wait(
        "customer-2", f"mkdir /customer_2/cat_shared/", verbose=ssh_cmd_verbose
    )
    ...
    customer_2_cat_own = run_ssh_command_and_wait(
        "customer-2", f"cat /customer_2/cat_mine/cat.txt", verbose=ssh_cmd_verbose
    )[0].strip()
    customer_2_cat_shared = run_ssh_command_and_wait(
        "customer-2", f"cat /customer_2/cat_shared/cat.txt", verbose=ssh_cmd_verbose
    )[0].strip()
...
def scenario7():
    ...
    kw.service_info_v1("customer-1", "service_shared_data", "ON")
    (
        customer_1_local_filepath,
        customer_1_remote_path,
        customer_1_download_filepath,
    ) = kw.verify_file_create_upload_start(
        node="customer-1",
...
def scenario8():
    ...
    customer_1_group_info_inactive = kw.group_info_v1(
        "customer-1", customer_1_group_key_id
    )["result"]
    ...
    customer_1_group_info_active = kw.group_info_v1(
        "customer-1", customer_1_group_key_id
    )["result"]
    ...
    kw.automat_info_v1(
        "customer-1",
        automat_index=customer_1_group_info_active["index"],
    ...
    assert customer_1_active_queue_id in kw.queue_list_v1(
        customer_1_active_broker_name, extract_ids=True
    )

    customer_1_broker_consumers = kw.queue_consumer_list_v1(
        customer_1_active_broker_name, extract_ids=True
    )
    customer_1_broker_producers = kw.queue_producer_list_v1(
        customer_1_active_broker_name, extract_ids=True
    )
    customer_1_broker_keepers = kw.queue_keeper_list_v1(
        customer_1_active_broker_name, extract_ids=True
    )
    ...
    assert customer_1_active_queue_id in kw.queue_peddler_list_v1(
        customer_1_active_broker_name, extract_ids=True
    )
    ...
    kw.file_list_all_v1(
        "customer-2", expected_reliable=100, reliable_shares=True, attempts=20
    )
    ...
    assert (
        kw.group_info_v1("customer-2", customer_1_group_key_id)["result"][
            "last_sequence_id"
        ]
        == -1
    )
    ...
    assert (
        len(
            kw.message_history_v1(
                "customer-1", customer_1_group_key_id, message_type="group_message"
            )["result"]
        )
        == 0
    )
    assert (
        len(
            kw.message_history_v1(
                "customer-2", customer_1_group_key_id, message_type="group_message"
            )["result"]
        )
        == 0
    )
    assert (
        len(
            kw.message_history_v1(
                "customer-3", customer_1_group_key_id, message_type="group_message"
            )["result"]
        )
        == 0
    )
    ...
    assert (
        kw.group_info_v1("customer-1", customer_1_group_key_id)["result"][
            "last_sequence_id"
        ]
        == 10
    )
    ...
    customer_1_group_info_offline = kw.group_info_v1(
        "customer-1", customer_1_group_key_id
    )["result"]
    ...
    assert (
        kw.group_info_v1("customer-2", customer_1_group_key_id)["result"]["state"]
        == "IN_SYNC!"
    )
    ...
    assert (
        kw.group_info_v1("customer-3", customer_1_group_key_id)["result"][
            "last_sequence_id"
        ]
        == 10
    )
    ...
    customer_2_group_info_offline = kw.group_info_v1(
        "customer-2", customer_1_group_key_id
    )["result"]
    ...
    customer_3_group_info_offline = kw.group_info_v1(
        "customer-3", customer_1_group_key_id
    )["result"]
    ...
    assert "customer-1@id-a_8084" in kw.queue_keeper_list_v1(
        customer_1_active_broker_name, extract_ids=True
    )
    assert customer_1_active_queue_id not in kw.queue_peddler_list_v1(
        customer_1_active_broker_name, extract_ids=True
    )
...
def scenario9():
    ...
    old_customer_keys = [
        k["key_id"] for k in kw.key_list_v1("customer-rotated")["result"]
    ]
    ...
    kw.config_set_v1(
        "proxy-rotated", "services/identity-propagate/automatic-rotate-enabled", "true"
    )
    kw.config_set_v1(
        "proxy-rotated",
        "services/identity-propagate/known-servers",
        "id-a:8084,id-b:8084,id-c:8084",
    )
    kw.config_set_v1("proxy-rotated", "services/identity-propagate/preferred-servers", "")

    kw.config_set_v1(
        "customer-rotated", "services/identity-propagate/automatic-rotate-enabled", "true"
    )
    kw.config_set_v1(
        "customer-rotated",
        "services/identity-propagate/known-servers",
    ...
    kw.config_set_v1(
        "customer-rotated", "services/identity-propagate/preferred-servers", ""
    )

    kw.config_set_v1(
        "supplier-rotated", "services/identity-propagate/automatic-rotate-enabled", "true"
    )
    kw.config_set_v1(
        "supplier-rotated",
        "services/identity-propagate/known-servers",
    ...
    kw.config_set_v1(
        "supplier-rotated", "services/identity-propagate/preferred-servers", ""
    )

    kw.config_set_v1(
        "broker-rotated", "services/identity-propagate/automatic-rotate-enabled", "true"
    )
    kw.config_set_v1(
        "broker-rotated",
        "services/identity-propagate/known-servers",
    ...
    kw.config_set_v1(
        "broker-rotated", "services/identity-propagate/preferred-servers", ""
    )
    ...
    for _ in range(20):
        ...
        assert (
            False
        ), "proxy-rotated automatic identity rotate did not happen after many attempts"
    ...
    for _ in range(20):
        ...
        assert (
            False
        ), "customer-rotated automatic identity rotate did not happen after many attempts"
    ...
    for _ in range(20):
        customer_rotated_event_log = run_ssh_command_and_wait(
            "customer-rotated",
            "cat /root/.bitdust/logs/event.log",
            verbose=ssh_cmd_verbose,
        )[0]
        ...
        assert (
            False
        ), 'event "my-identity-rotate-complete" was not triggered on customer-rotated after many attempts'
    ...
    for _ in range(20):
        ...
        assert (
            False
        ), "supplier-rotated automatic identity rotate did not happen after many attempts"
    ...
    for _ in range(20):
        ...
        assert (
            False
        ), "broker-rotated automatic identity rotate did not happen after many attempts"
...
def scenario10_begin():
    ...
    (
        customer_rotated_local_filepath,
        customer_rotated_remote_path,
        customer_rotated_download_filepath,
    ) = kw.verify_file_create_upload_start(
...
def scenario10_end(
    ...
    customer_rotated_download_filepath = old_customer_rotated_file_info[
        "download_filepath"
    ]
    ...
    customer_rotated_suppliers = kw.supplier_list_v1(
        "customer-rotated",
    ...
    first_supplier = (
        customer_rotated_suppliers[0]
        .replace("http://id-a:8084/", "")
        .replace("http://id-b:8084/", "")
        .replace(".xml", "")
    )
    second_supplier = (
        customer_rotated_suppliers[1]
        .replace("http://id-a:8084/", "")
        .replace("http://id-b:8084/", "")
        .replace(".xml", "")
    )
    ...
    new_share_id_customer_rotated = old_share_id_customer_rotated.replace(
        old_customer_global_id, new_customer_global_id
    )
    kw.share_open_v1("customer-rotated", new_share_id_customer_rotated)
    new_remote_path_customer_rotated = customer_rotated_remote_path.replace(
        old_customer_global_id, new_customer_global_id
    )
    run_ssh_command_and_wait(
        "customer-rotated",
        "rm -rfv %s" % customer_rotated_download_filepath,
        verbose=ssh_cmd_verbose,
    )[0].strip()
    ...
    old_folder_first_supplier = run_ssh_command_and_wait(
        first_supplier,
        f"ls -la ~/.bitdust/customers/{old_customer_global_id}/master/",
        verbose=ssh_cmd_verbose,
    )[0].strip()
    new_folder_first_supplier = run_ssh_command_and_wait(
        first_supplier,
        f"ls -la ~/.bitdust/customers/{new_customer_global_id}/master/",
        verbose=ssh_cmd_verbose,
    )[0].strip()
...
def scenario11_begin():
    ...
    t.start()
    kw.message_receive_v1(
        "customer-rotated", expected_data=random_message, timeout=31, polling_timeout=30
    )
...
def scenario11_end(
    ...
    t.start()
    kw.message_receive_v1(
        "customer-rotated", expected_data=random_message, timeout=16, polling_timeout=15
    )
    kw.wait_packets_finished(
        [
            "customer-2",
...
def scenario12_begin():
    ...
    kw.config_set_v1(
        ...
        "http://id-b:8084/broker-4.xml",
    )
    kw.config_set_v1(
        "broker-4",
        "services/message-broker/preferred-brokers",
        "http://id-a:8084/broker-3.xml",
    )
    kw.config_set_v1(
        "broker-3",
        "services/message-broker/preferred-brokers",
        "http://id-dead:8084/broker-rotated.xml",
    )
    ...
    customer_4_group_info_inactive = kw.group_info_v1(
        "customer-4", customer_4_group_key_id
    )["result"]
    ...
    customer_4_group_info_active = kw.group_info_v1(
        "customer-4", customer_4_group_key_id
    )["result"]
    ...
    assert customer_4_active_queue_id in kw.queue_list_v1(
        customer_4_active_broker_name, extract_ids=True
    )

    customer_4_broker_consumers = kw.queue_consumer_list_v1(
        customer_4_active_broker_name, extract_ids=True
    )
    customer_4_broker_producers = kw.queue_producer_list_v1(
        customer_4_active_broker_name, extract_ids=True
    )
    customer_4_broker_keepers = kw.queue_keeper_list_v1(
        customer_4_active_broker_name, extract_ids=True
    )
    assert "customer-4@id-b_8084" in customer_4_broker_consumers
    assert "customer-4@id-b_8084" in customer_4_broker_producers
    assert "customer-4@id-b_8084" in customer_4_broker_keepers
    assert customer_4_active_queue_id in kw.queue_peddler_list_v1(
        customer_4_active_broker_name, extract_ids=True
    )
    ...
    assert (
        kw.group_info_v1("customer-2", customer_4_group_key_id)["result"][
            "last_sequence_id"
        ]
        == -1
    )
    ...
    assert (
        len(
            kw.message_history_v1(
                "customer-4", customer_4_group_key_id, message_type="group_message"
            )["result"]
        )
        == 0
    )
    assert (
        len(
            kw.message_history_v1(
                "customer-2", customer_4_group_key_id, message_type="group_message"
            )["result"]
        )
        == 0
    )
    ...
    for i in range(5):
        group_customers_2_4_messages.append(
            kw.verify_message_sent_received(
    ...
    assert (
        kw.group_info_v1("customer-4", customer_4_group_key_id)["result"][
            "last_sequence_id"
        ]
        == 4
    )
    ...
    # create another group owned by customer-4 and join
    customer_4_group2_key_id = kw.group_create_v1("customer-4", label="MyGroupXYZ")
    kw.wait_service_state(CUSTOMERS_IDS, "service_shared_data", "ON")
    kw.wait_service_state(CUSTOMERS_IDS, "service_private_groups", "ON")
    kw.wait_packets_finished(PROXY_IDS + CUSTOMERS_IDS + BROKERS_IDS)
    ...
    assert (
        kw.group_info_v1("customer-4", customer_4_group_key_id, wait_state="IN_SYNC!")[
            "result"
        ]["state"]
        == "IN_SYNC!"
    )
    assert (
        kw.group_info_v1("customer-4", customer_4_group2_key_id, wait_state="IN_SYNC!")[
            "result"
        ]["state"]
        == "IN_SYNC!"
    )
    ...
    return {
        ...
        "group2_key_id": customer_4_group2_key_id,
    }
...
def scenario12_end(old_customer_4_info):
    ...
    customer_2_group_info_before = kw.group_info_v1(
        "customer-2", customer_4_group_key_id, wait_state="IN_SYNC!"
    )["result"]
    ...
    customer_2_group2_info_before = kw.group_info_v1(
        "customer-2", customer_4_group2_key_id, wait_state="IN_SYNC!"
    )["result"]
    assert customer_2_group2_info_before["state"] == "IN_SYNC!"

    # same for customer-4
    customer_4_group_info_before = kw.group_info_v1(
        "customer-4", customer_4_group_key_id, wait_state="IN_SYNC!"
    )["result"]
    assert customer_4_group_info_before["state"] == "IN_SYNC!"
    customer_4_group2_info_before = kw.group_info_v1(
        "customer-4", customer_4_group2_key_id, wait_state="IN_SYNC!"
    )["result"]
    assert customer_4_group2_info_before["state"] == "IN_SYNC!"
    ...
    customer_2_group_info_rotated = kw.group_info_v1(
        "customer-2", customer_4_group_key_id, wait_state="IN_SYNC!"
    )["result"]
    ...
    assert customer_2_rotated_queue_id in kw.queue_list_v1(
        customer_2_rotated_broker_name, extract_ids=True
    )
    assert customer_4_old_queue_id not in kw.queue_list_v1(
        customer_2_rotated_broker_name, extract_ids=True
    )

    # verify customer-4 reconnected to the new broker
    customer_4_group_info_rotated = kw.group_info_v1(
        "customer-4", customer_4_group_key_id, wait_state="IN_SYNC!"
    )["result"]
    ...
    assert customer_4_rotated_queue_id in kw.queue_list_v1(
        customer_4_rotated_broker_name, extract_ids=True
    )
    assert customer_4_old_queue_id not in kw.queue_list_v1(
        customer_4_rotated_broker_name, extract_ids=True
    )
    assert customer_4_rotated_queue_id == customer_2_rotated_queue_id

    # verify new broker accepted customer-2
    customer_2_rotated_broker_consumers = kw.queue_consumer_list_v1(
        customer_2_rotated_broker_name, extract_ids=True
    )
    customer_2_rotated_broker_producers = kw.queue_producer_list_v1(
        customer_2_rotated_broker_name, extract_ids=True
    )
    customer_2_rotated_broker_keepers = kw.queue_keeper_list_v1(
        customer_2_rotated_broker_name, extract_ids=True
    )
    ...
    assert "customer-4@id-b_8084" in customer_2_rotated_broker_consumers
    assert "customer-4@id-b_8084" in customer_2_rotated_broker_producers
    assert "customer-4@id-b_8084" in customer_2_rotated_broker_keepers
    assert customer_4_old_queue_id not in kw.queue_peddler_list_v1(
        customer_2_rotated_broker_name, extract_ids=True
    )
    assert customer_4_rotated_queue_id in kw.queue_peddler_list_v1(
        customer_2_rotated_broker_name, extract_ids=True
    )

    # verify new broker accepted customer-4
    customer_4_rotated_broker_consumers = kw.queue_consumer_list_v1(
        customer_4_rotated_broker_name, extract_ids=True
    )
    customer_4_rotated_broker_producers = kw.queue_producer_list_v1(
        customer_4_rotated_broker_name, extract_ids=True
    )
    customer_4_rotated_broker_keepers = kw.queue_keeper_list_v1(
        customer_4_rotated_broker_name, extract_ids=True
    )
    ...
    assert "customer-2@id-b_8084" not in customer_4_rotated_broker_keepers
    ...
    assert "customer-4@id-b_8084" in customer_4_rotated_broker_keepers
    assert customer_4_old_queue_id not in kw.queue_peddler_list_v1(
        customer_4_rotated_broker_name, extract_ids=True
    )
    assert customer_4_rotated_queue_id in kw.queue_peddler_list_v1(
    ...
    assert customer_4_group_info_rotated["state"] == "IN_SYNC!"
    assert customer_4_group_info_rotated["last_sequence_id"] == 6
    ...
    assert (
        kw.group_info_v1("customer-2", customer_4_group_key_id, wait_state="IN_SYNC!")[
            "result"
        ]["state"]
        == "IN_SYNC!"
    )
    assert (
        kw.group_info_v1("customer-2", customer_4_group2_key_id, wait_state="IN_SYNC!")[
            "result"
        ]["state"]
        == "IN_SYNC!"
    )
    ...
    # customer-2 and customer-4 leave from the both groups
    kw.group_leave_v1("customer-4", customer_4_group_key_id)
    kw.group_leave_v1("customer-2", customer_4_group_key_id)
    kw.group_leave_v1("customer-4", customer_4_group2_key_id)
    kw.group_leave_v1("customer-2", customer_4_group2_key_id)

    kw.wait_packets_finished(PROXY_IDS + CUSTOMERS_IDS + BROKERS_IDS)

    customer_4_broker_consumers = kw.queue_consumer_list_v1(
        customer_4_old_broker_name, extract_ids=True
    )
    customer_4_broker_producers = kw.queue_producer_list_v1(
        customer_4_old_broker_name, extract_ids=True
    )
    ...
    assert customer_4_rotated_queue_id not in kw.queue_peddler_list_v1(
        customer_4_old_broker_name, extract_ids=True
    )

    customer_4_group_info_offline = kw.group_info_v1(
        "customer-4", customer_4_group_key_id
    )["result"]
    assert customer_4_group_info_offline["state"] == "OFFLINE"
    assert customer_4_group_info_offline["label"] == "MyGroupABC"
    assert customer_4_group_info_offline["last_sequence_id"] == 11

    customer_2_group_info_offline = kw.group_info_v1(
        "customer-2", customer_4_group_key_id
    )["result"]
    assert customer_2_group_info_offline["state"] == "OFFLINE"
...
def scenario13_begin():
    ...
    old_customer_3_suppliers_idurls = kw.supplier_list_v1(
        "customer-3", expected_min_suppliers=2, expected_max_suppliers=2
    )
    ...
    (
        customer_3_local_filepath,
        customer_3_remote_path,
        customer_3_download_filepath,
    ) = kw.verify_file_create_upload_start(
        node="customer-3",
...
def scenario13_end(old_customer_3_info):
    ...
    run_ssh_command_and_wait(
        "customer-3",
        "rm -rfv %s" % old_customer_3_info["download_filepath"],
        verbose=ssh_cmd_verbose,
    )[0].strip()
...
def scenario14(old_customer_1_info, customer_1_shared_file_info):
    ...
    customer_1_supplier_idurls_before = kw.supplier_list_v1(
        "customer-1", expected_min_suppliers=2, expected_max_suppliers=2
    )
    ...
    kw.config_set_v1(
        "customer-1", "services/employer/candidates", ",".join(possible_suppliers)
    )
    ...
    while True:
        if count > 20:
            ...
            break
        customer_1_supplier_idurls_after = kw.supplier_list_v1(
            "customer-1",
            expected_min_suppliers=2,
            expected_max_suppliers=2,
            verbose=False,
...
def scenario16():
    ...
    customer_4_supplier_idurls_before = kw.supplier_list_v1(
        "customer-4", expected_min_suppliers=2, expected_max_suppliers=2
    )
    ...
    customer_4_supplier_idurls_increase = kw.supplier_list_v1(
        "customer-4", expected_min_suppliers=4, expected_max_suppliers=4
    )
    ...
    customer_4_supplier_idurls_decrease = kw.supplier_list_v1(
        "customer-4", expected_min_suppliers=2, expected_max_suppliers=2
    )
...
def scenario17(old_customer_2_info):
    ...
    kw.file_list_all_v1(
        "customer-2", expected_reliable=50, reliable_shares=False, attempts=20
    )
    ...
    for _ in range(5):
        ...
        assert (
            False
        ), "customer-restore was not able to recover identity after few attempts"
    ...
    kw.supplier_list_v1(
        "customer-restore", expected_min_suppliers=2, expected_max_suppliers=2
    )
    ...
    kw.file_list_all_v1(
        "customer-restore", expected_reliable=50, reliable_shares=False, attempts=20
    )
...
def scenario18():
    ...
    kw.config_set_v1(
        "broker-3",
        "services/message-broker/preferred-brokers",
        "http://id-b:8084/broker-2.xml",
    )
    kw.config_set_v1(
        "broker-2",
        "services/message-broker/preferred-brokers",
        "http://id-a:8084/broker-1.xml",
    )
    ...
    kw.wait_service_state(CUSTOMERS_IDS_124, "service_private_groups", "ON")
    kw.wait_packets_finished(PROXY_IDS + CUSTOMERS_IDS_124 + BROKERS_IDS)
    ...
    kw.group_join_v1("customer-4", customer_2_groupA_key_id)
    kw.wait_packets_finished(PROXY_IDS + CUSTOMERS_IDS_124 + BROKERS_IDS)
    ...
    customer_2_groupB_key_id = kw.group_create_v1("customer-2", label="MyGroupBBB")
    kw.wait_service_state(CUSTOMERS_IDS_124, "service_shared_data", "ON")
    kw.wait_service_state(CUSTOMERS_IDS_124, "service_private_groups", "ON")
    kw.wait_packets_finished(PROXY_IDS + CUSTOMERS_IDS_124 + BROKERS_IDS)
    ...
    # customer-4 also joins the second group
    kw.group_join_v1("customer-4", customer_2_groupB_key_id)
    kw.wait_packets_finished(PROXY_IDS + CUSTOMERS_IDS_124 + BROKERS_IDS)
    ...
    customer_4_groupA_info_before = kw.group_info_v1(
        "customer-4", customer_2_groupA_key_id
    )["result"]
    ...
    customer_4_active_broker_name_before = customer_4_active_broker_id_before.split("@")[
        0
    ]
    assert customer_4_active_broker_name_before == "broker-1"

    # also check the second group have similar info for customer-4
    customer_4_groupB_info_before = kw.group_info_v1(
        "customer-4", customer_2_groupB_key_id
    )["result"]
    ...
    customer_2_groupA_info_before = kw.group_info_v1(
        "customer-2", customer_2_groupA_key_id
    )["result"]
    ...
    customer_2_active_broker_name_before = customer_2_active_broker_id_before.split("@")[
        0
    ]
    assert customer_2_active_broker_name_before == "broker-1"

    # also check the second group have similar info for customer-2
    customer_2_groupB_info_before = kw.group_info_v1(
        "customer-2", customer_2_groupB_key_id
    )["result"]
    ...
    assert (
        customer_4_active_broker_id_before
        == customer_4_groupA_info_before["active_broker_id"]
    )
    assert (
        customer_4_active_broker_id_before
        == customer_4_groupB_info_before["active_broker_id"]
    )
    assert (
        customer_4_active_broker_id_before
        == customer_2_groupA_info_before["active_broker_id"]
    )
    assert (
        customer_4_active_broker_id_before
        == customer_2_groupB_info_before["active_broker_id"]
    )
    assert (
        customer_2_groupA_info_before["connected_brokers"]["0"]
        == customer_4_groupA_info_before["connected_brokers"]["0"]
    )
    assert (
        customer_2_groupA_info_before["connected_brokers"]["1"]
        == customer_4_groupA_info_before["connected_brokers"]["1"]
    )
    assert (
        customer_2_groupA_info_before["connected_brokers"]["2"]
        == customer_4_groupA_info_before["connected_brokers"]["2"]
    )
    assert (
        customer_2_groupB_info_before["connected_brokers"]["0"]
        == customer_4_groupA_info_before["connected_brokers"]["0"]
    )
    assert (
        customer_2_groupB_info_before["connected_brokers"]["1"]
        == customer_4_groupA_info_before["connected_brokers"]["1"]
    )
    assert (
        customer_2_groupB_info_before["connected_brokers"]["2"]
        == customer_4_groupA_info_before["connected_brokers"]["2"]
    )
    assert (
        customer_2_groupB_info_before["connected_brokers"]["0"]
        == customer_4_groupB_info_before["connected_brokers"]["0"]
    )
    assert (
        customer_2_groupB_info_before["connected_brokers"]["1"]
        == customer_4_groupB_info_before["connected_brokers"]["1"]
    )
    assert (
        customer_2_groupB_info_before["connected_brokers"]["2"]
        == customer_4_groupB_info_before["connected_brokers"]["2"]
    )
    ...
    # workaround config to make sure customer-4 will pick up a new broker
    kw.config_set_v1(
        "customer-4",
        "services/private-groups/preferred-brokers",
        "http://id-b:8084/broker-4.xml",
    ...
    customer_4_groupB_info_after = kw.group_info_v1(
        "customer-4", customer_2_groupB_key_id
    )["result"]
    ...
    customer_4_groupA_info_after = kw.group_info_v1(
        "customer-4", customer_2_groupA_key_id, wait_state="IN_SYNC!"
    )["result"]
    assert customer_4_groupA_info_after["state"] == "IN_SYNC!"
    assert len(customer_4_groupA_info_after["connected_brokers"]) >= 2
    assert (
        customer_4_active_broker_id_after
        == customer_4_groupA_info_after["active_broker_id"]
    )
    ...
    customer_2_groupB_info_after = kw.group_info_v1(
        "customer-2", customer_2_groupB_key_id, wait_state="IN_SYNC!"
    )["result"]
    ...
    customer_2_groupA_info_after = kw.group_info_v1(
        "customer-2", customer_2_groupA_key_id, wait_state="IN_SYNC!"
    )["result"]
    ...
    assert (
        customer_4_groupA_info_after["connected_brokers"]["0"]
        == customer_4_groupA_info_before["connected_brokers"]["1"]
    )
    assert (
        customer_4_groupA_info_after["connected_brokers"]["1"]
        == customer_4_groupA_info_before["connected_brokers"]["2"]
    )
    assert (
        customer_4_groupA_info_after["connected_brokers"]["2"]
        != customer_4_groupA_info_before["connected_brokers"]["2"]
    )
    assert (
        customer_4_groupB_info_after["connected_brokers"]["0"]
        == customer_4_groupB_info_before["connected_brokers"]["1"]
    )
    assert (
        customer_4_groupB_info_after["connected_brokers"]["1"]
        == customer_4_groupB_info_before["connected_brokers"]["2"]
    )
    assert (
        customer_4_groupB_info_after["connected_brokers"]["2"]
        != customer_4_groupB_info_before["connected_brokers"]["2"]
    )
    assert (
        customer_2_groupA_info_after["connected_brokers"]["0"]
        == customer_2_groupA_info_before["connected_brokers"]["1"]
    )
    assert (
        customer_2_groupA_info_after["connected_brokers"]["1"]
        == customer_2_groupA_info_before["connected_brokers"]["2"]
    )
    assert (
        customer_2_groupA_info_after["connected_brokers"]["2"]
        != customer_2_groupA_info_before["connected_brokers"]["2"]
    )
    assert (
        customer_2_groupB_info_after["connected_brokers"]["0"]
        == customer_2_groupB_info_before["connected_brokers"]["1"]
    )
    assert (
        customer_2_groupB_info_after["connected_brokers"]["1"]
        == customer_2_groupB_info_before["connected_brokers"]["2"]
    )
    assert (
        customer_2_groupB_info_after["connected_brokers"]["2"]
        != customer_2_groupB_info_before["connected_brokers"]["2"]
    )
    ...
    assert (
        customer_4_active_broker_id_after
        == customer_4_groupB_info_after["active_broker_id"]
    )
    assert (
        customer_4_active_broker_id_after
        == customer_2_groupA_info_after["active_broker_id"]
    )
    assert (
        customer_4_active_broker_id_after
        == customer_2_groupB_info_after["active_broker_id"]
    )
    assert (
        customer_2_groupA_info_after["connected_brokers"]["0"]
        == customer_4_groupA_info_after["connected_brokers"]["0"]
