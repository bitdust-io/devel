...
def supplier_list_v1(
    customer: str,
    expected_min_suppliers=None,
    expected_max_suppliers=None,
    attempts=20,
    delay=5,
    ...
    while True:
        if count > attempts:
            assert False, (
                f"{customer} failed to hire correct number of suppliers after many attempts. currently %d, expected min %d and max %d"
                % (
                    num_connected,
        ...
        for s in response.json()["result"]:
            if s["supplier_state"] == "CONNECTED" and s["contact_state"] == "CONNECTED":
                num_connected += 1
        if expected_min_suppliers is not None and (
            num_connected < expected_min_suppliers or num_total < expected_min_suppliers
        ):
            if verbose:
                dbg(
            ...
            time.sleep(delay)
            continue
        if expected_max_suppliers is not None and (
            num_connected > expected_max_suppliers or num_total > expected_max_suppliers
        ):
            if verbose:
                dbg(
...
def supplier_list_dht_v1(
    customer_id,
    observers_ids,
    expected_ecc_map,
    expected_suppliers_number,
    retries=30,
    delay=3,
    accepted_mistakes=1,
):
        ...
        while True:
            ...
            try:
                response = request_get(
                    obs, "supplier/list/dht/v1?id=%s" % customer_id, timeout=20
                )
            ...
            if not response.json()["result"]:
                ...
                time.sleep(delay)
                continue
            if not response.json()["result"]["customer_idurl"].count(
                "%s.xml" % customer_node
            ):
                warn(
    ...
    assert False, (
        "customer family [%s] [%s] was not re-published correctly, %d observers still see a wrong info"
        % (
            customer_node,
            expected_ecc_map,
            count,
...
def supplier_switch_v1(
    customer: str, supplier_idurl: str, position: int, validate_retries=30, delay=3
):
    ...
    while True:
        if count >= validate_retries:
            break
        current_suppliers_idurls = supplier_list_v1(
            customer, expected_min_suppliers=None, expected_max_suppliers=None, attempts=1
        )
        if supplier_idurl in current_suppliers_idurls:
            _pos = current_suppliers_idurls.index(supplier_idurl)
            assert position == _pos
            dbg(
                "found supplier %r at position %d for customer %r"
                % (supplier_idurl, position, customer)
            )
...
def group_info_v1(
    ...
    response = request_get(
        node, "group/info/v1?group_key_id=%s" % group_key_id, timeout=20
    )
    assert response.status_code == 200
...
def group_info_dht_v1(
    node: str, customer_id, timeout=20, wait_records=None, retries=10, delay=3
):
    response = request_get(
        node, "group/info/dht/v1?group_creator_id=%s" % customer_id, timeout=timeout
    )
    assert response.status_code == 200
    ...
    while True:
        ...
        if count >= retries:
            dbg(
                "group/info/dht/v1 [%s] attempt %d : %s\n"
                % (node, count, pprint.pformat(response.json()))
            )
            break
        count += 1
...
def group_reconnect_v1(customer: str, group_key_id, timeout=120):
    ...
    assert response.status_code == 200
    dbg(
        "group/reconnect/v1 [%s] group_key_id=%r : %s\n"
        % (customer, group_key_id, pprint.pformat(response.json()))
    )
    assert response.json()["status"] == "OK", response.json()
    packet_list_v1(customer, wait_all_finish=True)
...
def file_list_all_v1(
    node, expected_reliable=100, reliable_shares=True, attempts=20, delay=5, verbose=False
):
    ...
    while latest_reliable is None or latest_reliable < expected_reliable:
        ...
        if count >= attempts:
            warn(
                f"    latest reliable item info: {pprint.pformat(latest_reliable_fileinfo)}"
            )
            assert (
                False
            ), f"file {lowest_file} is not {expected_reliable} % reliable after {attempts} attempts"
            return
        time.sleep(delay)
    return response.json()
...
def file_create_v1(node, remote_path):
    response = request_post(
        node, "file/create/v1", json={"remote_path": remote_path}, timeout=20
    )
    assert response.status_code == 200
    dbg(
...
def file_upload_start_v1(
    ...
    if wait_job_finish:
        for _ in range(wait_finish_attempts):
            ...
            assert response.json()["status"] == "OK", response.json()
            if (
                len(response.json()["result"]["pending"]) == 0
                and len(response.json()["result"]["running"]) == 0
            ):
                break
            time.sleep(delay)
        else:
...
def file_download_start_v1(
    ...
    for _ in range(download_attempts):
        ...
        if response.json()["status"] == "OK":
            ...
            break
        if response.json()["errors"][0].count("downloading") and response.json()[
            "errors"
        ][0].count("already scheduled"):
            # print('file/download/start/v1 [%s] remote_path=%s destination_folder=%s : %s\n' % (
            #     customer, remote_path, destination, 'ALREADY STARTED', ))
            break
        if (
            response.json()["errors"][0].count("failed")
            and response.json()["errors"][0].count("downloading")
        ) or (response.json()["errors"][0].count("was not found in the catalog")):
            time.sleep(delay)
        else:
    ...
    for i in range(retries + 1):
        ...
        try:
            ...
            if expected_data == "not_exist":
                ...
                assert (
                    len(response.json()["result"]["closest_nodes"]) > 0
                ), response.json()
            else:
                ...
                assert (
                    response.json()["result"]["value"]["data"] in expected_data
                ), response.json()
                assert response.json()["result"]["value"]["key"] == key, response.json()
                assert (
                    response.json()["result"]["value"]["type"] == record_type
                ), response.json()
        except:
            time.sleep(2)
            if i == retries - 1:
                assert (
                    False
                ), f"DHT value read validation failed: {node} {key} {expected_data} : {response.json()}"
    return response.json()
...
def message_receive_v1(
    ...
    response = request_get(
        node,
        f"message/receive/{consumer}/v1?polling_timeout=%d" % polling_timeout,
        timeout=timeout,
        attempts=attempts,
    )
    assert response.status_code == 200
    dbg(
        f"message/receive/{consumer}/v1 [%s] : %s\n"
        % (node, pprint.pformat(response.json()))
    )
...
def message_history_v1(node, recipient_id, message_type="private_message", timeout=15):
    response = request_get(
        node,
        f"message/history/v1?id={recipient_id}&message_type={message_type}",
        timeout=timeout,
    )
    assert response.status_code == 200
    dbg(
...
def message_conversation_v1(
    node, message_types="private_message,group_message", timeout=15
):
    response = request_get(
        node, f"message/conversation/v1?message_types={message_types}", timeout=timeout
    )
    assert response.status_code == 200
    dbg(
...
def user_ping_v1(node, remote_node_id, timeout=95, ack_timeout=30, retries=2):
    ...
    try:
        response = request_get(
            node,
            f"user/ping/v1?id={remote_node_id}&timeout={ack_timeout}&retries={retries}",
            timeout=timeout,
        )
...
def service_info_v1(
    node, service_name, expected_state, attempts=90, delay=2, verbose=True
):
    current_state = None
    count = 0
    while current_state is None or current_state != expected_state:
        response = request_get(
            node, f"service/info/{service_name}/v1", timeout=20, verbose=verbose
        )
        assert response.status_code == 200
        assert response.json()["status"] == "OK", response.json()
        ...
        if current_state == expected_state:
            if verbose:
                dbg(
                    f"service/info/{service_name}/v1 [{node}] : %s"
                    % pprint.pformat(response.json())
                )
            break
        if verbose:
        ...
        if count >= attempts:
            assert (
                False
            ), f"service {service_name} is not {expected_state} after {attempts} attempts"
            return
        time.sleep(delay)
    if verbose:
...
def service_start_v1(node, service_name, timeout=10):
    response = request_post(
        node, f"service/start/{service_name}/v1", json={}, timeout=timeout
    )
    assert response.status_code == 200
    dbg(
...
def service_stop_v1(node, service_name, timeout=10):
    response = request_post(
        node, f"service/stop/{service_name}/v1", json={}, timeout=timeout
    )
    assert response.status_code == 200
    dbg(
...
def packet_list_v1(node, wait_all_finish=False, attempts=90, delay=2, verbose=False):
    ...
    for _ in range(attempts):
        ...
        for r in response.json()["result"]:
            ...
            #     continue
            if (
                r.get("command") == "Retrieve"
                and r.get("direction") == "outgoing"
                and r.get("label", "").count("-rotated")
            ):
                continue
            if (
                r.get("command") == "Data"
...
def automat_info_v1(node, automat_index, expected_state=None):
    response = request_get(
        node, "automat/info/v1?index=%d" % int(automat_index), timeout=20
    )
    assert response.status_code == 200
    dbg(
...
def wait_suppliers_connected(
    nodes, expected_min_suppliers=2, expected_max_suppliers=2, verbose=False
):
    dbg(
    ...
    for node in nodes:
        supplier_list_v1(
            node,
            expected_min_suppliers=expected_min_suppliers,
            expected_max_suppliers=expected_max_suppliers,
            verbose=verbose,
        )
    dbg("")
...
def wait_event(nodes, event, expected_count=1, attempts=10, delay=5, verbose=False):
    ...
    for node in nodes:
        for _ in range(attempts):
            event_log = run_ssh_command_and_wait(
                node, "cat /root/.bitdust/logs/event.log", verbose=verbose
            )[0].strip()
            ...
            assert (
                False
            ), f'event "{event}" did not occurred {expected_count} times on [{node}] after many attempts'
    dbg("")


def wait_specific_event(
    nodes, event, match_regex, expected_count=1, attempts=20, delay=5, verbose=False
):
    dbg(
...
def verify_message_sent_received(
    ...
    if expected_results:
        for consumer_id, expected_result in expected_results.items():
            if expected_result:
                if (
                    not consumer_results[consumer_id][0]
                    or not consumer_results[consumer_id][0]["result"]
                ):
                    assert False, "consumer %r did not received expected message %r" % (
                        consumer_id,
                        sample_message,
                    )
                if (
                    ...
                    assert (
                        False
                    ), "consumer %r received message %r, but expected is %r" % (
                        consumer_id,
                        consumer_results[consumer_id][0]["result"][0]["data"],
                        sample_message,
                    )
                ...
                assert (
                    consumer_results[consumer_id][0]["result"] == []
                ), "consumer %r received message while should not: %r" % (
                    consumer_id,
                    consumer_results[consumer_id],
                )
            if consumer_id in expected_last_sequence_id:
                consumer_last_sequence_id = group_info_v1(consumer_id, group_key_id)[
                    "result"
                ]["last_sequence_id"]
                assert (
                    consumer_last_sequence_id == expected_last_sequence_id[consumer_id]
...
def verify_file_create_upload_start(
    ...
    if randomize_bytes == 0:
        ...
        run_ssh_command_and_wait(
            node,
            f'python -c "import os, base64; print(base64.b64encode(os.urandom({randomize_bytes})).decode()[:{randomize_bytes}])" > {local_filepath}',
        )
    if verify_list_files:
        file_list_all_v1(
            node, reliable_shares=reliable_shares, expected_reliable=expected_reliable
        )
...
def verify_file_download_start(
    ...
    if verify_from_local_path is not None:
        file_body_source = run_ssh_command_and_wait(
            node, f"cat {verify_from_local_path}"
        )[0].strip()
        file_body_downloaded = run_ssh_command_and_wait(node, f"cat {destination_path}")[
            0
        ].strip()
