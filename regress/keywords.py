...
def supplier_list_dht_v1(customer_id, observers_ids, expected_ecc_map, expected_suppliers_number, retries=30, delay=3, accepted_mistakes=1):
    ...
    for observer_id in observers_ids:
        ...
        if _validate(observer_node):
            dbg('customer family [%s] [%s] info is correct for observer [%s] count=%d\n' % (
                customer_node,
                expected_ecc_map,
                observer_node,
                count,
...
def supplier_switch_v1(customer: str, supplier_idurl: str, position: int, validate_retries=30, delay=3):
    response = request_put(
        customer, 'supplier/switch/v1', json={
            'index': position,
...
def share_info_v1(node: str, key_id, wait_state=None, validate_retries=90, delay=2, stop_state=None, wait_suppliers=None):
    ...
    while True:
        ...
        dbg('  share/info/v1 [%s] attempt %d : state=%s' % (
            node,
            count,
...
def group_create_v1(customer: str, key_size=1024, label='', attempts=1):
    response = request_post(
        customer, 'group/create/v1', json={
            'key_size': key_size,
            'label': label,
        }, timeout=40, attempts=attempts
    )
    assert response.status_code == 200
...
def group_info_v1(node: str, group_key_id, wait_state=None, validate_retries=90, delay=2, stop_state=None):
    ...
    while True:
        ...
        dbg('  group/info/v1 [%s] attempt %d : state=%s' % (
            node,
            count,
            response.json()['result']['state'],
        ))
...
def group_leave_v1(customer: str, group_key_id, attempts=1):
    response = request_delete(
        customer, 'group/leave/v1', json={
            'group_key_id': group_key_id,
        }, timeout=20, attempts=attempts
    )
    assert response.status_code == 200
...
def group_reconnect_v1(customer: str, group_key_id, timeout=120):
    response = request_put(
        customer, 'group/reconnect/v1', json={
            'group_key_id': group_key_id,
        }, timeout=timeout
    )
    assert response.status_code == 200
...
def file_create_v1(node, remote_path):
    ...
    assert response.status_code == 200
    dbg('file/create/v1 [%s] remote_path=%s : %s\n' % (
        node,
        remote_path,
        pprint.pformat(response.json()),
    ))
...
def file_upload_start_v1(
    ...
    assert response.status_code == 200
    dbg('file/upload/start/v1 [%r] remote_path=%s local_path=%s : %s\n' % (
        customer,
        remote_path,
        local_path,
        pprint.pformat(response.json()),
...
def file_download_start_v1(customer: str, remote_path: str, destination: str, wait_result=True, download_attempts=1, wait_finish_attempts=20, delay=5, wait_tasks_finish=True):
    for _ in range(download_attempts):
        ...
        assert response.status_code == 200
        dbg('file/download/start/v1 [%s] remote_path=%s destination_folder=%s : %s\n' % (
            customer,
            remote_path,
            destination,
            pprint.pformat(response.json()),
        ...
        if response.json()['errors'][0].count('downloading') and response.json()['errors'][0].count('already scheduled'):
            ...
            break
        if (response.json()['errors'][0].count('failed') and response.json()['errors'][0].count('downloading')) or (response.json()['errors'][0].count('was not found in the catalog')):
            time.sleep(delay)
        else:
...
def dht_value_get_v1(node, key, expected_data, record_type='skip_validation', retries=2, fallback_observer=None):
    ...
    for i in range(retries + 1):
        ...
        response = request_get(
            node, 'dht/value/get/v1?record_type=%s&key=%s' % (
                record_type,
                key,
            ), timeout=20
        )
...
def dht_value_set_v1(
    ...
    assert response.status_code == 200
    dbg('dht/value/set/v1 [%s] key=%s : %s\n' % (
        node,
        key,
        pprint.pformat(response.json()),
    ))
...
def message_send_group_v1(node, group_key_id, data, timeout=120):
    dbg('message/send/group/v1 [%s] group_key_id=%r data=%r' % (
        node,
        group_key_id,
        data,
    ))
...
def message_history_v1(node, recipient_id, message_type='private_message', timeout=15):
    ...
    assert response.status_code == 200
    dbg('message/history/v1 [%s] recipient_id=%s : %s\n' % (
        node,
        recipient_id,
        pprint.pformat(response.json()),
    ))
...
def message_conversation_v1(node, message_types='private_message,group_message', timeout=15):
    ...
    assert response.status_code == 200
    dbg('message/conversation/v1 [%s] message_types=%s : %s\n' % (
        node,
        message_types,
        pprint.pformat(response.json()),
    ))
...
def automat_info_v1(node, automat_index, expected_state=None):
    ...
    assert response.status_code == 200
    dbg('automat/info/v1?index=%d [%s] : %s\n' % (
        int(automat_index),
        node,
        pprint.pformat(response.json()),
    ))
...
def friend_add_v1(node, friend_idurl, friend_alias=''):
    ...
    assert response.status_code == 200
    dbg('friend/add/v1 [%s] idurl=%r alias=%r : %s\n' % (
        node,
        friend_idurl,
        friend_alias,
        pprint.pformat(response.json()),
...
def queue_consumer_list_v1(node, extract_ids=False):
    ...
    assert response.status_code == 200
    dbg('queue/consumer/list/v1 [%s] : %s\n' % (
        node,
        pprint.pformat(response.json()),
    ))
    assert response.json()['status'] == 'OK', response.json()
...
def queue_producer_list_v1(node, extract_ids=False):
    ...
    assert response.status_code == 200
    dbg('queue/producer/list/v1 [%s] : %s\n' % (
        node,
        pprint.pformat(response.json()),
    ))
    assert response.json()['status'] == 'OK', response.json()
...
def wait_service_state(nodes, service_name, state, verbose=False):
    dbg('wait service %r state to become %r on %d nodes' % (
        service_name,
        state,
        len(nodes),
    ))
...
def wait_suppliers_connected(nodes, expected_min_suppliers=2, expected_max_suppliers=2, verbose=False):
    dbg('wait min %d and max %d suppliers to be connected on %d nodes' % (
        expected_min_suppliers,
        expected_max_suppliers,
        len(nodes),
    ))
...
def wait_event(nodes, event, expected_count=1, attempts=10, delay=5, verbose=False):
    dbg('wait event "%s" to occur %d times on %d nodes' % (
        event,
        expected_count,
        len(nodes),
    ))
...
def wait_specific_event(nodes, event, match_regex, expected_count=1, attempts=20, delay=5, verbose=False):
    dbg('wait event "%s" with "%s" to occur %d times on %d nodes' % (
        event,
        match_regex,
        expected_count,
        len(nodes),
...
def verify_message_sent_received(group_key_id, producer_id, consumers_ids, message_label='A', expected_results={}, expected_last_sequence_id={}, receive_timeout=31, polling_timeout=30):
    sample_message = {
        ...
        for consumer_id, expected_result in expected_results.items():
            if expected_result:
                ...
                if consumer_results[consumer_id][0]['result'][0]['data'] != sample_message:
                    assert False, 'consumer %r received message %r, but expected is %r' % (consumer_id, consumer_results[consumer_id][0]['result'][0]['data'], sample_message)
            else:
                assert consumer_results[consumer_id][0]['result'] == [], 'consumer %r received message while should not: %r' % (consumer_id, consumer_results[consumer_id])
