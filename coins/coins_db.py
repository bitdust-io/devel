...
from coins import coins_index, coins_io
from logs import lg
    ...
    for source_index_file in source_indexes:
        if source_index_file != "00id.py":
            ...
            if not bpio.WriteTextFile(
                destination_index_path, bpio.ReadTextFile(source_index_path)
            ):
            ...
            if not bpio.WriteBinaryFile(
                destination_buck_path, bpio.ReadBinaryFile(source_buck_path)
            ):
            ...
            if not bpio.WriteBinaryFile(
                destination_stor_path, bpio.ReadBinaryFile(source_stor_path)
            ):
            ...
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "        wrote index %s from %s" % (index_name, source_index_path),
                )
...
def get_many(
    ...
    **kwargs
    ...
    try:
        for r in db().get_many(
            index_name, key, limit, offset, with_doc, with_storage, start, end, **kwargs
        ):
            yield r
    except (
        PreconditionsException,
...
def _test_coin_worker(customer_idurl, duration, amount, price=1.0, trustee=None):
    ...
    storage_coin = coins_io.storage_contract_open(
        customer_idurl, duration, amount, price, trustee
    )
...
def _test_coin_mined(coin_json, customer_idurl, duration, amount, price, trustee):
    ...
    reactor.callLater(
        1, _test_coin_worker, customer_idurl, duration, amount, price, trustee
    )
    return coin_json
