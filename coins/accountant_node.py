...
from coins import coins_db, coins_io
...
class AccountantNode(automat.Automat):
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---READ_COINS---
            ...
            if event == "shutdown":
                ...
                event == "timer-1min" and not self.isAnyCoinsReceived(*args, **kwargs)
            ):
                self.state = "OFFLINE"
            elif event == "new-coin-mined":
                self.doPushCoin(*args, **kwargs)
                ...
                self.doWriteCoins(*args, **kwargs)
                self.doRetrieveCoins(*args, **kwargs)
            elif event == "valid-coins-received" and not self.isMoreCoins(
                *args, **kwargs
            ):
                self.state = "READY"
                self.doWriteCoins(*args, **kwargs)
        ...
        # ---ACCOUNTANTS?---
            ...
            if event == "shutdown":
                ...
                self.doAddAccountant(*args, **kwargs)
                self.doLookupAccountants(*args, **kwargs)
            elif event == "accountant-connected" and not self.isMoreNeeded(
                *args, **kwargs
            ):
                ...
                event == "lookup-failed"
                and self.Attempts < 5
                and self.isAnyAccountants(*args, **kwargs)
            ):
                self.Attempts += 1
                self.doLookupAccountants(*args, **kwargs)
            elif (
                event == "lookup-failed"
                and (self.Attempts >= 5 or not self.isAnyAccountants(*args, **kwargs))
            ) or event == "timer-2min":
                self.state = "OFFLINE"
        return None
    ...
    def doAddAccountant(self, *args, **kwargs):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "accountant_node.doAddAccountant NEW %s connected, %d total accountants"
    ...
    def _on_command_retrieve_coin(self, newpacket, info):
        ...
        p2p_service.SendCoin(
            newpacket.CreatorID, result_coins, packet_id=newpacket.PacketID
        )
        return True
