...
from coins import coins_io, contract_chain_consumer
from logs import lg
...
def is_connected():
    ...
    if contract_chain_consumer.A().state != "CONNECTED":
        if _Debug:
            lg.out(
                _DebugLevel,
                "contract_chain_node._connected OFFLINE, nodes connection is not ready: %s"
                % contract_chain_consumer.A(),
            )
        return False
    return True
...
class Query(object):
    def __init__(self, query_dict):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "contract_chain_node.Query created to request from %d accountants"
                % len(self.out_packets),
            )
    ...
    def _on_coin_received(self, response_packet, info):
        ...
        if _Debug:
            lg.out(
                _DebugLevel, "contract_chain_node._on_coin_received %r" % response_packet
            )
    ...
    def _on_coin_failed(self, response_packet, info):
        ...
        if _Debug:
            lg.out(
                _DebugLevel, "contract_chain_node._on_coin_failed %r" % response_packet
            )
        assert response_packet.PacketID in self.out_packets
        self.out_packets[response_packet.PacketID].callback(
            (
    ...
    def _on_results_collected(self, accountant_results):
        ...
        for success, result_tuple in accountant_results:
            ...
            for coin in coins_list:
                ...
                if coins_io.coin_to_string(
                    unique_coins[coin_hash]
