...
class Parser(object):
    ...
    def parse_ex(self):
        ...
        return "".join(self.custom_header), "".join(self.tokens_head) + (
            " ".join(self.tokens)
        )
        ...
        for i in d:
            if len(encs) > 0 and encs[-1] in ['"', "'"]:
                ...
                if len(encs) < 1 or contr[encs[-1]] != i:
                    raise IndexCreatorValueException(
                        "Missing opening enclosure for '%s'" % i, self.cnt_line_nr(d, st)
                    )
                del encs[-1]
                ...
                if c_b_cnt == n_b_cnt == s_b_cnt == 0:
                    if i == ",":
                        ...
                        if (s == 1 and comas_cnt > 1) or (s == 2 and comas_cnt > 0):
                            raise IndexCreatorFunctionException(
                                "Too much arguments to return", self.cnt_line_nr(line, st)
                            )
                        ...
                        raise IndexCreatorFunctionException(
                            "Two colons don't make any sense", self.cnt_line_nr(line, st)
                        )
    ...
    def cnt_line_nr(self, l, stage):
        ...
        if stage == 0:
            ...
            return (
                nr + self.cnt_lines[0] + (self.cnt_lines[2] - 1 if self.funcs_rev else 0)
            )
        elif stage == 2:
            return (
                nr
