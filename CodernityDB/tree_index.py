...
class IU_TreeBasedIndex(Index):
    ...
    def _read_single_node_key(self, node_start, key_index):
        self.buckets.seek(self._calculate_key_position(node_start, key_index, "n"))
    ...
    def _update_element(self, leaf_start, key_index, new_data):
        self.buckets.seek(
            self._calculate_key_position(leaf_start, key_index, "l") + self.key_size
        )
    ...
    def _find_key_in_node_using_binary_search(
        ...
        if (
            ...
            matching_record_index = self._node_linear_key_search(
                key,
                self._calculate_key_position(node_start, imin, "n"),
                imin,
                chosen_key_position,
            )
        else:
    ...
    def _update_leaf_ready_data(
        ...
        start_position = self._calculate_key_position(leaf_start, start_index, "l")
    ...
    def _update_leaf(
        ...
        if nr_of_records_to_rewrite == 0:  # just write at set position
            self.buckets.seek(
                self._calculate_key_position(leaf_start, new_record_position, "l")
            )
            self.buckets.write(
                struct.pack(
                    "<" + self.single_leaf_record_format,
            ...
            start = self._calculate_key_position(leaf_start, new_record_position, "l")
            self.buckets.seek(start)
            data = self.buckets.read(
    ...
    def _create_new_root_from_leaf(
        ...
        blanks = (
            (self.node_capacity - new_leaf_size) * self.single_leaf_record_size * "\x00"
        )
        ...
        left_leaf_data += (
            (self.node_capacity - old_leaf_size) * self.single_leaf_record_size * "\x00"
        )
            ...
            if nr_of_records_to_rewrite > half_size:  # insert key into first half of leaf
                self.buckets.seek(
                    self._calculate_key_position(
                        leaf_start, self.node_capacity - nr_of_records_to_rewrite, "l"
                    )
                )
                ...
                self.buckets.seek(
                    self._calculate_key_position(leaf_start, old_leaf_size, "l")
                )
                data = self.buckets.read(
    ...
    def _prepare_new_root_data(
        self, root_key, left_pointer, right_pointer, children_flag="n"
    ):
        ...
        new_root += (
            (self.key_size + self.pointer_size) * (self.node_capacity - 1) * "\x00"
        )
        return new_root
    ...
    def _split_node(
        ...
        if create_new_root:
            ...
            if (
                ...
                # reading second half of node
                self.buckets.seek(
                    self._calculate_key_position(node_start, old_node_size, "n")
                    + self.pointer_size
                )
                # read all keys with key>new_key
                data = self.buckets.read(
                ...
                self.buckets.seek(
                    self._calculate_key_position(
                        node_start, self.node_capacity - nr_of_keys_to_rewrite, "n"
                    )
                    + self.pointer_size
                )
                # read all keys with key>new_key
