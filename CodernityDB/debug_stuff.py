        ...
        def __inner(*args, **kwargs):
            ...
            if funct_name == "insert":
                try:
                    ...
                    packed = json.dumps((funct_name, meth_args, kwargs_copy, None))
                    f_obj.write("%s\n" % packed)
                    f_obj.flush()
                    raise
                else:
                    packed = json.dumps((funct_name, meth_args, kwargs_copy, res))
                f_obj.write("%s\n" % packed)
                f_obj.flush()
            else:
                if funct_name == "get":
                    for curr in __stack:
                        if ("delete" in curr or "update" in curr) and not curr.startswith(
                            "test"
                        ):
