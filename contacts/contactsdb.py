...
from lib import jsn, nameurl, strng
from logs import lg
...
def init():
    ...
    if _Debug:
        d.addErrback(
            lg.errback, debug=_Debug, debug_level=_DebugLevel, method="contactsdb.init"
        )
    return d
...
def set_suppliers(idlist, customer_idurl=None):
    ...
    if _Debug:
        lg.args(
            _DebugLevel,
            suppliers=_SuppliersList[customer_idurl],
...
def add_supplier(idurl, position=None, customer_idurl=None):
    ...
    if _Debug:
        lg.args(
            _DebugLevel, idurl=idurl, position=position, customer_idurl=customer_idurl
        )
    ...
    if position >= len(current_suppliers):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "contactsdb.add_supplier   %d empty suppliers added for customer %r"
                % (len(empty_suppliers), customer_idurl),
            )
...
def contacts(include_all=False, include_enabled=True):
    ...
    if (
        include_all
        or (include_enabled and driver.is_enabled("service_customer"))
        or driver.is_on("service_customer")
    ):
        result.update(set(all_suppliers()))
    if (
        include_all
        or (include_enabled and driver.is_enabled("service_supplier"))
        or driver.is_on("service_supplier")
    ):
        result.update(set(customers() + known_customers()))
    if (
        include_all
        or (include_enabled and driver.is_enabled("service_private_messages"))
        or driver.is_on("service_private_messages")
    ):
        result.update(set(correspondents_ids()))
    if include_all or include_enabled:
        if driver.is_enabled("service_message_broker") or driver.is_on(
            ...
            result.update(
                set(
                    message_peddler.list_consumers_producers(
                        include_consumers=True, include_producers=True
                    )
                )
            )
...
def contacts_remote(include_all=False, include_enabled=True):
    ...
    """
    l = id_url.to_bin_list(
        contacts(include_all=include_all, include_enabled=include_enabled)
    )
    return [
        i for i in id_url.fields_list(l) if not id_url.is_the_same(i, my_id.getIDURL())
    ]
...
def set_correspondents(idlist):
    ...
    global _CorrespondentsList
    _CorrespondentsList = [
        (id_url.field(idurl_name[0]).to_bin(), idurl_name[1]) for idurl_name in idlist
    ]
...
def is_supplier(idurl, customer_idurl=None):
    ...
    if id_url.is_empty(idurl):
        return False
    return id_url.field(idurl).to_bin() in id_url.to_bin_list(
        suppliers(customer_idurl=customer_idurl)
    )
...
def supplier_position(idurl, customer_idurl=None):
    ...
    idurl = id_url.field(idurl)
    try:
        index = id_url.to_bin_list(suppliers(customer_idurl=customer_idurl)).index(
            idurl.to_bin()
        )
    except:
        index = -1
...
def load_suppliers(path=None, customer_idurl=None, all_customers=False):
    ...
    if all_customers:
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "contactsdb.load_suppliers %d known customers"
                % len(list_local_customers),
            )
        for customer_id in list_local_customers:
            if not global_id.IsValidGlobalUser(customer_id):
                lg.warn(
                    "invalid customer record %s found in %s"
            ...
            if not one_customer_idurl.is_latest():
                latest_customer_path = os.path.join(
                    settings.SuppliersDir(), one_customer_idurl.to_id()
                )
                ...
                if not os.path.exists(latest_customer_path):
                    ...
                    lg.info(
                        "detected and processed idurl rotate when loading suppliers for customer : %r -> %r"
                        % (customer_id, one_customer_idurl.to_id())
                    )
                else:
            ...
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "    loaded %d known suppliers for customer %r"
                    % (len(lst), one_customer_idurl),
                )
        return True
    if not customer_idurl:
        customer_idurl = my_id.getIDURL()
    ...
    if _Debug:
        lg.out(
            _DebugLevel, "contactsdb.load_suppliers %d items from %s" % (len(lst), path)
        )
    return True
...
def cache_suppliers(path=None):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "contactsdb.cache_suppliers prepared %d idurls to be cached" % len(dl),
        )
...
def cache_customers(path=None):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "contactsdb.cache_customers prepared %d idurls to be cached" % len(dl),
        )
    return DeferredList(dl, consumeErrors=True)
...
def cache_correspondents(path=None):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "contactsdb.cache_correspondents prepared %d idurls to be cached" % len(dl),
        )
    return DeferredList(dl, consumeErrors=True)
    ...
    if not customer_idurl.is_latest():
        if customer_idurl.original() in _CustomersMetaInfo:
            if customer_idurl.to_bin() not in _CustomersMetaInfo:
                _CustomersMetaInfo[customer_idurl.to_bin()] = _CustomersMetaInfo.pop(
...
def get_customer_meta_info(customer_idurl):
    ...
    customer_idurl = id_url.to_bin(customer_idurl)
    return jsn.dict_keys_to_text(
        jsn.dict_values_to_text(_CustomersMetaInfo.get(customer_idurl, {}))
    )
