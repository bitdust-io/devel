...
def init(filepath=None):
    ...
    if not os.path.isfile(filepath):
        ...
        _HistoryCursor.execute(
            'CREATE INDEX "sender local key id" on history(sender_local_key_id)'
        )
        _HistoryCursor.execute(
            'CREATE INDEX "recipient local key id" on history(recipient_local_key_id)'
        )
        ...
        _HistoryCursor.execute(
            'CREATE INDEX "conversation id" on conversations(conversation_id)'
        )
...
def build_json_conversation(**record):
    ...
    if conv["type"] == "private_message":
        ...
        conv_label = None
        user_idurl = None
        if (id_url.is_cached(idurl1) and idurl1 == my_id.getIDURL()) or usr1.split("@")[
            0
        ] == my_id.getIDName():
            user_idurl = idurl2
            conv_key_id = global_id.UrlToGlobalID(idurl2, include_key=True)
            conv_label = conv_key_id.replace("master$", "").split("@")[0]
        if (id_url.is_cached(idurl2) and idurl2 == my_id.getIDURL()) or usr2.split("@")[
            0
        ] == my_id.getIDName():
            user_idurl = idurl1
...
def insert_message(
    ...
    db().commit()
    conversation_id = update_conversation(
        sender_local_key_id,
        recipient_local_key_id,
        payload_type,
        payload_time,
    ...
    listeners.push_snapshot(
        "message", snap_id=snap_id, created=payload_time, data=message_json
    )
    return message_json
...
def update_conversation(
    ...
    conversation_id = get_conversation_id(
        sender_local_key_id, recipient_local_key_id, payload_type
    )
    ...
    if _Debug:
        lg.args(
            _DebugLevel,
            conversation_id=conversation_id,
            found_conversation=len(found_conversation),
            params=params,
        )
...
def query_messages(
    ...
    for row in cur().execute(sql, params):
        ...
        if sender_local_key_id not in local_key_ids:
            local_key_ids[sender_local_key_id] = my_keys.get_local_key(
                sender_local_key_id
            )
        if recipient_local_key_id not in local_key_ids:
            local_key_ids[recipient_local_key_id] = my_keys.get_local_key(
                recipient_local_key_id
            )
        if not local_key_ids.get(sender_local_key_id) or not local_key_ids.get(
            recipient_local_key_id
        ):
        ...
        json_msg = build_json_message(
            ...
            conversation_id=get_conversation_id(
                sender_local_key_id, recipient_local_key_id, int(row[5])
            ),
...
def rebuild_conversations():
    cur().execute(
        'SELECT count(name) FROM sqlite_master WHERE type="table" AND name="conversations"'
    )
    if cur().fetchone()[0]:
        return
    cur().execute(
    ...
    for message_json in list(query_messages()):
        msg_typ = message_json["payload"].get("msg_type") or message_json["payload"].get(
            "type"
        )
        payload_type = MESSAGE_TYPES.get(msg_typ, 1)
        recipient_local_key_id = my_keys.get_local_key_id(
        ...
        if payload_type in [
            ...
            sender_local_key_id = my_keys.get_local_key_id(
...
def check_create_rename_key(new_public_key, new_key_id, new_local_key_id):
    ...
    if found_public_keys:
        if len(found_public_keys) > 1:
            raise Exception(
                "found multiple records for same public key: %r" % found_public_keys
            )
...
def populate_conversations(message_types=[], offset=0, limit=100, order_by_time=True):
    for conv in fetch_conversations(
        ...
        listeners.push_snapshot(
            "conversation", snap_id=conv["conversation_id"], data=conv
        )


def populate_messages(
    recipient_id=None, sender_id=None, message_types=[], offset=0, limit=100
):
    if recipient_id:
