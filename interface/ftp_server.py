...
from twisted.protocols.ftp import (  # classes; methods; exceptions; FTP codes constants
...
class BitDustFTP(FTP):
    ...
    def _ebReadOpened(self, err, newsegs):
        ...
        if not err.check(PermissionDeniedError, FileNotFoundError, IsADirectoryError):
            lg.warn(
                "Unexpected error attempting to open file for transmission: " + str(err)
            )
        if err.check(FTPCmdError):
    ...
    def _cbRestoreDone(self, ret, path_segments, result_defer):
        ...
        fp = filepath.FilePath(
            os.path.join(ret["local_path"], os.path.basename(ret["remote_path"]))
        )
        try:
            fobj = fp.open("r")
        except:
            ...
            result.append(
                (
                    ...
                    [  # name
                        ...
                        0,  # hardlinks
                        time.mktime(time.strptime(itm["latest"], "%Y-%m-%d %H:%M:%S"))
                        if itm["latest"]
                        else None,  # time
                        itm["customer"],  # owner
                        key_alias,  # group
