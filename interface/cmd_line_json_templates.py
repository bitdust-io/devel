...
TPL_KEYS_LIST = tpl_4_items.format(
    tpl_status, tpl_execution, ls("{key_id}\n"), tpl_errors
)
...
TPL_BACKUPS_LIST = tpl_4_items.format(
    ...
    tpl_execution,
    ls(
        "{global_id} {type} {size} {path} {.section versions}{.repeated section @}[{backup_id}: {size}]{.end}{.end} {.section key_id}<{@}>{.end}\n"
    ),
    tpl_errors,
)

TPL_BACKUPS_LIST_IDS = tpl_4_items.format(
    tpl_status, tpl_execution, ls("{backup_id} {size} {path}\n"), tpl_errors
)
...
TPL_OPTIONS_LIST_KEY_TYPE_VALUE = tpl_4_items.format(
    tpl_status, tpl_execution, ls("{key} ({type}) : {value}\n"), tpl_errors
)
...
TPL_SUPPLIERS = tpl_4_items.format(
    ...
    tpl_execution,
    ls(
        "{position}: {idurl}, since {connected}, keeps {.section fragments}{files}{.end} files, {contact_state}\n"
    ),
    tpl_errors,
)
...
TPL_CUSTOMERS = tpl_4_items.format(
    tpl_status, tpl_execution, ls("{position}: {idurl}, {status}\n"), tpl_errors
)
