...
from lib import jsn, serialization, strng
from lib.txrestapi.txrestapi.json_resource import JsonAPIResource, _JsonResource
from lib.txrestapi.txrestapi.methods import ALL, DELETE, GET, POST, PUT
from logs import lg
...
def serve_https(port):
    ...
    try:
        ...
        _APIListener = reactor.listenSSL(
            port, site, cert.options(auth), interface="127.0.0.1"
        )  # @UndefinedVariable
    except:
        lg.exc()
        os._exit(1)
...
class BitDustRESTHTTPServer(JsonAPIResource):
    ...
    def log_request(self, request, callback, args):
        ...
        if not _args:
            ...
            _args = {
                k: (v[0] if (v and isinstance(v, list)) else v) for k, v in _args.items()
            }
        if _Debug:
            if (
                uri
    ...
    def process_stop_v1(self, request):
        return api.process_stop(
            instant=bool(
                _request_arg(request, "instant", "1")
                in [
                    "1",
                    "true",
                ]
    ...
    def config_list_v1(self, request):
        return api.configs_list(
            ...
            include_info=bool(
                _request_arg(request, "include_info", "0")
                in [
                    "1",
                    "true",
                ]
    ...
    def identity_get_v1(self, request):
        return api.identity_get(
            include_xml_source=bool(
                _request_arg(request, "xml_source", "0")
                in [
                    "1",
                    "true",
                ]
    ...
    def key_list_v1(self, request):
        return api.keys_list(
            ...
            include_private=bool(
                _request_arg(request, "include_private", "0")
                in [
                    "1",
                    "true",
                ]
    ...
    def key_get_v1(self, request):
        return api.key_get(
            ...
            include_signature=bool(
                _request_arg(request, "include_signature", "0")
                in [
                    "1",
                    "true",
                ]
            ...
            generate_signature=bool(
                _request_arg(request, "generate_signature", "0")
                in [
                    "1",
                    "true",
                ]
    ...
    def file_list_v1(self, request):
        return api.files_list(
            ...
            recursive=bool(
                _request_arg(request, "recursive", "0")
                in [
                    "1",
                    "true",
                ]
            ...
            all_customers=bool(
                _request_arg(request, "all_customers", "0")
                in [
                    "1",
                    "true",
                ]
            ...
            include_uploads=bool(
                _request_arg(request, "uploads", "0")
                in [
                    "1",
                    "true",
                ]
            ...
            include_downloads=bool(
                _request_arg(request, "downloads", "0")
                in [
                    "1",
                    "true",
                ]
    ...
    def file_list_all_v1(self, request):
        return api.files_list(
            all_customers=True, include_uploads=True, include_downloads=True
        )
    ...
    def file_info_v1(self, request):
        return api.file_info(
            remote_path=_request_arg(request, "remote_path", mandatory=True),
            include_uploads=bool(
                _request_arg(request, "uploads", "1")
                in [
                    "1",
                    "true",
                ]
            ...
            include_downloads=bool(
                _request_arg(request, "downloads", "1")
                in [
                    "1",
                    "true",
                ]
    ...
    def files_uploads_v1(self, request):
        return api.files_uploads(
            include_running=bool(
                _request_arg(request, "running", "1")
                in [
                    "1",
                    "true",
                ]
            ...
            include_pending=bool(
                _request_arg(request, "pending", "1")
                in [
                    "1",
                    "true",
                ]
    ...
    def file_explore_v1(self, request):
        return api.file_explore(
            local_path=_request_arg(request, "local_path", mandatory=True)
        )
    ...
    def share_list_v1(self, request):
        return api.shares_list(
            ...
            include_granted=bool(
                _request_arg(request, "granted", "1")
                in [
                    "1",
                    "true",
                ]
    ...
    def share_grant_v1(self, request):
        ...
        return api.share_grant(
            ...
            or data.get("trusted_global_id")
            or data.get("trusted_idurl")
            or data.get("trusted_id"),
    ...
    def group_info_dht_v1(self, request):
        return api.group_info_dht(
            group_creator_id=_request_arg(request, "group_creator_id")
            or _request_arg(request, "group_creator_idurl")
            or _request_arg(request, "id"),
        )
    ...
    def user_ping_get_v1(self, request):
        return api.user_ping(
            user_id=_request_arg(request, "user_id")
            or _request_arg(request, "global_id")
            or _request_arg(request, "idurl")
            or _request_arg(request, "id"),
    ...
    def user_status_v1(self, request):
        return api.user_status(
            user_id=_request_arg(request, "global_id")
            or _request_arg(request, "idurl")
            or _request_arg(request, "id")
        )
    ...
    def user_search_v1(self, request, nickname):
        return api.user_search(
            nickname, attempts=int(_request_arg(request, "attempts", 1))
        )
    ...
    def message_conversation_v1(self, request):
        return api.message_conversations_list(
            message_types=list(
                filter(None, _request_arg(request, "message_types", "").split(","))
            ),
            offset=int(_request_arg(request, "offset", "0")),
            limit=int(_request_arg(request, "limit", "100")),
        )
    ...
    def message_receive_v1(self, request, consumer_callback_id):
        return api.message_receive(
            ...
            message_types=_request_arg(
                request, "message_types", "private_message,group_message"
            ),
    ...
    def supplier_list_v1(self, request):
        return api.suppliers_list(
            customer_id=_request_arg(request, "customer_id")
            or _request_arg(request, "customer_idurl")
            or _request_arg(request, "id"),
            verbose=bool(
                _request_arg(request, "verbose", "0")
                in [
                    "1",
                    "true",
                ]
    ...
    def supplier_change_v1(self, request):
        ...
        return api.supplier_change(
            ...
            or data.get("supplier_idurl")
            or data.get("supplier_glob_id"),
            new_supplier_id=data.get("new_global_id")
            or data.get("new_idurl")
            or data.get("new_supplier_id"),
        )
    ...
    def customer_reject_v1(self, request):
        ...
        return api.customer_reject(
            ...
            or data.get("idurl")
            or data.get("id"),
            erase_customer_key=bool(
                _request_arg(request, "erase_customer_key", "1")
                in [
                    "1",
                    "true",
                ]
    ...
    def service_list_v1(self, request):
        return api.services_list(
            with_configs=bool(
                _request_arg(request, "with_configs", "0")
                in [
                    "1",
                    "true",
                ]
    ...
    def connection_list_v1(self, request):
        return api.connections_list(
            protocols=map(
                ...
                filter(
                    None,
                    _request_arg(request, "protocols", "").strip().lower().split(","),
                ),
            )
            or None,
        )
    ...
    def network_connected_v1(self, request):
        return api.network_connected(
            wait_timeout=int(_request_arg(request, "wait_timeout", "5"))
        )
    ...
    def network_status_v1(self, request):
        return api.network_status(
            suppliers=bool(
                _request_arg(request, "suppliers", "0")
                in [
                    "1",
                    "true",
                ]
            ...
            customers=bool(
                _request_arg(request, "customers", "0")
                in [
                    "1",
                    "true",
                ]
    ...
    def network_info_v1(self, request):
        return api.network_status(
            suppliers=bool(
                _request_arg(request, "suppliers", "1")
                in [
                    "1",
                    "true",
                ]
            ...
            customers=bool(
                _request_arg(request, "customers", "1")
                in [
                    "1",
                    "true",
                ]
    ...
    def dht_node_find_v1(self, request):
        return api.dht_node_find(
            node_id_64=_request_arg(request, "node_id_64", mandatory=False, default=None)
            or _request_arg(request, "dht_id", mandatory=False, default=None),
    ...
    def dht_value_get_v1(self, request):
        return api.dht_value_get(
            key=_request_arg(request, "key", mandatory=True),
            record_type=_request_arg(
                request, "record_type", mandatory=False, default="skip_validation"
            ),
            layer_id=int(_request_arg(request, "layer_id", mandatory=False, default=0)),
        )
