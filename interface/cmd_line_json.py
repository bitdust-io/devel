...
{.end}""")
        ...
        return call_jsonrpc_method_and_stop('backups_id_list')

    elif args[1] == 'start' and len(args) >= 3:
        from lib import packetid
        if packetid.Valid(args[2]):
            return call_jsonrpc_method_and_stop('backup_start_id', args[2])
        if not os.path.exists(os.path.abspath(args[2])):
            print_text('path %s not exist\n' % args[2])
            return 1
        return call_jsonrpc_method_and_stop('backup_start_path', args[2])
        ...
        if bpio.pathIsDir(localpath):
            ...
            m = 'backup_file_add'
        return call_jsonrpc_method_and_stop(m, localpath)
        ...
        if not bpio.pathIsDir(localpath):
            ...
            return 1
        return call_jsonrpc_method_and_stop('backup_tree_add', localpath)

    elif args[1] == 'delete' and len(args) >= 3:
        if args[2] == 'local':
            if len(args) < 4:
                return 2
            return call_jsonrpc_method_and_stop('backup_delete_local', args[3].replace('/','_'))
        if packetid.Valid(args[2]):
            return call_jsonrpc_method_and_stop('backup_delete_id', args[2].replace('/','_'))
        return call_jsonrpc_method_and_stop('backup_delete_path', os.path.abspath(args[2]))

    elif args[1] == 'update':
        return call_jsonrpc_method_and_stop('backups_update')
...
def cmd_restore(opts, args, overDict):
    if len(args) == 2:
        return call_jsonrpc_method_and_stop('restore_single', args[1])
    elif len(args) == 3:
        return call_jsonrpc_method_and_stop('restore_single', args[1], args[2])
    return 2
...
def cmd_schedule(opts, args, overDict):
    ...
    if len(args) < 3:
        return call_jsonrpc_method_and_stop('getschedule', backupDir)
    from lib import schedule
    shed = schedule.from_compact_string(args[2])
    if shed is None:
        print_text(schedule.format()+'\n')
        return 0
    return call_jsonrpc_method_and_stop(
        'setschedule', backupDir, shed.type, shed.interval, shed.daytime, shed.details,)


def cmd_message(opts, args, overDict):
    if len(args) < 2 or args[1] == 'list':
        return call_jsonrpc_method_and_stop('list_messages')
    if len(args) >= 5 and args[1] in [ 'send', ]:
        return call_jsonrpc_method_and_stop('send_message', args[2], args[3], args[4]) 
    return 2


def cmd_friend(opts, args, overDict):
    if len(args) < 2:
        return call_jsonrpc_method_and_stop('list_correspondents')
    elif len(args) > 2 and args[1] == 'find':
        return call_jsonrpc_method_and_stop('find_peer_by_nickname', unicode(args[2]))
    return 2    
