...
from lib import jsn, jsontemplate, strng
...
def print_and_stop(result):
    ...
    sys.stdout.write(
        jsn.dumps(
            result, indent=2, keys_to_text=True, values_to_text=True, ensure_ascii=True
        )
        + "\n"
    )
...
def call_websocket_method(method, **kwargs):
    ...
    def _on_result(resp):
        ...
        if not isinstance(resp["payload"], dict):
            if not ret.called:
                ret.errback(
                    Exception("unexpected response received: %r" % resp["payload"])
                )
            return None
    ...
    websock.start(
        ...
        api_secret_filepath=os.path.join(
            deploy.current_base_dir(), "metadata", "apisecret"
        ),
    )
    return ret
...
def call_websocket_method_transform_template_and_stop(
    method, template, transform, **kwargs
):
    from twisted.internet import reactor  # @UnresolvedImport
...
def cmd_identity(opts, args, overDict, running, executablePath):
    ...
    def _do_cmd():
        ...
        if len(args) == 1 or args[1].lower() in [
            "info",
            "?",
            "show",
            "print",
        ...
        if (
            ...
            def _run_stand_alone_id_server():
                ...
                reactor.addSystemEventTrigger(
                    "before", "shutdown", id_server.A().automat, "shutdown"
                )  # @UndefinedVariable
            ...
            if len(args) <= 2:
                ...
                return call_websocket_method_template_and_stop(
                    "service_info", tpl, service_name="service_identity_server"
                )
            if args[2] == "stop":
                if not running:
                ...
                tpl = jsontemplate.Template(templ.TPL_RAW)
                return call_websocket_method_template_and_stop(
                    "service_stop", tpl, service_name="service_identity_server"
                )
            if args[2] == "start":
                if not running:
                    _run_stand_alone_id_server()
                ...
                tpl = jsontemplate.Template(templ.TPL_RAW)
                return call_websocket_method_template_and_stop(
                    "service_start", tpl, service_name="service_identity_server"
                )
            return 2
        ...
        def _register():
            ...
            initializer.A(
                "run-cmd-line-register", {"username": args[2], "pksize": pksize}
            )
            reactor.run()  # @UndefinedVariable
        ...
        def _recover():
            ...
            from automats import automat
            from logs import lg
            from main import config, initializer
            ...
            automat.SetLogOutputHandler(
                lambda debug_level, message: lg.out(
                    debug_level, message, log_name="state"
                )
            )
            # automat.OpenLogFile(settings.AutomatsLog())
        ...
        if args[1].lower() in [
            ...
            if running:
                print_text(
                    "BitDust is running at the moment, need to stop the software first\n"
                )
                return 0
            return _register()
        ...
        if len(args) >= 2 and args[1].lower() in [
            ...
            if args[1] in [
                ...
                if len(args) >= 3:
                    ...
                    filenameto = bpio.portablePath(
                        os.path.join(os.path.expanduser("~"), key_file_name)
                    )
                ...
                if not bpio.WriteTextFile(filenameto, TextToSave):
                    ...
                    return 1
                print_text(
                    'IDURL "%s" and "master" key was stored in "%s"'
...
def cmd_key(opts, args, overDict, running, executablePath):
    ...
    if len(args) == 1 or (
        ...
        return call_websocket_method_template_and_stop(
            "keys_list", tpl, include_private=False
        )
    ...
    if len(args) >= 3 and args[1] in [
        ...
        return call_websocket_method_template_and_stop(
            "key_create", tpl, key_alias=key_id, key_size=key_sz
        )
        ...
        def _on_key(key_json):
            ...
            if len(args) >= 4 and args[1] in [
                ...
                if not bpio.WriteTextFile(filenameto, TextToSave):
                    ...
                    return 1
                print_text(
                    'private key "%s" was stored in "%s"'
                    % (key_json["result"]["key_id"], filenameto)
                )
            else:
                from lib import misc
                ...
                print_text(
                    'key "%s" was sent to clipboard, you can use Ctrl+V to paste your private key where you want'
                    % key_json["result"]["key_id"]
                )
            del TextToSave
            if key_json["result"]["alias"] == "master":
                print_text(
                    'WARNING! keep your "master" key in safe place, do not publish it!\n'
                )
            reactor.stop()  # @UndefinedVariable
            return
    ...
    if len(args) >= 2 and args[1] in [
        ...
        key_id = "master" if len(args) < 3 else args[2]
        return call_websocket_method_template_and_stop(
            "key_get", tpl, key_id=key_id, include_private=True
        )
    ...
    if len(args) >= 4 and args[1] in [
        ...
        tpl = jsontemplate.Template(templ.TPL_RAW)
        return call_websocket_method_template_and_stop(
            "key_share", tpl, key_id=args[2], idurl=args[3]
        )
...
def cmd_file(opts, args, overDict, executablePath):
    if args[0] in [
        ...
        if len(args) > 2 and args[1] in [
            ...
            tpl = jsontemplate.Template(templ.TPL_RAW)
            return call_websocket_method_template_and_stop(
                "file_create", tpl, remote_path=args[2], as_folder=True
            )
        return 2
    ...
    if len(args) < 2 or args[1] in [
        ...
        return call_websocket_method_template_and_stop(
            "files_list", tpl, remote_path=remote_path
        )
    ...
    if len(args) > 2 and args[1] in [
        ...
        tpl = jsontemplate.Template(templ.TPL_RAW)
        return call_websocket_method_template_and_stop(
            "file_create", tpl, remote_path=args[2], as_folder=False
        )
    ...
    if len(args) > 2 and args[1] in [
        ...
        return call_websocket_method_template_and_stop(
            "file_download_start", tpl, remote_path=args[2], destination_path=local_path
        )
    ...
    if len(args) > 2 and args[1] in [
        ...
        tpl = jsontemplate.Template(templ.TPL_RAW)
        return call_websocket_method_template_and_stop(
            "file_delete", tpl, remote_path=args[2]
        )
    ...
    if len(args) >= 3 and args[1] in [
        ...
        if len(args) > 3:
            if args[2] in [
                ...
                return call_websocket_method_template_and_stop(
                    "file_download_stop", tpl, remote_path=args[3]
                )
            elif args[2] in [
                "upload",
                "up",
                ...
                return call_websocket_method_template_and_stop(
                    "file_upload_stop", tpl, remote_path=args[3]
                )
            return 2
        return call_websocket_method_template_and_stop(
            "file_upload_stop", tpl, remote_path=args[2]
        )
...
def cmd_set_request(opts, args, overDict):
    ...
    if name in [
        ...
        return call_websocket_method_transform_template_and_stop(
            "configs_list", tpl, _limit_length, sort=sort
        )
    path = "" if len(args) < 2 else args[1]
    path = option_name_to_path(name, path)
    if len(args) == 2:
    ...
    tpl = jsontemplate.Template(templ.TPL_OPTION_MODIFIED)
    return call_websocket_method_template_and_stop(
        "config_set", tpl, key=path, value=value
    )
...
def cmd_suppliers(opts, args, overDict):
    if len(args) < 2 or args[1] in ["list", "ls"]:
        ...
        tpl = jsontemplate.Template(templ.TPL_RAW)
        return call_websocket_method_template_and_stop(
            "supplier_change", tpl, position=args[2]
        )
        ...
        tpl = jsontemplate.Template(templ.TPL_RAW)
        return call_websocket_method_template_and_stop(
            "supplier_change", tpl, position=args[2], new_supplier_id=args[3]
        )
...
def cmd_customers(opts, args, overDict):
    if len(args) < 2 or args[1] in ["list", "ls"]:
        ...
        tpl = jsontemplate.Template(templ.TPL_RAW)
        return call_websocket_method_template_and_stop(
            "customer_reject", tpl, customer_id=args[2]
        )
...
def cmd_storage(opts, args, overDict):
    if len(args) < 2:
        ...
        def _got_local(result3, result2, result1):
            result = {
                "status": "OK",
                "execution": float(result1["execution"])
                + float(result2["execution"])
                + float(result3["execution"]),
                "result": [
                    {
...
def cmd_services(opts, args, overDict):
    ...
    if len(args) >= 3 and args[1] in [
        ...
        tpl = jsontemplate.Template(templ.TPL_RAW)
        return call_websocket_method_template_and_stop(
            "service_start", tpl, service_name=args[2]
        )
    ...
    if len(args) >= 3 and args[1] in [
        ...
        tpl = jsontemplate.Template(templ.TPL_RAW)
        return call_websocket_method_template_and_stop(
            "service_stop", tpl, service_name=args[2]
        )

    if len(args) >= 2:
        tpl = jsontemplate.Template(templ.TPL_SERVICE_INFO)
        return call_websocket_method_template_and_stop(
            "service_info", tpl, service_name=args[1]
        )
...
def cmd_message(opts, args, overDict):
    ...
    if len(args) >= 4 and args[1] in [
        ...
        tpl = jsontemplate.Template(templ.TPL_RAW)
        return call_websocket_method_template_and_stop(
            "message_send", tpl, recipient_id=args[2], data=args[3]
        )
    ...
    if len(args) < 2 or args[1] in [
        ...
        def _consume(x=None):
            ...
            d = call_websocket_method(
                "message_receive", consumer_callback_id="terminal_chat"
            )
...
def cmd_friend(opts, args, overDict):
    ...
    if len(args) < 2:
        ...
        return call_websocket_method_template_and_stop(
            "user_search", tpl_lookup, nickname=strng.text_type(args[2])
        )
    elif len(args) > 2 and args[1] in [
        "add",
        "append",
        ...
        if inp.startswith("http://"):
            return call_websocket_method_template_and_stop(
                "friend_add", tpl_add, trusted_user_id=inp
            )
        ...
        def _lookup(result):
            try:
                if result["result"] == "exist":
                    ...
                    d = call_websocket_method(
                        "friend_add", trusted_user_id=result["idurl"]
                    )
...
def cmd_dhtseed(opts, args, overDict):
    ...
    if len(args) > 1 and args[1] in [
        ...
        if len(appList) > 0:
            ...
            return 0
        print_text(
            "starting Distributed Hash Table seed node and detach main BitDust process"
        )
...
def run(opts, args, pars=None, overDict=None, executablePath=None):
    ...
    if not cmd or cmd == "start" or cmd == "go" or cmd == "run":
        ...
        if len(appList) > 0:
            print_text(
                "BitDust already started, found another process: %s" % str(appList)
            )
            return 0
    ...
    # ---restart---
        ...
        if cmd == "restart":
            ui = True
        print_text(
            'found main BitDust process: %s, executing "api.process_stop()" via WebSocket ... '
            % str(appList)
        )
        ...
        def failed(x):
            print_text(
                "soft restart failed, trying to kill running process and force the restart"
            )
            try:
                kill()
            except:
        ...
        try:
            ...
            call_websocket_method("process_restart", websocket_timeout=5).addCallbacks(
                done, failed
            )
            reactor.run()  # @UndefinedVariable
        except:
    ...
    # ---show---
        ...
        if len(appList_bpgui) > 0:
            if len(appList) == 0:
                ...
                print_text(
                    "BitDust GUI already opened, found another process: %s" % str(appList)
                )
                return 0
        if len(appList) == 0:
            from lib import misc
    ...
    # ---stop---
        ...
        if appList:
            ...
            try:
                ...
                call_websocket_method("process_stop", websocket_timeout=5).addBoth(
                    wait_then_kill
                )
                reactor.run()  # @UndefinedVariable
                return 0
            ...
            if appListAllChilds:
                print_text(
                    'found child BitDust processes: %s, perform "kill process" action ... '
                    % str(appList),
                    "",
                )
                ret = kill()
