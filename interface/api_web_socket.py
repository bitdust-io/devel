...
from lib import serialization, txws
from logs import lg
...
def init(port=None):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "api_web_socket.init  _WebSocketListener=%r with %d methods"
...
def shutdown():
    ...
    if _WebSocketListener:
        if _Debug:
            lg.out(
                _DebugLevel,
                "api_web_socket.shutdown calling _WebSocketListener.stopListening()",
            )
...
class BitDustWebSocketProtocol(Protocol):
    ...
    def dataReceived(self, data):
        try:
            json_data = serialization.BytesToDict(
                data, keys_to_text=True, values_to_text=True, encoding="utf-8"
            )
        except:
            lg.exc()
            return
        if _Debug:
            lg.dbg(
                _DebugLevel,
                "received %d bytes from web socket: %r" % (len(data), json_data),
            )
...
def do_process_incoming_message(json_data):
    ...
    if command == "api_call":
        ...
        if _APILogFileEnabled:
            lg.out(
                0,
                "*** %s  WS IN  %s(%r)" % (call_id, method, kwargs),
                log_name="api",
                showtime=True,
            )
