...
class DataSender(automat.Automat):
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---SENDING---
            ...
            if event == "restart":
                ...
                event == "timer-1sec"
                or event == "block-acked"
                or event == "block-failed"
                or event == "new-data"
            ) and self.isQueueEmpty(*args, **kwargs):
                self.state = "SCAN_BLOCKS"
                self.doScanAndQueue(*args, **kwargs)
        # ---AT_STARTUP---
    ...
    def doScanAndQueue(self, *args, **kwargs):
        ...
        if _ShutdownFlag:
            ...
            return
        from storage import backup_fs, backup_matrix
                    ...
                    for supplierNum in packetsBySupplier.keys():
                        ...
                        for packetID in packetsBySupplier[supplierNum]:
                            ...
                            if io_throttle.HasPacketInSendQueue(supplier_idurl, packetID):
                                ...
                                continue
                            latest_progress = self.statistic.get(supplier_idurl, {}).get(
                                "latest", ""
                            )
                            ...
                            if io_throttle.QueueSendFile(
                                ...
                                supplier_idurl,
                                my_id.getIDURL(),
                                lambda packet, ownerID, packetID: self._packetAcked(
                                    packet, ownerID, packetID, itemInfo
                                ),
                                lambda remoteID, packetID, why: self._packetFailed(
    ...
    def doRemoveUnusedFiles(self, *args, **kwargs):
        ...
        if settings.getGeneralWaitSuppliers() is True:
            ...
            if len(online_status.listOfflineSuppliers()) > 0 or (
                ...
                return
        count = 0
        from storage import backup_matrix, backup_rebuilder, restore_monitor
        ...
        for backupID in misc.sorted_backup_ids(list(backup_matrix.local_files().keys())):
            ...
            if backup_rebuilder.IsBackupNeedsWork(backupID):
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "        %s : SKIP, because needs rebuilding" % backupID,
                    )
                continue
            ...
            if backupID not in backup_matrix.remote_files():
                ...
                dirpath = os.path.join(
                    settings.getLocalBackupsDir(), customer, pathID, version
                )
            ...
            packets = backup_matrix.ScanBlocksToRemove(
                backupID, check_all_suppliers=settings.getGeneralWaitSuppliers()
            )
    ...
    def _packetFailed(self, remoteID, packetID, why, itemInfo):
        ...
        backupID, blockNum, supplierNum, dataORparity = packetid.BidBnSnDp(packetID)
        backup_matrix.RemoteFileReport(
            backupID, blockNum, supplierNum, dataORparity, False, itemInfo
        )
        ...
        self.statistic[remoteID]["latest"] = self.statistic[remoteID]["latest"][
            -STAT_KEEP_LATEST_RESULTS_COUNT:
        ]
