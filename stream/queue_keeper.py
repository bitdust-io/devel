    ...
    if customer_idurl not in list(queue_keepers().keys()):
        ...
        if event:
            ...
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "queue_keeper.check_create instance for customer %r was not found, made a new instance"
                    % customer_idurl,
                )
...
def close(customer_idurl):
    ...
    if not qk:
        lg.warn(
            "instance of queue_keeper() not found for given customer %r" % customer_idurl
        )
        return False
    qk.event("shutdown")
    del qk
...
class QueueKeeper(automat.Automat):
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---COOPERATE?---
            ...
            if event == "shutdown":
                ...
                or (event == "rejected" and self.InSync)
                or event == "accepted"
            ):
                ...
                (event == "rejected" and not self.InSync)
                or event == "failed"
                or event == "dht-mismatch"
                or event == "cooperation-mismatch"
            ):
                self.state = "DISCONNECTED"
    ...
    def doReadState(self, *args, **kwargs):
        ...
        """
        json_value = (
            read_state(customer_id=self.customer_id, broker_id=self.broker_id) or {}
        )
        self.cooperated_brokers = {
            int(k): id_url.field(v)
            for k, v in (json_value.get("cooperated_brokers") or {}).items()
        }
        try:
    ...
    def doEraseState(self, *args, **kwargs):
        ...
        """
        write_state(
            customer_id=self.customer_id, broker_id=self.broker_id, json_value=None
        )
        self.cooperated_brokers = {}
        self.known_position = -1
        self.known_streams = {}
    ...
    def doPullRequests(self, *args, **kwargs):
        ...
        if self.pending_connect_requests:
            ...
            if _Debug:
                lg.args(
                    _DebugLevel, more_pending=len(self.pending_connect_requests), **req
                )
    ...
    def doRememberCooperation(self, event, *args, **kwargs):
        ...
        if _Debug:
            lg.args(
                _DebugLevel,
                e=event,
                cooperated=kwargs.get("cooperated_brokers"),
                af_path=archive_folder_path,
            )
    ...
    def doRememberOwnPosition(self, *args, **kwargs):
        ...
        if (
            self.current_connect_request
            and self.current_connect_request.get("request") == "connect"
            and archive_folder_path
        ):
            try:
                self.known_streams[
                    self.current_connect_request["group_key_info"]["alias"]
                ] = archive_folder_path
            except:
    ...
    def doRunBrokerNegotiator(self, *args, **kwargs):
        ...
        if _Debug:
            lg.args(
                _DebugLevel,
                req=self.current_connect_request["request"],
                my_pos=self.known_position,
    ...
    def _do_dht_write(
        self, desired_position, archive_folder_path, revision, retry, event=None, **kwargs
    ):
        ...
        result.addCallback(
            self._on_write_customer_message_broker_success,
            desired_position,
            archive_folder_path,
            revision,
            event,
        ...
        if _Debug:
            result.addErrback(
                ...
                debug=_Debug,
                debug_level=_DebugLevel,
                method="queue_keeper.doDHTWrite",
            )
        result.addErrback(
            self._on_write_customer_message_broker_failed,
            desired_position,
            archive_folder_path,
            revision,
            retry,
    ...
    def _do_dht_push_state(self, event=None, **kwargs):
        if _Debug:
            lg.args(
                _DebugLevel,
                known_position=self.known_position,
                cooperated_brokers=self.cooperated_brokers,
                e=event,
                kw=kwargs,
            )
        ...
        archive_folder_path = None
        if self.current_connect_request and self.current_connect_request.get(
            "archive_folder_path"
        ):
            archive_folder_path = self.current_connect_request["archive_folder_path"]
        prev_revision = self.latest_dht_records.get(desired_position, {}).get(
            "revision", None
        )
    ...
    def _do_verify_cooperated_brokers(self):
        ...
        if _Debug:
            result.addErrback(
                ...
                debug=_Debug,
                debug_level=_DebugLevel,
                method="queue_keeper._do_verify_cooperated_brokers",
            )
    ...
    def _on_write_customer_message_broker_success(
        self,
        nodes,
        desired_broker_position,
        archive_folder_path,
        revision,
        event,
        **kwargs
    ...
    def _on_write_customer_message_broker_failed(
        self,
        err,
        desired_broker_position,
        archive_folder_path,
        ...
        **kwargs
        ...
        if _Debug:
            lg.args(
                _DebugLevel,
                err=err,
                desired_broker_position=desired_broker_position,
                e=event,
                kw=kwargs,
            )
        try:
        ...
        lg.err(
            "failed to write new broker info for %s : %s" % (self.customer_id, err_msg)
        )
