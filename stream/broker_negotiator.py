...
from p2p import lookup, p2p_service_seeker
...
class BrokerNegotiator(automat.Automat):
    ...
    def A(self, event, *args, **kwargs):
        ...
        # ---VERIFY_DEAL---
            ...
            if (
                ...
                event == "my-record-busy"
                or event == "my-record-empty"
                or event == "my-record-own"
            ):
                self.state = "CURRENT?"
                self.doRequestCurBroker(event, *args, **kwargs)
            elif (
                event == "my-top-record-busy"
                or event == "my-top-record-empty"
                or event == "my-top-record-own"
            ):
                self.state = "ACCEPT"
                self.doAccept(event, *args, **kwargs)
                self.doDestroyMe(*args, **kwargs)
            elif (
                event == "my-record-busy-replace"
                or event == "my-record-empty-replace"
                or event == "my-record-own-replace"
            ):
                self.state = "ROTATE?"
        ...
        # ---ROTATE?---
            ...
            if (
                event == "broker-rotate-failed"
                or event == "broker-rotate-timeout"
                or event == "broker-rotate-accepted"
            ):
                self.state = "ACCEPT"
                self.doAccept(event, *args, **kwargs)
                self.doDestroyMe(*args, **kwargs)
    ...
    def doInit(self, *args, **kwargs):
        ...
        Action method.
        """
        self.broker_negotiate_ack_timeout = config.conf().getInt(
            "services/message-broker/broker-negotiate-ack-timeout"
        )
        ...
        if not self.dht_brokers.get(self.desired_position):
            ...
            if self.desired_position == 0:
                ...
                if not self.dht_brokers.get(prev_position):
                    ...
                    if id_url.is_the_same(
                        self.dht_brokers[prev_position], self.my_broker_idurl
                    ):
            ...
            return
        if id_url.is_the_same(
            self.dht_brokers[self.desired_position], self.my_broker_idurl
        ):
    ...
    def doVerifyKnownBrokers(self, *args, **kwargs):
        ...
        if _Debug:
            lg.args(
                ...
                my=self.my_position,
                desired=self.desired_position,
                cooperated=self.cooperated_brokers,
            )
        ...
        if self.desired_position != self.my_position:
            ...
            self.automat(
                "request-invalid",
                Exception(
                    "position mismatch, current position is: %d" % self.my_position
                ),
        ...
        if not self.cooperated_brokers.get(self.my_position):
            ...
            lg.err(
                "my current cooperated info is not valid, there is no broker on position %d"
                % self.my_position
            )
            self.automat(
                "request-invalid",
                Exception(
                    "current cooperation is not valid, there is no broker on position %d"
                    % self.my_position
                ),
            )
            return
        if not id_url.is_the_same(
            ...
            lg.err(
                "my current cooperated info is not correct, another broker is taking my position"
            )
            self.automat(
                "request-invalid",
                Exception(
                    "current cooperation is not valid, another broker is taking position %d"
                    % self.my_position
                ),
            )
            return
        if self.my_position > 0:
            ...
            if not self.cooperated_brokers.get(prev_position):
                ...
                lg.err(
                    "my current cooperated info is not valid, there is no broker on previous position %d"
                    % prev_position
                )
                self.automat(
                    "request-invalid",
                    Exception(
                        "current cooperation is not valid, there is no broker on previous position %d"
                        % prev_position
                    ),
                )
                return
        ...
        if self.requestor_known_brokers.get(self.my_position):
            if not id_url.is_the_same(
                self.requestor_known_brokers[self.my_position], self.my_broker_idurl
            ):
                ...
                if not id_url.is_the_same(
                    self.dht_brokers[self.my_position], self.my_broker_idurl
                ):
    ...
    def doHireNewBroker(self, event, *args, **kwargs):
        ...
        exclude_brokers.extend(
            list(id_url.to_bin_list(filter(None, self.requestor_known_brokers.values())))
        )
        preferred_brokers = []
        preferred_brokers_raw = (
            config.conf().getData("services/message-broker/preferred-brokers").strip()
        )
        ...
        result.addCallback(
            self._on_new_broker_hired, target_pos, self.my_position, self.desired_position
        )
        if _Debug:
            result.addErrback(
                lg.errback,
    ...
    def doAccept(self, event, *args, **kwargs):
        ...
        reactor.callLater(
            0, self.result_defer.callback, self.cooperated_brokers
        )  # @UndefinedVariable
    ...
    def doReject(self, event, *args, **kwargs):
        ...
        reactor.callLater(
            0, self.result_defer.errback, Exception(event, args, kwargs)
        )  # @UndefinedVariable
    ...
    def _do_prepare_service_request_params(
        self, possible_broker_idurl, desired_broker_position, known_brokers, event
    ):
        req = {
        ...
        if _Debug:
            lg.args(
                ...
                desired=desired_broker_position,
                known=known_brokers,
            )
        return req

    def _on_cur_broker_connected(
        self, response_info, broker_pos, my_pos, desired_pos, event, *args, **kwargs
    ):
        try:
        ...
        if desired_pos >= 0:
            if id_url.is_the_same(
                cooperated_brokers.get(desired_pos), self.my_broker_idurl
            ):
                self.automat("broker-accepted", cooperated_brokers=cooperated_brokers)
                return
    ...
    def _on_cur_broker_connect_failed(self, err, broker_pos, event, *args, **kwargs):
        if _Debug:
            lg.args(
                _DebugLevel, err=err, pos=broker_pos, e=event, args=args, kwargs=kwargs
            )
    ...
    def _on_new_broker_hired(
        self, response_info, broker_pos, my_pos, desired_pos, *args, **kwargs
    ):
        try:
            # skip leading "accepted:" marker
            cooperated_brokers = jsn.loads(strng.to_text(response_info[0].Payload)[9:])
    ...
    def _on_rotate_broker_connected(
        self, response_info, broker_pos, event, *args, **kwargs
    ):
        try:
            # skip leading "accepted:" marker
            cooperated_brokers = jsn.loads(strng.to_text(response_info[0].Payload)[9:])
