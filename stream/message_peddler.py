...
from lib import jsn, packetid, serialization, strng, utime
from logs import lg
from main import config, events, settings
from p2p import p2p_service, propagate
from storage import archive_writer
from stream import message, p2p_queue, queue_keeper
...
def register_stream(queue_id):
    ...
    if _Debug:
        lg.args(
            ...
            customer_streams=len(customers().get(customer_idurl, [])),
        )
    return True
    ...
    for json_message in json_messages:
        ...
        if msg_type == "queue_message":
            if queue_id not in streams():
                ...
                if not group_creator_idurl.is_latest():
                    lg.warn(
                        "group creator idurl was rotated, consumer must refresh own identity cache: %r ~ %r"
                        % (
                    ...
                    if not known_ident:
                        lg.err(
                            "unknown group creator identity: %r"
                            % group_creator_idurl.to_bin()
                        )
                        p2p_service.SendFailNoRequest(
                            from_idurl, packet_id, "unknown group creator identity"
                        )
                        continue
                    p2p_service.SendFailNoRequest(
                        from_idurl,
                        packet_id,
                        "identity:%s" % known_ident.serialize(as_text=True),
                    )
                    continue
                lg.warn(
                    "skipped incoming queue_message, queue %r is not registered"
                    % queue_id
                )
                p2p_service.SendFailNoRequest(
                    from_idurl, packet_id, "queue ID not registered"
                )
                continue
            if not streams()[queue_id]["active"]:
                lg.warn(
                    "skipped incoming queue_message, queue %r is not active" % queue_id
                )
                p2p_service.SendFailNoRequest(
                    from_idurl, packet_id, "queue is not active"
                )
                continue
        my_queue_id = queue_id
        if msg_type == "queue_message_replica":
            queue_alias, owner_id, _ = global_id.SplitGlobalQueueID(queue_id)
            my_queue_id = global_id.MakeGlobalQueueID(
                queue_alias, owner_id, my_id.getID()
            )
            if my_queue_id not in streams():
                lg.warn(
                    "skipped incoming queue_message_replica, queue %r is not registered"
                    % my_queue_id
                )
                p2p_service.SendFailNoRequest(
                    from_idurl, packet_id, "queue ID not registered"
            ...
            if not streams()[my_queue_id]["active"]:
                lg.warn(
                    "skipped incoming queue_message_replica, queue %r is not active"
                    % my_queue_id
                )
                p2p_service.SendFailNoRequest(
                    from_idurl, packet_id, "queue is not active"
        ...
        if msg_action == "consume":
            ...
            if consumer_id not in streams()[queue_id]["consumers"]:
                lg.warn(
                    'skipped incoming "queue-read" request, consumer %r is not registered for queue %r'
                    % (
                        consumer_id,
                        queue_id,
                    )
                ...
                p2p_service.SendFailNoRequest(
                    from_idurl, packet_id, "consumer is not registered"
                )
                continue
            if not streams()[queue_id]["consumers"][consumer_id]["active"]:
                lg.warn(
                    'skipped incoming "queue-read" request, consumer %r is not active in queue %r'
                    % (
                        consumer_id,
                        queue_id,
                    )
                ...
                p2p_service.SendFailNoRequest(
                    from_idurl, packet_id, "consumer is not active"
                )
                continue
            ...
            if _Debug:
                lg.args(
                    ...
                    consumer_last_sequence_id=consumer_last_sequence_id,
                )
            A(
                "queue-read",
                queue_id=queue_id,
        ...
        if msg_action == "produce":
            ...
            if msg_type == "queue_message":
                if producer_id not in streams()[queue_id]["producers"]:
                    lg.warn(
                        "skipped incoming queue_message, producer %r is not registered for queue %r"
                        % (
                            producer_id,
                            queue_id,
                        )
                    ...
                    p2p_service.SendFailNoRequest(
                        from_idurl, packet_id, "producer is not registered"
                    )
                    continue
                if not streams()[queue_id]["producers"][producer_id]["active"]:
                    lg.warn(
                        "skipped incoming queue_message, producer %r is not active in queue %r"
                        % (
                            producer_id,
                            queue_id,
                        )
                    ...
                    p2p_service.SendFailNoRequest(
                        from_idurl, packet_id, "producer is not active"
                    )
                    continue
            if msg_type == "queue_message_replica":
                if producer_id not in streams()[my_queue_id]["producers"]:
                    lg.warn(
                        "skipped incoming queue_message_replica, producer %r is not registered for queue %r"
                        % (
                            producer_id,
                            my_queue_id,
                        )
                ...
                if not streams()[my_queue_id]["producers"][producer_id]["active"]:
                    lg.warn(
                        "skipped incoming queue_message_replica, producer %r is not active in queue %r"
                        % (
                            producer_id,
                            my_queue_id,
                        )
                ...
                do_store_message_replica(
                    from_idurl, packet_id, my_queue_id, producer_id, payload, created
                )
                handled = True
                continue
            try:
                known_brokers = {
                    int(k): id_url.field(v) for k, v in msg_data["brokers"].items()
                }
            except:
                lg.exc()
                continue
...
def do_push_message(
    from_idurl, packet_id, queue_id, producer_id, payload, created, known_brokers
):
    new_sequence_id = increment_sequence_id(queue_id)
    streams()[queue_id]["messages"].append(new_sequence_id)
    queued_json_message = store_message(
        queue_id, new_sequence_id, producer_id, payload, created
    )
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "<<< PUSH <<<    into %r by %r at sequence %d"
            % (
                queue_id,
                producer_id,
                new_sequence_id,
...
def do_store_message_replica(
    from_idurl, packet_id, queue_id, producer_id, payload, created
):
    try:
    ...
    if not queued_json_message:
        lg.err(
            "failed to store message replica %r in %r from %r via broker %r"
            % (
                packet_id,
                queue_id,
                producer_id,
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "<<< REPLICA <<<    into %r by %r at sequence %d via %r"
            % (
                queue_id,
                producer_id,
                new_sequence_id,
...
def on_message_processed(processed_message):
    ...
    if not unregister_delivery(
        ...
        lg.err(
            "failed to unregister message delivery attempt, message_id %r not found at position %d in queue %r"
    ...
    if processed_message.failed_notifications:
        if _Debug:
            lg.out(
                _DebugLevel,
                ">>> FAILED >>>    from %r at sequence %d, failed_consumers=%d"
                % (
                    processed_message.queue_id,
                    sequence_id,
                    len(processed_message.failed_notifications),
                ),
            )
    else:
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                ">>> PULL >>>    from %r at sequence %d with success count %d"
                % (
                    processed_message.queue_id,
                    sequence_id,
                    len(processed_message.success_notifications),
                ),
            )
    return True
...
def on_consumer_notify(message_info):
    ...
    if not qk:
        lg.exc(
            exc_value=Exception(
                "not possible to notify consumer %r because queue keeper for %r do not exist"
                % (
                    consumer_id,
                    group_creator_idurl,
                )
    ...
    ret = message.send_message(
        ...
        recipient_global_id=my_keys.make_key_id(
            alias="master", creator_glob_id=consumer_id
        ),
        packet_id=packet_id,
        message_ack_timeout=config.conf().getInt(
            "services/message-broker/message-ack-timeout"
        ),
        skip_handshake=True,
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            ">>> NOTIFY >>>    from %r by producer %r to consumer %r at sequence %d"
            % (
                queue_id,
                producer_id,
                consumer_id,
...
def set_latest_sequence_id(queue_id, new_sequence_id):
    ...
    if new_sequence_id == current_sequence_id + 1:
        ...
        if current_sequence_id >= 0:
            lg.warn(
                "message sequence_id update is not consistent in %r : %r -> %r"
                % (
                    queue_id,
                    current_sequence_id,
                    new_sequence_id,
...
def store_message(queue_id, sequence_id, producer_id, payload, created, processed=None):
    ...
    if not local_fs.WriteTextFile(message_path, jsn.dumps(stored_json_message)):
        lg.err(
            "failed to store message %d in %r from %r"
            % (
                sequence_id,
                queue_id,
                producer_id,
...
def update_processed_message(queue_id, sequence_id):
    ...
    if not stored_json_message:
        lg.err(
            "failed reading message %d from %r"
            % (
                sequence_id,
                queue_id,
            )
...
def read_messages(queue_id, sequence_id_list=[]):
    ...
    if not sequence_id_list:
        sequence_id_list = sorted(
            [int(sequence_id) for sequence_id in os.listdir(messages_dir)]
        )
    result = []
...
def get_messages_for_consumer(
    queue_id, consumer_id, consumer_last_sequence_id, max_messages_count=100
):
    service_dir = settings.ServiceDir("service_message_broker")
    queues_dir = os.path.join(service_dir, "queues")
    queue_dir = os.path.join(queues_dir, queue_id)
...
def register_delivery(queue_id, sequence_id, message_id):
    if _Debug:
        lg.args(
            _DebugLevel, queue_id=queue_id, sequence_id=sequence_id, message_id=message_id
        )
    service_dir = settings.ServiceDir("service_message_broker")
    queues_dir = os.path.join(service_dir, "queues")
    queue_dir = os.path.join(queues_dir, queue_id)
...
def check_rotate_queues():
    ...
    for queue_id in known_queueus:
        ...
        if id_url.is_cached(this_broker_idurl) and not this_broker_idurl.is_latest():
            latest_queue_id = global_id.MakeGlobalQueueID(
                queue_info["queue_alias"],
                queue_info["owner_id"],
                this_broker_idurl.to_id(),
            )
            if latest_queue_id != queue_id:
                ...
                if not os.path.isfile(latest_queue_path):
                    ...
                    rotated += 1
                    lg.info(
                        "detected and processed queue rotate : %r -> %r"
                        % (
                            queue_id,
                            latest_queue_id,
                        )
...
def save_stream(queue_id):
    ...
    for consumer_id, consumer_info in stream_info["consumers"].items():
        local_fs.WriteTextFile(
            os.path.join(consumers_dir, consumer_id), jsn.dumps(consumer_info)
        )
    for producer_id, producer_info in stream_info["producers"].items():
        local_fs.WriteTextFile(
            os.path.join(producers_dir, producer_id), jsn.dumps(producer_info)
        )
    return True
...
def erase_stream(queue_id):
    ...
    if _Debug:
        lg.args(
            _DebugLevel, queue_id=queue_id, queue_dir=queue_dir, erased_files=erased_files
        )
    return True
...
def start_consumer(queue_id, consumer_id):
    if queue_id not in streams():
        lg.warn(
            "queue % is not active, can not start consumer %r"
            % (
                queue_id,
                consumer_id,
            )
    ...
    if consumer_id not in streams()[queue_id]["consumers"]:
        lg.warn(
            "not able to start consumer %r because it was not added to the queue %r"
            % (
                consumer_id,
                queue_id,
            )
    ...
    lg.info(
        "consumer %s started in the queue %s"
        % (
            consumer_id,
            queue_id,
        )
...
def stop_consumer(queue_id, consumer_id):
    if queue_id not in streams():
        lg.warn(
            "queue % is not active, can not stop consumer %r"
            % (
                queue_id,
                consumer_id,
            )
    ...
    if consumer_id not in streams()[queue_id]["consumers"]:
        lg.warn(
            "not able to stop consumer %r because it was not added to the queue %r"
            % (
                consumer_id,
                queue_id,
            )
    ...
    save_consumer(queue_id, consumer_id)
    lg.info(
        "consumer %s stopped in the queue %s"
        % (
            consumer_id,
            queue_id,
        )
...
def start_producer(queue_id, producer_id):
    if queue_id not in streams():
        lg.warn(
            "queue % is not active, can not start producer %r"
            % (
                queue_id,
                producer_id,
            )
    ...
    if producer_id not in streams()[queue_id]["producers"]:
        lg.warn(
            "not able to start producer %r because it was not added to the queue %r"
            % (
                producer_id,
                queue_id,
            )
    ...
    lg.info(
        "producer %s started in the queue %s"
        % (
            producer_id,
            queue_id,
        )
...
def stop_producer(queue_id, producer_id):
    if queue_id not in streams():
        lg.warn(
            "queue % is not active, can not stop producer %r"
            % (
                queue_id,
                producer_id,
            )
    ...
    if producer_id not in streams()[queue_id]["producers"]:
        lg.warn(
            "not able to stop producer %r because it was not added to the queue %r"
            % (
                producer_id,
                queue_id,
            )
    ...
    save_producer(queue_id, producer_id)
    lg.info(
        "producer %s stopped in the queue %s"
        % (
            producer_id,
            queue_id,
        )
...
def stop_stream(queue_id):
    ...
    if p2p_queue.is_queue_exist(queue_id):
        p2p_queue.close_queue(
            queue_id, remove_empty_consumers=True, remove_empty_producers=True
        )
...
def ping_all_streams():
    target_nodes = list(
        set(list_customers() + list_consumers_producers() + list_known_brokers())
    )
    if _Debug:
...
class MessagePeddler(automat.Automat):
    ...
    def doInit(self, *args, **kwargs):
        ...
        self.archive_chunk_size = config.conf().getInt(
            "services/message-broker/archive-chunk-size"
        )
        self.send_message_ack_timeout = config.conf().getInt(
            "services/message-broker/message-ack-timeout"
        )
    ...
    def doStartKeeperJoinQueue(self, event, *args, **kwargs):
        ...
        if _Debug:
            lg.args(
                ...
                p=position,
                a=archive_folder_path,
            )
        if not my_keys.verify_key_info_signature(group_key_info):
            if _Debug:
                lg.exc(
                    "group key verification failed", exc_value=Exception(group_key_info)
                )
        ...
        if not group_creator_idurl.is_latest():
            ...
            p2p_service.SendFail(
                request_packet, "identity:%s" % known_ident.serialize(as_text=True)
            )
            result_defer.callback(False)
            return
        if my_keys.is_key_registered(group_key_id):
            ...
            if my_keys.get_public_key_raw(group_key_id) != key_object.toPublicString():
                ...
                if not my_keys.register_key(
                    group_key_id, key_object, group_key_info.get("label", "")
                ):
        ...
        dl.addCallback(
            lambda _: self._do_check_create_queue_keeper(
                group_creator_idurl,
                request_packet,
                queue_id,
                consumer_id,
        ...
        if _Debug:
            ...
            dl.addErrback(
                ...
                debug=_Debug,
                debug_level=_DebugLevel,
                method="message_peddler.doStartJoinQueue",
            )
        dl.addErrback(
            self._on_group_creator_idurl_cache_failed, request_packet, result_defer
        )
    ...
    def doLeaveQueueStopKeeper(self, *args, **kwargs):
        ...
        if not my_keys.verify_key_info_signature(group_key_info):
            if _Debug:
                lg.exc(
                    "group key verification failed while stopping consumer or producer",
                    exc_value=Exception(group_key_info),
                )
            p2p_service.SendFail(
                request_packet,
                "group key verification failed while stopping consumer or producer",
        ...
        if consumer_id:
            if not stop_consumer(queue_id, consumer_id):
                lg.err(
                    "failed to stop consumer %r for the queue %r"
                    % (
                        consumer_id,
                        queue_id,
                    )
                ...
                p2p_service.SendFail(
                    request_packet, "failed to stop consumer for the queue"
                )
                result_defer.callback(False)
                return
            if not remove_consumer(queue_id, consumer_id):
                lg.err(
                    "failed to remove consumer %r for the queue %r"
                    % (
                        consumer_id,
                        queue_id,
                    )
                ...
                p2p_service.SendFail(
                    request_packet, "consumer is not registered for the queue"
                )
                result_defer.callback(False)
                return
        if producer_id:
            if not stop_producer(queue_id, producer_id):
                lg.err(
                    "failed to stop producer %r for the queue %r"
                    % (
                        producer_id,
                        queue_id,
                    )
                ...
                p2p_service.SendFail(
                    request_packet, "failed to stop producer for the queue"
                )
                result_defer.callback(False)
                return
            if not remove_producer(queue_id, producer_id):
                lg.err(
                    "failed to remove producer %r for the queue %r"
                    % (
                        producer_id,
                        queue_id,
                    )
                ...
                p2p_service.SendFail(
                    request_packet, "producer is not registered for the queue"
                )
                result_defer.callback(False)
                return
        ...
        if customer_idurl in customers():
            if len(customers()[customer_idurl]) == 0:
                lg.info(
                    "no streams left for %r, clean up queue_keeper()" % customer_idurl
                )
                queue_keeper.close(customer_idurl)
                if my_keys.is_key_registered(group_key_id):
    ...
    def doConsumeMessages(self, *args, **kwargs):
        ...
        if consumer_last_sequence_id > queue_current_sequence_id:
            lg.warn(
                "consumer %r is ahead of queue %r position: %d > %d"
                % (
                    consumer_id,
                    queue_id,
                    consumer_last_sequence_id,
                    queue_current_sequence_id,
                )
            )
        list_messages = []
        if consumer_last_sequence_id < queue_current_sequence_id:
            list_messages = get_messages_for_consumer(
                queue_id, consumer_id, consumer_last_sequence_id
            )
    ...
    def doProcessMessage(self, *args, **kwargs):
        ...
        """
        message_in = args[0]
        self._do_replicate_message(
            message_in, known_brokers=kwargs.get("known_brokers", {})
        )
    ...
    def doVerifyCooperation(self, *args, **kwargs):
        ...
        p2p_service.SendAck(
            request_packet,
            "accepted:%s"
            % jsn.dumps(qk.cooperated_brokers, keys_to_text=True, values_to_text=True),
        )
        result_defer.callback(True)
    ...
    def _do_cache_known_customers(self):
        ...
        if _Debug:
            d.addErrback(
                ...
                debug=_Debug,
                debug_level=_DebugLevel,
                method="message_peddler._do_cache_known_customers",
            )
        ...
        for broker_id in os.listdir(keepers_dir):
            ...
            for customer_id in os.listdir(broker_dir):
                ...
                if not os.path.isfile(keeper_state_file_path):
                    continue
                try:
                    json_value = jsn.loads_text(
                        local_fs.ReadTextFile(keeper_state_file_path)
                    )
                    json_value["state"]
                    int(json_value["position"])
                    len(json_value["cooperated_brokers"])
                    json_value["cooperated_brokers"] = {
                        int(k): id_url.field(v)
                        for k, v in json_value["cooperated_brokers"].items()
                    }
                    json_value["time"]
                except:
                    lg.exc()
                ...
                if id_url.is_not_in(
                    my_id.getIDURL(),
                    json_value["cooperated_brokers"].values(),
                    as_field=False,
                ):
                    lg.warn(
                        "did not found my own idurl in the stored cooperated brokers list"
                    )
                    continue
                if utime.get_sec1970() - json_value["time"] > 60 * 60 * 2:
                    lg.warn(
                        "skip restoring queue_keeper for %s because stored state is too old"
                        % customer_id
                    )
                    continue
                customer_idurl = global_id.glob2idurl(customer_id)
                ...
                qk = queue_keeper.check_create(
                    customer_idurl=customer_idurl, auto_create=True, event="skip-init"
                )
    ...
    def _do_send_past_messages(self, queue_id, consumer_id, list_messages):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                ">>> PAST MSG >>>    from %r to consumer %r with %d messages at sequence %d"
                % (
                    queue_id,
                    consumer_id,
                    len(list_messages),
            ...
            if streams()[queue_id]["active"]:
                for consumer_id in list(streams()[queue_id]["consumers"]):
                    if consumer_id:
                        if not stop_consumer(queue_id, consumer_id):
                            lg.warn(
                                "failed to stop consumer %r in for queue %r"
                                % (
                                    consumer_id,
                                    queue_id,
                                )
                        ...
                        if not remove_consumer(queue_id, consumer_id):
                            lg.warn(
                                "consumer %r is not registered for queue %r"
                                % (
                                    consumer_id,
                                    queue_id,
                                )
                ...
                for producer_id in list(streams()[queue_id]["producers"]):
                    if producer_id:
                        if not stop_producer(queue_id, producer_id):
                            lg.warn(
                                "failed to stop producer %r in for queue %r"
                                % (
                                    producer_id,
                                    queue_id,
                                )
                        ...
                        if not remove_producer(queue_id, producer_id):
                            lg.warn(
                                "producer %r is not registered for queue %r"
                                % (
                                    producer_id,
                                    queue_id,
                                )
    ...
    def _do_check_create_queue_keeper(
        self,
        customer_idurl,
        request_packet,
        queue_id,
        ...
        if _Debug:
            queue_keeper_result.addErrback(
                ...
                debug=_Debug,
                debug_level=_DebugLevel,
                method="message_peddler._do_check_create_queue_keeper",
            )
        queue_keeper_result.addCallback(
    ...
    def _do_replicate_message(self, message_in, known_brokers={}):
        ...
        for other_broker_pos in range(groups.REQUIRED_BROKERS_COUNT):
            ...
            d = message.send_message(
                ...
                recipient_global_id=my_keys.make_key_id(
                    alias="master", creator_idurl=other_broker_idurl
                ),
            ...
            d.addCallback(
                self._on_replicate_message_success, message_in, other_broker_pos
            )
            if _Debug:
                d.addErrback(
                    lg.errback,
                    debug=_Debug,
                    debug_level=_DebugLevel,
                    method="message_peddler._do_replicate_message",
                )
        ...
        if replicate_attempts == 0:
            ...
            if _Debug:
                lg.args(
                    ...
                    replicas=replicate_attempts,
                    known_brokers=known_brokers,
                )

    def _do_build_archive_data(self, queue_id, archive_info):
        list_messages = read_messages(
            queue_id, sequence_id_list=archive_info["sequence_id_list"]
        )
    ...
    def _do_archive_other_messages(self, queue_id):
        ...
        if _Debug:
            lg.args(
                _DebugLevel,
                prepared_for_archive=prepared_for_archive,
        ...
        lg.info(
            "started archive backup with %d messages, archive_id=%s, archive_folder_path=%s"
            % (
                len(archive_snapshot_sequence_id_list),
                archive_id,
                known_archive_folder_path,
            )
        )
    ...
    def _on_queue_keeper_connect_result(
        self,
        result,
        consumer_id,
        producer_id,
        ...
        if _Debug:
            lg.args(
                ...
                producer_id=producer_id,
                last_sequence_id=last_sequence_id,
                request_packet_id=request_packet.PacketID,
            )
        ...
        try:
            ...
            if _Debug:
                lg.args(
                    _DebugLevel,
                    customer=target_customer_idurl,
                    customer_streams=known_customer_streams,
                )
            for current_queue_id in list(p2p_queue.queue().keys()):
                if current_queue_id == target_queue_id:
                    continue
                queue_alias, customer_id, _ = global_id.SplitGlobalQueueID(
                    current_queue_id, split_queue_alias=True
                )
                ...
                if current_queue_id not in streams():
                    raise Exception(
                        "rotated queue %r was not registered" % current_queue_id
                    )
            ...
            try:
                archive_folder_path = queue_keeper.queue_keepers()[
                    target_customer_idurl
                ].known_streams.get(target_queue_alias)
            except:
                lg.exc()
                raise Exception("failed reading archive folder path from queue keeper")
            if last_sequence_id > cur_sequence_id:
                lg.info(
                    "based on request from connected group member going to update last_sequence_id: %d -> %d"
                    % (
                        cur_sequence_id,
                        last_sequence_id,
                    )
        ...
        p2p_service.SendAck(
            request_packet,
            "accepted:%s" % jsn.dumps(p, keys_to_text=True, values_to_text=True),
        )
        result_defer.callback(True)
        return None

    def _on_queue_keeper_connect_failed(
        self,
        err,
        consumer_id,
        producer_id,
        ...
        if isinstance(err, Failure):
            ...
            if evt in [
                ...
                p2p_service.SendFail(
                    request_packet,
                    "mismatch:%s"
                    % jsn.dumps(kwargs, keys_to_text=True, values_to_text=True),
                )
                result_defer.callback(False)
                return None
    ...
    def _on_queue_keeper_restore_result(self, result, customer_idurl):
        ...
        for queue_id in known_queues:
            ...
            if queue_id not in streams():
                ...
                if not id_url.is_cached(customer_idurl):
                    lg.err(
                        "customer %r IDURL still is not cached, not able to load stream %r"
                        % (
                            customer_idurl,
                            queue_id,
                        )
            ...
            for _sequence_id in all_stored_queue_messages:
                ...
                stored_json_message = jsn.loads_text(
                    local_fs.ReadTextFile(
                        os.path.join(messages_dir, strng.to_text(_sequence_id))
                    )
                )
            ...
            for consumer_id in (
                os.listdir(consumers_dir) if os.path.isdir(consumers_dir) else []
            ):
                if consumer_id in streams()[queue_id]["consumers"]:
                    lg.warn(
                        "consumer %r already exist in stream %r"
                        % (
                            consumer_id,
                            queue_id,
                        )
                    ...
                    continue
                consumer_info = jsn.loads_text(
                    local_fs.ReadTextFile(os.path.join(consumers_dir, consumer_id))
                )
                if not consumer_info:
                    lg.err(
                        "failed reading consumer info %r from %r"
                        % (
                            consumer_id,
                            queue_id,
                        )
                ...
                loaded_consumers += 1
            for producer_id in (
                os.listdir(producers_dir) if os.path.isdir(producers_dir) else []
            ):
                if producer_id in streams()[queue_id]["producers"]:
                    lg.warn(
                        "producer %r already exist in stream %r"
                        % (
                            producer_id,
                            queue_id,
                        )
                    ...
                    continue
                producer_info = jsn.loads_text(
                    local_fs.ReadTextFile(os.path.join(producers_dir, producer_id))
                )
                if not producer_info:
                    lg.err(
                        "failed reading producer info %r from %r"
                        % (
                            producer_id,
                            queue_id,
                        )
    ...
    def _on_group_creator_idurl_cache_failed(self, err, request_packet, result_defer):
        ...
        p2p_service.SendFail(
            request_packet, "group creator, consumer or producer idurl cache failed"
        )
        result_defer.callback(False)
        return None
    ...
    def _on_replicate_message_success(self, result, message_in, other_broker_pos):
        if _Debug:
            lg.args(
                ...
                result=result,
                message_in=message_in,
                other_broker_pos=other_broker_pos,
            )
