...
from lib import jsn, misc, packetid, serialization, strng, utime
from logs import lg
...
def process_queues(interested_consumers=None):
    ...
    if _ProcessQueuesTask is None or _ProcessQueuesTask.called:
        ...
        # attenuation
        _ProcessQueuesTask = reactor.callLater(
            _ProcessQueuesDelay, process_queues
        )  # @UndefinedVariable
...
def touch_queues(interested_consumers=None):
    ...
    if time.time() - _ProcessQueuesLastTime < MIN_PROCESS_QUEUES_DELAY:
        return False
    reactor.callLater(
        0, process_queues, interested_consumers=interested_consumers
    )  # @UndefinedVariable
    return True
...
def close_queue(queue_id, remove_empty_consumers=False, remove_empty_producers=False):
    ...
    for producer_id in list(producer().keys()):
        if is_producer_connected(producer_id, queue_id):
            disconnect_producer(
                producer_id, queue_id, remove_empty=remove_empty_producers
            )
    for message_id in list(queue(queue_id).keys()):
        ...
        for consumer_id in list(queue(queue_id)[message_id].notifications.keys()):
            ...
            if msg_obj:
                callback_object = queue(queue_id)[message_id].notifications.get(
                    consumer_id
                )
                if callback_object and not callback_object.called:
                    lg.info(
                        "canceling non-finished notification in the queue %s" % queue_id
                    )
    ...
    for consumer_id in list(consumer().keys()):
        if is_consumer_subscribed(consumer_id, queue_id):
            unsubscribe_consumer(
                consumer_id, queue_id, remove_empty=remove_empty_consumers
            )
    ...
    for message_id, msg_obj in stored_messages.items():
        for consumer_id, callback_object in list(msg_obj.notifications.items()):
            if not callback_object.called:
                callback_object.addCallback(
                    on_notification_succeed, consumer_id, new_queue_id, message_id
                )
                callback_object.addErrback(
                    on_notification_failed, consumer_id, new_queue_id, message_id
                )
...
def subscribe_consumer(consumer_id, queue_id):
    ...
    lg.info(
        "consumer %s subscribed to read queue %s"
        % (
            consumer_id,
            queue_id,
        )
...
def unsubscribe_consumer(consumer_id, queue_id=None, remove_empty=False):
    ...
    lg.info(
        "consumer %s unsubscribed from queue %s"
        % (
            consumer_id,
            queue_id,
        )
...
def connect_producer(producer_id, queue_id):
    ...
    if queue_id not in producer(producer_id).queues:
        ...
        lg.info(
            "producer %s connected to queue %s"
            % (
                producer_id,
                queue_id,
            )
        ...
        lg.warn(
            "producer %s already connected to queue %s"
            % (
                producer_id,
                queue_id,
            )
...
def disconnect_producer(producer_id, queue_id=None, remove_empty=False):
    ...
    lg.info(
        "producer %s disconnected from queue %s"
        % (
            producer_id,
            queue_id,
        )
...
def start_notification(consumer_id, queue_id, message_id):
    ...
    callback_object.addCallback(
        on_notification_succeed, consumer_id, queue_id, message_id
    )
...
def on_notification_succeed(result, consumer_id, queue_id, message_id):
    if _Debug:
        lg.out(
            _DebugLevel,
            "p2p_queue.on_notification_succeed : message %r delivered to consumer %r in %r"
            % (
                message_id,
                consumer_id,
                queue_id,
    ...
    if is_queue_exist(queue_id) and is_consumer_exists(consumer_id):
        ...
        lg.warn(
            "notification %r was not finished for consumer %r in %r"
            % (
                message_id,
                consumer_id,
                queue_id,
...
def on_notification_failed(err, consumer_id, queue_id, message_id):
    if _Debug:
        lg.out(
            _DebugLevel,
            "p2p_queue.on_notification_failed FAILED message %r for consumer %r in %r : %r"
    ...
    if is_queue_exist(queue_id) and is_consumer_exists(consumer_id):
        ...
        lg.warn(
            "failed notification %r was not finished for consumer %r in %r"
            % (
                message_id,
                consumer_id,
                queue_id,
...
def write_message(producer_id, queue_id, data, creation_time=None):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "p2p_queue.write_message  %r added to queue %s with %r"
            % (
...
def pull_message(queue_id, message_id=None):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "p2p_queue.pull_message  %r removed from queue %s"
            % (
                message_id,
                queue_id,
            ),
...
def lookup_pending_message(consumer_id, queue_id):
    ...
    for message_id, message_obj in queue(queue_id).items():
        ...
        if consumer_id not in message_obj.consumers:
            ...
            continue
        if (
            consumer_id in message_obj.success_notifications
            or consumer_id in message_obj.failed_notifications
        ):
...
def push_signed_message(producer_id, queue_id, data, creation_time=None):
    ...
    return write_message(
        producer_id, queue_id, data=signed_data.Serialize(), creation_time=creation_time
    )
...
def on_event_packet_received(newpacket, info, status, error_message):
    global _EventPacketReceivedCallbacks
    try:
        e_json = serialization.BytesToDict(
...
def do_handle_event_packet(newpacket, e_json):
    ...
    if queue_id and producer_id and message_id:
        ...
        if _Debug:
            lg.info(
                "received new event %s from the queue at %s"
                % (
                    event_id,
                    queue_id,
                )
    ...
    if _Debug:
        lg.info(
            "pushing event %s to the queue %s on behalf of producer %s"
...
def do_notify(callback_method, consumer_id, queue_id, message_id):
    ...
    if id_url.is_idurl(callback_method):
        ...
        if isinstance(result, Deferred):
            result.addCallback(
                lambda ok: ret.callback(True) if ok else ret.callback(False)
            )
            if _Debug:
                result.addErrback(
                    lg.errback,
        ...
        for _message in queue(queue_id).values():
            if _message.state == "SENT":
                ...
                if not found_pending_notifications:
                    ...
                    continue
                if len(_message.failed_notifications) + len(
                    _message.success_notifications
                ) >= len(_message.consumers):
...
class QueueMessage(object):
    def __init__(self, producer_id, queue_id, json_data, created=None):
        ...
        if len(self.consumers) == 0:
            if _Debug:
                lg.warn(
                    "message %r from %r in queue %r will have no consumers"
                    % (
                        self.message_id,
                        self.producer_id,
                        self.queue_id,
...
class ProducerInfo(object):
    ...
    def do_push_message(self, evt):
        ...
        for queue_id in self.queues:
            try:
                ...
                lg.warn(
                    "queue_id=%s producer_id=%s: %s" % (queue_id, self.producer_id, exc)
                )
        return True
    ...
    def stop_publisher(self, event_id):
        ...
        if not self.publishers.pop(event_id, None):
            lg.warn(
                "publisher event %s for producer %s was not cleaned correctly"
                % (event_id, self.producer_id)
            )
        if not result:
...
def test():
    ...
    connect_producer(
        "alice@host-one.com", "event-test123&bob@server-second.com&carl@thirdnode.net"
    )
    ...
    subscribe_consumer(
        "bob@server-second.com", "event-test123&bob@server-second.com&carl@thirdnode.net"
    )
    add_consumer("dave@server-4.com")
    add_callback_method("dave@server-4.com", _test_callback_dave)
    subscribe_consumer(
        "dave@server-4.com", "event-test123&bob@server-second.com&carl@thirdnode.net"
    )
