...
from logs import lg
from main import config, events, listeners
...
def is_off(name):
    ...
    if svc is None:
        return False
    return (
        svc.state == "OFF" or svc.state == "NOT_INSTALLED" or svc.state == "DEPENDS_OFF"
    )
...
def is_started(name):
    ...
    return (
        svc.state == "ON"
        or svc.state == "STOPPING"
        or svc.state == "STARTING"
        or svc.state == "INFLUENCE"
    )
...
def start(services_list=[]):
    ...
    if _Debug:
        lg.args(
            ...
            starting=bool(_StartingDeferred),
            stoping=bool(_StopingDeferred),
        )
...
def restart(service_name, wait_timeout=None):
    ...
    def _on_started(start_result, stop_result, dependencies_results):
        if _Debug:
            lg.out(
                _DebugLevel,
                "driver.restart._on_started : %s with %s, dependencies_results=%r"
                % (service_name, start_result, dependencies_results),
            )
        try:
            stop_resp = {
    ...
    def _on_stopped(stop_result, dependencies_results):
        if _Debug:
            lg.out(
                _DebugLevel,
                "driver.restart._on_stopped : %s with %s" % (service_name, stop_result),
            )
    ...
    def _on_timeout(err):
        ...
        if (
            "INFLUENCE" in all_states
            or "STARTING" in all_states
            or "STOPPING" in all_states
        ):
    ...
    def _on_wait_timeout(err):
        if _Debug:
            lg.out(
                _DebugLevel,
                "driver.restart._on_wait_timeout %s : %s"
                % (service_name, err.getErrorMessage()),
            )
...
def get_network_configuration(services_list=[]):
    ...
    if _Debug:
        lg.out(
            _DebugLevel, "driver.get_network_info with %d services" % len(services_list)
        )
    result = {}
    for name in services_list:
        svc = services().get(name, None)
...
def get_attached_dht_layers(services_list=[]):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "driver.get_attached_dht_layers with %d services" % len(services_list),
        )
    result = {}
    for name in services_list:
        svc = services().get(name, None)
...
def on_service_callback(result, service_name):
    if _Debug:
        lg.out(
            _DebugLevel, "driver.on_service_callback %s : [%s]" % (service_name, result)
        )
    svc = services().get(service_name, None)
    if not svc:
        ...
        if len(relative_services) > 0:
            for relative_service in relative_services:
                ...
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "    making attempt to start relative service %r"
                        % relative_service,
                    )
...
def on_service_enabled_disabled(path, newvalue, oldvalue, result):
    ...
    if not path.endswith("/enabled"):
        return
    svc_name = (
        path.replace("services/", "service_").replace("/enabled", "").replace("-", "_")
    )
