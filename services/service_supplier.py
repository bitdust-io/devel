...
class SupplierService(LocalService):
    ...
    def start(self):
        ...
        events.add_subscriber(
            customer_space.on_identity_url_changed, "identity-url-changed"
        )
        events.add_subscriber(
            customer_space.on_customer_accepted, "existing-customer-accepted"
        )
        events.add_subscriber(
            customer_space.on_customer_accepted, "new-customer-accepted"
        )
        events.add_subscriber(
            customer_space.on_customer_terminated, "existing-customer-denied"
        )
        events.add_subscriber(
            customer_space.on_customer_terminated, "existing-customer-terminated"
        )
    ...
    def request(self, json_payload, newpacket, info):
        ...
        if key_id:
            ...
            if target_customer_idurl == customer_idurl:
                lg.warn(
                    "customer %s requesting shared access to own files" % customer_idurl
                )
            ...
            if not id_url.is_the_same(
                data_owner_idurl, target_customer_idurl
            ) and not id_url.is_the_same(data_owner_idurl, customer_idurl):
                ...
                return False
            customer_space.register_customer_key(
                customer_public_key_id, customer_public_key
            )
        ...
        if customer_idurl not in current_customers and customer_idurl.to_bin() in list(
            space_dict.keys()
        ):
            lg.warn("broken space file")
            return p2p_service.SendFail(newpacket, "broken space file")
        if customer_idurl in current_customers and customer_idurl.to_bin() not in list(
            space_dict.keys()
        ):
            ...
            for other_customer_idurl in space_dict.keys():
                if (
                    ...
                    and other_customer_idurl != "free"
                    and id_url.field(other_customer_idurl) == customer_idurl
                ):
                    lg.info(
    ...
    def _on_suppliers_dht_layer_connected(self, ok):
        ...
        if my_id.getIDURL():
            dht_service.set_node_data(
                "idurl", my_id.getIDURL().to_text(), layer_id=dht_records.LAYER_SUPPLIERS
            )
        return ok
