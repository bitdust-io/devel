...
class NetworkService(LocalService):
    ...
    def start(self):
        ...
        events.add_subscriber(
            self._on_my_identity_rotate_complete, "my-identity-rotate-complete"
        )
    ...
    def stop(self):
        ...
        events.remove_subscriber(
            self._on_my_external_ip_changed, "my-external-ip-changed"
        )
        events.remove_subscriber(
            self._on_my_identity_rotate_complete, "my-identity-rotate-complete"
        )
    ...
    def _do_check_network_interfaces(self):
        ...
        if self.current_network_interfaces is None:
            ...
            lg.info(
                "current network interfaces on START UP: %r"
