...
class AccountantService(LocalService):
    ...
    def start(self):
        from coins import accountant_node, accountants_finder, coins_db
    ...
    def stop(self):
        from coins import accountant_node, coins_db
    ...
    def request(self, json_payload, newpacket, info):
        ...
        if mode != "join" and mode != "write" and mode != "read":
            lg.out(
                8, "service_accountant.request DENIED, wrong mode provided : %s" % mode
            )
    ...
    def _on_accountant_node_switched(self, oldstate, newstate, evt, *args, **kwargs):
        ...
        if newstate == "OFFLINE" and oldstate != "AT_STARTUP":
            ...
            lg.out(
                8,
                "service_broadcasting._on_accountant_node_switched will try to reconnect again after 10 minutes",
            )
