...
class EmployerService(LocalService):
    ...
    def start(self):
        ...
        conf().addConfigNotifier(
            "services/customer/suppliers-number", self._on_suppliers_number_modified
        )
        conf().addConfigNotifier(
            "services/customer/needed-space", self._on_needed_space_modified
        )
    ...
    def _do_cleanup_dht_suppliers(self, use_cache=True):
        ...
        if driver.is_on("service_entangled_dht"):
            ...
            d = dht_relations.read_customer_suppliers(
                my_id.getIDURL(), use_cache=use_cache
            )
