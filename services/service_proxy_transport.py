...
class ProxyTransportService(LocalService):
    ...
    def start(self):
        ...
        conf().addConfigNotifier(
            "services/proxy-transport/enabled", self._on_enabled_disabled
        )
        conf().addConfigNotifier(
            "services/proxy-transport/sending-enabled", self._on_sending_enabled_disabled
        )
        conf().addConfigNotifier(
    ...
    def _check_reset_original_identity(self):
        ...
        from logs import lg
        from main.config import conf
        from userid import id_url, identity, my_id

        orig_ident_xmlsrc = (
            conf().getData("services/proxy-transport/my-original-identity", "").strip()
        )
        current_router_idurl = (
            conf().getString("services/proxy-transport/current-router", "").strip()
        )
    ...
    def _do_join_proxy_routers_dht_layer(self):
        ...
        lg.info(
            "going to join proxy routers DHT layer: %d" % dht_records.LAYER_PROXY_ROUTERS
        )
    ...
    def _on_proxy_routers_dht_layer_connected(self, ok):
        ...
        if self.transport:
            if self.starting_deferred and not self.starting_deferred.called:
                ...
                reactor.callLater(
                    0, self.transport.automat, "start"
                )  # @UndefinedVariable
        return ok
