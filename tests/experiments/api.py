...
class BitDust_API_Methods(unittest.TestCase):
    def test_ping_failed(self):
        def _t(r):
            ...
            self.assertEqual(
                r["errors"][0], "response was not received within 10 seconds"
            )
            return r

        d = cmd_line_json.call_websocket_method(
            "user_ping", user_id="http://p2p-id.ru/atg314.xml", timeout=10
        )
        d.addCallback(_t)
        return d
    ...
    def test_ping_success(self):
        ...
        d = cmd_line_json.call_websocket_method(
            "user_ping", user_id="http://p2p-id.ru/bitdust_j_vps1014.xml", timeout=10
        )
        d.addCallback(_t)
        return d
