...
class TestRaidWorker(TestCase):
    ...
    def _test_make_rebuild_read(
        ...
        def _read_done(cmd, taskdata, result):
            print("_read_done", cmd, taskdata, result)
            final_result = False
            ...
            if final_result:
                ...
                if read_success:
                    reactor.callLater(
                        0.1,
                        test_result.errback,
                        Exception("reconstructed data is not the same as source data"),
                    )  # @UndefinedVariable
                else:
                    reactor.callLater(
                        0.1,
                        test_result.errback,
                        Exception(
                            "reconstructed data is the same as source data, but expect to fail the raid read"
                        ),
                    )  # @UndefinedVariable
            return True
            ...
            if rebuild_one_success:
                if not newData:
                    ...
                    reactor.callLater(
                        ...
                        test_result.errback,
                        Exception(
                            "rebuild expected to succeed but new data was not created"
                        ),
                    )  # @UndefinedVariable
                    return
            else:
                if newData:
                    ...
                    reactor.callLater(
                        0.1,
                        test_result.errback,
                        Exception("rebuild expected to fail but new data was created"),
                    )  # @UndefinedVariable
                    return
        ...
        def _make_done(cmd, taskdata, result):
            ...
            for supplier_position in range(dead_suppliers):
                os.system(
                    "rm -rf '/tmp/raidtest/master$alice@somehost.com/0/F12345678/5-%d-Data'"
                    % supplier_position
                )
                os.system(
                    "rm -rf '/tmp/raidtest/master$alice@somehost.com/0/F12345678/5-%d-Parity'"
                    % supplier_position
                )
    ...
    def test_task_cancel(self):
        ...
        os.system("rm -rf /tmp/raidtest")
        os.system("mkdir -p '/tmp/raidtest/master$alice@somehost.com/0/F12345678'")
        open("/tmp/source1.txt", "w").write(
            base64.b64encode(os.urandom(1000000)).decode()
        )
        reactor.callWhenRunning(raid_worker.A, "init")  # @UndefinedVariable
        ...
        def _task_failed(c, t, r):
            ...
            if final_result:
                ...
                reactor.callLater(
                    0.1,
                    test_result.errback,
                    Exception("task expected to fail, but positive result was returned"),
                )  # @UndefinedVariable
        ...
        reactor.callLater(
            0.55, raid_worker.cancel_task, "make", "/tmp/source1.txt"
        )  # @UndefinedVariable
