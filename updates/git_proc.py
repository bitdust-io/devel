...
def sync(callback_func=None, update_method="rebase"):
    ...
    if bpio.portablePath(src_dir_path) != bpio.portablePath(expected_src_dir):
        if _Debug:
            lg.out(
                _DebugLevel,
                "git_proc.sync SKIP, non standard sources location: %r" % src_dir_path,
            )
        return
    ...
    def _fetch_done(response, error, retcode):
        ...
        result = "sync-error"
        if (
            response.count(b"Unpacking")
            or (response.count(b"master") and response.count(b"->"))
            or response.count(b"Updating")
            or response.count(b"Receiving")
            or response.count(b"Counting")
        ):
            result = "new-code"
...
def execute(cmdargs, base_dir=None, process_protocol=None, env=None, callback=None):
    ...
    if _Debug:
        lg.out(
            _DebugLevel, 'git_proc.execute: "%s" in %s' % (" ".join(cmdargs), base_dir)
        )
    ...
    if process_protocol is None:
        process_protocol = GitProcessProtocol(
            callbacks=[
                lambda out, err, ret_code: write2log(
                    "STDOUT:\n%s\nSTDERR:\n%s\n" % (out, err)
                ),
                callback,
            ]
