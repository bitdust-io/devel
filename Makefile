...
smoketestdht:
	@for srv in `$(PYTHON) -c "import dht.known_nodes; s=dht.known_nodes.default_nodes(); print ' '.join(['{}:{}'.format(i[0], i[1]) for i in s])"`; do echo "\n$$srv"; rndudpport=`echo $$RANDOM % 10000 + 10000 | bc`; rm -rf ~/.bitdust/metadata/dhtdb; ~/.bitdust/venv/bin/python dht/dht_service.py ping --udpport=$$rndudpport --seeds="$$srv"; done
