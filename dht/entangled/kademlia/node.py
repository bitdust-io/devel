...
class Node(object):
    ...
    def listenUDP(self):
        ...
        )  # IGNORE:E1101   @UndefinedVariable
        ...
        def executeStoreRPCs(nodes):
            ...
            if len(nodes) >= constants.k:
                ...
                if self._routingTable.distance(
                    key, self.id
                ) < self._routingTable.distance(key, nodes[-1].id):
                    nodes.pop()
                    try:
                        ok = self.store(
                            key,
            ...
            if isinstance(result, dict):
                if key in result:
                    ...
                    if "closestNodeNoValue" in result:
                        ...
                        if _Debug:
                            print(
                                "[DHT NODE]    republish %s to closest node with %d expire seconds"
                                % (key, expireSeconds)
                            )
                        contact.store(
                            key,
                    ...
                    if refresh_revision:
                        ...
                        for v in result["values"]:
                            if v[1] < latest_revision:
                                ...
                                if _Debug:
                                    print(
                                        "[DHT NODE]    will refresh revision %d on %r"
                                        % (latest_revision, _contact)
                                    )
                                d = _contact.store(
                                    key,
                                    result[key],
                    ...
                    if key in self._dataStore:
                        ...
                        if len(result["activeContacts"]) > 0:
                            ...
                            if _Debug:
                                print(
                                    "[DHT NODE]    refresh %s : %r with %d to %r"
                                    % (key, item["value"], expireSeconds, contact)
                                )
                            contact.store(
                                key, item["value"], None, 0, expireSeconds
                            ).addErrback(storeFailed)
                        outerDf.callback(
                            {
                                "key": item["value"],
                ...
                if key in self._dataStore:
                    ...
                    if len(result) > 0:
                        ...
                        if _Debug:
                            print(
                                "[DHT NODE]    refresh %s on closest node : %r with %d to %r"
                                % (key, item["value"], expireSeconds, contact)
                            )
                        contact.store(
                            key, item["value"], None, 0, expireSeconds
                    ...
                    if _Debug:
                        print(
                            "[DHT NODE]    key %s found in my local data store : %r"
                            % (key, ret)
                        )
    ...
    def findNode(self, key, **kwargs):
        ...
        for contact in contacts:
            contactTriples.append(
                (contact.id, encoding.to_text(contact.address), contact.port)
            )
    ...
    def findValue(self, key, **kwargs):
        ...
        if key in self._dataStore:
            ...
            if _Debug:
                print(
                    "[DHT NODE]  SINGLE    found key in local dataStore %r"
                    % self._dataStore[key]
                )
            return {
    ...
    def _iterativeFind(self, key, startupShortlist=None, rpc="findNode", deep=False):
        ...
        if startupShortlist is None:
            ...
            if len(shortlist) == 0:
                if _Debug:
                    print(
                        "[DHT NODE]  SINGLE  This node does not know of any other nodes !!!!!"
                    )
        ...
        def extendShortlist(responseTuple):
            ...
            if _Debug:
                print(
                    "[DHT NODE]   SINGLE   extendShortlist",
                    (responseMsg.nodeID, type(responseMsg.nodeID)),
                )
            ...
            if responseMsg.nodeID in shortlist:
                ...
                aContact = Contact(
                    encoding.to_text(responseMsg.nodeID),
                    originAddress[0],
                    originAddress[1],
                    self._protocol,
                )
        ...
        def cancelActiveProbe(contactID):
            activeProbes.pop()
            if len(activeProbes) <= int(constants.alpha / 2.0) and len(
                pendingIterationCalls
            ):
                # Force the iteration
        ...
        def searchIteration():
            ...
            activeContacts.sort(
                key=lambda cont: self._routingTable.distance(cont.id, key)
            )
            if _Debug:
            ...
            if key in findValueResult and not deep:
                if _Debug:
                    print(
                        "[DHT NODE] SINGLE ++++++++++++++ DONE (findValue found) +++++++++++++++\n\n"
                    )
                findValueResult["activeContacts"] = activeContacts
                outerDf.callback(findValueResult)
                return
            if len(activeContacts) and findValue == False:
                if (len(activeContacts) >= constants.k) or (
                    activeContacts[0] == prevClosestNode[0]
                    and len(activeProbes) == slowNodeCount[0]
                ):
                    ...
                    if len(activeContacts) >= constants.k:
                        if _Debug:
                            print(
                                "[DHT NODE] SINGLE ++++++++++++++ DONE (test for k active contacts) +++++++++++++++\n\n"
                            )
                    else:
                        if _Debug:
                            print(
                                "[DHT NODE] SINGLE ++++++++++++++ DONE (test for closest node) +++++++++++++++\n\n"
                            )
                    if findValue:
                        findValueResult["activeContacts"] = activeContacts
                        outerDf.callback(findValueResult)
            ...
            if len(activeProbes) > slowNodeCount[0] or (
                len(shortlist) < constants.k
                and len(activeContacts) < len(shortlist)
                and len(activeProbes) > 0
            ):
                if _Debug:
                    print(
                        "[DHT NODE] SINGLE ----------- scheduling next call -------------"
                    )
                ...
                call = twisted.internet.reactor.callLater(
                    constants.iterativeLookupDelay, searchIteration
                )  # IGNORE:E1101  @UndefinedVariable
            ...
            # Check for a quick contact response that made an update to the shortList
                ...
                if _Debug:
                    print(
                        "[DHT NODE] SINGLE ++++++++++++++ DONE (logically) +++++++++++++\n\n"
                    )
...
class MultiLayerNode(Node):
    ...
    def createLayer(self, layer_id, dataStore, nodeID=None, routingTable=None):
        if (
            layer_id in self.layers
            or layer_id in self._dataStores
            or layer_id in self._routingTables
        ):
            if _Debug:
        ...
        if self.nodeStateKey in self._dataStores[layer_id]:
            ...
            if layer_id not in self._routingTables:
                self._routingTables[
                    layer_id
                ] = routingTable or routingtable.TreeRoutingTable(
                    self.layers[layer_id], layerID=layer_id
                )
            for contactTriple in state["closestNodes"]:
                contact = LayeredContact(
                    encoding.to_text(contactTriple[0]),
                    contactTriple[1],
                    contactTriple[2],
                    self._protocol,
        ...
        if layer_id not in self._routingTables:
            self._routingTables[layer_id] = routingTable or routingtable.TreeRoutingTable(
                self.layers[layer_id], layerID=layer_id
            )
    ...
    def destroyLayer(self, layer_id):
        if layer_id == 0:
            return False
        if (
            layer_id not in self.layers
            and layer_id not in self._dataStores
            and layer_id not in self._routingTables
        ):
            if _Debug:
    ...
    def joinNetwork(
        self, knownNodeAddresses=None, layerID=0, attach=False, parallel_calls=None
    ):
        if knownNodeAddresses is not None:
            bootstrapContacts = []
            for address, port in knownNodeAddresses:
                contact = LayeredContact(
                    self._generateID(), address, port, self._protocol, layerID=layerID
                )
                bootstrapContacts.append(contact)
        else:
            bootstrapContacts = None
        if _Debug:
            print(
                "[DHT NODE]    joinNetwork bootstrapContacts=%r  layerID=%r  attach=%r"
                % (bootstrapContacts, layerID, attach)
            )
        ...
        self.refreshers[
            layerID
        ] = twisted.internet.reactor.callLater(  # IGNORE:E1101  @UndefinedVariable
    ...
    def iterativeStore(
        ...
        **kwargs
        ...
        def storeRPCsCollected(store_results, store_nodes):
            if _Debug:
                print(
                    "[DHT NODE]    iterativeStore.storeRPCsCollected",
                    store_results,
                    store_nodes,
                )
            ret.callback(
        ...
        def executeStoreRPCs(nodes):
            ...
            if len(nodes) >= constants.k:
                # If this node itself is closer to the key than the last (furthest) node in the list,
                # we should store the value at ourselves as well
                if self._routingTables[layerID].distance(
                    key, self.layers[layerID]
                ) < self._routingTables[layerID].distance(key, nodes[-1].id):
                    nodes.pop()
                    try:
                        ok = self.store(
                            key,
    ...
    def iterativeDelete(self, key, layerID=0, parallel_calls=None):
        ...
        df = self._iterativeFind(
            key, rpc="delete", layerID=layerID, parallel_calls=parallel_calls
        )
        return df

    def iterativeFindValue(
        self, key, rpc="findValue", refresh_revision=False, layerID=0, parallel_calls=None
    ):
        outerDf = defer.Deferred()
            ...
            if isinstance(result, dict):
                ...
                if key in self._dataStores[layerID]:
                    ...
                    if len(result) > 0:
                        ...
                        contact.store(
                            key, item["value"], None, 0, expireSeconds, layerID=layerID
                        ).addErrback(storeFailed)
                    ret = {
                        "key": item["value"],
                        "values": [
        ...
        # Execute the search
        df = self._iterativeFind(
            key, rpc=rpc, layerID=layerID, parallel_calls=parallel_calls
        )
        df.addCallback(checkResult)
        df.addErrback(lookupFailed)
        return outerDf
    ...
    def findContact(self, contactID, layerID=0, parallel_calls=None):
        try:
            ...
            df = self.iterativeFindNode(
                contactID, layerID=layerID, parallel_calls=parallel_calls
            )
            df.addCallback(parseResults)
        return df
        ...
        **kwargs
        ...
        if originalPublisherID is None:
            if rpcSenderID is not None:
                ...
                raise TypeError(
                    "No publisher specified, and RPC caller ID not available. Data requires an original publisher."
                )
        now = int(time.time())
        originallyPublished = now - age
        self._dataStores[layerID].setItem(
            key,
            value,
            now,
            originallyPublished,
    ...
    def delete(self, key, **kwargs):
        ...
        if _Debug:
            print(
                "[DHT NODE]    rpcmethod.delete %r layerID=%d : %r"
                % (key, layerID, kwargs)
            )
    ...
    def findNode(self, key, **kwargs):
        ...
        if _Debug:
            print(
                "[DHT NODE]    rpcmethod.findNode %r layerID=%d : %r"
                % (key, layerID, kwargs)
            )
        # Get the sender's ID (if any)
        if "_rpcNodeID" in kwargs:
        ...
        if layerID not in self._routingTables:
            ...
            return []
        contacts = self._routingTables[layerID].findCloseNodes(
            key, constants.k, rpcSenderID
        )
        contactTriples = []
        for contact in contacts:
            contactTriples.append(
                (contact.id, encoding.to_text(contact.address), contact.port, layerID)
            )
        if _Debug:
    ...
    def findValue(self, key, **kwargs):
        ...
        if _Debug:
            print(
                "[DHT NODE]    rpcmethod.findValue %r layerID=%r : %r"
                % (key, layerID, kwargs)
            )
        if key in self._dataStores[layerID]:
            exp = None
            ...
            if expireSecondsCall:
                exp = expireSecondsCall(key)
            originalPublishTimeCall = getattr(
                self._dataStores[layerID], "originalPublishTime"
            )
            published = None
            if originalPublishTimeCall:
                published = originalPublishTimeCall(key)
            if _Debug:
                print(
                    "[DHT NODE]        found key in local dataStore %r"
                    % self._dataStores[layerID][key]
                )
            return {
    ...
    def _iterativeFind(
        ...
        shortlist = []
        if startupShortlist is None:
            shortlist = self._routingTables[layerID].findCloseNodes(
                key, parallel_calls or constants.alpha
            )
            ...
            if len(shortlist) == 0:
                if _Debug:
                    print(
                        "[DHT NODE]  layerID=%d   This node doesnt know of any other nodes !!!!!"
                        % layerID
                    )
                # This node doesn't know of any other nodes
                fakeDf = defer.Deferred()
        ...
        def extendShortlist(responseTuple):
            ...
            if (
                responseMsg.nodeID in activeContacts
                or responseMsg.nodeID == self.layers[layerID]
            ):
                if _Debug:
            ...
            if findValue and isinstance(result, dict):
                ...
                if findValue:
                    ...
                    if "closestNodeNoValue" in findValueResult:
                        if self._routingTables[layerID].distance(
                            key, responseMsg.nodeID
                        ) < self._routingTables[layerID].distance(
        ...
        def cancelActiveProbe(contactID):
            activeProbes.pop()
            if len(activeProbes) <= int(
                (parallel_calls or constants.alpha) / 2.0
            ) and len(pendingIterationCalls):
                # Force the iteration
                pendingIterationCalls[0].cancel()
                del pendingIterationCalls[0]
                if _Debug:
        ...
        def searchIteration():
            if not self._routingTables or layerID not in self._routingTables:
                if _Debug:
                    print(
                        "[DHT NODE]    ++++++++++++++ searchIteration INTERRUPTED +++++++++++++++\n\n"
                    )
                return
            slowNodeCount[0] = len(activeProbes)
            # Sort the discovered active nodes from closest to furthest
            activeContacts.sort(
                key=lambda cont: self._routingTables[layerID].distance(cont.id, key)
            )
            if _Debug:
            ...
            if len(activeContacts) and findValue == False:
                if (len(activeContacts) >= constants.k) or (
                    ...
                    if len(activeContacts) >= constants.k:
                        if _Debug:
                            print(
                                "[DHT NODE]    ++++++++++++++ DONE (test for k active contacts) +++++++++++++++\n\n"
                            )
                    else:
                        if _Debug:
                            print(
                                "[DHT NODE]    ++++++++++++++ DONE (test for closest node) +++++++++++++++\n\n"
                            )
                    if findValue:
                        findValueResult["activeContacts"] = activeContacts
                        outerDf.callback(findValueResult)
            ...
            for contact in shortlist:
                if contact.id not in alreadyContacted:
                    ...
                    if _Debug:
                        print(
                            "[DHT NODE] calling RPC method %r with key=%r layerID=%d at %r"
    ...
    def _persistState(self, *args, **kwargs):
        ...
        now = int(time.time())
        self._dataStores[layerID].setItem(
            self.nodeStateKey, json_value, now, now, self.layers[layerID]
        )
        return args
    ...
    def _refreshRoutingTable(self, layerID=0, parallel_calls=None):
        ...
        def searchForNextNodeID(dfResult=None):
            if len(nodeIDs) > 0:
                searchID = nodeIDs.pop()
                df = self.iterativeFindNode(
                    searchID, layerID=layerID, parallel_calls=parallel_calls
                )
                df.addCallback(searchForNextNodeID)
                df.addErrback(searchFailed)
            else:
    ...
    def _republishData(self, *args, **kwargs):
        df = twisted.internet.threads.deferToThread(
            self._threadedRepublishData, *args, **kwargs
        )
        return df
    ...
    def _scheduleNextNodeRefresh(self, *args, **kwargs):
        ...
        self.refreshers[
            kwargs["layerID"]
        ] = twisted.internet.reactor.callLater(  # @UndefinedVariable
            constants.checkRefreshInterval,
            self._refreshNode,
            layerID=kwargs["layerID"],
        )  # @UndefinedVariable
