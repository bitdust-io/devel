...
class KademliaProtocol(protocol.DatagramProtocol):
    ...
    def __init__(
        self, node, msgEncoder=encoding.Bencode(), msgTranslator=msgformat.DefaultFormat()
    ):
        self._node = node
    ...
    def sendRPC(self, contact, method, args, rawResponse=False, **kwargs):
        ...
        if _Debug:
            print(
                "[DHT PROTO] <<< [%s] sendRPC" % time.time(),
                (method, msg.id, contact.address, contact.port, kwargs),
            )
        if self._counter:
            self._counter("sendRPC")
        # Set the RPC timeout timer
        timeoutCall = reactor.callLater(
            constants.rpcTimeout * 3, self._msgTimeout, msg.id
        )  # IGNORE:E1101
    ...
    def dispatch(self, datagram, address):
        if _Debug:
            print(
                "[DHT PROTO]             datagram of %d bytes to dispatch from %r"
                % (len(datagram), address)
            )
        ...
        remoteContact = Contact(
            encoding.to_text(message.nodeID), address[0], address[1], self
        )
        ...
        if isinstance(message, msgtypes.RequestMessage):
            ...
            if message.id in self._sentMessages:
                if _Debug:
                    print(
                        "[DHT PROTO]             RPC Request message received [%s]"
                        % message_request
                    )
            ...
            if message.id in self._sentMessages:
                ...
                if hasattr(df, "_rpcRawResponse"):
                    if _Debug:
                        print(
                            "[DHT PROTO]          sent  rpcRawResponse (%r, %r)"
                            % (message, address)
                        )
                    ...
                    if _Debug:
                        print(
                            "[DHT PROTO]         sent     message response: %r"
                            % message_response
                        )
    ...
    def datagramReceived(self, datagram, address):
        ...
        try:
            ...
            if not header_ok:
                if _Debug:
                    print(
                        "[DHT PROTO]  WARNING, dispatching old-style datagram, remote use is running old version"
                    )
                self.dispatch(datagram, address)
                return

            header = datagram[0:46]
            totalPackets = (ord(encoding.to_text(header[1:2])) << 8) | ord(
                encoding.to_text(header[2:3])
            )
            seqNumber = (ord(encoding.to_text(header[3:4])) << 8) | ord(
                encoding.to_text(header[4:5])
            )
            ...
            if len(self._partialMessages[msgID]) < totalPackets:
                if _Debug:
                    print(
                        "[DHT PROTO]   skip, _partialMessages=%r" % self._partialMessages
                    )
                return
            ...
            if _Debug:
                print(
                    "[DHT PROTO]      finished message of %d pieces: %r"
                    % (totalPackets, keys)
                )
        ...
        try:
            ...
            if len(data) > self.msgSizeLimit:
                ...
                while seqNumber < totalPackets:
                    ...
                    txHeader = (
                        encoding.to_bin(encTotalPackets)
                        + encoding.to_bin(encSeqNumber)
                        + encoding.to_bin(rpcID)
                    )
                    txData = b"\x00" + txHeader + b"\x00" + packetData
                    reactor.callLater(
                        self.maxToSendDelay * seqNumber + self.minToSendDelay,
                        self._write,
                        txData,
                        address,
                    )  # IGNORE:E1101
    ...
    def _write(self, data, address):
        ...
        if _Debug:
            print(
                "[DHT PROTO]                     dht._write %d bytes to %s"
                % (len(data), str(address))
            )
        return True
            ...
            try:
                try:
                    ...
                    result = func(
                        *args,
                        **{
                            "_rpcNodeID": senderContact.id,
                            "layerID": senderContact.layerID,
                        }
                    )
                except TypeError:
                    # import traceback
        ...
        if messageID in self._sentMessages:
            ...
            if messageID in self._partialMessages:
                ...
                if messageID in self._partialMessagesProgress:
                    if len(self._partialMessagesProgress[messageID]) == len(
                ...
                timeoutCall = reactor.callLater(
                    constants.rpcTimeout * 3, self._msgTimeout, messageID
                )  # IGNORE:E1101
            ...
            print(
                "[DHT PROTO]  ERROR: deferred timed out, but is not present in sent messages list!"
            )
...
class KademliaMultiLayerProtocol(KademliaProtocol):
    ...
    def sendRPC(self, contact, method, args, rawResponse=False, layerID=0, **kwargs):
        ...
        if only_ask:
            msg = msgtypes.QuestionMessage(
                self._node.layers[layerID], method, args, layerID=layerID
            )
        else:
    ...
    def dispatch(self, datagram, address):
        if _Debug:
            print(
                "[DHT PROTO]    dispatch datagram of %d bytes to dispatch from %r"
                % (len(datagram), address)
            )
        msgPrimitive = self._encoder.decode(datagram, encoding="utf-8")
        if _Debug:
        ...
        remoteContact = LayeredContact(
            encoding.to_text(message.nodeID),
            address[0],
            address[1],
            self,
            layerID=layerID,
        ...
        if isinstance(message, msgtypes.RequestMessage) or isinstance(
            message, msgtypes.QuestionMessage
        ):
            # This is an RPC method request
            message_request = message.request
            if isinstance(message_request, six.binary_type):
            ...
            if message.id in self._sentMessages:
                if _Debug:
                    print(
                        "[DHT PROTO]    dispatch   RPC Request message received [%s]"
                        % message_request
                    )
                # Cancel timeout timer for this RPC
                df, timeoutCall = self._sentMessages[message.id][1:3]
            ...
            if message.id in self._sentMessages:
                ...
                if hasattr(df, "_rpcRawResponse"):
                    if _Debug:
                        print(
                            "[DHT PROTO]    dispatch   respond with tuple (%r, %r)"
                            % (message, address)
                        )
                    # The RPC requested that the raw response message and originating address be returned; do not interpret it
                    df.callback((message, address))
                    ...
                    if _Debug:
                        print(
                            '[DHT PROTO]    dispatch   respond with error "%s"' % exc_msg
                        )
                    df.errback(remoteException)
                else:
                    # We got a result from the RPC
                    if _Debug:
                        print(
                            "[DHT PROTO]    dispatch  respond with message_response: %r"
                            % message_response
                        )
                    df.callback(message_response)
            else:
    ...
    def _sendResponse(self, contact, rpcID, response):
        ...
        msg = msgtypes.ResponseMessage(
            rpcID, self._node.layers[layerID], response, layerID=layerID
        )
        msgPrimitive = self._translator.toPrimitive(msg)
        encodedMsg = self._encoder.encode(msgPrimitive, encoding="utf-8")
        if _Debug:
