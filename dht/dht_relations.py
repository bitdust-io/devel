...
def read_customer_suppliers(customer_idurl, as_fields=True, use_cache=True):
    ...
    def _do_identity_cache(ret):
        ...
        for _supplier_idurl in ret["suppliers"]:
            if _supplier_idurl:
                ...
                if not id_url.is_cached(_supplier_idurl) or not identitycache.HasFile(
                    _supplier_idurl
                ):
        ...
        if _customer_idurl and (
            not id_url.is_cached(_customer_idurl)
    ...
    def _do_verify(dht_value, customer_idurl_bin):
        ...
        if not dht_value or not isinstance(dht_value, dict):
            ...
            d = dht_records.get_suppliers(
                another_customer_idurl_bin, return_details=True, use_cache=False
            )
    ...
    def _do_save_customer_suppliers(id_cached_result, ret):
        if my_id.getIDURL() != id_url.field(ret["customer_idurl"]):
            contactsdb.set_suppliers(
                ret["suppliers"], customer_idurl=ret["customer_idurl"]
            )
    ...
    #         rotated_idurls.remove(customer_idurl_bin)
    d = dht_records.get_suppliers(
        customer_idurl_bin, return_details=True, use_cache=use_cache
    )
...
def read_customer_message_brokers(
    ...
    def _do_broker_identity_cache(dht_record, position, broker_result):
        ...
        one_broker_task.addErrback(
            _on_borker_identity_cache_failed, position, broker_result
        )
        # if _Debug:
    ...
    def _do_verify(dht_value, position, broker_result):
        ...
        if not dht_value or not isinstance(dht_value, dict):
            if _Debug:
                lg.args(
                    _DebugLevel, c=customer_idurl, p=position, dht_value=type(dht_value)
                )
...
def write_customer_message_broker(
    customer_idurl, broker_idurl, position=0, revision=None
):
    if _Debug:
