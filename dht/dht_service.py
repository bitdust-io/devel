    ...
    def store(self, key, value, originalPublisherID=None, age=0, expireSeconds=KEY_EXPIRE_MAX_SECONDS, **kwargs):
        ...
        if 'store' in self.rpc_callbacks and layerID in self.active_layers:
            ...
            self.rpc_callbacks['store'](
                key=key, value=value, originalPublisherID=originalPublisherID, age=age, expireSeconds=expireSeconds, layerID=layerID, **kwargs
            )
        ...
        try:
            if len(args) == 0:
                ...
                if cmd == 'get':
                    ...
                    set_valid_data(
                        args[1], jsn.loads(args[2]), expire=int(args[3]), rules=jsn.loads(args[4]), layer_id=0 if len(args) < 6 else int(args[5])
                    ).addBoth(_r)
