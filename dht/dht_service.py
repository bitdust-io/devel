...
from dht.entangled.kademlia.datastore import (  # @UnresolvedImport
    SQLiteVersionedJsonDataStore,
)
from dht.entangled.kademlia.node import MultiLayerNode, rpcmethod  # @UnresolvedImport
from dht.entangled.kademlia.protocol import (  # @UnresolvedImport
    KademliaMultiLayerProtocol,
    encoding,
    msgformat,
)
...
def init(udp_port, dht_dir_path=None, open_layers=[]):
    ...
    if _Debug:
        lg.dbg(
            _DebugLevel,
            "dht_dir_path=%r list_layers=%r network_info=%r"
    ...
    for layer_filename in list_layers:
        ...
        try:
            ...
            lg.warn(
                "failed reading DHT records, removing %s and starting clean DB" % dbPath
            )
            lg.exc()
            try:
                os.remove(dbPath)
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.init UDP port is %d, DB file path is %s, my DHT ID is %s"
...
def connect(seed_nodes=[], layer_id=0, attach=False):
    ...
    if joinDeferred:
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service.connect SKIP, already joining layer %d" % layer_id,
            )
    ...
    if node().refreshers.get(layer_id, None) and node().refreshers[layer_id].active():
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service.connect SKIP seems like DHT already active, RESET current refresher task",
            )
        reactor.callLater(0, result.callback, True)  # @UndefinedVariable
        return result
    ...
    if not node().listener:
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service.connect opened a new listener : %r" % node().listener,
            )

    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.connect STARTING with %d known nodes in layer %d:"
            % (
                len(seed_nodes),
                layer_id,
            ),
    ...
    def _on_join_success(live_contacts, resolved_seed_nodes, _layer_id):
        ...
        if isinstance(live_contacts, dict):
            lg.err(
                "Unexpected result from joinNetwork: %s" % pprint.pformat(live_contacts)
            )
        else:
            if ok:
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "dht_service.connect DHT JOIN SUCCESS   layer_id=%d attach=%r"
                        % (_layer_id, attach),
                    )
            else:
                lg.warn(
                    "No live DHT contacts found...  your node is NOT CONNECTED TO DHT NETWORK at layer %d"
                    % _layer_id
                )
        if _Debug:
            lg.out(
                _DebugLevel,
                "for layer %d found DHT nodes: %s" % (_layer_id, live_contacts),
            )
            lg.out(_DebugLevel, "resolved SEED nodes: %r" % resolved_seed_nodes)
            lg.out(
                _DebugLevel,
                "DHT node is active, ID%d=[%s]" % (_layer_id, node().layers[_layer_id]),
            )
    ...
    def _on_hosts_resolved(resolved_seed_nodes, _layer_id):
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service.connect RESOLVED %d live nodes" % (len(resolved_seed_nodes)),
            )
        ...
        if not node().expire_task.running:
            ...
            node().expire_task.start(
                int(KEY_EXPIRE_MIN_SECONDS / 2), now=True
            )  # @UndefinedVariable
...
def open_layer(
    layer_id, seed_nodes=[], dht_dir_path=None, connect_now=False, attach=False
):
    global _MyNode
    if not node():
        result = Deferred()
    ...
    if not layer_id in node().layers:
        ...
        if not node().createLayer(
            layer_id, dataStore=SQLiteVersionedJsonDataStore(dbFile=dbPath)
        ):
    ...
    lg.info(
        "DHT layer %d opened and connecting to the seed nodes: %r"
        % (
            layer_id,
            seed_nodes,
        )
...
def resolve_hosts(nodes_list):
    ...
    for node_tuple in nodes_list:
        ...
        d.addErrback(
            on_host_failed, node_tuple[0], result_list, len(nodes_list), result_defer
        )
    return result_defer
...
def on_success(result, method, key, *args, **kwargs):
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.on_success   %s(%r)    result is %s"
            % (
                method,
                key,
                type(result),
...
def on_error(x, method, key):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.on_error   %s(%s)   returned an ERROR:\n%r"
            % (method, key, errmsg),
        )
    return x
...
def get_value(key, layer_id=0, parallel_calls=None):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.get_value key=[%r] key_hash=%s"
            % (
                key,
                key_hash,
            ),
...
def set_value(
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.set_value key=[%s] key_hash=%s with %d bytes for %d seconds"
            % (
                key,
                key_hash,
                sz_bytes,
...
def delete_key(key, layer_id=0, parallel_calls=None):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.delete_key [%s] key_hash=%s"
            % (
                key,
                key_hash,
            ),
...
def on_read_json_response(response, key, result_defer=None):
    if _Debug:
        lg.out(
            _DebugLevel + 6,
            "dht_service.on_read_json_response [%r] : %s" % (key, type(response)),
        )
    json_value = None
...
def get_json_value(key, layer_id=0, update_cache=True):
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.get_json_value key=[%r] layer_id=%d update_cache=%s"
            % (
                key,
                layer_id,
                update_cache,
...
def set_json_value(
    key, json_data, age=0, expire=KEY_EXPIRE_MAX_SECONDS, collect_results=True, layer_id=0
):
    if not node():
        return fail(Exception("DHT service is off"))
    try:
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.set_json_value key=[%r] layer_id=%d with %d bytes"
            % (key, layer_id, len(repr(value))),
        )
    return set_value(
        key=key,
        value=value,
...
def validate_rules(
    ...
    if not isinstance(value, dict):
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service.validate_rules    key=[%s] not found : %s"
                % (key, type(value)),
            )
        if not result_defer:
        ...
        for field, field_rules in rules.items():
            ...
            for rule in field_rules:
                if "op" not in rule:
                    ...
                    continue
                if rule["op"] == "equal" and rule.get("arg") != strng.to_text(
                    value.get(field)
                ):
                    passed = False
                    errors.append(
...
def validate_before_store(key, value, originalPublisherID, age, expireSeconds, **kwargs):
    ...
    if _Debug:
        lg.out(
            _DebugLevel + 8,
            "dht_service.validate_before_store key=[%s] with %d bytes value"
            % (
                key,
                len(value),
            ),
    ...
    if not new_record_type:
        if _Debug:
            lg.out(
                _DebugLevel,
                '        new json data do not have "type" field present, store operation FAILED',
            )
        raise ValueError(
            'input data do not have "type" field present: %r' % json_new_value
        )
    ...
    try:
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "        current value in DHT is not a json data, will be overwritten, store OK",
            )
        return True
    ...
    if prev_record_type and prev_record_type != new_record_type:
        if _Debug:
            lg.out(
                _DebugLevel,
                "        new json data type did not match to existing record type, store operation FAILED",
            )
        raise ValueError(
            "new json data type do not match to existing record type: %r"
            % json_prev_value
        )
    ...
    if prev_revision >= 0:
        if new_revision < 0:
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "        new json data must have a revision, store operation FAILED",
                )
        ...
        if new_revision < prev_revision:
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "        new json data must increment revision number, store operation FAILED",
                )
            raise ValueError(
                "new json data must increment revision number, current revision is %d "
                % prev_revision
            )
        if new_revision == prev_revision:
            if prev_record_type == "suppliers":
                ...
                if prev_ecc_map and new_ecc_map != prev_ecc_map:
                    if _Debug:
                        lg.out(
                            _DebugLevel,
                            "        new json data have same revision but different ecc_map, store operation FAILED",
                        )
                    raise ValueError(
                        "new json data have same revision but different ecc_map, current revision is %d "
                        % prev_revision
                    )
                ...
                if prev_suppliers != new_suppliers:
                    if _Debug:
                        lg.out(
                            _DebugLevel,
                            "        new json data have same revision but different suppliers list, store operation FAILED",
                        )
                    raise ValueError(
                        "new json data have same revision but different suppliers list, current revision is %d "
                        % prev_revision
                    )
            if prev_record_type == "message_broker":
                ...
                if prev_broker_idurl is not None and prev_position is not None:
                    if (
                        prev_broker_idurl != new_broker_idurl
                        or prev_position != new_position
                    ):
                        if _Debug:
                            lg.out(
                                _DebugLevel,
                                "        new json data have same revision but different broker info, store operation FAILED",
                            )
                        raise ValueError(
                            "new json data have same revision but different broker info, current revision is %d "
                            % prev_revision
                        )
    if _Debug:
        lg.out(
            _DebugLevel,
            "        new json data is valid and matching existing DHT record, store OK",
        )
    return True
...
def validate_data_written(store_results, key, json_data, result_defer):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.validate_data_written key=[%s]  %s  collected=%s  nodes=%r"
            % (
                key,
                type(store_results),
                results_collected,
        ...
        for result in store_results[1]:
            ...
            if response != "OK":
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "    store operation failed, unexpected response received: %r"
                        % response,
                    )
                result_defer.errback(ValueError(response))
                return None
    ...
    if isinstance(store_results, list):
        ...
        result_defer.errback(
            Exception("store operation failed, result is %s" % type(store_results))
        )
    return None
...
def validate_after_receive(
    value, key, rules, result_defer, raise_for_result, populate_meta_fields
):
    """
...
def get_valid_data(
    ...
    d.addCallback(
        ...
        result_defer=ret,
        raise_for_result=raise_for_result,
        populate_meta_fields=return_details,
    )
    d.addErrback(ret.errback)
    return ret
...
def set_valid_data(
    ...
    valid_json_data = validate_before_send(
        value=json_data, key=key, rules=rules, populate_meta_fields=True
    )
...
def write_verify_republish_data(
    key, json_data, age=0, expire=KEY_EXPIRE_MAX_SECONDS, rules={}, layer_id=0
):
    try:
    ...
    if _Debug:
        _join.addErrback(
            ...
            method="write_verify_republish_data",
        )
    ...
    def _do_verify(write_response, found_nodes):
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service._do_verify  %r via nodes: %r"
                % (
                    write_response,
                    found_nodes,
                ),
...
def on_nodes_found(result, node_id64):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.on_nodes_found   node_id=[%s], %d nodes found"
            % (node_id64, len(result)),
        )
    # _ActiveLookupLayerID = None
    # _ActiveLookup = None
    return result
...
def on_lookup_failed(result, node_id64):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.on_lookup_failed   node_id=[%s], result=%s"
            % (node_id64, result),
        )
    # _ActiveLookupLayerID = None
    # _ActiveLookup = None
...
def find_node(node_id, layer_id=0):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.find_node   node_id=[%s]  layer_id=%d" % (node_id64, layer_id),
        )
...
def get_node_data(key, layer_id=0):
    ...
    if layer_id not in node().data:
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service.get_node_data   layer_id=%d   is not exist" % layer_id,
            )
        return None
    ...
    if key not in node().data[layer_id]:
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service.get_node_data   key=[%s] not exist   layer_id=%d"
                % (
                    key,
                    layer_id,
                ),
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.get_node_data key=[%s] read %d bytes   layer_id=%d"
            % (
                key,
...
def set_node_data(key, value, layer_id=0):
    if not node():
        lg.warn(
            "DHT node is not ready yet, not able to store key %r for layer_id=%d locally"
            % (
                key,
                layer_id,
            )
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.set_node_data key=[%s] wrote %d bytes  layer_id=%d"
            % (
                key,
                len(strng.to_text(value) or ""),
                layer_id,
...
def delete_node_data(key, layer_id=0):
    ...
    if layer_id not in node().data:
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service.delete_node_data layer_id=%d  is not exist" % layer_id,
            )
        return False
    key = strng.to_text(key)
    if key not in node().data[layer_id]:
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.delete_node_data key=[%s]   layer_id=%d"
            % (
                key,
                layer_id,
            ),
...
def load_cache(cache_dir_path):
    ...
    for layer_id_str in os.listdir(cache_dir_path):
        ...
        for hash_key in os.listdir(cache_dir_path):
            _Cache[layer_id][hash_key] = jsn.loads_text(
                local_fs.ReadTextFile(os.path.join(cache_dir_path, hash_key))
            )
            total_records += 1
            records_per_layer[layer_id] += 1
    if _Debug:
        lg.args(
            _DebugLevel, total_records=total_records, records_per_layer=records_per_layer
        )
...
def store_cached_key(hash_key, json_value, layer_id=0, timestamp=None):
    ...
    if not local_fs.WriteTextFile(cached_record_file_path, jsn.dumps(cached_json_record)):
        lg.err(
            "failed to store cached dht key %r in layer %d"
            % (
                hash_key,
                layer_id,
            )
    ...
    if _Debug:
        lg.args(
            ...
            layer_id=layer_id,
            timestamp=timestamp,
            cached_records=len(_Cache[layer_id]),
        )
    return True
...
def get_cached_json_value(key, layer_id=0, cache_ttl=DEFAULT_CACHE_TTL):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            "dht_service.get_cached_json_value key=[%r] layer_id=%d cache_ttl=%d"
            % (
                key,
                layer_id,
                cache_ttl,
...
class DHTNode(MultiLayerNode):
    ...
    def reset_my_dht_id(self, new_id=None):
        ...
        if nodeStateKey in self._dataStore:
            ...
            for contactTriple in state["closestNodes"]:
                contact = Contact(
                    encoding.to_text(contactTriple[0]),
                    contactTriple[1],
                    contactTriple[2],
                    self._protocol,
                )
            ...
            if _Debug:
                print(
                    '[DHT NODE]    found "nodeState" key in local db and added %d contacts to routing table'
        ...
        for layer_id in self._dataStores.keys():
            ...
            for key in expired_keys:
                if _Debug:
                    lg.out(
                        _DebugLevel,
                        "dht_service.expire   [%s] removed from layer %d"
                        % (key, layer_id),
                    )
    ...
    def store(
        ...
        age=0,
        expireSeconds=KEY_EXPIRE_MAX_SECONDS,
        **kwargs
    ):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service.DHTNode.store key=[%s] for %d seconds layerID=%d"
    ...
    def request(self, key, **kwargs):
        ...
        if layerID not in self.active_layers:
            if _Debug:
                lg.out(
                    _DebugLevel,
                    "dht_service.DHTNode.request key=[%r] SKIP because layer %d is not active"
                    % (
                        key,
                        layerID,
                    ),
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service.DHTNode.request key=[%r] layerID=%d"
                % (
                    key,
                    layerID,
                ),
    ...
    def verify_update(
        ...
        **kwargs
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service.DHTNode.verify_update key=[%s]"
                % strng.to_text(key, errors="ignore")[:10],
            )
        return True
    ...
    def findNode(self, key, **kwargs):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service.DHTNode.findNode key=[%s] layerID=%d"
                % (
                    key,
                    layerID,
                ),
    ...
    def findValue(self, key, **kwargs):
        ...
        if _Debug:
            lg.out(
                _DebugLevel,
                "dht_service.DHTNode.findValue key=[%s] layerID=%d"
                % (
                    key,
                    layerID,
                ),
...
class DHTProtocol(KademliaMultiLayerProtocol):
    def __init__(
        ...
        msgTranslator=msgformat.MultiLayerFormat(),
    ):
        KademliaMultiLayerProtocol.__init__(
            self, node, msgEncoder=msgEncoder, msgTranslator=msgTranslator
        )
        self._counter = count
...
def parseCommandLine():
    ...
    oparser.add_option(
        "-s", "--seeds", dest="seeds", help="specify list of DHT seed nodes"
    )
    oparser.set_default("seeds", "")
    oparser.add_option(
        "-l", "--layers", dest="layers", help="specify list of layers to be created"
    )
    ...
    oparser.add_option(
        ...
        "--attach_layer",
        dest="attach_layer",
        help="specify which layer to be attached at start",
    )
    ...
    oparser.add_option(
        ...
        "--join_layer",
        dest="join_layer",
        help="specify which layer to be joined at start",
    )
...
def main(options=None, args=None):
    ...
    lg.out(
        _DebugLevel,
        "Init   udpport=%d   dhtdb=%s   node=%r   connect_layers=%r"
    ...
    def _go():
        ...
        try:
            if len(args) == 0:
                ...
                if cmd == "get":
                    get_value(
                        args[1], layer_id=0 if len(args) < 3 else int(args[2])
                    ).addBoth(_r)
                elif cmd == "set":
                    set_value(
                        args[1],
                        args[2],
                        expire=int(args[3]),
                        layer_id=0 if len(args) < 5 else int(args[4]),
                    ).addBoth(_r)
                elif cmd == "get_json":
                    get_json_value(
                        args[1], layer_id=0 if len(args) < 3 else int(args[2])
                    ...
                    set_json_value(
                        ...
                        jsn.loads(args[2]),
                        expire=int(args[3]),
                        layer_id=0 if len(args) < 3 else int(args[2]),
                    ).addBoth(_r)
                elif cmd == "get_valid_data":
                    get_valid_data(
                        args[1],
                        rules=jsn.loads(args[2]),
                        return_details=True,
                        layer_id=0 if len(args) < 4 else int(args[3]),
                    ).addBoth(_r)
                    ...
                    set_valid_data(
                        ...
                        layer_id=0 if len(args) < 6 else int(args[5]),
                    ).addBoth(_r)
                    ...
                    write_verify_republish_data(
                        args[1], args[2], expire=int(args[3])
                    ).addBoth(_r)
                elif cmd == "find":
