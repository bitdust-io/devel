...
def verify_local_install():
    ...
    if not os.path.isdir(ethereum_location):
        raise ValueError(
            "Ethereum geth process folder not found: {}".format(ethereum_location)
        )
    geth_bin_path = os.path.join(geth_location, "build", "bin", "geth")
    if not os.path.isfile(geth_bin_path):
        raise ValueError(
            "Ethereum geth process executable not found: {}".format(geth_bin_path)
        )
    return True
...
def execute(cmdargs, base_dir=None, process_protocol=None, env=None, callback=None):
    ...
    if _Debug:
        lg.out(
            _DebugLevel,
            'geth_service.execute: "%s" in %s' % (" ".join(cmdargs), base_dir),
        )
