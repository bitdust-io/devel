...
def listen(port, proto=None):
    if port in listeners().keys():
        io.log(6, 'udp.listen  WARNING already started on port %d' % port)
        io.log(6, '            %s' % str(listeners().keys()))
    ...
    io.log(6, 'udp.listen on port %d started' % port)
    return listeners()[port]


def port_closed(x):
    io.log(6, 'udp.port_closed   listeners: %d' % (len(listeners())))
    return x
    
    
def close(port):
    io.log(6, 'udp.close  %r' % port)
    ...
    io.log(6, 'udp.close  STOP listener on UDP port %d' % port)
    return d
...
def close_all():
    ...
    for port in l:
        ...
        if d:
            # d.addCallback(port_closed)
            shutlist.append(d)
            # io.log(6, 'udp.close_all  STOP listener on UDP port %d' % port)
    # _Listeners.clear()
    io.log(6, 'udp.close_all  %d UDP listeners were closed' % len(shutlist))
...
def send_command(from_port, command, data, address):
    p = proto(from_port)
    if not p:
        io.log(6, 'udp.send_command WARNING port %d is not opened to listen' % from_port)
        return False
...
class BasicProtocol(protocol.DatagramProtocol):
    ...
    def __init__(self):
        """
        """
        # io.log(6, 'udp.BasicProtocol.__init__ %r' % id(self))
        self.port = None
        self.callbacks = []
        self.stopping = False
    ...
    def __del__(self):
        """
        """
        # io.log(6, 'udp.BasicProtocol.__del__ %r' % id(self))
    ...
    def startProtocol(self):
        """
        """
        io.log(6, 'udp.startProtocol %r' % self)

    def stopProtocol(self):
        """
        """
        io.log(6, 'udp.stopProtocol %r' % self)
        self.port = None
        self.callbacks = []
...
class CommandsProtocol(BasicProtocol):
    ...
    def datagramReceived(self, datagram, address):
        ...
        if version != self.SoftwareVersion:
            return
        io.log(24, '>>> [%s] (%d bytes) from %s' % (command, len(payload), str(address)))
    ...
    def sendCommand(self, command, data, address):
        try:
            ...
            io.exception()
            return False
        io.log(24, '<<< [%s] (%d bytes) to %s' % (command, len(data), address))
