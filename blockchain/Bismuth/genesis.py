...
block_hash = hashlib.sha224(
    str((timestamp, transaction)).encode("utf-8")
).hexdigest()  # first hash is simplified
...
if os.path.isfile("static/ledger.db"):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO misc (difficulty, block_height) VALUES ({},1)".format(DIFFICULTY)
        )
        ...
        mem_cur.execute(
            "CREATE TABLE transactions (timestamp, address, recipient, amount, signature, public_key, operation, openfield)"
        )
        mempool.commit()
        conn.close()
        mempool.close()
