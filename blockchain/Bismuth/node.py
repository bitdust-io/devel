...
from digest import *
from essentials import download_file, fee_calculate
from fork import Fork
from libs import client, keys, logger
from libs import node as _node
...
def bootstrap():
    ...
    try:
        ...
        hdd_cursor.execute(
            'CREATE TABLE IF NOT EXISTS "misc" ("block_height" INTEGER, "difficulty" TEXT)'
        )
        hdd_cursor.execute(
        ...
        hdd_cursor.execute(
            "CREATE INDEX TXID4_Index ON transactions(substr(signature,1,4))"
        )
        hdd_cursor.execute('CREATE INDEX "Misc Block Height Index" on misc(block_height)')
        hdd_cursor.execute(
            "INSERT INTO misc (difficulty, block_height) VALUES ({},1)".format(
                INITIAL_DIFFICULTY
            )
        )
        hdd_cursor.execute(
            ...
            (
                ...
                "1660386165.95425",
                "genesis",
                "3b9ca99a7804015f8eaebb78d4b50570bd8337e8a8196343dbbb59c4",
                "0",
                "",
                "",
                "06b63546d0f877f57e74980fdc961b5247c1f0713f827b9af6a36c29",
                0,
                1,
                1,
                "genesis",
        ...
        index_cursor.execute(
            'CREATE INDEX "Alias Index" ON "aliases" ("block_height", "address","alias")'
        )
        index_cursor.execute(
            'CREATE INDEX "Token Index" ON "tokens" ("block_height","timestamp","token","address","recipient","txid","amount")'
        )
        index_cursor.execute(
            "CREATE TABLE staking (block_height INTEGER, timestamp NUMERIC, address, balance, ip, port, pos_address)"
        )
        index.commit()
        index.close()
    ...
    with sqlite3.connect(database) as ledger_check:
        if node.trace_db_calls:
            ledger_check.set_trace_callback(
                functools.partial(
                    sql_trace_callback, node.logger.app_log, "CHECK_INTEGRITY"
                )
            )
...
def rollback(node, db_handler, block_height):
    ...
    node.logger.app_log.warning(
        f"Status: Chain rolled back below {block_height} and will be resynchronized"
    )
...
def recompress_ledger(node, rebuild=False, depth=15000):
    ...
    files_remove = [
        node.ledger_path + ".temp",
        node.ledger_path + ".temp-shm",
        node.ledger_path + ".temp-wal",
    ]
    ...
    if rebuild:
        ...
        hyper = sqlite3.connect(node.ledger_path + ".temp")
    if node.trace_db_calls:
        hyper.set_trace_callback(
            functools.partial(sql_trace_callback, node.logger.app_log, "HYPER")
        )
    hyper.text_factory = str
    hyp = hyper.cursor()

    hyp.execute(
        "UPDATE transactions SET address = 'Hypoblock' WHERE address = 'Hyperblock'"
    )
    ...
    )  # new addresses will be ignored until depth passed
    ...
    for x in set(unique_addressess):
        ...
        for entry in hyp.execute(
            ...
            try:
                credit = (
                    quantize_eight(credit)
                    + quantize_eight(entry[0])
                    + quantize_eight(entry[1])
                )
    ...
    hyp.execute(
        "DELETE FROM misc WHERE (block_height < ? AND block_height > ?);",
        (
            depth_specific,
            -depth_specific,
        ),
...
def ledger_check_heights(node, db_handler):
    ...
    if os.path.exists(node.hyper_path):
        ...
        # cross-integrity check
        if (
            hdd_block_max == hdd2_block_last == hdd2_block_last_misc == hdd_block_max_diff
            and node.hyper_recompress
        ):  # cross-integrity check
            node.logger.app_log.warning(
            ...
            lowest_block = min(
                hdd_block_max, hdd2_block_last, hdd_block_max_diff, hdd2_block_last_misc
            )
            highest_block = max(
                hdd_block_max, hdd2_block_last, hdd_block_max_diff, hdd2_block_last_misc
            )
            ...
            rollback(
                node, db_handler_initial, lowest_block
            )  # rollback to the lowest value
            node.recompress = False
...
def balanceget(balance_address, db_handler):
    ...
    balance_no_mempool = (
        float(credit_ledger) - float(debit_ledger) - float(fees) + float(rewards)
    )
...
def blocknf(node, block_hash_delete, peer_ip, db_handler, hyperblocks=False):
    ...
    node.logger.app_log.info(
        f"Rollback operation on {block_hash_delete} initiated by {peer_ip}"
    )
    ...
    if not node.db_lock.locked():
        ...
        try:
            ...
            if ip["ip"] == "no":
                ...
                hyperblocks and node.last_block_ago > 30000
            ):  # more than 5000 minutes/target blocks away
                ...
                node.logger.app_log.warning(
                    f"Node {peer_ip} didn't find block {db_block_height}({db_block_hash})"
                )
            ...
            if skip:
                ...
                try:
                    ...
                    for tx in backup_data:
                        ...
                        if tx[9] == 0:
                            try:
                                ...
                                node.logger.app_log.warning(
                                    f"Moved tx back to mempool: {tx_short}"
                                )
                            except Exception as e:
                                node.logger.app_log.warning(
                                    f"Error during moving tx back to mempool: {e}"
                                )
                        else:
                    ...
                    node.logger.app_log.warning(
                        f"Error during moving txs back to mempool: {e}"
                    )
...
def sequencing_check(db_handler):
    ...
    try:
        ...
        node.logger.app_log.warning(
            "Sequencing anchor not found, going through the whole chain"
        )
        sequencing_last = 0

    node.logger.app_log.warning(
        f"Status: Testing chain sequencing, starting with block {sequencing_last}"
    )
    ...
    for chain in chains_to_check:
        ...
        if node.trace_db_calls:
            conn.set_trace_callback(
                functools.partial(
                    sql_trace_callback, node.logger.app_log, "SEQUENCE-CHECK-CHAIN"
                )
            )
        c = conn.cursor()
        ...
        for row in c.execute(
            "SELECT block_height FROM misc WHERE block_height > ? ORDER BY block_height ASC",
            (300000,),
        ):
            y_init = row[0]
            ...
            if row[0] != y:
                ...
                for chain2 in chains_to_check:
                    ...
                    c2.execute(
                        "DELETE FROM transactions WHERE block_height >= ?", (row[0],)
                    )
                    conn2.commit()
                    c2.execute("DELETE FROM misc WHERE block_height >= ?", (row[0],))
                    conn2.commit()
...
class ThreadedTCPRequestHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        if node.IS_STOPPING:
            node.logger.app_log.warning(
                "Inbound: Rejected incoming cnx, node is stopping"
            )
            return
        ...
        if (
            threading.active_count() < node.thread_limit / 3 * 2
            or node.peers.is_whitelisted(peer_ip)
        ):  # inbound
            client_instance.connected = True
        else:
            try:
                node.logger.app_log.info(
                    f"Free capacity for {peer_ip} unavailable, disconnected"
                )
                self.request.close()
        ...
        while (
            not node.peers.is_banned(peer_ip)
            and node.peers.version_allowed(peer_ip, node.version_allow)
            ...
            try:
                ...
                if (
                    not time.time() <= timer_operation + timeout_operation
                ):  # return on timeout
                ...
                if data.startswith("regtest_"):
                    if not node.is_regnet:
                        ...
                        (
                            ...
                            db_handler_instance.conn,
                            db_handler_instance.c,
                            db_handler_instance.hdd,
                            db_handler_instance.h,
                            db_handler_instance.hdd2,
                            db_handler_instance.h2,
                            db_handler_instance.h,
                        )
                        regnet.command(
                            self.request, data, block_hash, node, db_handler_instance
                        )
                ...
                if data == "version":
                    ...
                    if data not in node.version_allow:
                        ...
                        node.logger.app_log.warning(
                            f"Inbound: Protocol version matched with {peer_ip}: {data}"
                        )
                    ...
                    node.logger.app_log.info(
                        mp.MEMPOOL.merge(segments, peer_ip, db_handler_instance.c, False)
                    )
                    ...
                    if node.is_regnet:
                        node.logger.app_log.info(
                            "Inbound: Got hello but I'm in regtest mode, closing."
                        )
                        return
                    ...
                    node.logger.app_log.info(
                        f"Inbound: Client {peer_ip} has the block(s)"
                    ...
                    if node.db_lock.locked():
                        ...
                        node.last_block_timestamp = (
                            db_handler_instance.last_block_timestamp()
                        )
                        ...
                        if (
                            ...
                            try:  # they claim to have the longest chain, things must go smooth or ban
                                ...
                                node.plugin_manager.execute_action_hook(
                                    "sync", {"what": "syncing_from", "ip": peer_ip}
                                )
                                ...
                                if node.peers.warning(
                                    self.request,
                                    peer_ip,
                                    "Failed to deliver the longest chain",
                                ):
                                    node.logger.app_log.info(f"{peer_ip} banned")
                                    break
                            else:
                            ...
                            node.logger.app_log.warning(
                                f"Rejecting to sync from {peer_ip}"
                            )
                    ...
                    try:
                        received_block_height = receive(
                            self.request
                        )  # receive client's last block height
                        node.logger.app_log.info(
                        ...
                        node.peers.consensus_add(
                            peer_ip, consensus_blockheight, self.request, node.hdd_block
                        )
                        # consensus pool 1 (connection from them)
                        ...
                        if int(received_block_height) > node.hdd_block:
                            node.logger.app_log.warning(
                                "Inbound: Client has higher block"
                            )

                            node.logger.app_log.info(
                                f"Inbound: block_hash to send: {node.hdd_hash}"
                            )
                            ...
                            data = receive(
                                self.request
                            )  # receive client's last block_hash
                            # send all our followup hashes
                            if data == "*":
                                # connection lost, no need to go on, that was banning the node like it forked.
                                node.logger.app_log.warning(
                                    f"Inbound: {peer_ip} dropped connection"
                                )
                                break
                            node.logger.app_log.info(
                                f"Inbound: Will seek the following block: {data}"
                            )
                            ...
                            if client_block is None:
                                node.logger.app_log.warning(
                                    f"Inbound: Block {data[:8]} of {peer_ip} not found"
                                )
                                if node.full_ledger:
                                    send(
                                        self.request, "blocknf"
                                    )  # announce block hash was not found
                                else:
                                    send(
                                        self.request, "blocknfhb"
                                    )  # announce we are on hyperblocks
                                ...
                                node.logger.app_log.info(
                                    f"Inbound: Client is at block {client_block}"
                                )  # now check if we have any newer

                                if node.hdd_hash == data or not node.egress:
                                    if not node.egress:
                                        node.logger.app_log.warning(
                                            f"Inbound: Egress disabled for {peer_ip}"
                                        )
                                    else:
                                        node.logger.app_log.info(
                                            f"Inbound: Client {peer_ip} has the latest block"
                                        )
                                    ...
                                    node.logger.app_log.info(
                                        f"Inbound: Selected {blocks_fetched}"
                                    )
                                    ...
                                    if confirmation == "blockscf":
                                        node.logger.app_log.info(
                                            "Inbound: Client confirmed they want to sync from us"
                                        )
                    ...
                    if consensus_blockheight == node.peers.consensus_max:
                        ...
                        if node.peers.warning(self.request, peer_ip, "Rollback", 2):
                            ...
                            break
                    node.logger.app_log.info(
                        "Inbound: Deletion complete, sending sync request"
                    )
                    ...
                    if node.peers.is_allowed(peer_ip, data):  # from miner
                        ...
                        node.logger.app_log.info(
                            f"Inbound: Received a block from miner {peer_ip}"
                        )
                        # receive block
                        segments = receive(self.request)
                        ...
                        try:
                            mined["miner"] = segments[0][-1][
                                1
                            ]  # sender, to be consistent with block event.
                        except:
                        ...
                        if node.is_mainnet:
                            if len(
                                node.peers.connection_pool
                            ) < 5 and not node.peers.is_whitelisted(peer_ip):
                                ...
                                node.plugin_manager.execute_action_hook("mined", mined)
                                node.logger.app_log.info(
                                    "Inbound: Processing block from miner"
                                )
                                try:
                                    digest_block(
                                        node,
                                    ...
                                    node.logger.app_log.warning(
                                        "Inbound: block {}".format(str(e))
                                    )
                                    return
                                except Exception as e:
                                    node.logger.app_log.error(
                                        "Inbound: Processing block from miner {}".format(
                                            e
                                        )
                                    )
                                    return
                                ...
                                reason = (
                                    f"Inbound: Mined block was orphaned because node was not synced, "
                            ...
                            try:
                                ...
                                node.logger.app_log.error(
                                    "Inbound: Processing block from miner {}".format(e)
                                )
                                return
                    else:
                        receive(self.request)  # receive block, but do nothing about it
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for block command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for blocklast command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for blocklastjson command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for blockget command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        mpinsert_result = mp.MEMPOOL.merge(
                            mempool_insert, peer_ip, db_handler_instance.c, True, True
                        )
                        node.logger.app_log.warning(f"mpinsert result: {mpinsert_result}")
                        send(self.request, mpinsert_result)
                    else:
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for mpinsert command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        balanceget_result = balanceget(
                            balance_address, db_handler_instance
                        )

                        send(
                            self.request, balanceget_result
                        ...
                        node.logger.app_log.info(
                            "{peer_ip} not whitelisted for balanceget command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for balancegetjson command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for balancegethyperjson command"
                        )
                    ...
                    data == "mpclear" and peer_ip == "127.0.0.1"
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        (
                            gen_private_key_readable,
                            gen_public_key_readable,
                            gen_address,
                        ) = wallet_keys.generate()
                        send(
                            self.request,
                            (
                                gen_private_key_readable,
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for keygen command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for addlist command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for listlimjson command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for listlim command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for addlistlim command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for addlistlimjson command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for addlistlimmir command"
                        )
                    ...
                    data == "aliasget"
                ):  # all for a single address, no protection against overlapping
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for aliasget command"
                        )

                elif (
                    data == "aliasesget"
                ):  # only gets the first one, for multiple addresses
                    if node.peers.is_allowed(peer_ip, data):
                        aliases.aliases_update(node, db_handler_instance)
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for aliasesget command"
                        )
                ...
                # Not mandatory, but may help to reindex with minimal sql queries
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        for token in tokens_user:
                            token = token[0]
                            db_handler_instance.execute_param(
                                db_handler_instance.index_cursor,
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for tokensget command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.warning(
                            f"Fetched the following alias address: {address_fetch}"
                        )
                        send(self.request, address_fetch)

                    else:
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for addfromalias command"
                        )

                elif data == "pubkeyget":
                    if node.peers.is_allowed(peer_ip, data):
                        pub_key_address = receive(self.request)
                        target_public_key_b64encoded = db_handler_instance.pubkeyget(
                            pub_key_address
                        )
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for pubkeyget command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for aliascheck command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        node.logger.app_log.warning(
                            "txsend is unsafe and deprecated, please don't use."
                        )
                        ...
                        remote_tx_pubkey = (
                            tx_remote_key.publickey().exportKey().decode("utf-8")
                        )

                        remote_tx_pubkey_b64encoded = base64.b64encode(
                            remote_tx_pubkey.encode("utf-8")
                        ).decode("utf-8")

                        remote_tx_address = hashlib.sha224(
                            remote_tx_pubkey.encode("utf-8")
                        ).hexdigest()
                        # derive remaining data
                        ...
                        remote_signature_enc = base64.b64encode(remote_signature).decode(
                            "utf-8"
                        )
                        # construct tx
                        ...
                        node.logger.app_log.info(
                            mp.MEMPOOL.merge(
                                mempool_data, peer_ip, db_handler_instance.c, True, True
                            )
                        )
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for txsend command"
                        )
                ...
                # less important methods
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for addvalidate command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for annget command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for peersget command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        send(
                            ...
                            (
                                ...
                                threads_count,
                                uptime,
                                node.peers.consensus,
                                node.peers.consensus_percentage,
                                VERSION,
                                diff,
                                server_timestamp,
                            ),
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for statusget command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        status = {
                            ...
                            "uptime": uptime,
                            "consensus": node.peers.consensus,
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for statusjson command"
                        )
                elif data[:4] == "api_":
                    if node.peers.is_allowed(peer_ip, data):
                        try:
                            node.apihandler.dispatch(
                                data, self.request, db_handler_instance, node.peers
                            )
                        except Exception as e:
                            if node.debug:
                                raise
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for diffget command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for portget command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for diffgetjson command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            "f{peer_ip} not whitelisted for difflastget command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for difflastjson command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for stop command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        node.logger.app_log.info(
                            f"{peer_ip} not whitelisted for block_height_from_hash command"
                        )
                    ...
                    if node.peers.is_allowed(peer_ip, data):
                        ...
                        try:
                            ...
                            node.logger.app_log.warning(
                                f"{peer_ip} sent invalid peers list"
                            )
                            raise
                        ...
                        node.logger.app_log.warning(
                            f"{peer_ip} not whitelisted for addpeers"
                        )
                    ...
                    if not extra:
                        raise ValueError(
                            "Unexpected error, received: " + str(data)[:32] + " ..."
                        )
...
def setup_net_type():
    ...
    if "testnet" in node.version or node.is_testnet:
        ...
        redownload_test = input(
            "Status: Welcome to the testnet. Redownload test ledger? y/n"
        )
        if redownload_test == "y":
            types = [
                "static/ledger_test.db-wal",
                "static/ledger_test.db-shm",
                "static/index_test.db",
                "static/hyper_test.db-wal",
                "static/hyper_test.db-shm",
            ]
            for type in types:
    ...
    try:
        if node.ram:
            ...
            if node.py_version >= 370:
                temp_target = sqlite3.connect(
                    node.ledger_ram_file, uri=True, isolation_level=None, timeout=1
                )
                if node.trace_db_calls:
                    temp_target.set_trace_callback(
                        functools.partial(
                            sql_trace_callback, node.logger.app_log, "TEMP-TARGET"
                        )
                    )

                temp_source = sqlite3.connect(
                    node.hyper_path, uri=True, isolation_level=None, timeout=1
                )
                if node.trace_db_calls:
                    temp_source.set_trace_callback(
                        functools.partial(
                            sql_trace_callback, node.logger.app_log, "TEMP-SOURCE"
                        )
                    )
                ...
                if node.trace_db_calls:
                    source_db.set_trace_callback(
                        functools.partial(
                            sql_trace_callback, node.logger.app_log, "SOURCE-DB"
                        )
                    )
                database.to_ram = sqlite3.connect(
                    node.ledger_ram_file, uri=True, timeout=1, isolation_level=None
                )
                if node.trace_db_calls:
                    database.to_ram.set_trace_callback(
                        functools.partial(
                            sql_trace_callback, node.logger.app_log, "DATABASE-TO-RAM"
                        )
                    )
...
def initial_db_check():
    ...
    if os.path.exists("fresh_sync") and node.is_mainnet:
        node.logger.app_log.warning(
            "Status: Fresh sync required, bootstrapping from the website"
        )
    ...
    if node.is_mainnet:
        ...
        if node.trace_db_calls:
            upgrade.set_trace_callback(
                functools.partial(
                    sql_trace_callback, node.logger.app_log, "INITIAL_DB_CHECK"
                )
            )
        u = upgrade.cursor()
        try:
...
def verify(db_handler):
    ...
    try:
        ...
        try:
            ...
            if str(genesis) != node.genesis and int(block_height) == 0:
        ...
        for row in db_handler.h.execute(
            ...
            try:
                ...
                SignerFactory.verify_bis_signature(
                    db_signature_enc, db_public_key_b64encoded, db_transaction, db_address
                )
            except Exception as e:
                ...
                try:
                    if (
                        sha_hash.hexdigest()
                        != db_hashes[db_block_height + "-" + db_timestamp]
                    ):
                        node.logger.app_log.warning(
                            "Signature validation problem: {} {}".format(
                                db_block_height, db_transaction
                            )
                        )
                        invalid = invalid + 1
                except Exception as e:
...
def add_indices(db_handler: dbhandler.DbHandler):
    CREATE_TXID4_INDEX_IF_NOT_EXISTS = (
        "CREATE INDEX IF NOT EXISTS TXID4_Index ON transactions(substr(signature,1,4))"
    )
    CREATE_MISC_BLOCK_HEIGHT_INDEX_IF_NOT_EXISTS = (
...
if __name__ == "__main__":
    ...
    if (
        os.path.exists("../wallet.der")
        and not os.path.exists("wallet.der")
    ...
    if not node.full_ledger:
        ...
        shutil.copy(
            node.hyper_path, node.ledger_path
        )  # hacked to remove all the endless checks
    try:
        # create a plugin manager, load all plugin modules and init
        node.plugin_manager = plugins.PluginManager(
            app_log=node.logger.app_log, config=config, init=True
        )
        # get the potential extra command prefixes from plugin
        extra_commands = {}  # global var, used by the server part.
        extra_commands = node.plugin_manager.execute_filter_hook(
            "extra_commands_prefixes", extra_commands
        )
        ...
        try:
            ...
            if not node.tor:
                ...
                node.logger.app_log.warning(
                    f"Status: Server loop running on {ip}:{node.port}"
                )

            else:
                node.logger.app_log.warning(
                    "Status: Not starting a local server to conceal identity on Tor network"
                )
