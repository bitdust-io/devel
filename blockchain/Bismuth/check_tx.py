...
def is_in_mempool(txid):
    ...
    m = mempool.cursor()
    m.execute(
        "SELECT timestamp, address, recipient, amount, openfield FROM transactions WHERE signature like ?;",
        (txid + "%",),
    )
    result = m.fetchone()
    if result:
...
def is_in_ledger(txid):
    ...
    m = ledger.cursor()
    m.execute(
        "SELECT timestamp, address, recipient, amount, openfield, block_height FROM transactions WHERE signature like ?;",
        (txid + "%",),
    )
    result = m.fetchone()
    if result:
