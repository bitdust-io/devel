...
bin_format_dict = dict(
    (x, format(ord(x), "8b").replace(" ", "0")) for x in "0123456789abcdef"
)
...
def payout(payout_threshold, myfee, othfee):
    ...
    for row in c.execute(
        "SELECT * FROM transactions WHERE address = ? AND CAST(timestamp AS INTEGER) >= ? AND reward != 0",
        (address,) + (block_threshold,),
    ):
    ...
    if reward_total > 0:
        ...
        for r in paylist:
            ...
            keep = 0
            fee = float(
                "%.8f" % float(0.01 + (float(len(openfield)) / 100000) + (keep))
            )  # 0.01 + openfield fee + keep fee
            # make payout
...
if not os.path.exists("shares.db"):
    ...
    execute(
        s,
        "CREATE TABLE IF NOT EXISTS shares (address, shares, timestamp, paid, rate, name, workers, subname)",
    )
...
class MyTCPHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        try:
            ...
            app_log.warning(
                "Received: {} from {}".format(data, peer_ip)
            )  # will add custom ports later

            if (
                data == "getwork"
            ):  # sends the miner the blockhash and mining diff for shares
                ...
                if not s_test(miner_address):
                    ...
                    app_log.warning(
                        "Received a solution from miner {} ({})".format(
                    ...
                    """
                    app_log.warning(
                        "Solution has {} difficulty, current difficulty is {}".format(
                            real_diff, diff
                        )
                    )
                    if real_diff >= int(diff):
                        ...
                        app_log.warning(
                            "Local node ip {} on port {}".format(cnode_ip_conf, cport)
                        )
                        ...
                        with open(m_peer_file) as f:
                            ...
                            for k, v in peer_dict.items():
                                ...
                                try:
                                    ...
                                    app_log.warning(
                                        "Miner: Block submitted to {}".format(peer_ip)
                                    )
                                except Exception as e:
                                    app_log.warning(
                                        "Miner: Could not submit block to {} because {}".format(
                                            peer_ip, e
                                        )
                                    )
                    ...
                    execute_param(
                        s, ("SELECT nonce FROM nonces WHERE nonce = ?"), (nonce,)
                    )
