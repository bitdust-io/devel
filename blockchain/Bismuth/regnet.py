            ...
            for i in range(100):
                    ...
                    possibles = [
                        ...
                        in (
                            mining.anneal3(
                                ...
                                int.from_bytes(
                                    sha224(
                                        (prefix + nonce + blockhash).encode("utf-8")
                                    ).digest(),
                                    "big",
                                ),
                            )
                ...
                if possibles:
                    ...
                    node.logger.app_log.warning(
                        "Generate got a block in {} tries len {}".format(
                            i, len(possibles)
                        )
                    )
                    ...
                    if signer.verify(hash, signature):
                        ...
                        node.logger.app_log.warning(
                            "Block to send: {}".format(block_send)
                        )
                    # calc hash

                    new_hash = DIGEST_BLOCK(
                        node, [block_send], None, "regtest", db_handler
                    )
...
def command(sdef, data, blockhash, node, db_handler):
    try:
        ...
        if data == "regtest_generate":
            ...
            node.logger.app_log.warning(
                "regtest_generate {} {}".format(how_many, blockhash)
            )
...
def init(app_log, trace_db_calls=False):
    ...
    with sqlite3.connect(REGNET_DB) as source_db:
        if trace_db_calls:
            source_db.set_trace_callback(
                functools.partial(sql_trace_callback, app_log, "REGNET-INIT")
            )
    ...
    with sqlite3.connect(REGNET_INDEX) as source_db:
        if trace_db_calls:
            source_db.set_trace_callback(
                functools.partial(sql_trace_callback, app_log, "REGNET-INIT-INDEX")
            )
