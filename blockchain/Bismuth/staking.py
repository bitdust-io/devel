...
def check_db(index, index_cursor):
    index_cursor.execute(
        "CREATE TABLE IF NOT EXISTS staking (block_height INTEGER, timestamp NUMERIC, address, balance)"
    )
    index.commit()
...
def staking_update(conn, c, index, index_cursor, mode, reg_phase_end, app_log):
    ...
    for row in results:
        try:
            ...
            try:
                index_cursor.execute(
                    "SELECT * from staking WHERE address = ?", (address,)
                )
            ...
            app_log.warning(
                "Staking registration ran into the following issue: {}".format(e)
            )
...
def mirror_hash_generate(c):
    # new hash
    c.execute(
        "SELECT * FROM transactions WHERE block_height = (SELECT block_height FROM transactions ORDER BY block_height ASC LIMIT 1)"
    )
    result = c.fetchall()
    ...
    for staking in staking:
        ...
        if block_staking <= block_height:
            ...
            try:
                ...
                conn.commit()
                app_log.warning(
                    "Staking fuel payout added: {} {}".format(block_height, address)
                )
                # fuel
        else:
...
def staking_revalidate(conn, c, index, index_cursor, block, app_log):
    ...
    for staking in staking:
        ...
        if quantize_eight(balance) < 10000:
            ...
            index_cursor.execute(
                "UPDATE staking SET balance = ? WHERE address = ?", (balance, address)
            )
            index.commit()
            app_log.warning(
