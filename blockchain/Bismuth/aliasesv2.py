...
def aliases_update(node, db_handler_instance):
    ...
    db_handler_instance.index_cursor.execute(
        "SELECT block_height FROM aliases ORDER BY block_height DESC LIMIT 1;"
    )
    try:
    ...
    for openfield in result:
        ...
        try:
            db_handler_instance.index_cursor.execute(
                "SELECT * from aliases WHERE alias = ?", (openfield[2],)
            )
            ...
            db_handler_instance.index_cursor.execute(
                ...
                (openfield[0], openfield[1], openfield[2]),
            )
            db_handler_instance.index.commit()
            node.logger.app_log.warning(
                f"Added alias to the database: {openfield[2]} from block {openfield[0]}"
            )
