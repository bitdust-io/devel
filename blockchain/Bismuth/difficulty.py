...
from quantizer import quantize_ten, quantize_two
...
def difficulty(node, db_handler):
    try:
        ...
        timestamp_before_last = (
            timestamp_last if previous is None else Decimal(previous[1])
        )
        ...
        db_handler.execute(
            db_handler.c, "SELECT difficulty FROM misc ORDER BY block_height DESC LIMIT 1"
        )
        ...
        if node.is_regnet:
            return (
                float("%.10f" % regnet.REGNET_DIFF),
                float("%.10f" % (regnet.REGNET_DIFF - 8)),
                float(time_to_generate),
                float(regnet.REGNET_DIFF),
                float(block_time),
                float(0),
                float(0),
                block_height,
        ...
        difficulty_new = Decimal(
            (2 / math.log(2))
            * math.log(
                hashrate * target * math.ceil(28 - diff_block_previous / Decimal(16.0))
            )
        )
        # Feedback controller
        Kd = 10
        ...
        diff_adjustment = (
            difficulty_new - diff_block_previous
