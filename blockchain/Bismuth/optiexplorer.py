...
def main():
    ...
    for x in addresses:
        ...
        s.execute(
            "SELECT timestamp FROM shares WHERE address = ? ORDER BY timestamp ASC LIMIT 1",
            (x,),
        )
        ...
        for n in shares_names:
            s.execute(
                "SELECT * FROM shares WHERE address = ? AND name = ? ORDER BY timestamp DESC LIMIT 1",
                (x, n[0]),
            )
    ...
    for row in c.execute(
        "SELECT * FROM transactions WHERE address = ? and openfield = ? ORDER BY timestamp DESC LIMIT 80",
        (address,) + ("pool",),
    ):
        ...
        data_ptime.append(
            format(time.strftime("%Y/%m/%d,%H:%M:%S", time.gmtime(float(row[1]))))
        )
