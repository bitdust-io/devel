...
class MainHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        execute(
            c,
            "SELECT * FROM transactions ORDER BY block_height DESC, timestamp DESC LIMIT 100;",
        )
        all = c.fetchall()[::-1]
        ...
        execute(
            c,
            "SELECT * FROM transactions ORDER BY block_height DESC, timestamp DESC LIMIT 500;",
        )
        all = c.fetchall()
        ...
        for x in all:
            ...
            view.append(
                "<td>{}".format(
                    time.strftime("%Y/%m/%d,%H:%M:%S", time.gmtime(float(x[1])))
                )
            )
        ...
        html.append(
            "<center><a href='http://bismuth.cz/ledger.tar.gz' class='btn btn-info' role='button'>Download Blockchain</a></center>"
        )
        html.append("</div>")
        ...
        html.append(
            "<tr><td>Transactions per block: </td><td>{}</td>".format(tx_count / 500)
        )
        html.append(
            "<tr><td>Total BIS transferred: </td><td>{}</td>".format(transferred_total)
        )
