...
def ledger_balance_node(address, c):
    ...
    for entry in execute_param(
        c, "SELECT amount FROM transactions WHERE recipient = ?;", (address,)
    ):
    ...
    for entry in execute_param(
        c, "SELECT amount FROM transactions WHERE address = ?;", (address,)
    ):
    ...
    fees = Decimal("0")
    for entry in execute_param(
        c, "SELECT fee FROM transactions WHERE address = ?;", (address,)
    ):
        try:
    ...
    rewards = Decimal("0")
    for entry in execute_param(
        c, "SELECT reward FROM transactions WHERE recipient = ?;", (address,)
    ):
        try:
...
def ledger_balance2(address, c):
    ...
    credit_ledger = Decimal(0)
    for entry in execute_param(
        c, "SELECT amount, reward FROM transactions WHERE recipient = ?;", (address,)
    ):
        credit_ledger += quantize_eight(entry[0]) + quantize_eight(entry[1])

    debit_ledger = Decimal(0)
    for entry in execute_param(
        c, "SELECT amount, fee FROM transactions WHERE address = ?;", (address,)
    ):
