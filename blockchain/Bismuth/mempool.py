...
class Mempool:
    ...
    def __init__(
        self, app_log, config=None, db_lock=None, testnet=False, trace_db_calls=True
    ):
        try:
            ...
            if self.ram:
                ...
                if self.trace_db_calls:
                    self.db.set_trace_callback(
                        functools.partial(sql_trace_callback, self.app_log, "MEMPOOL-RAM")
                    )
                ...
                self.db = sqlite3.connect(
                    self.mempool_path, timeout=1, check_same_thread=False
                )
                if self.trace_db_calls:
                    self.db.set_trace_callback(
                        functools.partial(sql_trace_callback, self.app_log, "MEMPOOL")
                    )
                self.db.text_factory = str
                self.cursor = self.db.cursor()
    ...
    def merge(
        ...
        if not essentials.is_sequence(data):
            if peer_ip != "127.0.0.1":
                ...
                self.app_log.warning(
                    "Freezing mempool from {} for 10 min - Bad TX format".format(peer_ip)
                )
                ...
                for transaction in block_list:
                    if size_bypass or self.space_left_for_tx(transaction, mempool_size):
                        ...
                        try:
                            ...
                            mempool_result.append(
                                "Mempool: Invalid timestamp {}".format(transaction[0])
                            )
                        if not essentials.address_validate(transaction[1]):
                            mempool_result.append(
                                "Mempool: Invalid address {}".format(transaction[1])
                            )
                            continue
                        ...
                        if not essentials.address_validate(transaction[2]):
                            mempool_result.append(
                                "Mempool: Invalid recipient {}".format(transaction[2])
                            )
                            continue
                        ...
                        try:
                            ...
                            mempool_result.append(
                                "Mempool: Invalid amount {}".format(transaction[3])
                            )
                            continue
                            ...
                            if mempool_recipient in self.config.mandatory_message.keys():
                                mempool_result.append(
                                    "Mempool: Missing message - {}".format(
                                        self.config.mandatory_message[mempool_recipient]
                                    )
                                )
                                continue
                        ...
                        if mempool_in and ledger_in:
                            try:
                                ...
                                if self.config.old_sqlite:
                                    self.execute(
                                        SQL_DELETE_TX_OLD, (mempool_signature_enc,)
                                    )
                                else:
                        ...
                        if result:
                            for x in result:
                                ...
                                fee = essentials.fee_calculate(
                                    x[1], x[2], last_block
                                )  # fee_calculate sends back a Decimal 8
