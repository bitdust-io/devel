...
if __name__ == '__main__':
    ...
    transaction = (
        str(timestamp), str(address), str(recipient_input), '%.8f' % float(amount_input), str(operation_input), str(openfield_input)
    )  # this is signed
    ...
    if verifier.verify(h, signature):
        if float(amount_input) < 0:
            ...
            tx_submit = (
                str(timestamp), str(address), str(recipient_input), '%.8f' % float(amount_input), str(signature_enc.decode('utf-8')),
                str(public_key_b64encoded.decode('utf-8')), str(operation_input), str(openfield_input)
            )
            while True:
                try:
                    s._send('mpinsert')
