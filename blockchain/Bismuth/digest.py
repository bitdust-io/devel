...
def digest_block(node, data, sdef, peer_ip, db_handler):
    ...
    def transaction_validate():
        ...
        buffer = str((tx.received_timestamp, tx.received_address, tx.received_recipient, tx.received_amount, tx.received_operation, tx.received_openfield)
                    ).encode('utf-8')
            ...
            if entry_signature:  # prevent empty signature database retry hack
                ...
                if node.old_sqlite:
                    ...
                    db_handler.execute_param(
                        db_handler.h, 'SELECT block_height FROM transactions WHERE substr(signature,1,4) = substr(?1,1,4) and signature = ?1;',
                        (entry_signature,)
                    )
                ...
                if node.old_sqlite:
                    ...
                    db_handler.execute_param(
                        db_handler.c, 'SELECT block_height FROM transactions WHERE substr(signature,1,4) = substr(?1,1,4) and signature = ?1;',
                        (entry_signature,)
                    )
    ...
    def sort_transactions(block):
        ...
        for tx_index, transaction in enumerate(block):
            ...
            block_instance.transaction_list_converted.append((
                tx.received_timestamp, tx.received_address, tx.received_recipient, tx.received_amount, tx.received_signature_enc,
                tx.received_public_key_b64encoded, tx.received_operation, tx.received_openfield
            ))
