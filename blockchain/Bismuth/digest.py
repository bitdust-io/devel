...
def digest_block(node, data, sdef, peer_ip, db_handler):
    ...
    def rewards():
        if int(block_instance.block_height_new) % 10 == 0:  # every 10 blocks
            db_handler.dev_reward(
                ...
                block_instance,
                miner_tx,
                block_instance.mining_reward,
                block_instance.mirror_hash,
            )
            db_handler.hn_reward(
                node, block_instance, miner_tx, block_instance.mirror_hash
            )
    ...
    def sort_transactions(block):
        ...
        for tx_index, transaction in enumerate(block):
            ...
            if (
                tx_index == block_instance.tx_count - 1
            ):  # faster than comparing the whole tx
                if float(tx.received_amount) != 0:
                    raise ValueError(
                        "Coinbase (Mining) transaction must have zero amount"
                    )
            ...
            for tx_index, transaction in enumerate(block):
                ...
                if tx_index == block_instance.tx_count - 1:
                    ...
                    if node.is_testnet and node.last_block >= fork.POW_FORK_TESTNET:
                        block_instance.mining_reward = (
                            15
                            - (block_instance.block_height_new - fork.POW_FORK_TESTNET)
                            / 1100000
                            - 9.5
                        )
                        ...
                        block_instance.mining_reward = (
                            15
                            - (block_instance.block_height_new - fork.POW_FORK) / 1100000
                            - 9.5
                        )
                    else:
                        block_instance.mining_reward = (
                    ...
                    reward = "{:.8f}".format(
                        Decimal(block_instance.mining_reward) + sum(fees_block)
                    )
                    ...
                    balance_pre = ledger_balance3(
                        db_address, balances, db_handler
            ...
            for block in b_data:
                if node.IS_STOPPING:
                    node.logger.app_log.warning(
                        "Process_blocks aborted, node is stopping"
                    )
                    return
                ...
                if miner_tx.q_block_timestamp <= node.last_block_timestamp:
                    ...
                    raise ValueError(
                        f"!Block is older {miner_tx.q_block_timestamp} "
                ...
                node.logger.app_log.warning(
                    f"Time to generate block {node.last_block + 1}: {'%.2f' % diff[2]}"
                )
                ...
                block_instance.block_hash = hashlib.sha224(
                    (
                        str(block_instance.transaction_list_converted)
                ...
                node.logger.app_log.info(
                    f"Calculated block sha_hash: {block_instance.block_hash}"
                )
                ...
                block_instance.mirror_hash = hashlib.blake2b(
                    str(tx_list_to_hash).encode(), digest_size=20
                ).hexdigest()
                # /new sha_hash
    ...
    if not node.db_lock.locked():
        ...
        try:
            ...
            node.last_block = db_handler.block_max_ram()[
                "block_height"
            ]  # get actual data from database on exception
            node.last_block_hash = (
                db_handler.last_block_hash()
            )  # get actual data from database on exception
        ...
        node.plugin_manager.execute_action_hook(
            "digestblock", {"failed": "skipped", "ip": peer_ip}
        )
