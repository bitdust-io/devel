...
def send(sdef, data, slen=SLEN):
    ...
    return sdef.sendall(
        str(len(json.dumps(data))).encode("utf-8").zfill(slen)
...
if "Linux" in platform.system():
    READ_OR_ERROR = (
        select.POLLIN | select.POLLPRI | select.POLLHUP | select.POLLERR | select.POLLNVAL
    )
