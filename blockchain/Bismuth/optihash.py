...
def diffme(pool_address, nonce, db_block_hash):
    ...
    mining_hash = bin_convert(
        sha224((pool_address + nonce + db_block_hash).encode("utf-8")).hexdigest()
    )
                ...
                possibles = [
                    ...
                    in (
                        mining.anneal3(
                            ...
                            int.from_bytes(
                                sha224(
                                    (prefix + nonce + db_block_hash).encode("utf-8")
                                ).digest(),
                                "big",
                            ),
                        )
                ...
                if possibles:
                    ...
                    for nonce in possibles:
                        ...
                        if xdiffx < diff:
                            ...
                            print(
                                "Thread {} solved work with difficulty {} in {} cycles - YAY!".format(
                                    q, xdiffx, tries
                                )
                            )
                            ...
                            try:
                                ...
                                print(
                                    "Miner: connected to pool, proceeding to submit solution"
                                )
                                ...
                                mined_coins += 1
                                print(
                                    "Miner: solution submitted to pool",
                                    mined_coins,
                                    how_much_coins,
                                )
                                if mined_coins >= how_much_coins:
    ...
    while True:
        try:
            ...
            for q in instances:
                ...
                p.start()
            print(
                "{} miners searching for solutions at difficulty {} and condition {}".format(
                    mining_threads_conf, str(diff), str(mining_condition)
                )
            )
            ...
            dh = sum(results)
            print(
                "Current total hash rate is {} kh/s : in total {} coins mined ".format(
                    str(dh), totoally_mined_coins
                )
            )
            ...
            print(e)
            print(
                "Miner: Unable to connect to pool check your connection or IP settings."
            )
            time.sleep(1)
