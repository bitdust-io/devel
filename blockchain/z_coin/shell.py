...
class zc(cmd.Cmd):
    ...
    def do_send(self, line):
        ...
        try:
            ...
            threading.Thread(target=send_coin.send,
                             args=(to, amount, self.name_space)).start()
    
    def do_check(self, lines):
        get_db.send(self.name_space)

    def do_totalcoins(self, line):
        coin = ns(self.name_space).db.find("coins", "all")
        if not coin:
            coin = 0
        else:
            coin = len(coin)
    ...
    def do_coins(self, line):
        addr = ns(self.name_space).wallet.find("data", "all")[0]
        addr = addr['address']
        coins = ns(self.name_space).db.find("coins", {"address":addr})
        if not coins:
            coins = 0
        else:
            coins = len(coins)
    ...
    def do_addr(self, lines):
        addr = ns(self.name_space).wallet.find("data", "all")[0]['address']
    ...
    def do_transactions(self, line):
        ...
        if not line:
            ...
            data = ns(self.name_space).db.find("transactions", {"from":line})
            if not data:
    ...
    def do_update(self, line):
        ...
        for x in files:
            ...
            with open(x,'wb') as outfile:
                outfile.write(urllib.urlopen(files[x]).read())

    def do_stats(self, line):
        coins = ns(self.name_space).db.find("coins", "all")
        trans = ns(self.name_space).db.find("transactions","all")
